...
%    noise (or signal) above that frequency is not treated, in units of
%    the sample frequency.
%    NPERS is the number of periods to use for estimation.
%    This only works for vectors XX.
...
if prod(size(xx)) ~= length(xx)
  ...
  yy = zeros(X,Y);
  for y=1:Y
    yy(:,y) = templatefilter(xx(:,y),period_sams,f_mxax,npers);
  end
  yy = reshape(yy,S);
end
...
% Step 2b: remove DC from the template-to-be
...
% Step six: reshape back to a vector, and resample back to original frequency.
zz = zz(:);
Z=length(zz);
zz = interp1([1:Z],zz,[1:1/rat:Z],'linear');

% Step seven: subtract the template from the original signal.
