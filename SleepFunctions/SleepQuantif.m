function [A,t,Atsd,Perc,Sleep,Sleep2,idFr,th]=SleepQuantif(filename,fac,thtps,ThMice,lim)
...
% we set the limits of the frame for tracking
margeinfX=180;
margeinfY=120;
margesupX=260;
margesupY=190;
...
[x, y, bodyline, sqr] = FindFly(max(max(J))-J, param);
...
%----------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   if plo
    ...
    subplot(2,3,1),imagesc(vidFrames1(margeinfY:margesupY,margeinfX:margesupX,:,1)),axis xy    
        % we set a global title to the figure
  %      text(0,margesupY+20, figuretitle ,'FontSize',15,'Color','k');
        %text(xlim(1)-0.05*(xlim(2)-xlim(1)),ylim(1)+1.06*(ylim(2)-ylim(1)), figuretitle ,'FontSize',15,'Color','k');
        
    subplot(2,3,2),imagesc(Mov2th(margeinfY:margesupY,margeinfX:margesupX,:,1)),axis xy, colorbar
    subplot(2,3,3),hold on,axis xy, colorbar
...
Sleep=thresholdIntervals(Atsd,th,'Direction','Below');
Sleep2=dropShortIntervals(Sleep,thtps*1E4);

Perc=sum(End(Sleep2,'s')-Start(Sleep2,'s'))/dur*100;
