function SleepScoring_OBgamma_RemoveStimBeforeSmoothing_AF(varargin)
...
%                          - Added Theta channel
%   Updated 2024 Sophie and Alexane to remove stimulations before smoothing
%   for audiodream project
%      
...
if ~recompute
    if exist('SleepScoring_OBGamma_Audiodream.mat','file')==2 
        disp('Scoring  already generated')
        return
    end
end
...
diary('SleepScoring_history_AF.txt')
...
if doob
    save('SleepScoring_OBGamma_Audiodream','Info')
end
clear Info
...
disp('------------------------------------------------------------')
disp(' STEP 1: LOADING PREVIOUS NOISE EPOCHS')
disp('------------------------------------------------------------')
disp(' ')
load('SleepScoring_OBGamma.mat', 'Info','Epoch','TotalNoiseEpoch','SubNoiseEpoch')
Info_OB = Info;
...
if doob
    ...
    disp(' ')
    save('SleepScoring_OBGamma_Audiodream','REMEpoch','SWSEpoch','Wake','REMEpochWiNoise', ...
...
if PlotFigure==1
    ...
    if exist('dowiob','var')
        if ~dowiob
            Figure_SleepScoring_OBGamma_Audiodream(foldername)
        end
    else
        Figure_SleepScoring_OBGamma_Audiodream(foldername)
    end
