...
%     xlim(xl)
    ...
    durS_accelero = durSS_Piez_accelero + durWS_Piez_accelero;
    proba_S_S_ob_accelero(i,1) = (durSS_Piez_accelero/durS_accelero);
    proba_W_S_ob_accelero(i,1) = (durWS_Piez_accelero/durS_accelero);
    ...
    durW_accelero = durWW_Piez_accelero + durSW_Piez_accelero;
    proba_W_W_ob_accelero(i,1) = (durWW_Piez_accelero/durW_accelero) ; 
    proba_S_W_ob_accelero(i,1) = (durSW_Piez_accelero/durW_accelero) ; 
    ...
    durS_accelero = durSS_Piez_accelero + durWS_Piez_accelero;
    proba_S_S_accelero_ob(i,1) = (durSS_Piez_accelero/durS_accelero);
    proba_W_S_accelero_ob(i,1) = (durWS_Piez_accelero/durS_accelero);
    ...
    durW_accelero = durWW_Piez_accelero + durSW_Piez_accelero;
    proba_W_W_accelero_ob(i,1) = (durWW_Piez_accelero/durW_accelero) ; 
    proba_S_W_accelero_ob(i,1) = (durSW_Piez_accelero/durW_accelero) ;
    
    % Long Sleep
    durSS_Piez_accelero = sum(Stop(and(Sleep_LongOnly_accelero,Sleep_LongOnly_OB),'s') - Start(and(Sleep_LongOnly_accelero,Sleep_LongOnly_OB),'s'));
    durWS_Piez_accelero = sum(Stop(and(Wake_modified_accelero,Sleep_LongOnly_OB),'s') - Start(and(Wake_modified_accelero,Sleep_LongOnly_OB),'s'));
    durS_accelero = durSS_Piez_accelero + durWS_Piez_accelero;
    proba_S_S_ob_accelero_longsleep_only(i,1) = (durSS_Piez_accelero/durS_accelero);
    proba_W_S_ob_accelero_longsleep_only(i,1) = (durWS_Piez_accelero/durS_accelero);

    
    durWW_Piez_accelero = sum(Stop(and(Wake_modified_accelero,Wake_modified_OB),'s') - Start(and(Wake_modified_accelero,Wake_modified_OB),'s'));
    durSW_Piez_accelero = sum(Stop(and(Sleep_LongOnly_accelero,Wake_modified_OB),'s') - Start(and(Sleep_LongOnly_accelero,Wake_modified_OB),'s'));
    durW_accelero = durWW_Piez_accelero + durSW_Piez_accelero;
    proba_W_W_ob_accelero_longsleep_only(i,1) = (durWW_Piez_accelero/durW_accelero) ; 
    proba_S_W_ob_accelero_longsleep_only(i,1) = (durSW_Piez_accelero/durW_accelero) ;
    
    durSS_Piez_accelero = sum(Stop(and(Sleep_LongOnly_OB,Sleep_LongOnly_accelero),'s') - Start(and(Sleep_LongOnly_OB,Sleep_LongOnly_accelero),'s'));
    durWS_Piez_accelero = sum(Stop(and(Wake_modified_OB,Sleep_LongOnly_accelero),'s') - Start(and(Wake_modified_OB,Sleep_LongOnly_accelero),'s'));
    durS_accelero = durSS_Piez_accelero + durWS_Piez_accelero;
    proba_S_S_accelero_ob_longsleep_only(i,1) = (durSS_Piez_accelero/durS_accelero);
    proba_W_S_accelero_ob_longsleep_only(i,1) = (durWS_Piez_accelero/durS_accelero);

    
    durWW_Piez_accelero = sum(Stop(and(Wake_modified_OB,Wake_modified_accelero),'s') - Start(and(Wake_modified_OB,Wake_modified_accelero),'s'));
    durSW_Piez_accelero = sum(Stop(and(Sleep_LongOnly_OB,Wake_modified_accelero),'s') - Start(and(Sleep_LongOnly_OB,Wake_modified_accelero),'s'));
    durW_accelero = durWW_Piez_accelero + durSW_Piez_accelero;
    proba_W_W_accelero_ob_longsleep_only(i,1) = (durWW_Piez_accelero/durW_accelero) ; 
    proba_S_W_accelero_ob_longsleep_only(i,1) = (durSW_Piez_accelero/durW_accelero) ;
end
...
m = i + 1;
    ...
    mean_proba_W_W_Piezo_OB_longsleep_only = mean(proba_W_W_Piezo_OB_longsleep_only(:,1));
    ...
    mean_proba_W_S_Piezo_accelero_longsleep_only = mean(proba_W_S_Piezo_accelero_longsleep_only(:,1));
    ...
    mean_proba_S_S_ob_accelero_longsleep_only = mean(proba_S_S_ob_accelero_longsleep_only(:,1));
    mean_proba_W_S_ob_accelero_longsleep_only = mean(proba_W_S_ob_accelero_longsleep_only(:,1));

    mean_proba_W_W_ob_accelero_longsleep_only = mean(proba_W_W_ob_accelero_longsleep_only(:,1));
    mean_proba_S_W_ob_accelero_longsleep_only = mean(proba_S_W_ob_accelero_longsleep_only(:,1));
    
    mean_proba_S_S_accelero_ob_longsleep_only = mean(proba_S_S_accelero_ob_longsleep_only(:,1));
    mean_proba_W_S_accelero_ob_longsleep_only = mean(proba_W_S_accelero_ob_longsleep_only(:,1));

    mean_proba_W_W_accelero_ob_longsleep_only = mean(proba_W_W_accelero_ob_longsleep_only(:,1));
    mean_proba_S_W_accelero_ob_longsleep_only = mean(proba_S_W_accelero_ob_longsleep_only(:,1));
...
xticklabels({'Piezo','OB Gamma', 'Mouvement'});
xlabel('Eveil prédit par','Position', [1.5 0.25]);
yticks(1:3);
yticklabels({'Piezo','OB Gamma', 'Mouvement'});
ylabel('Validé par');
...
xlabel('Sommeil prédit par','Position', [1.5 0.25]);
yticks(1:3);
...
% Matrix Wake Long Sleep
matrix_wake = zeros([3,3])
matrix_wake(1,1) = 1
matrix_wake(2,1) = mean_proba_W_W_Piezo_OB_longsleep_only
matrix_wake(3,1) = mean_proba_W_W_Piezo_accelero_longsleep_only
matrix_wake(1,2) = mean_proba_W_W_OB_longsleep_only
matrix_wake(2,2) = 1
matrix_wake(3,2) = mean_proba_W_W_ob_accelero_longsleep_only
matrix_wake(1,3) = mean_proba_W_W_accelero_longsleep_only
matrix_wake(2,3) = mean_proba_W_W_accelero_ob_longsleep_only
matrix_wake(3,3) = 1
...
ax.XAxisLocation = 'top';
xticklabels({'Piezo','OB Gamma', 'Movement'});
xlabel('Période Sommeil : Eveil prédit par','Position', [1.5 0.25]);
yticks(1:3);
yticklabels({'Piezo','OB Gamma', 'Movement'});
ylabel('Validé par');
...
matrix_sleep = zeros([3,3])
matrix_sleep(1,1) = 1
matrix_sleep(2,1) = mean_proba_S_S_Piezo_OB_longsleep_only
matrix_sleep(3,1) = mean_proba_S_S_Piezo_accelero_longsleep_only
matrix_sleep(1,2) = mean_proba_S_S_OB_longsleep_only
matrix_sleep(2,2) = 1
matrix_sleep(3,2) = mean_proba_S_S_ob_accelero_longsleep_only
matrix_sleep(1,3) = mean_proba_S_S_accelero_longsleep_only
matrix_sleep(2,3) = mean_proba_S_S_accelero_ob_longsleep_only
matrix_sleep(3,3) = 1
...
ax.XAxisLocation = 'top';
xticklabels({'Piezo','OB Gamma', 'Movement'});
xlabel('Période Sommeil : Sommeil prédit par','Position', [1.5 0.25]);
yticks(1:3);
yticklabels({'Piezo','OB Gamma', 'Movement'});
ylabel('Validé par');
...
matrix_ob_accelero_PSC = zeros([2,2])
matrix_ob_accelero_PSC(1,1) = mean_proba_W_W_ob_accelero_longsleep_only
matrix_ob_accelero_PSC(2,1) = mean_proba_S_W_ob_accelero_longsleep_only
matrix_ob_accelero_PSC(1,2) = mean_proba_W_S_ob_accelero_longsleep_only
matrix_ob_accelero_PSC(2,2) = mean_proba_S_S_ob_accelero_longsleep_only

matrix_Piezo_OB_PSC = zeros([2,2])
matrix_Piezo_OB_PSC(1,1) = mean_proba_W_W_Piezo_OB_longsleep_only
matrix_Piezo_OB_PSC(2,1) = mean_proba_S_W_Piezo_OB_longsleep_only
matrix_Piezo_OB_PSC(1,2) = mean_proba_W_S_Piezo_OB_longsleep_only
matrix_Piezo_OB_PSC(2,2) = mean_proba_S_S_Piezo_OB_longsleep_only

matrix_Piezo_accelero_PSC = zeros([2,2])
matrix_Piezo_accelero_PSC(1,1) = mean_proba_W_W_Piezo_accelero_longsleep_only
matrix_Piezo_accelero_PSC(2,1) = mean_proba_S_W_Piezo_accelero_longsleep_only
matrix_Piezo_accelero_PSC(1,2) = mean_proba_W_S_Piezo_accelero_longsleep_only
matrix_Piezo_accelero_PSC(2,2) = mean_proba_S_S_Piezo_accelero_longsleep_only
...
for i = 1:numel(matrix_ob_accelero)
    text(x(i),y(i),sprintf('%.2f',matrix_Piezo_accelero_PSC(i)), 'HorizontalAlignment','center','VerticalAlignment','middle');
end
xticks(1:3);
...
cohen_kappa = CohenKappaSleepStaging(matrix_ob_accelero)
cohen_kappa = CohenKappaSleepStaging(matrix_Piezo_OB)
cohen_kappa = CohenKappaSleepStaging(matrix_Piezo_accelero)



cohen_kappa = CohenKappaSleepStaging(matrix_ob_accelero_PSC)
cohen_kappa = CohenKappaSleepStaging(matrix_Piezo_OB_PSC)
cohen_kappa = CohenKappaSleepStaging(matrix_Piezo_accelero_PSC)
