...
Periode_sommeil_OB = intervalSet(0,max(Range(SmoothGamma)));

    Sleep_LongOnly_OB = and(Sleep, Periode_sommeil_OB);
    Sleep_LongOnly_OB = dropShortIntervals(Sleep_LongOnly_OB,10*1e4);
...
ylabel('Overlap(%)');
xlabel('OB gamma');
title('Période de Sommmeil Consolidé');
lgd = legend('Sommeil','Eveil','Location','southoutside');
title(lgd,'Mouvement');
ylim([0 100]);
