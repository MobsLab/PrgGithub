...
%     load('SleepScoring_Accelero.mat','tsdMovement');
    ...
    Wake_accelerosup300 = dropShortIntervals(Wake,300*1e4);
        Wake_accelerosup240all = Wake_accelero - Wake_accelerosup300;
    Wake_accelerosup240 = dropShortIntervals(Wake_accelerosup240all,240*1e4);
        Wake_accelerosup180all = Wake_accelerosup240all -Wake_accelerosup240;
    Wake_accelerosup180 = dropShortIntervals(Wake_accelerosup180all,180*1e4);
        Wake_accelerosup120all = Wake_accelerosup180all -Wake_accelerosup180;    
    Wake_accelerosup120 = dropShortIntervals(Wake_accelerosup120all,120*1e4);
        Wake_accelerosup60all = Wake_accelerosup120all -Wake_accelerosup120;
    Wake_accelerosup60 = dropShortIntervals(Wake_accelerosup60all,60*1e4);

    Wake_accelerosup300 = dropShortIntervals(Wake_accelero,50*1e4);
        Wake_accelerosup300 = Wake_accelero - Wake_accelerosup300;
    Wake_accelerosup240 = dropShortIntervals(Wake_accelero,40*1e4);
        Wake_accelerosup240 = Wake_accelero -Wake_accelerosup240;
    Wake_accelerosup180 = dropShortIntervals(Wake_accelero,30*1e4);
        Wake_accelerosup180 = Wake_accelero -Wake_accelerosup180;    
    Wake_accelerosup120 = dropShortIntervals(Wake_accelero,20*1e4);
        Wake_accelerosup120 = Wake_accelero -Wake_accelerosup120;
...
%     load('SleepScoring_OBGamma.mat','SmoothGamma');
    ...
    Wake_OBsup300 = dropShortIntervals(Wake,300*1e4);
        Wake_OBsup240all = Wake_OB - Wake_OBsup300;
    Wake_OBsup240 = dropShortIntervals(Wake_OBsup240all,240*1e4);
        Wake_OBsup180all = Wake_OBsup240all -Wake_OBsup240;
    Wake_OBsup180 = dropShortIntervals(Wake_OBsup180all,180*1e4);
        Wake_OBsup120all = Wake_OBsup180all -Wake_OBsup180;    
    Wake_OBsup120 = dropShortIntervals(Wake_OBsup120all,120*1e4);
        Wake_OBsup60all = Wake_OBsup120all -Wake_OBsup120;
    Wake_OBsup60 = dropShortIntervals(Wake_OBsup60all,60*1e4);
...
%     load('PiezoData_SleepScoring.mat','Smooth_actimetry');
    ...
    Wake_piezosup300 = dropShortIntervals(WakeEpoch_Piezo,300*1e4);
        Wake_piezosup240all = WakeEpoch_Piezo - Wake_piezosup300;
    Wake_piezosup240 = dropShortIntervals(Wake_piezosup240all,240*1e4);
        Wake_piezosup180all = Wake_piezosup240all -Wake_piezosup240;
    Wake_piezosup180 = dropShortIntervals(Wake_piezosup180all,180*1e4);
        Wake_piezosup120all = Wake_piezosup180all -Wake_piezosup180;    
    Wake_piezosup120 = dropShortIntervals(Wake_piezosup120all,120*1e4);
        Wake_piezosup60all = Wake_piezosup120all -Wake_piezosup120;
    Wake_piezosup60 = dropShortIntervals(Wake_piezosup60all,60*1e4);
    ...
    durWW_Piez_accelero = sum(Stop(and(Wake_accelerosup300,Wake_piezo50),'s') - Start(and(Wake_accelerosup300,Wake_piezo50),'s'));
    proba_W_W_accelero50 = (durWW_Piez_accelero/durW_accelero)*100 ; 
    
    durW_accelero = sum(Stop(Wake_piezo40,'s') - Start(Wake_piezo40,'s'));
    durWW_Piez_accelero = sum(Stop(and(Wake_accelerosup240,Wake_piezo40),'s') - Start(and(Wake_accelerosup240,Wake_piezo40),'s'));
    proba_W_W_accelero40 = (durWW_Piez_accelero/durW_accelero)*100 ; 
    
    durW_accelero = sum(Stop(Wake_piezo30,'s') - Start(Wake_piezo30,'s'));
    durWW_Piez_accelero = sum(Stop(and(Wake_accelerosup180,Wake_piezo30),'s') - Start(and(Wake_accelerosup180,Wake_piezo30),'s'));
    proba_W_W_accelero30 = (durWW_Piez_accelero/durW_accelero)*100 ;     

    durW_accelero = sum(Stop(Wake_piezo20,'s') - Start(Wake_piezo20,'s'));
    durWW_Piez_accelero = sum(Stop(and(Wake_accelerosup120,Wake_piezo20),'s') - Start(and(Wake_accelerosup120,Wake_piezo20),'s'));
    ...
    durWW_Piez_OB = sum(Stop(and(Wake_OB50,Wake_accelerosup300),'s') - Start(and(Wake_OB50,Wake_accelerosup300),'s'));
    proba_W_W_OBacc50 = (durWW_Piez_OB/durW_OB)*100 ; 
    
    durW_OB = sum(Stop(Wake_accelerosup240,'s') - Start(Wake_accelerosup240,'s'));
    durWW_Piez_OB = sum(Stop(and(Wake_OB40,Wake_accelerosup240),'s') - Start(and(Wake_OB40,Wake_accelerosup240),'s'));
    proba_W_W_OBacc40 = (durWW_Piez_OB/durW_OB)*100 ; 
    
    durW_OB = sum(Stop(Wake_accelerosup180,'s') - Start(Wake_accelerosup180,'s'));
    durWW_Piez_OB = sum(Stop(and(Wake_OB30,Wake_accelerosup180),'s') - Start(and(Wake_OB30,Wake_accelerosup180),'s'));
    proba_W_W_OBacc30 = (durWW_Piez_OB/durW_OB)*100 ;     

    durW_OB = sum(Stop(Wake_accelerosup120,'s') - Start(Wake_accelerosup120,'s'));
    durWW_Piez_OB = sum(Stop(and(Wake_OB20,Wake_accelerosup120),'s') - Start(and(Wake_OB20,Wake_accelerosup120),'s'));
...
groups = {'>300s','240-300s','180-240s','120-180s','60-120s','<60s','<50s','<40s','<30s','<20s','<10s','<5s'};
