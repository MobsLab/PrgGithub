...
for ep=1:2
    for k=1:2
        ...
        AllInfoyx=[];
        for mm=1:17
            if isfield(RememberInfo{mm},'Fx2y')
                try
            AllInfoxy=[AllInfoxy;RememberInfo{mm}.Fx2y{ep,k}];
            AllInfoyx=[AllInfoyx;RememberInfo{mm}.Fy2x{ep,k}];
                end
            end
        end
subplot(2,3,k+3*(ep-1))
plot(RememberInfo{2}.freqBin,nanmean(AllInfoxy),'r'), hold on
plot(RememberInfo{2}.freqBin,nanmean(AllInfoyx),'b'), hold on
g=shadedErrorBar(RememberInfo{2}.freqBin,nanmean(AllInfoxy),[stdError(AllInfoxy);stdError(AllInfoxy)],'r')
g=shadedErrorBar(RememberInfo{2}.freqBin,nanmean(AllInfoyx),[stdError(AllInfoyx);stdError(AllInfoyx)],'b')
xlim([0 20])
StrucName={Structures{StrucPairs(k,1)},Structures{StrucPairs(k,2)}};
legend([StrucName{2},'->',StrucName{1}],[StrucName{1},'->',StrucName{2}]);
