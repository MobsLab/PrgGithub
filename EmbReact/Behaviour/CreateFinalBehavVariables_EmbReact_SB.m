...
for ss=1:length(FileNames)
    clearvars -except ss FileNames n AllDat th_immob thtps_immob smoofact_Acc smoofact th_immob_Acc DoSave GlobalFigure DoPosition FolderName
    disp(FileNames{ss})
    cd(FileNames{ss})
    load('ExpeInfo.mat')
    ...
    if ExpeInfo.SleepSession==0
        ...
        if exist([cd filesep 'behavResources.mat'])>0
            ...
            % What was this there for?????
            %             PosMat(PosMat(:,3)>39.5,3)=39.5;
            %              save('behavResources.mat','PosMat','-append')
