...
SessOfInterest = {'_Habituation','_TestPre','_Cond','_TestPost','_Extinction'};
SubSession = {{},{'TestPre1','TestPre2','TestPre3','TestPre4'},...
    {'Cond1','Cond2','Cond3','Cond4','Cond5'},{'TestPost1','TestPost2','TestPost3','TestPost4'},{}};
n=1;
...
%% Function called by the others
    ...
    function GetAllFolders(obj,event)
        FolderNameAllUMaze={};
        DirName=uigetdir('','Choose folder containing all sessions for UMaze');
        cd(DirName)
        ls=dir;
        subFolders = ls([ls.isdir]);
        for k=1:length(SessOfInterest)
            FoldNum=find(~cellfun(@isempty,(strfind({subFolders.name}, SessOfInterest{k}))));
            cd(subFolders(FoldNum).name)
            if not(isempty(SubSession{k}))
                for l=1:length(SubSession{k})
                    cd(SubSession{k}{l})
                    cd('raw')
                    RawList=dir;
                    FoldNumFrames=find(~cellfun(@isempty,(strfind({RawList.name}, 'FEAR'))));
                    cd(RawList(FoldNumFrames).name)
                    load('behavResources.mat','mask','ref','pixratio')
                    save('InfoTrackingOffline.mat','mask','ref','pixratio')
                    clear('mask','ref','pixratio')
                    FolderNameAllUMaze{1}{k,l}= [cd filesep];
                    RawList=dir;
                    RawList=RawList([RawList.isdir]);
                    FoldNumAllFrames=find(~cellfun(@isempty,(strfind({RawList.name}, 'F'))));
                    FolderNameAllUMaze{2}{k,l}= [RawList(FoldNumAllFrames).name];
                    cd(DirName)
                    cd(subFolders(FoldNum).name)
                end
            else
                ...
                save('InfoTrackingOffline.mat','mask','ref','pixratio')
                clear('mask','ref','pixratio')
                FolderNameAllUMaze{1}{k,1}= [cd filesep];
                RawList=dir;
                RawList=RawList([RawList.isdir]);
                FoldNumAllFrames=find(~cellfun(@isempty,(strfind({RawList.name}, 'F'))));
                FolderNameAllUMaze{2}{k,1}= [RawList(FoldNumAllFrames).name];
            end
            cd(DirName)
        ...
        cd(DirName)
        for k=1:length(SessOfInterest)
            disp(SessOfInterest{k})
            if not(isempty(SubSession{k}))
                for l=1:length(SubSession{k})
                    disp(FolderNameAllUMaze{1}{k,l})
                end
            else
                disp(FolderNameAllUMaze{1}{k,1})
            end
        end
        keyboard
        save('FolderUMazeRetracking.mat','FolderNameAllUMaze')
        set(hand(4),'enable','on')
    end

    function SetSessionToTrack(obj,event)
        fctname=SessId(get(obj,'value'));
        cd(FolderNameAllUMaze{1}{fctname{1}(1),fctname{1}(2)})
        load('InfoTrackingOffline.mat','ref','pixratio','mask')
        AllSess=strsplit(AllSess,'|');
        set(hand(5),'string',AllSess{get(obj,'value')});
        AllSess=strjoin(AllSess,'|');
        cd(FolderNameAllUMaze{2}{fctname{1}(1),fctname{1}(2)})
        lis=dir;
        index=[];
        for i=1:length(lis)
