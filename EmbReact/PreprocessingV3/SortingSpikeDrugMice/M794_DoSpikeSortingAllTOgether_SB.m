...
CopyLocation = '/media/mobsmorty/DataMOBS98/Mouse794/20181116/SpikeSortingTogether/'


% copy the ones for the UMaze day into folder
for g=35:length(FileNames)
    
    FileNames{g} = strrep(FileNames{g},'/media/nas4/ProjetEmbReact/','/media/mobsmorty/DataMOBS98/');
    
    cd(FileNames{g})
    load('ExpeInfo.mat')
    FolderName =  dir('*.dat');
    for i = 1:length(FolderName)
        if not(isempty(strfind(FolderName(i).name,'M794')))
