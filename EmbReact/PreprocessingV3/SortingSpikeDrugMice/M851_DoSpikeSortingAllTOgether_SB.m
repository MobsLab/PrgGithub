...
CopyLocation = '/media/mobsmorty/DataMOBS98/MidazolamExperiment/Mouse851/24012019/SpikeSorting/'


% copy the ones for the UMaze day into folder
for g=5:length(FileNames)
    
    FileNames{g} = strrep(FileNames{g},'/media/nas4/ProjetEmbReact/','/media/mobsmorty/DataMOBS98/MidazolamExperiment/');
    
    cd(FileNames{g})
    load('ExpeInfo.mat')
    FolderName =  dir('*.dat');
    for i = 1:length(FolderName)
        if not(isempty(strfind(FolderName(i).name,'M851')))
    ...
    if ExpeInfo.PreProcessingInfo.DoSpikes
        ...
        if sum(ExpeInfo.SpikeGroupInfo.UseForElecSub)>0
            ...
            tmp = ExpeInfo.SpikeGroupInfo.ElecsToSub;
            ExpeInfo.SpikeGroupInfo.ElecsToSub = [];
            for tet = 1:length(ExpeInfo.SpikeGroupInfo.ChanNames)
                ExpeInfo.SpikeGroupInfo.ElecsToSub{tet}=tmp;
            end
            
            % New version with different reference channels
            clear ChannelsToAverageForSub
