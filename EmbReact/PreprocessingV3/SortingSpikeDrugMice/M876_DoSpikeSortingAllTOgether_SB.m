...
CopyLocation = '/media/mobsmorty/3EB95039742FF152/Mouse876/SpikeSorting/'


% copy the ones for the UMaze day into folder
for g=2:length(FileNames)
    
    FileNames{g} = strrep(FileNames{g},'/media/nas4/ProjetEmbReact/','/media/mobsmorty/3EB95039742FF152/');
    
    cd(FileNames{g})
    load('ExpeInfo.mat')
    FolderName =  dir('*.dat');
    for i = 1:length(FolderName)
        if not(isempty(strfind(FolderName(i).name,'M876')))
