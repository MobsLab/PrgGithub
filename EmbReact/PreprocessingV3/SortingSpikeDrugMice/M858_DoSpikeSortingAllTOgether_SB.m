...
CopyLocation = '/media/mobsmorty/DataClemence/PreProcessed/Mouse858/SpikeSorting/'


% copy the ones for the UMaze day into folder
for g=1:length(FileNames)
    
    FileNames{g} = strrep(FileNames{g},'/media/nas4/ProjetEmbReact/','/media/mobsmorty/DataClemence/PreProcessed/');
    
    cd(FileNames{g})
    load('ExpeInfo.mat')
    FolderName =  dir('*.dat');
    for i = 1:length(FolderName)
        if not(isempty(strfind(FolderName(i).name,'M858')))
    ...
    % Do subtraction
        if ExpeInfo.PreProcessingInfo.DoSpikes
            ...
            if sum(ExpeInfo.SpikeGroupInfo.UseForElecSub)>0
                ...
                clear ChannelsToAverageForSub ChannelsToPerfSub
                ToDel = [];
                numtet = 1;
                for tet = 1:length(ExpeInfo.SpikeGroupInfo.ChanNames)
                    try ExpeInfo.SpikeGroupInfo.ElecsToSub{tet}
                    catch
                       ExpeInfo.SpikeGroupInfo.ElecsToSub{tet} = []; 
                    end
...
% rename as .fil
system(['mv ' 'M' num2str(ExpeInfo.nmouse)  '-' ExpeInfo.date '-SpikesAllTogether.dat' ' M' MouseNum '-' ExpeInfo.date '-SpikesAllTogether.fil'])
