...
ParNames = {'All','NoResp','NoHear','NoOB','NoHpc'};
...
OB_MaxFreq_Maze_BM;
clear DATA_Ctrl
DATA_Ctrl(1,:) = [OB_Max_Freq.RipControl.Cond.Shock OB_Max_Freq.RipControl.Cond.Safe];
n=2;
for par=[2:5 7:8]
    DATA_Ctrl(n,:) = [OutPutData.Cond.(Params{par}).mean(:,5)' OutPutData.Cond.(Params{par}).mean(:,6)'];
    n=n+1;
end

% Drug mice
clear DATA_Drug
load('Data_Physio_Freezing_RipInhib_Cond_2sFullBins.mat')
OB_MaxFreq_Maze_BM
DATA_Drug(1,:) = [OB_Max_Freq.RipInhib.Cond.Shock OB_Max_Freq.RipInhib.Cond.Safe];
n=2;
for par=[2:5 7:8]
    DATA_Drug(n,:) = [OutPutData.Cond.(Params{par}).mean(:,5)' OutPutData.Cond.(Params{par}).mean(:,6)'];
    n=n+1;
end
...
for parToUse = 1:length(ParNames)
    ...
    for gg = 1:length(GoodGuys)
        ...
        SVMChoice_Sf(parToUse,GoodGuys(gg)) = prediction(totnummice_train+gg,1);
...
codename = mfilename;
save([SaveLoc,'SVM_Ripples.mat'],'SVMScores_Sk','SVMScores_Sf','SVMChoice_Sk','SVMChoice_Sf',...
    'SVMScores_Sk_Ctrl','SVMScores_Sf_Ctrl','SVMChoice_Sk_Ctrl','SVMChoice_Sf_Ctrl',...
    'ParNames','ParToKeep','codename')

load([SaveLoc,'SVM_Ripples.mat'])
