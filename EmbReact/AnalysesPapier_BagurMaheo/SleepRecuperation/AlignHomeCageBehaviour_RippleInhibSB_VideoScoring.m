...
for group=1:length(Drug_Group)
    ...
    for sess=2
        for mouse=1:length(Folder.(Drug_Group{group}).(Session_type{sess}))
            ...
            disp(Folder.(Drug_Group{group}).(Session_type{sess}){mouse})
            ...
            save('AlignedCagePos_BM_Video.mat', 'AlignedYtsd','AlignedXtsd')
            %
        end
    end
end
...
for group=1:length(Drug_Group)
    ...
    for sess=1:2
        for mouse=1:length(Folder.(Drug_Group{group}).(Session_type{sess}))
            ...
            MovEpoch = and(PreSleep,MovEpoch);
            if sess==1
                load('AlignedCagePos.mat', 'AlignedYtsd','AlignedXtsd')
            else
                load('AlignedCagePos_SBVideo.mat', 'AlignedYtsd','AlignedXtsd')
            end
