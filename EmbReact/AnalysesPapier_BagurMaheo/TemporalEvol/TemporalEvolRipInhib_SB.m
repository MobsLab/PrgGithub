...
for param=[1:8]
    for mouse = 1:length(Mouse)
        clear DSk, DSk = Data(OutPutData.(Session_type{1}).(Params{param}).tsd{mouse,5});
        clear DSf, DSf = Data(OutPutData.(Session_type{1}).(Params{param}).tsd{mouse,6});
        if param==1, DSk(DSk<1.5) = NaN; end
        if param==1, DSf(DSf<1.5) = NaN; end
        
        DATA_Ctrl(param,mouse) = nanmean(DSk);
        DATA_Ctrl(param,mouse+length(Mouse)) = nanmean(DSf);
...
clearvars -except classifier ParToKeep
parToUse=1;
NumBins = 20;
