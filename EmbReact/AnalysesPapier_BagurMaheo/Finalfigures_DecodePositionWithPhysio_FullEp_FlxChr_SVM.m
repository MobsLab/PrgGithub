...
% Controls
load('Data_Physio_Freezing_FlxChr_Ctrl_Ext_2sFullBins.mat')
clear DATA_Ctrl
for n= 1:8
    DATA_Ctrl(n,:) = [OutPutData.Ext.(Params{n}).mean(:,5)' OutPutData.Ext.(Params{n}).mean(:,6)'];
end

% Drug mice
clear DATA_Drug
load('Data_Physio_Freezing_FlxChr_Ext_2sFullBins.mat')

for n=1:8
    DATA_Drug(n,:) = [OutPutData.Ext.(Params{n}).mean(:,5)' OutPutData.Ext.(Params{n}).mean(:,6)'];
end
...
codename = mfilename;
save([SaveLoc,'SVM_Flx_Chr.mat'],'SVMScores_Sk','SVMScores_Sf','SVMChoice_Sk','SVMChoice_Sf',...
    'SVMScores_Sk_Ctrl','SVMScores_Sf_Ctrl','SVMChoice_Sk_Ctrl','SVMChoice_Sf_Ctrl',...
    'ParNames','ParToKeep','codename')

%% Figures

load([SaveLoc,'SVM_Flx_Chr.mat'])
