...
[XmlFile,PathNameXML]=uigetfile('','please show the xml file');
XMLToCopy=[PathNameXML,XmlFile];
[XmlFile,PathNameXML]=uigetfile('','please show the xml spike file');
XMLToCopySpike=[PathNameXML,XmlFile];

Response = inputdlg({'Mouse Number','Stim Type (PAG,Eyeshock,MFB)','ExperimentDay','CalibrationDay','BaselineSleepDays',...
    'Ripples','DeltaPF [sup (delta down) deep (delta up)]','SpindlePF','Calibration Int','Calibration dur'},'Inputs for EMbrReactFolders',1);
% Mouse and date info
...
%     FolderName=OrganizeFilesEmbReactSB(ExpeInfo.nmouse,ExpeInfo.date,[SaveFolderName num2str(ExpeInfo.date) filesep],ChannelsToAnalyseLocation,InfoLFP,ExpeInfo,answer,answerdigin,XMLToCopy,XMLToCopySpike)
    FolderName=OrganizeFilesEmbReactNewProtocolSB(ExpeInfo.nmouse,ExpeInfo.date,[SaveFolderName num2str(ExpeInfo.date) filesep],ChannelsToAnalyseLocation,InfoLFP,ExpeInfo,answer,answerdigin,XMLToCopy,XMLToCopySpike)
