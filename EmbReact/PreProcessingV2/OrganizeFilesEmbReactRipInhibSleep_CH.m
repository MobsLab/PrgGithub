function FolderName=OrganizeFilesEmbReactRipInhibSleep_CH(MouseNum,Date,SaveFolderName,ExpeInfo)
...
ExperimentalConditions={'Habituation24HPre_PreDrug' 'Habituation_PreDrug' 'HabituationBlockedShock_PreDrug' 'HabituationBlockedSafe_PreDrug' 'SleepPre_PreDrug',...
    ...
    'SleepPost_PreDrug'  'TestPost_PreDrug' 'ExtinctionBlockedShock_PreDrug' 'ExtinctionBlockedSafe_PreDrug' 'Recall_PreDrug' 'UMazeCondExplo_PostDrug' 'UMazeCondBlockedShock_PostDrug' 'UMazeCondBlockedSafe_PostDrug',...
    'SleepPost_PostDrug' 'TestPost_PostDrug'  'ExtinctionBlockedShock_PostDrug' 'ExtinctionBlockedSafe_PostDrug'};

IsSleep=zeros(21,1);
IsSleep(5)=1;IsSleep(10)=1;IsSleep(18)=1;
...
for e=1:length(ExperimentalConditions)
    ...
    if strcmp(ExperimentalConditions{e},'TestPre_PreDrug')
        for i=1:4
           ...
           elseif strcmp(ExperimentalConditions{e},'TestPost_PreDrug')
        for i=1:4
    ...
    elseif strcmp(ExperimentalConditions{e},'UMazeCondBlockedSafe_PostDrug')
        ...
        elseif strcmp(ExperimentalConditions{e},'ExtinctionBlockedShock_PreDrug')
        for i=1:3
            mkdir(['Ext',num2str(i)])
            cd(['Ext',num2str(i)])
            ExpeInfo.SessionNumber=i; % if repeated sessions ie conditionning; 0 otherwise
        ...
        elseif strcmp(ExperimentalConditions{e},'ExtinctionBlockedSafe_PreDrug')
        for i=1:3
            mkdir(['Ext',num2str(i)])
            cd(['Ext',num2str(i)])
            ExpeInfo.SessionNumber=i; % if repeated sessions ie conditionning; 0 otherwise
    ...
    elseif strcmp(ExperimentalConditions{e},'Recall_PreDrug')
        for i=1:2
