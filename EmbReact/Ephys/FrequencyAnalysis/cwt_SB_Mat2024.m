function [wt,p,coi,Outputparams] = cwt_SB_Mat2024(x,varargin)
...
%   and maximum scales are determined automatically for each wavelet based
%   on the wavelet's energy spread in frequency and time. The default value
%   of NV is 10.
%
%   [...] = CWT(...,'TimeBandwidth',TB) specifies the time-bandwidth
%   parameter of the Morse wavelet with the symmetry parameter (gamma)
%   fixed at 3. TB is a positive number strictly greater than 3 and less
%   than or equal to 120. The larger the time-bandwidth parameter, the more
%   spread out the wavelet is in time and narrower the wavelet is in
%   frequency. The standard deviation of the Morse wavelet in time is
%   approximately sqrt(TB/2). The standard deviation in frequency is
%   approximately 1/2*sqrt(2/TB). The 'TimeBandwidth' and
%   'WaveletParameter' name-value pairs cannot both be specified.
%
...
%   element is the symmetry parameter (gamma), which must be greater than
%   or equal to 1. The second element is the time-bandwidth parameter,
%   which must be strictly greater than gamma. The ratio of the
%   time-bandwidth parameter to gamma cannot exceed 40. When gamma is equal
%   to 3, the Morse wavelet is perfectly symmetric in the frequency domain.
%   The skewness is equal to 0. Values of gamma greater than 3 result in
...
%   the number of octaves. NO cannot exceed log2(fmax/fmin) where fmax and
%   fmin are the maximum and minimum CWT frequencies (or periods) as
%   determined by the signal length, sampling frequency, and wavelet. See
%   CWTFREQBOUNDS for details. The 'NumOctaves' name-value pair is not
%   recommended and will be removed in a future release. The recommended
%   way to modify the frequency or period range of the CWT is with the
%   'FrequencyLimits' or 'PeriodLimits' name-value pairs. You cannot
%   specify both the 'NumOctaves' and 'FrequencyLimits' or 'PeriodLimits'
%   name-value pairs.
%
...
%   wavelet transform if the analyzing wavelet is 'Morse' or 'amor'.
...
[FourierFactor,sigmaT] = wavelet.internal.cwt.wavCFandSD(fb.Wavelet,...
        ga,be);
Outputparams.sigmaT = sigmaT;
Outputparams.FourierFactor = FourierFactor;
