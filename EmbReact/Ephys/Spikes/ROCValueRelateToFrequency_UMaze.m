...
for mm=1:length(MiceNumber)
    ...
    AllW = [AllW, 0.5-RocVal{mm}(find(IsPFCNeuron{mm}))];
    GoodNeur = [GoodNeur,IsSig{mm}(find(IsPFCNeuron{mm}))];
...
A(find(sum(isnan(A)')),:) = [];
subplot(121)
imagesc(FreqLims,[1:size(A,1)]/size(A,1),SmoothDec(A,[0.1 1]))
hold on
clim([-1.5 1.5])
plot(sort(FreqLims(ind)),[1:size(B,1)]/size(B,1),'color','r','linewidth',2)
plot(sort(FreqLims(ind1)),[1:size(A,1)]/size(A,1),'color','b','linewidth',2)
...
B(find(sum(isnan(B)')),:) = [];
subplot(122)
imagesc(FreqLims,[1:size(B,1)]/size(B,1),SmoothDec(B,[0.1 1]))
hold on
plot(sort(FreqLims(ind)),[1:size(B,1)]/size(B,1),'color','r','linewidth',2)
plot(sort(FreqLims(ind1)),[1:size(A,1)]/size(A,1),'color','b','linewidth',2)
