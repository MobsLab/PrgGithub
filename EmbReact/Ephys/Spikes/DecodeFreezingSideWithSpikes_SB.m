...
%% SPiking correlation matrices sorted by OB frequency or position
figure;
for mm= 1:length(MiceNumber)
    subplot(5,2,[1,3,5])
...
% Decod freezing side with PFC spiking activity
...
hold on
MakeSpreadAndBoxPlot_SB(A,Cols,1:2,{},1)
xlim([0.5 1.5])
ylim([0 1])


%% Look at where the mistakes were made
...
Variable = {'OBFreq','HPCPower','HR','HRVar','LinPos'};
clear ParamsErr
...
% Get the values of different parameters depending in error or correct
for mm= 1:length(MiceNumber)
    for var = 1 :length(Variable)
...
violinplot(A,'mc',[],'facecolor',Cols,'medc','k')
...
X = LinPos';
Y = dat*DecodeBrainState';
nans = find(or(isnan(X),isnan(Y)));
Y(nans) = [];
X(nans) = [];
interp1(X,Y,[0:0.1:1])
...
for k = 1 :length(LinPosVals)-1
    ind = find(X>LinPosVals(k) & X<LinPosVals(k+1)) ;
    if not(isempty(ind))
    Val(k) = (nanmean((Y(ind))));
    Err(k) = (stdError((Y(ind))));
    else
       Val(k) = NaN; 
    end
end

errorbar(LinPosVals(1:end-1)+0.1,naninterp(Val),naninterp(Err),'color','k','linewidth',2)
xlim([0 1])
ylabel('Projection - Physiology')
xlabel('Dist to shock zone')
...
%     DataPCATogether = nanzscore([MouseByMouse.OBFreq{(mm)}',log(MouseByMouse.HPCPower{(mm)})',MouseByMouse.HR{(mm)}',...
%         MouseByMouse.HRVar{(mm)}',(MouseByMouse.RippleDensity{(mm)}')]);
        DataPCATogether = nanzscore([MouseByMouse.OBFreq{(mm)}',log(MouseByMouse.HPCPower{(mm)})',MouseByMouse.HR{(mm)}',...
        MouseByMouse.HRVar{(mm)}']);

%     Proj = DataPCATogether*DecodeBrainState';
        Proj = DataPCATogether*DecodeBrainState(1:4)';

    for k = 1 :length(LinPosVals)-1
        ind = find(MouseByMouse.LinPos{mm}>LinPosVals(k) & MouseByMouse.LinPos{mm}<LinPosVals(k+1)) ;
        if not(isempty(ind))
            Val(mm,k) = nanmean(nanmean((Proj(ind,:))));
        else
            Val(mm,k) = NaN;
        end
    end
