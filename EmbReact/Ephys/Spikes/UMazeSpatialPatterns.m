...
% to be analysed
% num_bootstraps : number of shuffles to perform to evaluate significance


function [map,stats,ZoneFr,ZoneFrRand]=UMazeSpatialPatterns(Sp,Xtsd,Ytsd,TotEpoch,SpeedThresh,ZoneEpoch,MinNumSpikes,num_bootstraps)
...
if  length(Sp)>MinNumSpikes
    ...
    % Average firing rate in each zone
    for z=1:2
        for zz=1:5
    ...
    for z=1:2
        for zz=1:5
            ZoneFrRand.(UmazeZoneNames{zz}).(TypeNames{z})(1:num_bootstraps)=nan(1,num_bootstraps);
        end
        ZoneFrRand.ModInd.(TypeNames{z})=nan(1,num_bootstraps);
    end
end
