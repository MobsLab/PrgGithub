...
for mm = 1 : length(MiceNumber)
    ...
    Dir = Dir(find((cellfun(@isempty,strfind(Dir,'Sleep')))));
    Dir = Dir(find((cellfun(@isempty,strfind(Dir,'Sound')))));
    ...
    clear SkSfProj
    Q = MakeQfromS(Spikes(numNeurons),Binsize);
    Q = Restrict(Q,or(SessNames.EPM{1},UMaze));   
    ...
    for k  = 1:4
        figure(1)
        Projtsd = Restrict(tsd(Range(Q),full(Data(Q))*SkSfProj{k}'),SessNames.EPM{1});
        ...
        scatter(Data(Restrict(Xtsd,ts(Range(Projtsd)))),Data(Restrict(Ytsd,ts(Range(Projtsd)))),15,Data(Projtsd)','filled')
        ...
        figure(2)
        Projtsd = Restrict(tsd(Range(Q),full(Data(Q))*SkSfProj{k}'),UMaze);
    ...
    saveas(1,[FolderName 'EPMDataProjectedEPMUMazeZSCoreTogether',num2str(mm),'.png'])
    saveas(2,[FolderName 'UMazeDataProjectedEPMUMazeZSCoreTogether',num2str(mm),'.png'])
    
end
cd /media/DataMOBsRAIDN/ProjectEmbReact/Figures/PopulationCodingAcrossTasks
save('AllMiceEPMUmazeZSCoreTogether.mat','AllMice')
...
for k = 1:4
subplot(2,2,k)
PlotErrorBarN_KJ(Vals{k}([1,3]),'newfig',0,'ShowSigStar','none')
YL = ylim;
ylim([YL(1) YL(2)*1.2])
