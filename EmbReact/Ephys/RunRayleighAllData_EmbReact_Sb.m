        ...
        if ismember(Dir.ExpeInfo{dd}{1}.nmouse,MiceNumber)
            ...
            for ddd=1:length(Dir.path{dd})
                ...
                for ff =  1:length(FileToLoad)
                    ...
                    load(FileToLoad{ff})
                    disp(FileToLoad{ff})
                    [HS,PhaseSpikes,ModInfo,PhaseLFP]=RayleighFreq_SB(Restrict(LFP,Epoch),Restrict(S,Epoch),'FilterBands',FilterBands);
                    save(['RayleighFreqAnalysis/Rayleigh_BandWidth_' FileToLoad{ff}(9:end-4) 'Spike.mat'],'HS','PhaseSpikes','ModInfo','FilterBands','-v7.3')
                    ...
                    if not(isempty(Start(FreezeEpoch)))
                        ...
                        save(['RayleighFreqAnalysis/Rayleigh_BandWidth_FreezingOnly' FileToLoad{ff}(9:end-4) 'Spike.mat'],'HS','PhaseSpikes','ModInfo','FilterBands','PhaseLFP','-v7.3')
                        clear('HS','PhaseSpikes','ModInfo','PhaseLFP')
                    end
                    clear LFP
                    toc
