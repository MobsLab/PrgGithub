...
line([Start(FreezeEpoch,'s') Stop(FreezeEpoch,'s')]',[Start(FreezeEpoch,'s') Stop(FreezeEpoch,'s')]'*0+160,'color','c','linewidth',2)
Zones = {'shock','safe','center','safe','centersafe'};
for z = 1:5
    line([Start(ZoneEpoch{z},'s') Stop(ZoneEpoch{z},'s')]',[Start(ZoneEpoch{z},'s') Stop(ZoneEpoch{z},'s')]'*0+180,'color',UMazeColors(Zones{z}),'linewidth',5)
end
xlim([0 180])
