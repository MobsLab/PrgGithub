        ...
        for d=1:length(Dir.path)
            ...
            for dd=1:length(Dir.path{d})
                ...
                if go ==1
                    ...
                    % Pag vs Eyeshock
                    try
                        switch lower(Dir.ExpeInfo{d}{dd}.StimElecs)
                            case 'pag'
                                StimType = 1;
                            case 'eyeshock'
                                StimType = 2;
                    ...
                    %Blocked or not
                    Blocked = 0;
                    if not(isempty(findstr(Dir.ExpeInfo{d}{dd}.SessionType,'Block')))
                        Blocked = 1;
                    end
                    ...
                    if not(isempty(Start(CleanFreezeEpoch)))
                        if sum(Stop(CleanFreezeEpoch)-Start(CleanFreezeEpoch))>WndwSz
                            ...
                            AmbigZone = (Behav.FreezeEpoch-or(SafeZone,ShockZone))-RemovEpoch;
                            
                            MouseByMouse.ShockDur{Dir.ExpeInfo{d}{1}.nmouse}  = [MouseByMouse.ShockDur{Dir.ExpeInfo{d}{1}.nmouse},sum(Stop(ShockZone,'s')-Start(ShockZone,'s'))];
                            MouseByMouse.SafeDur{Dir.ExpeInfo{d}{1}.nmouse}  = [MouseByMouse.SafeDur{Dir.ExpeInfo{d}{1}.nmouse},sum(Stop(SafeZone,'s')-Start(SafeZone,'s'))];
                            ...
                            for s=1:length(Start(CleanFreezeEpoch))
                                ...
                                if  dur<(WndwSz*2-0.5*1e4) & dur>(WndwSz-0.5*1e4)
                                    ...
                                    MouseByMouse.LinPos{Dir.ExpeInfo{d}{1}.nmouse} = [MouseByMouse.LinPos{Dir.ExpeInfo{d}{1}.nmouse},nanmean(Data(Restrict(Behav.LinearDist,LitEpoch)))];
                                    ...
                                    MouseByMouse.Stim{Dir.ExpeInfo{d}{1}.nmouse} = [MouseByMouse.Stim{Dir.ExpeInfo{d}{1}.nmouse},StimType];
                                    MouseByMouse.Blocked{Dir.ExpeInfo{d}{1}.nmouse} = [MouseByMouse.Blocked{Dir.ExpeInfo{d}{1}.nmouse},Blocked];
                                    
                                    [~,Zone] = max([length((Restrict(Behav.Vtsd,and(ShockZone,LitEpoch)))),...
                                        length((Restrict(Behav.Vtsd,and(AmbigZone,LitEpoch)))),length((Restrict(Behav.Vtsd,and(SafeZone,LitEpoch))))]);
                                    MouseByMouse.WhichZone{Dir.ExpeInfo{d}{1}.nmouse} = [MouseByMouse.WhichZone{Dir.ExpeInfo{d}{1}.nmouse},Zone];
...
LinPos = [LinPos,MouseByMouse.LinPos{AllMice(m)}];
ZoneFz = [ZoneFz,MouseByMouse.WhichZone{AllMice(m)}];
IsBlocked = [IsBlocked,MouseByMouse.Blocked{AllMice(m)}];
StimType = [StimType,MouseByMouse.Stim{AllMice(m)}];
EarlyLateSk = [EarlyLateSk,cumsum(MouseByMouse.WhichZone{AllMice(m)}==1)];
EarlyLateSf = [EarlyLateSf,cumsum(MouseByMouse.WhichZone{AllMice(m)}==3)];
MouseID = [MouseID,ones(1,length(MouseByMouse.Stim{AllMice(m)}))*AllMice(m)];
    
end

LinPos(DataPCA(:,1)>7) = [];
EarlyLateSk(DataPCA(:,1)>7) = [];
EarlyLateSf(DataPCA(:,1)>7) = [];
ZoneFz(DataPCA(:,1)>7) = [];
IsBlocked(DataPCA(:,1)>7) = [];
StimType(DataPCA(:,1)>7) = [];
MouseID(DataPCA(:,1)>7) = [];
DataPCA(DataPCA(:,1)>7,:) = [];
...
for c = 1:3
    subplot(1,3,c)
    Score = coeff(:,c)'*dat(SelData,:)';
    nhist({Score(ZoneFz(SelData)==3),Score(ZoneFz(SelData)==1)})
    title(['PCA ' num2str(c)])
end
