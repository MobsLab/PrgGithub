...
for mm=1:length(MiceNumber)
    ...
    Sleep = load('/home/gruffalo/Dropbox/Mobs_member/SophieBagur/Figures/phD/ReactivationRipples/AllRippleInfo_DiffEpochs.mat','Ripples','Spikes')
    Q_Sleep = MakeQfromS(Sleep.Spikes{mm}.SleepPre,Binsize);
    Epoch.PostRipples_Sleep = mergeCloseIntervals(intervalSet(Range(Sleep.Ripples.SleepPre{mm})-0.05*1e4,Range(Sleep.Ripples.SleepPre{mm})+0.22*1e4),0.1*1e4);
    EpochNames = fieldnames(Epoch);
    k = length(EpochNames);
    QTemplate_Sleep = Restrict(Q_Sleep,Epoch.PostRipples_Sleep);
...
box off
set(gca,'FontSize',15,'linewidth',2)
legend({'PostRipples','PreRipples','SLeepPreRip'})
ylabel('Mean R')
...
% Quantification
figure
A = {nanmax(AllTriggeredStim{9}(:,10:30)'),nanmax(AllTriggeredStim{9}(:,45:65)'),nanmax(AllTriggeredStim{8}(:,10:30)'),nanmax(AllTriggeredStim{8}(:,45:65)')};
