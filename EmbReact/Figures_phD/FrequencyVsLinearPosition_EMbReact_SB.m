...
for ss=1:length(SessNames)
    ...
    for d=1:length(FinalResults.LinDistTempFz{ss})
        ...
        % the nanmean was changed to nanmax
...
axes(ha(1))
[val,ind] = max(SmoothDec(zscore(MeanFreqPTWV'),[0.5,2]));
imagesc(0:0.1:1,Fr,SmoothDec((MeanFreqPTWV(1:20,:)'),[0.5,2])); axis xy, ylim([0 10]), hold on
