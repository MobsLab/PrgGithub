...
[op,NamesOp]=FindNREMepochsML;
%NamesOp={'PFsupOsci','PFdeepOsci','BurstDelta','REM','WAKE','SWS','PFswa','OBswa'}
[Epochs,nameEpochs]=DefineSubStages(op);
%nameEpochs={'N1','N2','N3','REM','WAKE','SWS','swaPF','swaOB','TOTSleep'}
[SleepStages,FinalEpochs,nameFinal]=PlotPolysomnoML(Epochs{1},Epochs{3},Epochs{6},Epochs{4},Epochs{5});
...
[params,movingwin]=SpectrumParametersML('high');
for man=1:length(Dir.path)
    disp(' '),disp(Dir.path{man})
    cd(Dir.path{man})
    clear Sp t f LFP channel
    try
        ...
        disp('high PFCx Spectrum does not exist, computing ....')
        % do a simple spectrogram high frequencies
        ...
        if ~exist('SpectrumDataH','dir'),mkdir('SpectrumDataH');end
        save(['SpectrumDataH/Spectrum',num2str(channel)],'-v7.3','Sp','t','f','params','movingwin');
    end
    disp('Done')
end

%% mtspecgramc.m
% do a simple spectrogram low frequencies
[params,movingwin]=SpectrumParametersML('low');
LFP_temp=ResampleTSD(LFP,params.Fs);
[Sp,t,f]=mtspecgramc(Data(LFP_temp),movingwin,params);
save(['SpectrumDataL/Spectrum',num2str(channel)],'-v7.3','Sp','t','f','params','movingwin');
