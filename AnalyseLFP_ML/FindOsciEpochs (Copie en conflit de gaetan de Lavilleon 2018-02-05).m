%FindOsciEpochs.m
function [OsciEpoch,WholeEpoch,SWAEpoch,BurstEpoch,spindles,sdTH,ISI_th]=FindOsciEpochs(nameChannel,SWS,sdTH,ISI_th)

% e.g. OsciEpoch=FindOsciEpochs('PFCx_deep',SWSEpoch);
%
% inputs:
...
[tpeaks]=FindExtremPeaksML(LFP,sdTH,dropShortIntervals(SWS,1.5E4));

disp('        ObsExtremPeaksML')
[h,BurstEpoch]=ObsExtremPeaksML(tpeaks,ISI_th);
...
% <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    function [tpeaks,t_waveP,t_waveT,brst]=FindExtremPeaksML(lfp,thD,Epoch)
        ...
        % for 2Hz freq step [2-20Hz], for each epoch of Epoch
        ...
        t_waveT=tdeltaT(find(tdeltaT(:,1)+1E4<rg(end)&tdeltaT(:,1)-1E4>0),:);
...
% <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    function [Distrib,BurstEpoch,brst]=ObsExtremPeaksML(t1,ISI_th)
        ...
        brst = burstinfo(Range(t1,'s'), ISI_th);
    ...
    function [Spi,SWA,stdev]=FindSpindlesML(LFP,freq,Epoch,showingORnot)
        ...
        for i=1:length(Start(Epoch))
            ...
            try
                ...
                % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                [ripples,swa,stdev] = FindspindlesMarie(filtered,'durations',[100 350 200000],...
