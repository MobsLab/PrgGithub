...
FolderToSave='/media/DataMOBsRAIDN/ProjetNREM/Figures/AnalyseNREMsubstagesNew';
savFig=1; clear MATEP Dir
analyname='AnalySubStagesNewML.mat';
...
%     Dir=MergePathForExperiment(Dir,Dir3);
    ...
    for man=1:length(Dir.path)
        ...
        try
            ...
            op; opNew; noise ;disp('... loading epochs from NREMepochsML.m')
        ...
        if ~isempty(Range(Dpfc))        
            ...
            MATepochs(man,1:length(opNew))=opNew;
            MATepochs{man,length(opNew)+1}=noise;
    ...
    save([res,'/',analyname],'Dir','MATepochs','MATOSCI','MATZT','OptionSpiLow')
end
...
if ~exist('MATEP','var') || forceReDo==1
    ...
    for man=1:length(Dir.path)
        disp(Dir.path{man})
        [temp,nameEpochs]=DefineSubStagesNew(MATepochs(man,1:end-1),MATepochs{man,end},1,0,1);
    ...
    disp(['Adding in ',analyname])
    save([res,'/',analyname],'-append','MATEP','nameEpochs')
    disp('Done'); 
else
...
figure('Color',[1 1 1],'unit','Normalized','Position', [0.05 0.05 0.5 0.8]); numF(1)=gcf;
figure('Color',[1 1 1],'unit','Normalized','Position', [0.3 0.05 0.5 0.8]); numF(2)=gcf;
% delta
...
figure('Color',[1 1 1],'unit','Normalized','Position', [0.1 0.05 0.6 0.8]);numF=gcf;
% delta
for i=1:3
...
for f=1%:2
    Y=nan(length(nameEpochs),length(HourDay)-1);
    ...
    for op=1:5%length(nameEpochs),
        ...
        for mi=1:length(mice)
            ...
            %         subplot(3,length(nameEpochs),op),hold on,
            %         plot(HourDay(1:end-1),A1(mi,:),'Color',[0.5 0.5 0.5]);xlim(HourDay([1,end]))
            %         subplot(3,length(nameEpochs),length(nameEpochs)+op),hold on,
            %         plot(HourDay(1:end-1),A2(mi,:),'Color',[0.5 0.5 0.5]);xlim(HourDay([1,end]))
            %         subplot(3,length(nameEpochs),2*length(nameEpochs)+op),hold on,
        ...
        try
            ...
            plot(x,smooth(y,2),'Linewidth',2,'Color',colori(op,:)); line([min(x),max(x)],p(2)+[min(x),max(x)]*p(1),'Color','k')
            plot(x,smooth(y+stdError(A1),2),'Color',colori(op,:)); plot(x,smooth(y-stdError(A1),2),'Color',colori(op,:));
            xlim([9 20]);if dozscore, ylim([-1.5 1.5]);end; if op==1, ylabel([legzsc,' %TotRec']);end
...
%                 'Linewidth',3,'Color',colori(op,:));xlim([9 20]);if dozscore, ylim([-1.5 1.5]);end
            ...
            p= polyfit(x,y,1);[r,pval]=corrcoef(x,y);
            plot(x,smooth(y,2),'Linewidth',2,'Color',colori(op,:)); line([min(x),max(x)],p(2)+[min(x),max(x)]*p(1),'Color','k')
            plot(x,smooth(y+stdError(A2),2),'Color',colori(op,:)); plot(x,smooth(y-stdError(A2),2),'Color',colori(op,:));
            xlim([9 20]);if dozscore, ylim([-1.5 1.5]);end; if op==1, ylabel([legzsc,' %NREM']);end
            title(sprintf([nameEpochs{op},' r=%0.1f, p=%0.3f'],r(1,2),pval(1,2)),'Color',colori(op,:));
...
%                 'Linewidth',3,'Color',colori(op,:));xlim([9 20]);if dozscore, ylim([-1.5 1.5]);end
            ...
            p= polyfit(x,y,1);[r,pval]=corrcoef(x,y);
            plot(x,smooth(y,2),'Linewidth',2,'Color',colori(op,:)); line([min(x),max(x)],p(2)+[min(x),max(x)]*p(1),'Color','k')
            plot(x,smooth(y+stdError(A3),2),'Color',colori(op,:)); plot(x,smooth(y-stdError(A3),2),'Color',colori(op,:));
            xlim([9 20]);if dozscore, ylim([-1.5 1.5]);end; if op==1, ylabel([legzsc,' %TotSleep']);end
            title(sprintf([nameEpochs{op},' r=%0.1f, p=%0.3f'],r(1,2),pval(1,2)),'Color',colori(op,:));
            Y(op,:)=y;
        end
    end
