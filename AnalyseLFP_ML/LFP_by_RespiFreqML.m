function [FSp_pooled,MatrixSpStrain,MatrixSpMice,MatrixSpInfMice,MatrixSpSupMice]=LFP_by_RespiFreqML(NameDir,StructureName,NameEpoch,plotIndividual,Freq)

% function LFP_by_RespiFreqML
%
% inputs:
...
if ErasePreviousAnalysis || plotIndividual
    ...
    for man=1:length(Dir.path)
        ...
        if sum(strcmp(listLFP.name,StructName))~=0
            ...
            if ErasePreviousAnalysis
                % --------------------------------------------------------------
                % ----- load respi info ------------
                clear indexInf indexSup SmoFrequency
                
                SmoFrequency=SmoothDec(Data(Frequency),1);
                SmoFrequency=tsd(Range(Frequency),SmoFrequency);
                

                % epoch frequency
                clear epochFreqInf_temp1 epochFreqInf_temp2 epochFreqInf
                epochFreqInf_temp1=thresholdIntervals(SmoFrequency,FreqInf(1),'Direction','Above');
                epochFreqInf_temp2=thresholdIntervals(SmoFrequency,FreqInf(2),'Direction','Below');
                epochFreqInf=and(and(epochFreqInf_temp1,epochFreqInf_temp2),epoch);
                epochFreqInf=mergeCloseIntervals(epochFreqInf,4E3);
                epochFreqInf=dropShortIntervals(epochFreqInf,2E4);
                
                clear epochFreqSup_temp1 epochFreqSup_temp2 epochFreqSup
                epochFreqSup_temp1=thresholdIntervals(SmoFrequency,FreqSup(1),'Direction','Above');
                epochFreqSup_temp2=thresholdIntervals(SmoFrequency,FreqSup(2),'Direction','Below');
                epochFreqSup=and(and(epochFreqSup_temp1,epochFreqSup_temp2),epoch);
                epochFreqSup=mergeCloseIntervals(epochFreqSup,4E3);
                epochFreqSup=dropShortIntervals(epochFreqSup,2E4);
                ...
                subplot(3,1,2)
                plot(Range(Restrict(Frequency,epoch),'s'),Data(Restrict(Frequency,epoch)),'k')
                hold on, plot(Range(Restrict(SmoFrequency,epoch),'s'),Data(Restrict(SmoFrequency,epoch)),'b')

                subplot(3,1,2)
                
                ok=input('Are you satisfied with frequency periods (y/n)? ','s');
                ...
                AllEpoch{man,2}=epochFreqInf;
                AllEpoch{man,3}=epochFreqSup;
    ...
    if ErasePreviousAnalysis
        ...
        save([res,'/',ANALYNAME,'.mat'],'fi','AllEpoch','-append');
    end
end
