...
FolderSaveFig='/media/DataMOBsRAID5/ProjetAstro/SlowOscillationsML';
...
try
    ...
    if ~exist('channel','var') || ~exist('LFP','var')
        tempload=load([PathManipe,'/ChannelsToAnalyse/',Structure,'_',depth,'.mat'],'channel');
        eval(['load(''',PathManipe,'/LFPData/LFP',num2str(tempload.channel),'.mat'',''LFP'');'])
    end
    disp('Calculating spectrogram for high gamma')
    [paramsG,movingwinG]=SpectrumParametersML('SuperHigh');
    paramsG.Fs=1250;
    
    [SpG,tG,fG]=mtspecgramc(Data(LFP),movingwinG,paramsG);
end
...
try
    Start(Epoch60);
    if ~exist('Step60min','var'), Step60min=unique([Start(Epoch60,'s'),Stop(Epoch60,'s')]);end
catch
...
subplot(5,3,1:3),imagesc(ti,fAll,10*log10(SpAll)'), axis xy, caxis([20 65]);
title([nameManipe,' ',Structure,'_',depth,'    Spectrogramm [',num2str(fpass1(1)),'-',num2str(fpass1(2)),']Hz']);

subplot(5,3,4:6),imagesc(ti,fAll,10*log10(SpNorm)'), axis xy,
title(['Spectrogramm [',num2str(fpass1(1)),'-',num2str(fpass1(2)),']Hz normalized by sum(power all freq)']);

subplot(5,3,7:9),imagesc(ti,fAll,10*log10(SpNorm)'), axis xy,
title(['Spectrogramm [',num2str(fpass1(1)),'-',num2str(fpass1(2)),']Hz normalized by sum(power all freq)']);
...
save([PathManipe,'/AnlySlowOscillationML/',Structure,depth,'.mat'],'-append','SpG')
