...
ChLFP=[ChLFPRight(1:min(2,length(ChLFPRight))),ChLFPLeft(1:min(2,length(ChLFPLeft)))];
    ...
    for i=1:length(indexU)
        ...
        if plotPETH
            ...
            for ch=1:length(ChLFP)
                ...
                hold on, plot(tps/1E3,m/1E3-(ch-1)*3,colori{ch},'Linewidth',2)
            end
