...
if 0 % new
    analyname='AnalySubstagesN1notWake_New';
    ...
    for man=1:length(Dir.path)
        ...
        try
            ...
            if length(VarName)>2
                ...
                disp('loading spectrum from OB and PFCx...')
                % -----------------------
                % load gamma OB
                tsdGhi=tsd([],[]);
                clear Spectro
                ...
                disp('skip loading spectrum from OB and PFCx...')
            ...
            for n=1:length(DoEp)
                ...
                % get mtspectrum
                disp('calculating AverageSpectrogram')
...
for v=1:3
    ...
    matbar=nan(length(Dir.path),size(dt,2)); matbar1=matbar; matbar2=matbar;
    for man=1:length(Dir.path)
        matbar(man,:)=nanmean(dt(find(info(:,3)==man & info(:,2)==iW),:),1); % N1 after wake
        matbar1(man,:)=nanmean(dt(find(info(:,3)==man & info(:,2)~=iW & info(:,2)<6),:),1); % N1 after other
        matbar2(man,:)=nanmean(dt2(find(info2(:,3)==man & info2(:,2)<6),:),1); % WAKE after other
    end
