...
if DPCPX, 
    ANALYNAME='AnalyDPCPXfromSCRATCH';
    InjName={'Pre' 'VEH' 'DPCPX'};
     %nameSpectre={'Bulb','PFCx','PaCx','dHPC'};
    %nameSp={'BO','PFC','PA','HPC'};
    nameSp={'BO'};nameSpectre={'Bulb'};
    restrictInj={'Pre','VEH','DPCPX'};
    StrainTOpool='dKO'; %'WT'
    
elseif LPS, 
    ANALYNAME='AnalyLPSML';
    ...
    for man=numDir
        ...
        for ll=1:length(nameSpectre)
            ...
            if sum(strcmp(listLFP.name,nameSpectre{ll}))~=0
                ...
                [Spi,ti,fi]=ComputeSpectrogramML(LFP,movingwin,params,[res,'/',nameMan,'.mat'],[nameSpectre(ll),nameSp(ll)],info);
                % ------------------------------------------------------
                % ------------------------------------------------------ 
...
if ploSleep
    ...
    for i=1:4
        ...
        if length(ANOV)>1,
            ...
            title(['Strain ',ANOVgroup{1}]); ylabel(Ttitle{i});
            set(gca,'xtick',1:length(Injindex));set(gca,'xticklabel',InjName(Injindex));
