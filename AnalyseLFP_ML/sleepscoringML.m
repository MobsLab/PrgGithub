...
%         MovThresh=6E7; % valeur par défaut proposée avant 25.01.2017
        MovThresh=GetGammaThresh(Data(Mmov)); % modif Julie 25.01.2017
        MovThresh=exp(MovThresh);
        MovThreshDefault=MovThresh;
        disp('use Gaussian Fit to determine MovThresh')
    ...
    while Ok~='y'
        ...
        if Ok=='n'
%             MovThresh=input(['   threshold for movement (Default= ',sprintf('%1.1E',nanmean(Data(Mmov))+2*nanstd(Data(Mmov))),') : ']); % removed Julie 26.01.2018
             MovThresh=input(['   threshold for movement (Default thresh from gaussian fit = ',sprintf('%1.1E',MovThreshDefault),') : ']); % added Julie 26.01.2018
