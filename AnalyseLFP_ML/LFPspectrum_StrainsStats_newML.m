function [numF2,MatrixTitles,MatrixData,AllSpectrum]=LFPspectrum_StrainsStats_newML(NameDir,nameStructure,depthstructure,nameEpoch,FilterFrequencies,removeNoisyEpochs,FolderToSaveFigure)
...
%
% numF=LFPspectrum_StrainsStats_newML('BASAL',{'Bulb' 'BO'},'deep','MovEpoch',[2 4],1,'/home/karim/Dropbox/MOBS-ProjetAstro/ProjetBulbe/FiguresPosterFENS');
...
if length(nameStructure)>1
    ...
    StructName=nameStructure{1};
    StructNickName=nameStructure{1};
end

colori={'b','r','k','g','c','m','b','r','k','g','c','m','b','r','k','g','c','m'};
typo={ '-','-','-','-','-','-','--','--','--','--','--','--','-.','-.','-.','-.','-.','-.'};
scrsz = get(0,'ScreenSize');
figure('Color',[1 1 1],'Position',[2 scrsz(4)/2 scrsz(3)/4 scrsz(4)/3]), numF=gcf;
figure('Color',[1 1 1],'Position',[2 scrsz(4)/2 scrsz(3)/4 scrsz(4)/3]), numF2=gcf;
...
for man=1:length(Dir.path)
    ...
    clear MovEpoch  SWSEpoch REMEpoch NoiseEpoch GndNoiseEpoch WeirdNoiseEpoch InfoLFP
    ...
    FileToSave=[Dir.path{man},'/SpectrumDataL'];
    disp(['* * * ',nameMan,' * * *'])
    ...
    if ~exist('num_channel','var')
        try
            load([Dir.path{man},'/ChannelsToAnalyse/',StructName,'_',depthstructure],'channel');
            num_channel=channel;
            disp(['Analyzing channel ',num2str(num_channel),' (found in ChannelsToAnalyse/',StructName,'_',depthstructure,'.mat)'])
        catch
            disp(['No ',StructName,'_',depthstructure,' found in ChannelsToAnalyse'])
        end
    end
    ...
    if exist('num_channel','var') && ~isempty(num_channel)
        ...
        clear Sp t f LFP LFP_temp fi ti Spi
        try
            %if strcmp(Dir.name{man},'Mouse051'), error;end
            load([FileToSave,'/Spectrum',num2str(num_channel),'.mat'],'Sp','t','f','params','movingwin');
            %Spi=Sp*Dir.CorrecAmpli(man); ti=t; fi=f; params_out=params; movingwin_out=movingwin;
            Spi=Sp; ti=t; fi=f; params_out=params; movingwin_out=movingwin;
            if size(fi,2)==263
                disp(['... /Spectrum',num2str(num_channel),' has been loaded'])
                disp(['Correction for amplification = ',num2str(Dir.CorrecAmpli(man))])
                %if Dir.CorrecAmpli(man)~=1, error;end
            else
                copyfile([FileToSave,'/Spectrum',num2str(num_channel),'.mat'],[FileToSave,'/OldSpectrum',num2str(num_channel),'.mat'])
                disp(['... RE computing and saving /Spectrum',num2str(num_channel),' (old has been renamed OldSpectrum)'])
                load([Dir.path{man},'/LFPData/LFP',num2str(num_channel),'.mat'],'LFP')
                LFP_temp=ResampleTSD(LFP,params.Fs);
                [Sp,t,f]=mtspecgramc(Data(LFP_temp)*Dir.CorrecAmpli(man),movingwin,params);
                Spi=Sp; ti=t; fi=f;
        ...
        legendL{gg}=[legendL{gg},{[Dir.group{man},'-',Dir.path{man}(min(strfind(Dir.path{man},'Mouse'))+6:min(strfind(Dir.path{man},'Mouse'))+7),' ch',num2str(num_channel)]}];
        legend(legendL{gg});
        % ------------------------------------------------
        %         keyboard
        ...
        disp(['Problem for ',StructName,'_',depthstructure,' in ',Dir.path{man}])
        indexNan=[indexNan,man];
    end
...
disp(['Keyboard before saving figures in ',FolderToSaveFigure])
keyboard
saveFigure(numF,['ChosenChannels_MeanFilteredSpectrum_StrainEffect_',nameEpoch],FolderToSaveFigure);
saveFigure(numF2,['BILAN_MeanFilteredSpectrum_StrainEffect_',nameEpoch],FolderToSaveFigure);
