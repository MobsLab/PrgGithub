...
%                 Mmov=tsd(rg(1:10:end),val(1:10:end));
            ...
            try
                ...
                load StateEpoch.mat MovThresh
            end
            save([res,'/StateEpochSubStages.mat'],'-append','MovThresh');
        end
        % sleep
...
if exist('Immob','var')
    ...
    % --------------------------------------------------------------------- 
    % --------------------------- save ------------------------------------
    save([res,'/StateEpochSubStages.mat'],'-append','win','mergeSleep','dropSleep','ThetaEpoch','Immob','SIEpoch')
    save([res,'/StateEpochSubStages.mat'],'-append','WholeEpoch','noise','SWS','REM','WAKE');
end  
