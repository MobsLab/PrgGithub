...
if ~exist('MATEP','var') || forceReDo==1
    ...
    disp('Adding in AnalySubStagesML.mat')
    save([res,'/AnalySubStagesML.mat'],'-append','MATEP','nameEpochs')
    disp('Done'); 
else
...
for f=1:2
    ...
    if savFig, saveFigure(gcf,['NREMSubStagesQuantif',num2str(f)],FolderToSave);end
end
%% bar plot and stats for nb of rhythms
idchange=[1,2,3,6,4,5];
%idchange=1:size(MATDELT,2);

figure('Color',[1 1 1],'unit','Normalized','Position', [0.1 0.05 0.3 0.8]);
% delta
for i=1:3
...
if savFig,saveFigure(gcf,'NREMSubStagesQuantifRhythms',FolderToSave);end
