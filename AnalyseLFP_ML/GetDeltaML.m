%GetDeltaML
function [tDelta,DeltaEpoch]=GetDeltaML(NameStructure,Epoch,SWSEpoch,newmethod)
...
freqDelta=[1 5];
disp('GetDeltaML...')
if ~exist('newmethod','var')
    newmethod=0;
end
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% load if exists
try
    temp=load('AllDeltaPFCx.mat','tDelta','DeltaEpoch','Epoch','ch1');
    if newmethod, temp.ch1;end %new methods save sup and deep channels
    tDelta=temp.tDelta;
...
if ~exist('tDelta','var')
    try
        ...
        try
            eval(['ch1=load(''ChannelsToAnalyse/',NameStructure,'_deltadeep.mat'');'])
        catch
        ...
        try
            ...
            disp('No LFP sup found, detecting delta on deep channel only')
            eegSup=tsd(Range(eegDeep),zeros(length(Range(eegDeep)),1));
        end
        % change on 01/12/2016
        % eegDeep=temp1.LFP; 
        % eegSup=temp2.LFP;
        clear temp1 temp2
        ...
        if length(Factor)>1, Factor=0;end
        ...
        save AllDeltaPFCx tDelta DeltaEpoch SWSEpoch Epoch ch1 ch2
