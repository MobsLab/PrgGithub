...
% ------------ 450 ------------ 
Dir{5,1}='/media/DataMOBsRAID/ProjetNREM/Mouse450/20161010';
%Dir{5,2}='/media/DataMOBsRAID/ProjetNREM/Mouse450/20161011';
%Dir{5,3}='/media/DataMOBsRAID/ProjetNREM/Mouse450/20161012';

% ------------ 451 ------------ 
%Dir{6,1}='/media/DataMOBsRAID/ProjetNREM/Mouse451/20161010';
    ...
    for a=1:size(Dir,1)
        for i=1:3
            try
                ...
                %%%%%%%%%%%%%%%%%% IdentityNeuronsML %%%%%%%%%%%%%%%%%%%%%%
                [DirFigure,Epochs,NamesEp,tempFR,tempFRz,tempFR2,tempDurEp,tempZTFRz,tempZTFR,tempZTFR2] = IdentityNeuronsML(S(nN),option);
                movefile(DirFigure,sprintf([DirFigure(1:strfind(DirFigure,date)-1),date,'_M%d-',nameSessions{i}],indMAT(3)))
                % ------------------
                % add to MAT
                MATtemp(:,8)=tempFR;
                [m,B]=max(tempFRz');
                MATtemp(:,9)=B';
                MATtemp(:,10:10+2*length(NamesEp)-1)=[tempFR2,tempFRz];
                
                MATneuron=[MATneuron;MATtemp];
                
                AllZT(LNZT+(1:length(nN)),1:length(NamesEp),:)=tempZTFR;
                AllZTz(LNZT+(1:length(nN)),1:length(NamesEp),:)=tempZTFRz;
                LNZT=size(MATneuron,1);
                
                indM=[]; for n=1:length(NamesEp), indM=[indM ; indMAT,n];end
                DurEpZT=[DurEpZT; [indM, tempDurEp] ];
                
                % %%%%%%%%%%%%%%%%%%%%%%%%% PLOT %%%%%%%%%%%%%%%%%%%%%%%%%%
                if 1
                    figure('Color',[1 1 1],'Unit','Normalized','Position',[0.2 0.2 0.4 0.8]),
                    for n=1:length(NamesEp)
                        ttd=20*squeeze(tempZTFR(:,n,:));
                        ttdz=squeeze(tempZTFRz(:,n,:));
                        ...
                        if n>2
                            ...
                            if ~isempty(prefN)
                                ...
                                for nn=3:length(NamesEp)
                                    Prefttdz=squeeze(tempZTFRz(prefN,nn,:));
                                    errorbar(timeZT(1:end-1),nanmean(Prefttdz,1),stdError(Prefttdz),'Linewidth',(n==nn)+1,'Color',colori(nn,:))
                                end
                            end
                    ...
                    saveFigure(gcf,sprintf('AnalyORspikes_Day%dCond%d',a,i),saveFolder);
                end
                
                
                % %%%%%%%%%%%%%%%%%%%%%%%%% SAVE %%%%%%%%%%%%%%%%%%%%%%%%%%
                disp(['Saving in ',nameAnaly])
                save([res,'/',nameAnaly],'-append','AllZTz','AllZT','DurEpZT','MATneuron');
