...
if doNew==1
    ...
    nameAnaly='Analyse_meanSpectrumNREMStages_new.mat';
else
...
if dodKO
    ...
    nameAnaly='Analyse_meanSpectrumNREMStages_dKO.mat';
end
% <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
for n=2:5
    ...
    for mi=1:length(mice)
        ...
        MiPF(mi,:)=nanmean(squeeze(MATPF(ind,n,:)),1);
        MiOB(mi,:)=nanmean(squeeze(MATOB(ind,n,:)),1);
        MiHP(mi,:)=nanmean(squeeze(MATHP(ind,n,:)),1);
        end
    end
    % display PF
    subplot(2,3,3*(i-1)+1), hold on
    ...
    plot(Fsamp,nanmean(MiHP,1)+nanstd(MiHP)/sqrt(sum(~isnan(nanmean(MiHP,2)))),'Color',colori(n,:))
    plot(Fsamp,nanmean(MiHP,1)-nanstd(MiHP)/sqrt(sum(~isnan(nanmean(MiHP,2)))),'Color',colori(n,:))
...
%saveFigure(numF.Number,'AnalyseNREM_MeanPFSpectrum','/home/mobsyoda/Dropbox/MOBs_ProjetBulbe/FIGURES/FigureNREMstages');
        
% <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
%% <<<<<<<<<<<<<<<<<<<<<<<< OB freq depending on stages <<<<<<<<<<<<<<<<<<<

FolderToSave='/media/DataMOBsRAIDN/ProjetAstro/AnalyseNREMsubstages/meanSpectrumNREMstages';
...
save([res,'/',nameAnaly],'-append','MATf');
end
...
FolderToSave='/media/DataMOBsRAIDN/ProjetAstro/AnalyseNREMsubstages/CrossFreqCoupling/';
