...
if dofreez || dotrack
    ...
    try
        ...
        for a=1:length(nameSess)
            ...
            for i=1:length(list)
                % if the file is a '.mat' and its name correponds to the session name:
...
if doaccelero
    ...
    try
        ...
        try 
            load ([pwd '/behavResources','MovAcctsd']) 
        catch
            load ([pwd '\behavResources','MovAcctsd']) 
        end
...
%--------------------------------------------------------------------------
try
    Structure={'Bulb_sup','Bulb_deep','dHPC_sup','dHPC_deep','dHPC_rip','DG','PaCx_sup','PaCx_deep','PFCx_sup','PFCx_deep','Amyg','PiCx'};
    disp(' ');
    dodisp=0;
