function [DirFigure,Epochs,NamesEp,FR,FRez,FRe2,DurEp,ZTFRz,ZTFR,ZTFR2,timeZT,Windo] = IdentityNeuronsML(S,option)
%
...
try filPFCt=FilterLFP(Restrict(option.PFC,TrackEpoch),[5 8],2048);DoModu=1; disp('PFC 5-8Hz done.');end
try filHPCt=FilterLFP(Restrict(option.HPC,TrackEpoch),[5 8],2048);DoModu=1; disp('HPC 5-8Hz done.');end
...
for s=1:length(S)
    try
        ...
        if DoModu
            ...
            if doRip
                figure, [fh,sq,sweeps, rasterAx, histAx,dArea] = RasterPETH(S{s}, option.RIP, -5000, 7000); close
                subplot(3,5,10), plot(Range(sq,'ms'),Data(sq)); xlabel('Time of Ripples (ms)')
                title('Spike at HPC ripples')
            end
        ...
        % ---------------- Save figure ----------------------
        numF=gcf;
        saveFigure(numF,sprintf('IdentityNeuronsML_neuron%d',s),DirFigure)
        disp(sprintf('Saving figure for neuron %d',s)); pause(1)
        if numF.Number >25, close all;end
        
    catch
        disp(sprintf('Problem neuron %d',s)); keyboard; close
    end
    %keyboard
end
