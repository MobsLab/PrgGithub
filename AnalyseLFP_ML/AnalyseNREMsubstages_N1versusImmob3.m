% AnalyseNREMsubstages_N1versusImmob3.m
...
MATNameTrans={'WKoN1','SWSoN1','REMoN1','REMoWK','SWSoWK','MVoN1','MVoIMwk','IMwkoSWS'};
...
%                     end
            ...
            Still=Immob-REM;Still=mergeCloseIntervals(Still,10);
            Still=Immob-N2;Still=mergeCloseIntervals(Still,10);
            Still=Immob-N3;Still=mergeCloseIntervals(Still,10);
            WakeMov=WholeEpoch-Immob;WakeMov=mergeCloseIntervals(WakeMov,10);WakeMov=dropShortIntervals(WakeMov,1);
            WakeMov=(((WakeMov-noise)-REM)-N2)-N3;
            WakeMov=mergeCloseIntervals(WakeMov,10);
            WakeMov=dropShortIntervals(WakeMov,10);
            ...
            M(diff(M(:,1:2)')'< 5 | diff(M(:,[2,4])')'<1.5,:)=[]; % consider only stage if previous >5s & next >1.5s
            
            id_wkn1 = find(M(:,3)==1 & M(:,5)==2); % WK-N1
            id_swsn1 = find((M(:,3)==3 | M(:,3)==4) & M(:,5)==2); % SWS-N1
            id_REMn1 = find(M(:,3)==5 & M(:,5)==2); % REM-N1
            id_REMwk = find(M(:,3)==5 & M(:,5)==1); % REM-WK
            id_swswk = find((M(:,3)==3 | M(:,3)==4) & M(:,5)==1); % SWS-WK
            
            WKoN1=intervalSet((M(id_wkn1,2)-5)*1E4,(M(id_wkn1,2)+3)*1E4);% 5s avant - 3s après
            SWSoN1=intervalSet((M(id_swsn1,2)-5)*1E4,(M(id_swsn1,2)+3)*1E4);
            REMoN1=intervalSet((M(id_REMn1,2)-5)*1E4,(M(id_REMn1,2)+3)*1E4);
            REMoWK=intervalSet((M(id_REMwk,2)-5)*1E4,(M(id_REMwk,2)+3)*1E4);
            SWSoWK=intervalSet((M(id_swswk,2)-5)*1E4,(M(id_swswk,2)+3)*1E4);
            ...
            id_ImWsws = find(M(:,3)==3 & M(:,5)==4 ); % StillWake-sws 
            
            MVoN1=intervalSet((M(id_MvImN1,2)-5)*1E4,(M(id_MvImN1,2)+3)*1E4); % 3s avant - 3s après
            MVoIMwk=intervalSet((M(id_MvImW,2)-5)*1E4,(M(id_MvImW,2)+3)*1E4);
            IMwkoSWS=intervalSet((M(id_ImWsws,2)-5)*1E4,(M(id_ImWsws,2)+3)*1E4);
           ...
           % remove noise
            ...
            for n=1:length(MATNameTrans)
                ...
                title(sprintf(['PFC ',tt,' (n=%d)'],length(Start(epoch))))
                ...
                if sum(MeanE)~=0,plot([-3:6/(size(MeanE,1)-1):3],2*MeanE+15,'Color',[0.5 0.5 0.5],'Linewidth',2);end
                line([0 0],ylim,'Color',[0.5 0.5 0.5]); xlim([-3 3]); ylim([0 20])
                axis xy; caxis([35 50]); xlabel('Time (s)'); colormap('jet')
                tt=MATNameTrans{n}; it=strfind(tt,'o'); tt=[tt(1:it-1),' -> ',tt(it+1:end)];
                title(sprintf(['HPC ',tt,' (n=%d)'],length(Start(epoch))))
            end
            %%
            if saveFig
...
try 
    load AnalySubstagesN1versusImmob_GoodSpectrum.mat do 
    do(1);
catch
    for man=1:length(Dir.path)
