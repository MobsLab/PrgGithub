...
for DoControls = 1
    ...
    for mm = 1:length(enregistrements)
        ...
        mm
        % load SeepScoring
        load('SleepScoring_Accelero.mat','REMEpoch','Wake','SWSEpoch','SmoothTheta')
        if OnlyLongEpochs
        ...
        VarToMes.SmoothTheta = SmoothTheta;
        clear SmoothTheta
        ...
        SmoothTheta = tsd(Range(ThetaRatioTSD),runmean(Data(ThetaRatioTSD),ceil(3/median(diff(Range(ThetaRatioTSD,'s'))))));
        VarToMes.SmoothTheta_PFC = SmoothTheta;
        clear SmoothTheta_PFC
    ...
    AllTransitions = fieldnames(T.SmoothTheta);
    AllVar= fieldnames(T);
    ...
    cd /home/pinky/Documents/Figure_PapierMathilde/TransitionsBis
    ...
    for var = 1:length(AllVar)
        if not(strcmp(AllVar{var},'Sptsd_H') | strcmp(AllVar{var},'Sptsd_P'))
            ...
            saveas(fig.Number,['TransitionsFor',AllVar{var},'_',GroupNames{DoControls},'_Long',num2str(OnlyLongEpochs),'SmooTheta.fig'])
            saveas(fig.Number,['TransitionsFor',AllVar{var},'_',GroupNames{DoControls},'_Long',num2str(OnlyLongEpochs),'SmooThetaSmooTheta.png'])
    ...
    saveas(fig.Number,['HPCSpec',AllVar{var},'_',GroupNames{DoControls},'_Long',num2str(OnlyLongEpochs),'SmooTheta.fig'])
    saveas(fig.Number,['HPCSpec',AllVar{var},'_',GroupNames{DoControls},'_Long',num2str(OnlyLongEpochs),'SmooThetaSmooTheta.png'])
    ...
    saveas(fig.Number,['PFCSpec',AllVar{var},'_',GroupNames{DoControls},'_Long',num2str(OnlyLongEpochs),'SmooTheta.fig'])
    saveas(fig.Number,['PFCSpec',AllVar{var},'_',GroupNames{DoControls},'_Long',num2str(OnlyLongEpochs),'SmooThetaSmooTheta.png'])
...
for var = 1:length(AllVar)
    ...
    saveas(fig.Number,['CompareTransitions_',AllVar{var},'_Long',num2str(OnlyLongEpochs),'_SDS_Vs_CtrlSmooTheta.fig'])
    saveas(fig.Number,['CompareTransitions_',AllVar{var},'_Long',num2str(OnlyLongEpochs),'_SDS_Vs_CtrlSmooTheta.png'])
end


%% Compare the dynamics of PFC and HPC
AllVar = {'SmoothTheta','SmoothTheta_PFC'};
Cols = {'c','r'}
...
for grp = 1:2
    ...
    saveas(fig.Number,['CompareTransitions_HPCVsPFC_Long',num2str(OnlyLongEpochs),'_SDS_Vs_CtrlSmooTheta.fig'])
    saveas(fig.Number,['CompareTransitions_HPCVsPFC_Long',num2str(OnlyLongEpochs),'_SDS_Vs_CtrlSmooTheta.png'])
...
saveas(fig.Number,['Transitions_AccMiceAndEvents_Long',num2str(OnlyLongEpochs),'_SDS_Vs_CtrlSmooTheta.fig'])
saveas(fig.Number,['Transitions_AccMiceAndEvents_Long',num2str(OnlyLongEpochs),'_SDS_Vs_CtrlSmooTheta.png'])




%% Quantify transition on spectra
figure
Cols = {'c','r'}
fig = figure('Name','HPCVsPFC');
for v = 1:2
    load(['TransitionInfo_',GroupNames{v},'_Long',num2str(OnlyLongEpochs),'.mat'],'T','AllEv','AllEvMiceNum','AllEvPostDur','MiceAv','time_spec')
    
    
    ThetFreq = [6 8];
    DeltaFreq = [2 4];
    for tr = 1:length(AllTransitions)
        ThetaBand = [find(Spectro{3}>ThetFreq(1),1,'first'):find(Spectro{3}>ThetFreq(2),1,'first')];
        DeltaBand = [find(Spectro{3}>DeltaFreq(1),1,'first'):find(Spectro{3}>DeltaFreq(2),1,'first')];
        
        clear Spec ThetaDeltaRation_Spec
        for mm = 1:length(T.Sptsd_P.(AllTransitions{tr}))
            temp = squeeze(nanmean(T.Sptsd_H.(AllTransitions{tr}){mm},2));
            if size(temp,2)<2
            else
                temp = temp(:,1:100);
                Spec(mm,:,:) = temp;
                Spec(mm,:,:) = Spec(mm,:,:)./squeeze(nanmean(nanmean(Spec(mm,:,:))));
                Spec(Spec==0) = NaN;
                ThetaDeltaRation_Spec(mm,:) = nanmean(Spec(mm,ThetaBand,:),2)./nanmean(Spec(mm,DeltaBand,:),2);
                
            end
        end
        ThetaDeltaRation_Spec(ThetaDeltaRation_Spec==0) = NaN;
        subplot(2,4,tr)
        shadedErrorBar(time_spec.Sptsd_P(1:100),nanmean(ThetaDeltaRation_Spec),stdError(ThetaDeltaRation_Spec),Cols{v})
        hold on
        axis xy
        title(AllTransitions{tr})
        ylim([0.9 1.2])
        xlabel('Time to switch (s)')
        if tr==1
    ...
    for tr = 1:length(AllTransitions)
        ...
        clear Spec ThetaDeltaRation_Spec
        for mm = 1:length(T.Sptsd_P.(AllTransitions{tr}))
            temp = squeeze(nanmean(T.Sptsd_P.(AllTransitions{tr}){mm},2));
            if  size(temp,2)<2
            else
                temp = temp(:,1:100);
...
for v = 1:2
    ...
    for tr = 1:length(AllTransitions)
        ...
        ThetaDeltaRation_Spec(ThetaDeltaRation_Spec==0) = NaN;
        subplot(2,4,tr+4*(v-1))
        shadedErrorBar(time_spec.Sptsd_P(1:100),nanmean(ThetaDeltaRation_Spec),stdError(ThetaDeltaRation_Spec),Cols{1})
        hold on
        axis xy
        title(AllTransitions{tr})
        ylim([0.9 1.2])
    ...
    for tr = 1:length(AllTransitions)
        ...
        ThetaDeltaRation_Spec(ThetaDeltaRation_Spec==0) = NaN;
        subplot(2,4,tr+4*(v-1))
        shadedErrorBar(time_spec.Sptsd_P(1:100),nanmean(ThetaDeltaRation_Spec),stdError(ThetaDeltaRation_Spec),Cols{2})
        hold on
        axis xy
        title(AllTransitions{tr})
        ylim([0.8 1.05])
