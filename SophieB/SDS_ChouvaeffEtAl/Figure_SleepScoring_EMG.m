...
function Figure_SleepScoring_EMG(coeff_display, foldername)
...
try
    load('SleepScoring_EMG.mat','Wake','SWSEpoch','REMEpoch')
    WakeAcc = Wake;
    SWSEpochAcc = SWSEpoch;
    REMEpochAcc = REMEpoch;
catch
    ...
    try
        res=pwd;
        cd(foldername);
        saveas(SleepScoringFigure, 'SleepScoringEMG.png', 'png');
        cd(res);
    catch
        res=pwd;
        cd(foldername);
        saveas(SleepScoringFigure.Number, 'SleepScoringEMG.png', 'png');
        cd(res);
    end
