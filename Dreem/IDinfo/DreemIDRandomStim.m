...
%
%
% infos = DreemIDRandomStim(filereference)
%
% infos = DreemIDRandomStim(p, Dir)
%
%
% OPTIONAL ARGUMENT:
...
%
% SEE 
%   PlotIDDreemRandomStim
%
%


function infos = DreemIDRandomStim(filereference,Dir,varargin)
...
%recompute ?
savefile = fullfile(FolderRandomStimID, ['IdFigureData_' num2str(Dir.filereference{p}) '.mat']);
infos=cell(0);
if recompute
    save(savefile,'infos') 
...
%load slow waves
sw_file = fullfile(FolderRandomStimPreprocess, 'SlowWaves', ['SlowWaves_' num2str(Dir.filereference{p}) '.mat']);
if exist(sw_file,'file')==2
    load(sw_file);
else
   [SlowWaveEpochs, ~] = MakeSlowWavesDreemRecord(p,Dir,'savefolder',fullfile(FolderRandomStimPreprocess, 'SlowWaves'));
end
...
if recompute || ~exist('meancurves_stim','var')    
    [meancurves_stim, nb_stim, intensities] = DreemIDfunc_Stimcurves('signals',signals,'stimulations',stimulations);
    save(savefile,'-append', 'meancurves_stim', 'nb_stim', 'intensities', 'infos')
else
    disp('already done')
end
...
%% Substages of stim
disp('Stim Stages analysis')
load(savefile,'stage_event');
if recompute || ~exist('stage_event','var')    
    stage_event = DreemIDfunc_Stagetones('stimulations',stimulations,'hypnogram', StageEpochs);
    save(savefile,'-append', 'stage_event', 'infos')
else
    disp('already done')
end
...
if recompute || ~exist('t_spg','var')
    filespectro = fullfile(FolderRandomStimPreprocess, 'Spectrograms',num2str(Dir.filereference{p}), ['Spectro_' num2str(Dir.filereference{p}) '_ch_' num2str(infos.channel_specg) '.mat']);
