...
%
% SEE 
%   DreemIDRandomStim PlotIDDreemStimImpact
%
%


function infos = PlotIDDreemRandomStim(filereference)

%% Load Data
try
    datafile = fullfile(FolderRandomStimID,['IdFigureData_' num2str(filereference) '.mat']);
    load(datafile)
catch
...
%left
textbox_dim = [0.05 0.9 0.2 0.06];
table_dim = [0.05 0.72 0.205 0.17];

StimStage_Axes = axes('position', [0.05 0.53 0.18 0.17]);

MeanCurves_Axes = axes('position', [0.05 0.05 0.24 0.45]);

Phase1_Axes = axes('position', [0.28 0.29 0.22 0.2]);
Phase2_Axes = axes('position', [0.31 0.05 0.22 0.2]);
...
textbox_str = {['Subject ' num2str(infos.subject) ' - ' infos.date], [num2str(stat_sw.nb_tones) ' stimulations']}; 
...
nb_tones = stat_sw.nb_tones;
for ep=1:length(stage_event.tones)
    h = bar(ep, 100*stage_event.tones{ep}/nb_tones); hold on
    set(h,'FaceColor', colori{ep}), hold on
end
set(gca, 'XTickLabel',{'N1','N2','N3','REM','WAKE'}, 'XTick',1:5), hold on
title('Tones occurence'); ylabel('%')
...
try
    ...
    ylim([-100 100]), xlim([-1500 3000]), hold on
    ...
    lgd = legend(h, name_channels(ch_curves));
...
channel_phase = 1;
phase_tone = phase_event.tones{channel_phase};
rose(phase_tone,36);
...
channel_phase = 1;
phase_sham = phase_event.sham{channel_phase};
rose(phase_sham,36);
title('Sham Phase Histogram')
...
plot(density_curves.sham.x, density_curves.sham.y,'-',  'color',[0.7 0.7 0.7]), hold on,
