...
%
%
% infos = DreemIDSlowDyn(filereference)
%
% infos = DreemIDSlowDyn(p, Dir)
%
%
% OPTIONAL ARGUMENT:
...
function infos = DreemIDSlowDyn(filereference,Dir,varargin)
...
%recompute ?
savefile = fullfile(FolderSlowDynID,['IdFigureData_' num2str(Dir.filereference{p}) '.mat']);
infos=cell(0);
if recompute
    save(savefile,'infos') 
...
if recompute || ~exist('t_spg','var')
    filespectro = fullfile(FolderSlowDynPreprocess, 'Spectrograms',num2str(Dir.filereference{p}), ['Spectro_' num2str(Dir.filereference{p}) '_ch_' num2str(infos.channel_specg) '.mat']);
