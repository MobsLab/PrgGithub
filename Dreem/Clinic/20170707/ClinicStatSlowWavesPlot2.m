% ClinicStatSlowWavesPlot2
% 06.07.2017 KJ
%
% Delta Stat - only delta after tones or sham
% 
% 
%   see ClinicStatSlowWaves ClinicStatSlowWavesPlot1 ClinicStatSlowWavesPlot3
%
...
for p=1:length(quantity_res.filename)
    ...
    data.slope(p) = nanmean(quantity_res.slowwaves.slope{p}(sw_induced));
end
...
for cond=1:length(conditions)
    ...
    %data
    slowwaves{cond} = cell2mat(quantity_res.slowwaves.total(path_cond));

    success{cond} = data.success(path_cond);
...
figure, hold on
[~,eb] = PlotErrorBarN_KJ(slowwaves,'newfig',0,'barcolors',colori,'ShowSigstar',show_sig,'showPoints',show_points,'paired',paired);
...
%% Number of success tones
figure, hold on
[~,eb] = PlotErrorBarN_KJ(success,'newfig',0,'barcolors',colori,'ShowSigstar',show_sig,'showPoints',show_points,'paired',paired);
set(eb,'Linewidth',2); %bold error bar
title('Number of evoking tones'),
ylabel('Number of success Tones'),
set(gca, 'XTickLabel',conditions, 'XTick',1:numel(conditions),'FontName','Times','fontsize',12), hold on,
...
set(eb,'Linewidth',2); %bold error bar
title('AUC of Slow waves after event'),
ylabel('AUC'),
set(gca, 'XTickLabel',conditions, 'XTick',1:numel(conditions),'FontName','Times','fontsize',12), hold on,
...
set(eb,'Linewidth',2); %bold error bar
title('Amplitude of Slow waves after event'),
ylabel('µV'),
set(gca, 'XTickLabel',conditions, 'XTick',1:numel(conditions),'FontName','Times','fontsize',12), hold on,
...
set(eb,'Linewidth',2); %bold error bar
title('Slope of Slow waves after event'),
ylabel('µV/s'),
set(gca, 'XTickLabel',conditions, 'XTick',1:numel(conditions),'FontName','Times','fontsize',12), hold on,
