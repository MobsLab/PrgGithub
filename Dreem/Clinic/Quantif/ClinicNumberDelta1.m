...
% 
% 
%   see ClinicQuantitySleep ClinicEpisodeDuration
%
...
for cond=1:length(conditions)
    ...
    %data
    tones{cond} = cell2mat(quantity_res.tones.total(path_cond));
    success{cond} = cell2mat(quantity_res.success.total(path_cond));
    ...
    for p=1:length(path_cond)
        n3_duration{cond} = [n3_duration{cond} quantity_res.sleepstages.total{path_cond(p)}(3)/1E4]; 
    end
