% QuantifClinicSuccessSubstage3
% 15.01.2017 KJ
%
% Plot of the ratio of tones evoking slow waves
%   - Sleep stages = N3
%   - 1st success, 2nd success, 2-click sucess
%
%
%   see QuantifClinicSuccessSubstage QuantifClinicSuccessSubstage2 QuantifClinicSuccessSubstage4
%
            ...
            if strcmpi(success_res.condition{p},conditionsWithTones{cond})     
                ...
                if ranks(r)==3
                    ...
                    for i=1:length(success_res.rank_tones{p})-1
                       ...
                       if success_res.sleepstage_tone{p}(i)==n3_stage
                           if success_res.rank_tones{p}(i)==1 && success_res.rank_tones{p}(i+1)==2; %succession of first and second
                               nb_tones(end) = nb_tones(end) + 1;
                               if success_res.induce_slow_wave{p}(i)+success_res.induce_slow_wave{p}(i+1)>0; %at least one SW induced
...
labels={'Alone','First','Second','2-click'};
...
for cond=1:length(conditionsWithTones)
    ...
    set(gca, 'XTickLabel',labels, 'XTick',1:numel(labels),'YTick',0:20:70,'YLim',[0 80],'FontName','Times','fontsize',12), hold on,
