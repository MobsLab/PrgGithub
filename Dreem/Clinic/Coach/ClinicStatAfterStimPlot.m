...
%
%   see 
%       ClinicStatAfterStim ClinicStatAfterStimPlot2
%
...
for cond=1:length(conditions)
    ...
    for p=1:length(quantity_res.filename)
        if strcmpi(quantity_res.condition{p},conditions{cond})
            auc1{cond} = [auc1{cond} ; quantity_res.auc1{p}];
            amplitude1{cond} = [amplitude1{cond} ; quantity_res.amplitude1{p}];
            auc2{cond} = [auc2{cond} ; quantity_res.auc2{p}];
            amplitude2{cond} = [amplitude2{cond} ; quantity_res.amplitude2{p}];
            phasegroup{cond} = [phasegroup{cond} ; quantity_res.first.phasegroup{p}];
        end
    end
...
for cond=1:length(conditions)
    for i=1:size(phases_group,1)
        ...
        stat_stim.auc1{cond,i} = auc1{cond}(phasegroup{cond}==i);
        stat_stim.amplitude1{cond,i} = amplitude1{cond}(phasegroup{cond}==i);
        stat_stim.auc2{cond,i} = auc2{cond}(phasegroup{cond}==i);
        stat_stim.amplitude2{cond,i} = amplitude2{cond}(phasegroup{cond}==i);
...
for i=1:length(phases_group)
    ...
    [~,eb] = PlotErrorBarN_KJ(data,'newfig',0,'barcolors',colori,'columntest',[1 2;3 4],'ShowSigstar','sig','showPoints',0,'paired',0);
