...
%% When all is done, make Epochs
    function Done(~,~)
        ...
        if ~isempty(Incoh)
            ...
            save(strcat(SaveName, 'LongSleepScoringManual.mat'),'SWSEpoch','Wake','REMEpoch','Transition','SaveName')
...
%% General plotting function
    function DoPlot
        ...
        line([max(Range(Restrict(EMGLFP,intervalSet(begin*1e4,begin*1e4+winsize*1e4/3)),'s')) max(Range(Restrict(EMGLFP,intervalSet(begin*1e4,begin*1e4+winsize*1e4/3)),'s'))],[ (max(X)*1.2-min(X))/2+min(X) max(X)*1.2],'color','k')
        line([max(Range(Restrict(EMGLFP,intervalSet(begin*1e4+winsize*1e4/3,begin*1e4+2*winsize*1e4/3)),'s')) max(Range(Restrict(EMGLFP,intervalSet(begin*1e4+winsize*1e4/3,begin*1e4+2*winsize*1e4/3)),'s'))],[ (max(X)*1.2-min(X))/2+min(X) max(X)*1.2],'color','k')
        line([max(Range(Restrict(EMGLFP,intervalSet(begin*1e4+2*winsize*1e4/3,begin*1e4+3*winsize*1e4/3)),'s')) max(Range(Restrict(EMGLFP,intervalSet(begin*1e4+2*winsize*1e4/3,begin*1e4+3*winsize*1e4/3)),'s'))],[ (max(X)*1.2-min(X))/2+min(X) max(X)*1.2],'color','k')
