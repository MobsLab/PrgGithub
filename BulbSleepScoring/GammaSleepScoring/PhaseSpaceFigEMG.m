...
for file =1:m
    ...
    file/m
    cd(filename{file})
    clear Epoch smooth_Theta EMGData
    ...
    [C,I]=min([max(Range(smooth_Theta)),max(Range(EMGData))]);
    ...
    ghi_new=Restrict(EMGData,time);
    ghi_new=tsd(Range(theta_new),Data(ghi_new));
    %center on slow wave sleep
    GThresh{file}=log(EMG_thresh)-nanmean(log(Data(Restrict(ghi_new,SWSEpoch))));
...
for file=1:m
   ...
   plot(Xg{file},Yg{file}/sum(Yg{file}(Xg{file}<0.5)),'color',[0.6 0.6 0.6],'linewidth',1), hold on
    plot(GThresh{file},0.3*file/m,'k*')
    subplot(3,3,[1,4])
