function SleepScoreFigureAllOB(filename,PlotEp)
...
load(strcat(filename,'StateEpochSBAllOB'))
...
try
    ...
    theta_new=Restrict(smooth_1015,PlotEp);
catch
    ghi_new=smooth_ghi;
    theta_new=smooth_1015;   
end
...
for k=1:length(sleepstart)
    line([sleepstart(k)/1e4 sleepstop(k)/1e4],[19 19],'color',[0 1 0],'linewidth',5);
end
sleepstart=Start(SWSEpoch);
sleepstop=Stop(SWSEpoch);
for k=1:length(sleepstart)
    line([sleepstart(k)/1e4 sleepstop(k)/1e4],[19 19],'color',[1 0 0]  ,'linewidth',5);
end
sleepstart=Start(Wake);
sleepstop=Stop(Wake);
for k=1:length(sleepstart)
    line([sleepstart(k)/1e4 sleepstop(k)/1e4],[19 19],'color',[0 0 1]  ,'linewidth',5);
end
...
sleepstop=Stop(REMEpoch);
for k=1:length(sleepstart)
    line([sleepstart(k)/1e4 sleepstop(k)/1e4],[90 90],'color',[0 1 0],'linewidth',5);
end
sleepstart=Start(SWSEpoch);
sleepstop=Stop(SWSEpoch);
for k=1:length(sleepstart)
    line([sleepstart(k)/1e4 sleepstop(k)/1e4],[90 90],'color',[1 0 0]  ,'linewidth',5);
end
sleepstart=Start(Wake);
sleepstop=Stop(Wake);
for k=1:length(sleepstart)
...
[theText, rawN, x] =nhist(log(Data(Restrict(smooth_1015,sleepper))),'maxx',max(log(Data(Restrict(smooth_1015,sleepper)))),'noerror','xlabel','Beta power OB','ylabel',[]); axis xy
view(90,-90)
line([log(TenFif_thresh) log(TenFif_thresh)],[0 max(rawN)],'linewidth',4,'color','r')
...
try
saveFigure(h,'SleepScoringAllOB',filename)
catch
saveFigure(h.Number,'SleepScoringAllOB',filename)
end
