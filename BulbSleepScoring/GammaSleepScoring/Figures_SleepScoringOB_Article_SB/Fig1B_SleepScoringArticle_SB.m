...
for st=3
    fig=figure;
    temp=log(GetSpectraH{st,1}([1:3,5,8:10],:)./(meanvalH(st,[1:3,5,8:10])'*ones(1,32)));
    g=shadedErrorBar(f2,runmean(nanmean(temp),3),[stdError(temp);stdError(temp)],'b');
    hold on
    temp=log(GetSpectraH{st,2}([1:3,5,8:10],:)./(meanvalH(st,[1:3,5,8:10])'*ones(1,32)));
    g=shadedErrorBar(f2,runmean(nanmean(temp),3),[stdError(temp);stdError(temp)],'k');
    temp=log(GetSpectraH{st,3}([1:3,5,8:10],:)./(meanvalH(st,[1:3,5,8:10])'*ones(1,32)));
