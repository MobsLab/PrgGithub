...
subplot(7,1,[1:2])
 rg=Range(sptsdH,'s');rg=rg(1:1:end);
datH=Data(sptsdH);datH=datH(1:1:end,:);

imagesc(rg,fH,10*log10(datH')), axis xy, caxis([20 65]);
xlim([begin endin])
...
datb=Data(sptsdB);
clear datbnew
rg=Range(sptsdB,'s');rg=rg(1:100:end);
datbnew=datb(1:100:end,:);

for k=1:size(datb,2)
    datbnew(:,k)=runmean(datbnew(:,k),10);
end
imagesc(rg,fB,10*log10(datbnew')), axis xy, caxis([25 50]);
set(gca,'XTick',[],'TickLength',[0 0])
xlim([begin endin])
