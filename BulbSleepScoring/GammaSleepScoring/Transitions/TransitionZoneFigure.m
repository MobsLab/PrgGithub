...
for mm=1:m
    ...
    cd(filename{mm})
    load('StateEpochEMGSB.mat','SWSEpoch','Wake','REMEpoch')%     %Find transition zone
    TotDur=max([max(Stop(Wake)),max(Stop(SWSEpoch)),max(Stop(REMEpoch))])/1e4;
    ...
    %Wk to Sleep
    temp=dropShortIntervals(AbortTrans{3},0.2e4);
    DurE(1,nn)=mean((Stop(temp)-Start(temp))/1e4);
    
    %Sleep to Wk
    temp=dropShortIntervals(AbortTrans{2},0.2e4);
    DurE(2,nn)=mean((Stop(temp)-Start(temp))/1e4);
    
    %Sleep to Sleep
    temp=dropShortIntervals(AbortTrans{4},0.2e4);
    DurE(3,nn)=mean((Stop(temp)-Start(temp))/1e4);
    DurE(5,nn)=length(Start(temp))./TotDur;
    
    %Wake to Wake
    temp=dropShortIntervals(AbortTrans{1},0.2e4);
    DurE(4,nn)=mean((Stop(temp)-Start(temp))/1e4);
    DurE(6,nn)=length(Start(temp))./TotDur;
    nn=nn+1;
end
...
PlotErrorBar(DurE(1:2,:)')
[h,p]=ttest(DurE(1,:),DurE(2,:));
sigstar({[1,2]},p)
box off
ylabel('Transition duration')
title('EMG')
...
PlotErrorBar(DurE(3:4,:)')
[h,p]=ttest(DurE(3,:),DurE(4,:));
sigstar({[1,2]},p)
box off
ylabel('Transition duration')
title('EMG')
...
title('OB')
set(gca,'XTick',[1,2],'XTickLabel',{'S. to S.','W. to W.'})
PlotErrorBar(DurE(5:6,:)')
[h,p]=ttest(DurE(5,:),DurE(6,:));
sigstar({[1,2]},p)
box off
ylabel('Transition frequency')
title('EMG')
