...
for mm=1:m
    ...
    % REMEpoch
    if exist('SptsdH')>0
        SpH{mm,6}=nanmean(Data(Restrict(SptsdH,REMEpoch)));
    end
    if exist('SptsdPa')>0
        SpPa{mm,6}=nanmean(Data(Restrict(SptsdPa,REMEpoch)));
    end
    if exist('SptsdPF')>0
        SpPF{mm,6}=nanmean(Data(Restrict(SptsdPF,REMEpoch)));
    end
    if exist('SptsdB')
        SpB{mm,6}=nanmean(Data(Restrict(SptsdB,REMEpoch)));
    end
    if exist('SptsdHH')
        SpHH{mm,6}=nanmean(Data(Restrict(SptsdHH,REMEpoch)));
    end
    if exist('SptsdHPa')>0
        SpPaH{mm,6}=nanmean(Data(Restrict(SptsdHPa,REMEpoch)));
    end
    if exist('SptsdHPF')>0
        SpPFH{mm,6}=nanmean(Data(Restrict(SptsdHPF,REMEpoch)));
    end
    if exist('SptsdHB')>0
        SpBH{mm,6}=nanmean(Data(Restrict(SptsdHB,REMEpoch)));
    end
     save TransitionZoneAnalysisREM.mat SpBH SpB SpPaH SpPa SpPF SpPFH SpHH SpH Points SpPFHPost SpPaHPost SpHHPost SpBHPost SpPFPost  SpHPost SpBPost X Y Dur SpPaPost
...
for mm=1:m
    cd(filename2{mm})
    load TransitionZoneAnalysisREM.mat 
...
%REMEpoch to Sleep/REMEpoch to REMEpoch
...
%Sleep to REMEpoch/Sleep to Sleep
% Low frequency before transition
figure
subplot(141)
