...
Folder='G:\FEAR_DATA\FEAR_ManipTest_Nov2015\Data_Matlab\FEAR_EXT24_DATA';

list=dir(Folder);
a=1;
for i=1:length(list)
     if list(i).isdir==1&list(i).name(1)~='.'&list(i).name(1)=='F' % if the folder is indeed a mouse name folder
...
%% Freezing Imetronics
cd('G:\FEAR_DATA\FEAR_ManipTest_Nov2015\Data_imetronics\EXT');
...
for i=1:length(DATA_IMETRONICS)
    ...
    StartFr=TpsFr(find(diff(FrData)==1)+1)*(1E-3)-2; % +1 car décalage dû au diff  '-1' car Imetronics ne commence à compter le freezing qu' partir d'1sec
