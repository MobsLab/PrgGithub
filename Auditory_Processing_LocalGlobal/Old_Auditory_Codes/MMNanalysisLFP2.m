...
for i=1:length(LFP);   
    ...
    title(['local effect, channel',num2str(i),InfoLFP.structure(i)])
    ...
    hold on, plot(tps(1:end-1),SmoothDec((Mb-Eb),smo),'r') 
    hold on, plot(rg(p<pval),pr(p<pval),'gx')
    title(['Global effect, channel',num2str(i),InfoLFP.structure(i)])
    hold on, axis([-200 1300 -600 600])
    for a=0:150:600
        hold on, plot(a,min(axis):max(axis),'b','linewidth',1);
    end
    ...
    hold on, plot(tps(1:end-1),SmoothDec((Mb-Eb),smo),'r') 
    hold on, plot(rg(p<pval),pr(p<pval),'gx')
    title(['Global Omission effect, channel',num2str(i),InfoLFP.structure(i)])
    hold on, axis([-200 1300 -600 600])
    for a=0:150:600
        hold on, plot(a,min(axis):max(axis),'b','linewidth',1);
    end
