...
for i=1:7
figure('color',[1 1 1]),
[fh,sqSca{i},sweepsSca{i}] = RasterPETH(S{num}, Restrict(tstim1,EpochSca{i}), -deb, +fin,'BinSize',100,'Markers',{ts1},'MarkerTypes',{'c.'});title(['Neuron n',num2str(num), ' ', cellnames{num},' ',num2str(freqSca(i)),' Hz'])%, close
end
...
subplot(2,1,1), title(['Neuron n',num2str(num), ' ', cellnames{num},' 2500-17500 Hz'])
yl=ylim;
line([0 0],yl,'color','k')
clear MSca
for i=1:7
...
% line([-300 -300],yl,'color','k')
% line([-400 -400],yl,'color','k')
title(['Neuron n',num2str(num), ' ', cellnames{num},' 2500-17500 Hz'])
...
if sav
            ...
            figure('color',[1 1 1]), [fh,sq,sweeps] = RasterPETH(S{num}, ts(TlocalSTD), -6000, +6000,'BinSize',100);
            figure('color',[1 1 1]), [fh2,sq2,sweeps2] = RasterPETH(S{num}, ts(TlocalDEV), -6000, +6000,'BinSize',100);
            figure('color',[1 1 1]), [fh3,sq3,sweeps3] = RasterPETH(S{num}, ts(TglobalSTD), -6000, +6000,'BinSize',100);
            figure('color',[1 1 1]), [fh4,sq4,sweeps4] = RasterPETH(S{num}, ts(TglobalDEV), -6000, +6000,'BinSize',100);
            ...
            title([cellnames{num}, 'Local Effect'])
            yl=ylim;
            line([0 0],yl,'color','k')
            xlim([-550 550])
            ...
            for i=1:20
                try
             eval(['saveFigure(',num2str(i),',''Figure',num2str(num),'-',cellnames{num},'-Mouse039-10kHz-',num2str(i),''',''',filenameSav,''')'])
                end
