...
function [Vlt,firings,Ne,Nfs,No]=TortSpkWaveForm(t,Ek,Eapp,FSk,FSapp,NoiseLevel,Nnetwork)
...
try 
    ...
    Ne = Nec*Nmod;   Nfs = Nfsc*Nmod;  No = Noc*Nmod; N = Ne + Nfs + No;
catch
...
tON = 0;        	% time at which the weights are turned on (ms)  
...
nstdO = 50 * NoiseLevel;     % note that our OL-M model fires at ~3Hz spontaneously anyway, so we don't have to apply
