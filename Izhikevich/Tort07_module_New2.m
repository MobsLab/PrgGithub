...
t = 10000;         % length of simulation (ms)
tON = 0;        	% time at which the weights are turned on (ms)  par default 2000
tOFF = 10000;        % time at which the weights are turned off (ms)  par default 12000
tDopaON = 5000;        	% time at which dopamine is turned on (ms)  par default 6000
tDopaOFF = 10000;        % time at which dopamine is turned off (ms)  par default 10000
...
%     s_OX = s_OX * expsynO_X;
%     
%      WED(i)=w(ixE(Efired(k)),:)';
%      WFD(i)=w(ixFS(FSfired(k)),:)';
%      WOD(i)= w(ixO(Ofired(k)),:)';
%     
%     for k = 1:length(Efired)
%         %try
%         %WED(i)=w(ixE(Efired(k)),:)';
%         s_EX = s_EX + WED(i+D*10);  
%         %end
%     end
%         
%     for k = 1:length(FSfired)
