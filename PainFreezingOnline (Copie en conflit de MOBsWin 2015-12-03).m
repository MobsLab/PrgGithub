function [numF,filename]=PainFreezingOnline(~,~)
...
global JumpMatrix;JumpMatrix=[];
global RearMatrix;RearMatrix=[];
global LickMatrix;LickMatrix=[];
global PlotJump PlotRear PlotLick
...
Fig_Click=figure('name','ClickForBehaviour','menubar','none','tag','Behaviour Observation');
ClickButton(1)= uicontrol(Fig_Click,'style','pushbutton',...
    'units','normalized','position',[0.1 0.69 0.8 0.1],...
    'string','Licking Paw','callback', @LickPaw);
set(ClickButton(1),'enable','on','FontWeight','bold','BackgroundColor','m','ForegroundColor','k')
set(ClickButton(1),'enable','on','FontWeight','bold')
ClickButton(2)= uicontrol(Fig_Click,'style','pushbutton',...
    'units','normalized','position',[0.1 0.49 0.8 0.1],...
    'string','Rearing','callback', @Rearing);
set(ClickButton(2),'enable','on','FontWeight','bold','BackgroundColor','c','ForegroundColor','k')
set(ClickButton(2),'enable','on','FontWeight','bold')
ClickButton(3)= uicontrol(Fig_Click,'style','pushbutton',...
    'units','normalized','position',[0.1 0.29 0.8 0.1],...
    'string','Jump','callback', @Jumping);
set(ClickButton(3),'enable','on','FontWeight','bold','BackgroundColor','r','ForegroundColor','k')
set(ClickButton(3),'enable','on','FontWeight','bold')
ClickButton(4)= uicontrol(Fig_Click,'style','pushbutton',...
    'units','normalized','position',[0.3 0.1 0.4 0.05],...
    'string','Cancel','callback', @Cancel);
set(ClickButton(4),'enable','on','FontWeight','bold','BackgroundColor','b','ForegroundColor','w')
...
% -------------------------------------
%% Clicking Buttons
    function Rearing(obj,event)
        RearMatrix=[RearMatrix,chrono];
    end
    function LickPaw(obj,event)
        LickMatrix=[LickMatrix,chrono];
    end
    function Jumping(obj,event)
        JumpMatrix=[JumpMatrix,chrono];
        if length(JumpMatrix)==20
            enableTrack=0;
        end
    end
    function Cancel(obj,event)
        
        [C,I]=max([max(RearMatrix),max(LickMatrix),max(JumpMatrix)]);
        if I==1
            RearMatrix=RearMatrix(1:end-1);
        elseif I==2
            LickMatrix=LickMatrix(1:end-1);
        else I==3
            JumpMatrix=JumpMatrix(1:end-1);
        end
    end
% -----------------------------------------------------------------
...
%% track mouse
    function Track_Freeze(obj,event)
        ...
        hold off, PlotFreez=plot([1:10],[1:10],'-b');
        hold on, thimmobline=line([1,2000],[1 1]*th_immob,'Color','r');
        PlotJump=plot([1:2],th_immob*0.8,'r*');
        PlotRear=plot([1:2],th_immob*0.85,'c*');
        PlotLick=plot([1:2],th_immob*0.9,'m*');
        ...
        while enableTrack
            ...
            if StartChrono
                ...
                try 
                    ...
                    set(PlotFreez,'XData',immob_val(max(1,num_fr_f-maxfrvis):end,2),'YData',immob_val(max(1,num_fr_f-maxfrvis):end,1));      
                    set(PlotJump,'XData',JumpMatrix,'YData',th_immob*0.8*ones(1,length(JumpMatrix)));
                    set(PlotRear,'XData',RearMatrix,'YData',th_immob*0.85*ones(1,length(RearMatrix)));
                    set(PlotLick,'XData',LickMatrix,'YData',th_immob*0.9*ones(1,length(LickMatrix)));
        ...
        FinalTemp=0;
        if length(JumpMatrix)==10
            FinalTemp=inputdlg(   sprintf('Remove the mouse \n What temperature did you remove mouse?'));
        end
        ...
        save([name_folder,mark,'Behavior.mat'],'PosMat','frame_rate','FreezeEpoch','StimInfo','Movtsd','th_immob',...
            'JumpMatrix','LickMatrix','RearMatrix','imageRef','shape_ratio2','BW_threshold2','mask','smaller_object_size2','FinalTemp');
        pause(0.5)
        JumpMatrix=[];         LickMatrix=[];         RearMatrix=[];
        ...
        keyboard
        try,plot(JumpMatrix,max(Data(Movtsd))*ones(1,length(JumpMatrix)),'b*'), end
        try,plot(RearMatrix,max(Data(Movtsd))*ones(1,length(RearMatrix)),'r*'), end
        try,plot(LickMatrix,max(Data(Movtsd))*ones(1,length(LickMatrix)),'k*'), end
        title('Raw Data')
        legend({'mov','frz','jump','rear','lick'})
...
%% StartSession
    function StartSession(obj,event)
        ...
        try, fwrite(a,8);disp('Arduino ON');end
        tDeb = clock;
    end
...
%% OnlineGuiReglage
    function guireg_fig=OnlineGuiReglage(obj,event);
        ...
        function freeze_thresh(obj,event)
            th_immob = (get(slider_freeze,'value')*2000);
