...
for i=1:TriggerOnTheta
    ...
    LFPOB=LFP;
    LFPOB=FilterLFP(LFPOB,[7 12],1024);
    LFPOBHil=hilbert(Data(LFPOB));
    LFPOBPhase=angle(LFPOBHil);
    Phasetsd=tsd(Range(LFPOB),LFPOBPhase);
...
for i=1:LookAtRipProfile
    ...
    title('LFP')
    subplot(132)
    imagesc(M(:,1),[1:14],AllRip),hold on
    line([0 0],ylim,'color','k','linewidth',3)
    title('LFP')
    subplot(133)
    AllRipCSD=interp2(diff(diff(AllRip)),3);
    imagesc(M(:,1),[1:14],AllRipCSD),hold on
    line([0 0],ylim,'color','k','linewidth',3)
    title('CSD')
end
...
for i=1:MakeCohFigs
    ...
    figure
    subplot(231)
    imagesc(f,[1:16],AllCohPFCx),clim([0.4 0.7])
    title('Coh LFP HPC / LFP PFCx')
    xlabel('Frequency (Hz)')
    subplot(232)
    imagesc(f,[1:14],AllCohDiffPFCx),clim([0.4 0.7])
    title('Coh Diff HPC / LFP PFCx')
    xlabel('Frequency (Hz)')
    subplot(233)
    imagesc(f,[1:14],AllCohCSDPFCx),clim([0.4 0.7])
    title('Coh CSD HPC / LFP PFCx')
    xlabel('Frequency (Hz)')
    
    subplot(234)
    imagesc(f,[1:16],AllCohOB),clim([0.4 0.7])
    title('Coh LFP HPC / LFP OB')
    xlabel('Frequency (Hz)')
    subplot(235)
    imagesc(f,[1:14],AllCohDiffOB),clim([0.4 0.7])
    title('Coh CSD HPC / LFP OB')
    xlabel('Frequency (Hz)')
    subplot(236)
    imagesc(f,[1:14],AllCohCSDOB),clim([0.4 0.7])
    title('Coh CSD HPC / LFP OB')
    xlabel('Frequency (Hz)')
