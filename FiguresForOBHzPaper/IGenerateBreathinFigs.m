...
saveas(fig,'RespiOBcoherence.fig'), close all
fig=figure;
line([0.7 1.3],[1 1]*nanmean(ValsNoFz),'color','k','linewidth',2), hold on
line([1.7 2.3],[1 1]*nanmean(ValsFz),'color','k','linewidth',2)
plotSpread({ValsNoFz,ValsFz},'distributionColors',[Cols1(2,:);Cols1(1,:)]), hold on
...
H=sigstar({[1,2]},p);set(H(1),'Color','w');set(H(2),'FontSize',14)
set(gca,'XTickLabel',{'NoFz','Fz'})
saveas(fig,'RespiOBcoherenceQuantif.fig'), close all
...
saveas(fig,'RespiPFCxcoherence.fig'), close all
fig=figure;
line([0.7 1.3],[1 1]*nanmean(ValsNoFz),'color','k','linewidth',2), hold on
line([1.7 2.3],[1 1]*nanmean(ValsFz),'color','k','linewidth',2)
plotSpread({ValsNoFz,ValsFz},'distributionColors',[Cols1(2,:);Cols1(1,:)]), hold on
...
H=sigstar({[1,2]},p);set(H(1),'Color','w');set(H(2),'FontSize',14)
set(gca,'XTickLabel',{'NoFz','Fz'})
saveas(fig,'RespiPFCxcoherenceQuantif.fig'), close all
...
for mm=1:length(Dir.path)
    ...
    load('behavResources.mat')
    AllFreq.NoFz{mm}=Data(Restrict(Frequecytsd,NoFreezeEpoch-BreathNoiseEpoch));
    AllFreq.Fz{mm}=Data(Restrict(Frequecytsd,FreezeEpoch-BreathNoiseEpoch));
    ...
    clear Frequecytsd BreathNoiseEpoch NoFreezeEpoch FreezeEpoch
end
...
[p,h]=signrank(MeanFreq(:,1),MeanFreq(:,2));
H=sigstar({[1,2]},p);set(H(1),'Color','w');set(H(2),'FontSize',14)
set(gca,'XTickLabel',{'NoFz','Fz'})
xlim([0 3])
saveas(fig,'MeanFreqbr.fig')
...
set(handles{1}(1),'MarkerSize',20)
set(handles{1}(2),'MarkerSize',20)
[p,h,stat]=signrank(StdFreq(:,1),StdFreq(:,2));
H=sigstar({[1,2]},p);set(H(1),'Color','w');set(H(2),'FontSize',14)
set(gca,'XTickLabel',{'NoFz','Fz'})
xlim([0 3])
ylabel('Std frequency (Hz)')
saveas(fig,'StdFreqbr.fig')
...
set(handles{1}(1),'MarkerSize',20)
set(handles{1}(2),'MarkerSize',20)
[p,h,stat]=signrank(MeanVol(:,1),MeanVol(:,2));
H=sigstar({[1,2]},p);set(H(1),'Color','w');set(H(2),'FontSize',14)
set(gca,'XTickLabel',{'NoFz','Fz'})
xlim([0 3])
ylabel('nanmean tidal Volume (AU)')
saveas(fig,'MeanTidalVol.fig')
...
set(handles{1}(1),'MarkerSize',20)
set(handles{1}(2),'MarkerSize',20)
[p,h,stat]=signrank(StdVol(:,1),StdVol(:,2));
H=sigstar({[1,2]},p);set(H(1),'Color','w');set(H(2),'FontSize',14)
set(gca,'XTickLabel',{'NoFz','Fz'})
xlim([0 3])
ylabel('Std tidal Volume (AU)')
set(gca,'FontSize',14,'YTick',[800:400:1600])
saveas(fig,'StdTidalVol.fig')

figure
clear AllY
for mm=1:length(Dir.path)
[Y,X]=hist(AllFreq.NoFz{mm},[0:0.2:15]);
Y=Y/sum(Y);
stairs(X,Y,'color',[0.6 0.6 0.6],'linewidth',2), hold on
AllY.NoFz(mm,:)=Y;

[Y,X]=hist(AllFreq.Fz{mm},[0:0.2:15]);
Y=Y/sum(Y);
stairs(X,Y,'color',[49,106,179]/263,'linewidth',2), hold on
AllY.Fz(mm,:)=Y;
end
Ytemp=mean(AllY.Fz);
line([1 1].*X(find(Ytemp==max(Ytemp))),ylim,'color','b','linewidth',3)
Ytemp=mean(AllY.NoFz);
line([1 1].*X(find(Ytemp==max(Ytemp))),ylim,'color','k','linewidth',3)
box off
set(gca,'FontSize',20)