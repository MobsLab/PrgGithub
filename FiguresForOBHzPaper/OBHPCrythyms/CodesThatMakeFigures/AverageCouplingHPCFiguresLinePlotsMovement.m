...
for mm=1:length(Dir.path)
    ...
    if exist('LFPData/LocalHPCActivity.mat')>0
        ...
        % HPC non local - PFCx
        load('MovPhaseCouplingHPC1_PFCx.mat')
        FinalSig1=GetPhaseCouplingSig(Index,IndexRand,FreqRange);
        load('MovPhaseCouplingHPC2_PFCx.mat')
        ...
        % HPC  local - PFCx
        load('MovPhaseCouplingHPCLoc_PFCx.mat')
        ...
        %HPC local - OB
        try
            load('MovPhaseCouplingOB1_HPCLoc.mat')
            FinalSig1=GetPhaseCouplingSig(Index,IndexRand,FreqRange);
            load('MovPhaseCouplingOB2_HPCLoc.mat')
            FinalSig2=GetPhaseCouplingSig(Index,IndexRand,FreqRange);
            FinalSig.VectLength=(FinalSig1.VectLength+FinalSig2.VectLength)/2;
            FinalSig.Shannon=(FinalSig1.Shannon+FinalSig2.Shannon)/2;
...
ylabel('Vect Length'), ylim([0 0.9]), xlim([2 13])
subplot(2,2,3)
AvCouplingAndSignifPlot(AllHPCnonlocPFCX.Shannon,FreqRange,[1 0 0],0.15,0.005)
AvCouplingAndSignifPlot(AllHPClocPFCX.Shannon,FreqRange,[0 0 1],0.1,0.005)
title('Local/NonLocal HPC wi PFCx')
ylabel('Entropy'), ylim([0 0.2]), xlim([2 13])
subplot(2,2,4)
AvCouplingAndSignifPlot(AllHPCnonlocOB.Shannon,FreqRange,[1 0 0],0.15,0.005)
AvCouplingAndSignifPlot(AllHPClocOB.Shannon,FreqRange,[0 0 1],0.1,0.005)
title('Local/NonLocal HPC wi OB')
ylabel('Entropy'), ylim([0 0.2]), xlim([2 13])
