function [Index,IndexRand,Phase]=PhaseCouplingSlowOscillSameFreq(Sig1,Sig2,FreqRange,Epoch,DoStats,DoFig)
...
for f=1:size(FreqRange,2)
    ...
    for ff=f
        ...
        S=-sum(Y1.*log(Y1));
        Index.Shannon(1,f)=(Smax-S)/Smax;
        Index.VectLength(1,f)=sqrt(sum(cos(AllPhaseDiff)).^2+sum(sin(AllPhaseDiff)).^2)/length(AllPhaseDiff);
        ...
        if DoStats>0
            for sh=1:DoStats
                ...
                Y=Y/sum(Y);
                S=-sum(Y.*log(Y));
                IndexRand.Shannon{1,f}(sh)=(Smax-S)/Smax;
                IndexRand.VectLength{1,f}(sh)=sqrt(sum(cos(AllPhaseDiff)).^2+sum(sin(AllPhaseDiff)).^2)/length(AllPhaseDiff);
                if DoFig
...
if   DoStats>1
    ...
    for f=1:size(FreqRange,2)
        for ff=1:size(FreqRange,2)
            Perc95=prctile(IndexRand.Shannon{1,f},99);
            if Index.Shannon(1,f)>Perc95
                FinalSig.Shannon(1,f)=Index.Shannon(1,f);
            end
            Perc95=prctile(IndexRand.VectLength{1,f},99);
            if Index.VectLength(1,f)>Perc95
                FinalSig.VectLength(1,f)=Index.VectLength(1,f);
            end
        end
    end
end
