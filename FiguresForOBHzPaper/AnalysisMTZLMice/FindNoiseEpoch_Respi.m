...
% 21.11.2017 SB
%
%  [Epoch,TotalNoiseEpoch,SubNoiseEpoch,Info]=FindNoiseEpoch_SleepScoring(channel_respi, foldername)
%
...
% channel_respi       : HPC channel, spectrum will be calculated if necessary
...
function [Epoch,TotalNoiseEpoch,SubNoiseEpoch,Info]=FindNoiseEpoch_Respi(channel_respi, varargin)
...
% Get HPC spectrum
if ~(exist([foldername '/' 'Respi_Low_Spectrum.mat'], 'file') == 2)
    LowSpectrumSB(foldername, channel_respi,'H');
end
load(strcat(foldername, '/Respi_Low_Spectrum.mat'))
fH = Spectro{3};
tH = Spectro{2};
SpH = Spectro{1};
...
load([foldername '/LFPData/LFP' num2str(channel_respi) '.mat'])
