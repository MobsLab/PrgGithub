        ...
        for dd = 1:length(Dir.path{d})
            ...
            if not(isempty(numNeurons))
                ...
                for chan = 1: length(channel_accelero)
                    ...
                    times = Start(and(mergeCloseIntervals(AccBurst,5*1e4),ImmobilityEpoch));
                    ...
                    if not(StrongEvents)
                        save(['NeuronResponseToMovement/NeuronPredictedPhaseMovementEventsNEWNeurBinSize',num2str(BinsizeNeur),'_ZScore',strrep(num2str(ZscoreThresh),'.',','),'chan',num2str(channel_accelero(chan)),'.mat'],'Index','BinsizeNeur','muPred','KappaPred','times','SaveNeurResp','hPhase')
                    else
                        save(['NeuronResponseToMovement/NeuronPredictedPhaseMovementEventsNEWStrongCorrNeurBinSize',num2str(BinsizeNeur),'_ZScore',strrep(num2str(ZscoreThresh),'.',','),'chan',num2str(channel_accelero(chan)),'.mat'],'BinsizeNeur','Index','muPred','KappaPred','times','SaveNeurResp','hPhase')
                    end
