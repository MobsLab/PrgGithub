...
for d = 1:length(Dir.path)
    for dd = 1:length(Dir.path{d})
        ...
        if (exist('NeuronModulation/NeuronMod_Respi_ZeroCross_total_nonoise'))>0 & isempty(numNeurons)==0
            ...
            clf
            for ep = 1:5
                [mat,ind] = sortrows([nanmean(EvokPotOB{ep}(:,600:650)')',(EvokPotOB{ep})]);
                AllTimes = Range(Restrict(Breathtsd,Epoch{ep}));
                SortedTimes = AllTimes(ind);
                
                subplot(5,4,(ep-1)*4+1)
                imagesc(zscore(mat')'), clim([-3 3])
                TenPercNum = floor(size(mat,1)/10);
                line([1 1]*200,[1 TenPercNum],'linewidth',2,'color','b')
                line([1 1]*200,[(TenPercNum*4.5),floor(TenPercNum*5.5)],'linewidth',2,'color','g')
                line([1 1]*200,[size(mat,1)-TenPercNum size(mat,1)],'linewidth',2,'color','r')
                ...
                EvokOB = tsd(AllTimes,EvokPotOB{ep});
                ...
                PlotErrorBarN_KJ([Kappa_Up{ep}(pval_All{ep}<0.05);Kappa_Down{ep}(pval_All{ep}<0.05);Kappa_Mid{ep}(pval_All{ep}<0.05)]','newfig',0)
                set(gca,'XTick',[1:3],'XTickLabel',{'Up','Down','Mid'})
           ...
           % saveas(fig,['/home/vador/Dropbox/Mobs_member/SophieBagur/Figures/MTZLmice/NeuronModulation/SleepSession/EffectOfOBResponseToBreathingOnNeuronLocking_',num2str(num),'.png'])
           % saveas(fig,['/home/vador/Dropbox/Mobs_member/SophieBagur/Figures/MTZLmice/NeuronModulation/SleepSession/EffectOfOBResponseToBreathingOnNeuronLocking_',num2str(num),'.fig'])
            save('NeuronModulation/EffectOfOBResponseToBreathingOnNeuronLocking.mat','mu_All','mu_Mid','mu_Up','mu_Down',...
                'pval_All','pval_Mid','pval_Up','pval_Down','Kappa_All','Kappa_Mid','Kappa_Down','Kappa_Down')
...
for ep=1:5
    ...
    PlotErrorBarN_KJ(AllKappa.SALINE(2:4,Allpval.SALINE(1,:)<0.05)','newfig',0)
    set(gca,'XTick',[1:3],'XTickLabel',{'Up','Mid','Down'})
    title(NameEpoch{ep})
    ylabel('Kappa - SALINE')
    subplot(5,3,(ep-1)*3+2)
    PlotErrorBarN_KJ(AllKappa.METHIMAZOLE(2:4,Allpval.METHIMAZOLE(1,:)<0.05)','newfig',0)
    set(gca,'XTick',[1:3],'XTickLabel',{'Up','Mid','Down'})
    ylabel('Kappa - MTZL')
