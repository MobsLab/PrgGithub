...
for d = 1:length(Dir.path)
    for dd = 1:length(Dir.path{d})
        ...
        if (exist('NeuronModulation/NeuronMod_Respi_ZeroCross_total_nonoise'))>0 & isempty(numNeurons)==0
            ...
            freq = f;
            Cohtsd = tsd(t*1e4,C);
            maxTps = max(Range(Frequecytsd,'s'));
            Tps = [0:1:maxTps];
            Freq_interp = interp1(Range(Frequecytsd,'s'),Data(Frequecytsd),[0:1:maxTps]);
            Freq_interp_binned = discretize(Freq_interp,freq);
            Coh_interp = interp1(Range(Cohtsd,'s'),Data(Cohtsd),[0:1:maxTps]);
            Tps(find(isnan(Freq_interp_binned))) = [];
            Coh_interp(find(isnan(Freq_interp_binned)),:) = [];
            Freq_interp_binned(find(isnan(Freq_interp_binned))) = [];
            
            Coh_BreathFreq = Coh_interp(sub2ind(size(Coh_interp),1:length(Freq_interp_binned),Freq_interp_binned));
            
            
            Ctsd = tsd(Tps*1e4,Coh_BreathFreq');
            ...
            saveas(fig,[dropbox '/Mobs_member/SophieBagur/Figures/MTZLmice/NeuronModulation/SleepSession/EffectOfOBREspiCohRespiFreqToBreathingOnNeuronLocking_',num2str(Dir.ExpeInfo{d}{dd}.nmouse),'_',num2str(Dir.ExpeInfo{d}{dd}.date),'.png'])
            saveas(fig,[dropbox '/Mobs_member/SophieBagur/Figures/MTZLmice/NeuronModulation/SleepSession/EffectOfOBREspiCohRespiFreqToBreathingOnNeuronLocking_',num2str(Dir.ExpeInfo{d}{dd}.nmouse),'_',num2str(Dir.ExpeInfo{d}{dd}.date),'.fig'])
            save('NeuronModulation/EffectOfOBRespiCohRespiFreqOnNeuronLocking.mat','mu_Coh','Kappa_Coh', 'pval_Coh')
