...
for ev = 1:3
    switch AfterOnly
        case 'AferOnly'
            for i=0:TimeAroundEvent*10
                [hPhase{ev}(i+1,:),b]=hist(Data(Restrict(PhaseInterpol,intervalSet(EventType{ev}+(i-1)*1E3,EventType{ev}+i*1E3))),[0.05:0.1:6.25]);
            end
        case 'BefAndAfter'
            for i=-TimeAroundEvent*10:TimeAroundEvent*10
                [hPhase{ev}(i+TimeAroundEvent*10+1,:),b]=hist(Data(Restrict(PhaseInterpol,intervalSet(EventType{ev}+(i-1)*1E3,EventType{ev}+i*1E3))),[0.05:0.1:6.25]);
            end
    end
    hPhase{ev}(:,1) = [];
    hPhase{ev}(:,end) = [];
...
fig = figure(1);
mkdir(['NeuronResponseToMovement/ModulationAroundAccEvents_' AfterOnly '_' NormBreathPhase])
for n = 1 :size(Qdat,2)
    ...
    for ev = 1:3
        ...
        switch AfterOnly
            case 'AferOnly'
            AngDist{ev} = nanmean(hPhase{ev}.*repmat(MSp{ev}(21:end,2),1,61));
            case 'BefAndAfter'
    ...
    saveas(fig.Number,['NeuronResponseToMovement/ModulationAroundAccEvents_' AfterOnly '_' NormBreathPhase '/' cellnames{n} '.png'])
    saveas(fig.Number,['NeuronResponseToMovement/ModulationAroundAccEvents_' AfterOnly '_' NormBreathPhase '/' cellnames{n} '.fig'])
    
        clf 
end

save(['NeuronResponseToMovement/ModulationAroundAccEvents_' AfterOnly '_' NormBreathPhase '/ModulationAroundAccEvents_' AfterOnly '_' NormBreathPhase '.mat'], 'muData','KappaData','pvalData','muPred','KappaPred','pvalPred')
