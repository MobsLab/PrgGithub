...
for mm=6%:length(Dir.path) % mouse 1 has no reliable PFCx
    cd(Dir.path{mm})
    ...
    tempsum=cumsum(Stop(FreezeEpoch,'s')-Start(FreezeEpoch,'s'));
    LastEpToKeep=find(tempsum>200,1,'first');
    if isempty(LastEpToKeep), LastEpToKeep=length(Stop(FreezeEpoch,'s'));end
    FreezeEpoch=subset(FreezeEpoch,[1:LastEpToKeep]);
    
    NoFreezeEpoch=NoFreezeEpoch-TotalNoiseEpoch;NoFreezeEpoch=dropShortIntervals(NoFreezeEpoch,3*1e4);
    tempsum=cumsum(Stop(NoFreezeEpoch,'s')-Start(NoFreezeEpoch,'s'));
    LastEpToKeep=find(tempsum>200,1,'first');
    if isempty(LastEpToKeep), LastEpToKeep=length(Stop(NoFreezeEpoch,'s'));end
    NoFreezeEpoch=subset(NoFreezeEpoch,[1:LastEpToKeep]);
    Ep{1}=FreezeEpoch; Ep{2}=NoFreezeEpoch,
    load('ChannelsToAnalyse/Bulb_deep.mat')
    load(['LFPData/LFP',num2str(channel),'.mat'])
    FilLFP=FilterLFP((LFP),[1 80],1024);
    tps=Range(FilLFP);dat=Data(FilLFP);
    DataLFP.OB=tsd(tps(1:10:end),dat(1:10:end))
    load('ChannelsToAnalyse/PFCx_deep.mat')
    load(['LFPData/LFP',num2str(channel),'.mat'])
    FilLFP=FilterLFP((LFP),[1 80],1024);
    tps=Range(FilLFP);dat=Data(FilLFP);
    DataLFP.P=tsd(tps(1:10:end),dat(1:10:end))
    load('ChannelsToAnalyse/Respi.mat')
    load(['LFPData/LFP',num2str(channel),'.mat'])
    FilLFP=FilterLFP((LFP),[1 80],1024);
    tps=Range(FilLFP);dat=Data(FilLFP);
    DataLFP.Respi=tsd(tps(1:10:end),dat(1:10:end))
    clear LFP FilLFP
    ...
    for ep=1:2
        ...
        for st=1:length(Start(Ep{ep}))
            begin=Start(subset(Ep{ep},st));
            endin=Stop(subset(Ep{ep},st));
            if endin-begin<6.2*1e4
                LitEp{ep}{n}=subset(Ep{ep},st);
                n=n+1;
            else
                dur=endin-begin;
                numbins=round(dur/(3*1E4));
                epdur=(dur/1E4)/numbins;
                for nn=1:numbins
                    startcounting=begin+(nn-1)*dur/numbins+0.1*1e4;
                    stopcounting=begin+nn*dur/numbins;
                    LitEp{ep}{n}=intervalSet(startcounting,stopcounting);
                    n=n+1;
                end
            end
        end
        
        for st=1:length(LitEp{ep})
            LitEpToUse=(LitEp{ep}{st});
