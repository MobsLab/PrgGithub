%function SynchroCohen(lfp1,lfp2)


EEG.times=Range(lfp1,'ms');
EEG.data(1,:,:)=Data(lfp1);
EEG.data(2,:,:)=Data(lfp2);
EEG.srate=1/median(diff(Range(lfp1,'s')));
EEG.trials=1;
...
for chani=1:2
    fft_data = fft(squeeze(EEG.data(chani,:,1)),n_convolution);
...
data_fft2 = fft(reshape(EEG.data(2,:,:),1,n_data),n_convolution);
...
%     trialdata1 = EEG.data(1,:,triali);
%     trialdata2 = EEG.data(2,:,triali);
%     
...
% initialize
spectcoher = zeros(length(freqs2use),length(EEG.times));
...
trial2plot = 1; % any trial between 1 and 99 (book uses trial 10)
...
for idx=animate_start:animate_stop
    ...
    % update angles
    for i=1:length(hpol)
        set(hpol(i),'XData',[0; cos(angle(xsd(idx+i)))],'YData',[0; sin(angle(xsd(idx+i)))]);
    end
    title([ num2str(round(EEG.times(idx))) '-' num2str(round(EEG.times(idx+i))) ' ms' ])

    % compute ICPS and dwPLI
    ispc(idx) = abs(mean(exp(1i*angle(xsd(idx:idx+i))),1));
