...
for p=1:length(Dir.path)
    ...
    Res.delay_delta_in{p} = delay_delta_tone;
    Res.delay_down_in{p} = delay_down_tone;
    ...
    Res.delay_delta_out{p} = delay_delta_tone;
    Res.delay_down_out{p} = delay_down_tone;   
...
delay_delta_in = [];
for p=1:length(Res.path)
    delay_delta_in = [delay_delta_in ; Res.delay_delta_in{p}];
end
delay_down_in = [];
for p=1:length(Res.path)
    delay_down_in = [delay_down_in ; Res.delay_down_in{p}];
end
delay_delta_out = [];
for p=1:length(Res.path)
    delay_delta_out = [delay_delta_out ; Res.delay_delta_out{p}];
end
delay_down_out = [];
for p=1:length(Res.path)
    delay_down_out = [delay_down_out ; Res.delay_down_out{p}];
end
...
edges = 0:20:400;
figure, hold on
subplot(2,2,1),histogram(delay_delta_in/10,edges), title('delta waves (in burst)'),hold on,
subplot(2,2,2),histogram(delay_down_in/10,edges), title('down states (in burst)'),hold on,
subplot(2,2,3),histogram(delay_delta_out/10,edges), title('delta waves (no burst)'),hold on,
subplot(2,2,4),histogram(delay_down_out/10,edges), title('down states (no burst)'),hold on,
suplabel('Delay after tone','t');
