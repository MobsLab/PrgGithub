% Figure7StimEffectSleepStructurePlot4
% 05.12.2016 KJ
%
% Collect data to plot the figures from the Figure7.pdf (f-g) of Gaetan PhD
%   - Same as Figure7StimEffectSleepStructurePlot3, but with Random condition
% 
% 
%   see Figure7StimEffectSleepStructure Figure7StimEffectSleepStructurePlot3
%
...
thresh_high_s1 = 0.8; %  threshold for considering high delta frequency S1
...
for p=1:length(figure7_res.path)
    if strcmpi(figure7_res.manipe{p}, 'RdmTone')
        delta_density_random_s1s2 = [delta_density_random_s1s2 ; squeeze(figure7_res.delta.density(p,1:2))];
    end
end
...
for p=1:length(figure7_res.path)
    if strcmpi(figure7_res.manipe{p}, 'RdmTone')
        delta_density_random_s3s4 = [delta_density_random_s3s4 ; squeeze(figure7_res.delta.density(p,3:4))];
    end
end
