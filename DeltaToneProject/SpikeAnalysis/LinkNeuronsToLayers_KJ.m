...
for p=1:length(Dir.path)
    ...
    %% Link neurons and channels
    load('SpikeData.mat', 'tetrodeChannels', 'TT')


    tetrodeLayers = [];
    for i=1:length(tetrodeChannels)
        idx = ismember(channels,tetrodeChannels{i});
        if any(idx)
            tetrodeLayers(i) = clusters(idx);
        end
    end
    ...
    for n=1:length(TT)    
        try
            neuronsLayers(n) = tetrodeLayers(TT{n}(1));
        catch
            neuronsLayers(n) = nan;
        end
    end

    save('SpikeData.mat', '-append', 'tetrodeLayers', 'neuronsLayers')
