...
for p=1:length(Dir_sham.path)
    ...
    for i=1:nb_sham
        idx_delta_before = find(end_deltas < sham_tmp(i), 1,'last');
        delay_delta_sham(i) = sham_tmp(i) - end_deltas(idx_delta_before);    
    end        
...
for p=1:length(Dir_rdm.path)
    ...
    for i=1:nb_tones
        idx_delta_before = find(end_deltas < tones_tmp(i), 1,'last');
        delay_delta_tone(i) = tones_tmp(i) - end_deltas(idx_delta_before);    
    end        
