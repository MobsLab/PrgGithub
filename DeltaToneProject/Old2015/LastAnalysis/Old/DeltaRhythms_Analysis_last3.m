...
for a=1:length(Path_M243_basal)
    ...
    load behavResources
    startSleep=Start(SWSEpoch);startSleep=startSleep(1);
    endSleep=Stop(SWSEpoch);endSleep=endSleep(end);
    DurSWSEpoch=endSleep-startSleep;
    
    for i=1:5
        EpochA{i}=intervalSet(startSleep+((DurSWSEpoch/5)*(i-1)),startSleep+((DurSWSEpoch/5)*(i))); 
        EpochSWS{i}=and(EpochA{i},SWSEpoch); 
        EpochREM{i}=and(EpochA{i},REMEpoch); 
    end
    
    % restrict Delta occurence to the 5 succesive Sleep epochs
    load([res,'/newDelta',struc]);
    for i=1:5
        Delta_M243_basal{a,i}=Restrict(ts(tDelta),EpochSWS{i});
        DurDeltaEpoch_M243_basal{a,i}=tot_length(EpochSWS{i},'s');
        DurREMEpoch_M243_basal{a,i}=tot_length(EpochREM{i},'s');
    end
end
...
for a=1:length(Path_M244_basal)
    ...
    load([res,'/newDelta',struc]);
    for i=1:5
        Delta_M244_basal{a,i}=Restrict(ts(tDelta),SWSEpoch{i});
        DurDeltaEpoch_M244_basal{a,i}=tot_length(SWSEpoch{i},'s');
        DurREMEpoch_M244_basal{a,i}=tot_length(EpochREM{i},'s');
    end
end
...
for a=1:length(Path_Mouse243_Delta)
    ...
    for i=1:5
        EpochREM{i}=and(EpochSleep{i},REMEpoch); 
    end
