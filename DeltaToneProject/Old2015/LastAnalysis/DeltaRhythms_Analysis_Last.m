...
for a=1:length(Path_Mouse243_Delta)
    ...
    load([res,'/newDelta',struc]);
    for i=1:5
        Delta_M243_Tone{a,i}=Restrict(ts(tDelta),EpochAnalysed{i});
        DurSWSEpoch_M243_Tone{a,i}=tot_length(and(SWSEpoch,EpochAnalysed{i}),'s');
        DurREMEpoch_M243_Tone{a,i}=tot_length(and(REMEpoch,EpochAnalysed{i}),'s');
        Dur5Epoch_M243_Tone{a,i}=tot_length(EpochAnalysed{i},'s');
        RatioSWS_M243_Tone{a,i}=DurSWSEpoch_M243_Tone{a,i}/Dur5Epoch_M243_Tone{a,i};
        RatioREM_M243_Tone{a,i}=DurREMEpoch_M243_Tone{a,i}/Dur5Epoch_M243_Tone{a,i};
    end
end
...
for a=1:length(Path_Mouse244_Delta)
    ...
    load([res,'/newDelta',struc]);
    for i=1:5
        Delta_M244_Tone{a,i}=Restrict(ts(tDelta),EpochAnalysed{i});
        DurSWSEpoch_M244_Tone{a,i}=tot_length(and(SWSEpoch,EpochAnalysed{i}),'s');
        DurREMEpoch_M244_Tone{a,i}=tot_length(and(REMEpoch,EpochAnalysed{i}),'s');
        Dur5Epoch_M244_Tone{a,i}=tot_length(EpochAnalysed{i},'s');
        RatioSWS_M244_Tone{a,i}=DurSWSEpoch_M244_Tone{a,i}/Dur5Epoch_M244_Tone{a,i};
        RatioREM_M244_Tone{a,i}=DurREMEpoch_M244_Tone{a,i}/Dur5Epoch_M244_Tone{a,i};
    end
end
...
for i=1:length(Path_M243_basal)
    for j=1:5
        SWS_basal(i,j)=DurSWSEpoch_M243_basal{i,j};
        SWS_basal2(i,j)=DurSWSEpoch_M244_basal{i,j};
    end
end
for i=1:length(Path_Mouse243_Delta)
    for j=1:5
        SWS_Tone(i,j)=DurSWSEpoch_M243_Tone{i,j};
        SWS_Tone2(i,j)=DurSWSEpoch_M244_Tone{i,j};
    end
end
% SWS Duration (mean & normalized)
SWSb=(SWS_basal./(SWS_basal(:,1)*ones(1,5))+SWS_basal2./(SWS_basal2(:,1)*ones(1,5)))/2;
SWSt=(SWS_Tone./(SWS_Tone(:,1)*ones(1,5))+SWS_Tone2./(SWS_Tone2(:,1)*ones(1,5)))/2;

% SWS Ratio
for i=1:length(Path_M243_basal)
    for j=1:5
        SWSr_basal(i,j)=RatioSWS_M243_basal{i,j};
        SWSr_basal2(i,j)=RatioSWS_M244_basal{i,j};
    end
end
for i=1:length(Path_Mouse243_Delta)
    for j=1:5
        SWSr_Tone(i,j)=RatioSWS_M243_Tone{i,j};
        SWSr_Tone2(i,j)=RatioSWS_M244_Tone{i,j};
    end
end
...
for i=1:length(Path_M243_basal)
    for j=1:5
        Mf(i,j)=M(i,j)/DurSWSEpoch_M243_basal{i,j};
        Mf2(i,j)=M2(i,j)/DurSWSEpoch_M244_basal{i,j};
    end
end
for i=1:length(Path_Mouse243_Delta)
    for j=1:5
        Tf(i,j)=T2(i,j)/DurSWSEpoch_M243_Tone{i,j};
        Tf2(i,j)=T2(i,j)/DurSWSEpoch_M244_Tone{i,j};
    end
end
...
for i=1:length(Path_M243_basal)
    for j=1:5
        REM_basal(i,j)=DurREMEpoch_M243_basal{i,j};
        REM_basal2(i,j)=DurREMEpoch_M244_basal{i,j};
    end
end
for i=1:length(Path_Mouse243_Delta)
    for j=1:5
        REM_Tone(i,j)=DurREMEpoch_M243_Tone{i,j};
        REM_Tone2(i,j)=DurREMEpoch_M244_Tone{i,j};
    end
end
% REM Duration (mean & normalized)
REMb=(REM_basal./(REM_basal(:,1)*ones(1,5))+REM_basal2./(REM_basal2(:,1)*ones(1,5)))/2;
REMt=(REM_Tone./(REM_Tone(:,1)*ones(1,5))+REM_Tone2./(REM_Tone2(:,1)*ones(1,5)))/2;

% REM Ratio
for i=1:length(Path_M243_basal)
    for j=1:5
        REMr_basal(i,j)=RatioREM_M243_basal{i,j};
        REMr_basal2(i,j)=RatioREM_M244_basal{i,j};
    end
end
for i=1:length(Path_Mouse243_Delta)
    for j=1:5
        REMr_Tone(i,j)=RatioREM_M243_Tone{i,j};
        REMr_Tone2(i,j)=RatioREM_M244_Tone{i,j};
    end
end
...
hold on, title('Mouse 243 - Delta Quantity')
PlotErrorBarN(M2)
...
hold on, title('Mouse 244 - Delta Quantity')
...
hold on, title('Mouse 243 - Delta Quantity (normalized)')
...
hold on, title('Mouse 244 - Delta Quantity (normalized)')
...
hold on, plot(1:5,t(4,:),'mo-')
hold on, plot(1:5,t(5,:),'ro-')
hold on, title('Both Mice - Delta Quantity (normalized)')
...
% SWS Duration
PlotErrorBarN(SWS_basal)
hold on, plot(1:5,SWS_Tone(1,:),'go-')
hold on, plot(1:5,SWS_Tone(2,:),'co-')
hold on, plot(1:5,SWS_Tone(3,:),'bo-')
hold on, plot(1:5,SWS_Tone(4,:),'mo-')
hold on, plot(1:5,SWS_Tone(5,:),'ro-')
hold on, title('Mouse 243 - SWS Duration')

PlotErrorBarN(SWS_basal2)
hold on, plot(1:5,SWS_Tone2(1,:),'go-')
hold on, plot(1:5,SWS_Tone2(2,:),'co-')
hold on, plot(1:5,SWS_Tone2(3,:),'bo-')
hold on, plot(1:5,SWS_Tone2(4,:),'mo-')
hold on, plot(1:5,SWS_Tone2(5,:),'ro-')
hold on, title('Mouse 244 - SWS Duration')

% SWS Ratio
PlotErrorBarN(SWSr_basal)
hold on, plot(1:5,SWSr_Tone(1,:),'go-')
hold on, plot(1:5,SWSr_Tone(2,:),'co-')
hold on, plot(1:5,SWSr_Tone(3,:),'bo-')
hold on, plot(1:5,SWSr_Tone(4,:),'mo-')
hold on, plot(1:5,SWSr_Tone(5,:),'ro-')
hold on, title('Mouse 243 - SWS Ratio')

PlotErrorBarN(SWSr_basal2)
hold on, plot(1:5,SWSr_Tone2(1,:),'go-')
hold on, plot(1:5,SWSr_Tone2(2,:),'co-')
hold on, plot(1:5,SWSr_Tone2(3,:),'bo-')
hold on, plot(1:5,SWSr_Tone2(4,:),'mo-')
hold on, plot(1:5,SWSr_Tone2(5,:),'ro-')
hold on, title('Mouse 244 - SWS Ratio')

% SWS (mean & normalized)
PlotErrorBarN(SWSb)
hold on, plot(1:5,SWSt(1,:),'go-')
hold on, plot(1:5,SWSt(2,:),'co-')
hold on, plot(1:5,SWSt(3,:),'bo-')
hold on, plot(1:5,SWSt(4,:),'mo-')
hold on, plot(1:5,SWSt(5,:),'ro-')
hold on, title('Both Mice - SWS Quantity (normalized)')
...
% REM Duration
PlotErrorBarN(REM_basal)
hold on, plot(1:5,REM_Tone(1,:),'go-')
hold on, plot(1:5,REM_Tone(2,:),'co-')
hold on, plot(1:5,REM_Tone(3,:),'bo-')
hold on, plot(1:5,REM_Tone(4,:),'mo-')
hold on, plot(1:5,REM_Tone(5,:),'ro-')
hold on, title('Mouse 243 - REM Duration')

PlotErrorBarN(REM_basal2)
hold on, plot(1:5,REM_Tone2(1,:),'go-')
hold on, plot(1:5,REM_Tone2(2,:),'co-')
hold on, plot(1:5,REM_Tone2(3,:),'bo-')
hold on, plot(1:5,REM_Tone2(4,:),'mo-')
hold on, plot(1:5,REM_Tone2(5,:),'ro-')
hold on, title('Mouse 244 - REM Duration')

% REM Ratio
PlotErrorBarN(REMr_basal)
hold on, plot(1:5,REMr_Tone(1,:),'go-')
hold on, plot(1:5,REMr_Tone(2,:),'co-')
hold on, plot(1:5,REMr_Tone(3,:),'bo-')
hold on, plot(1:5,REMr_Tone(4,:),'mo-')
hold on, plot(1:5,REMr_Tone(5,:),'ro-')
hold on, title('Mouse 243 - REM Ratio')

PlotErrorBarN(REMr_basal2)
hold on, plot(1:5,REMr_Tone2(1,:),'go-')
hold on, plot(1:5,REMr_Tone2(2,:),'co-')
hold on, plot(1:5,REMr_Tone2(3,:),'bo-')
hold on, plot(1:5,REMr_Tone2(4,:),'mo-')
hold on, plot(1:5,REMr_Tone2(5,:),'ro-')
hold on, title('Mouse 244 - REM Ratio')

% REM (mean & normalized)
PlotErrorBarN(REMb)
hold on, plot(1:5,REMt(1,:),'go-')
hold on, plot(1:5,REMt(2,:),'co-')
hold on, plot(1:5,REMt(3,:),'bo-')
hold on, plot(1:5,REMt(4,:),'mo-')
hold on, plot(1:5,REMt(5,:),'ro-')
hold on, title('Both Mice - REM Quantity (normalized)')
...
% Delta Frequency
PlotErrorBarN(Mf)
hold on, plot(1:5,Tf(1,:),'go-')
hold on, plot(1:5,Tf(2,:),'co-')
hold on, plot(1:5,Tf(3,:),'bo-')
hold on, plot(1:5,Tf(4,:),'mo-')
hold on, plot(1:5,Tf(5,:),'ro-')
hold on, title('Mouse 243 - Delta Wave Frequency')

PlotErrorBarN(Mf2)
hold on, plot(1:5,Tf2(1,:),'go-')
hold on, plot(1:5,Tf2(2,:),'co-')
hold on, plot(1:5,Tf2(3,:),'bo-')
hold on, plot(1:5,Tf2(4,:),'mo-')
hold on, plot(1:5,Tf2(5,:),'ro-')
hold on, title('Mouse 244 - Delta Wave Frequency')

% Delta Frequency (normalized)
PlotErrorBarN(Mf./(Mf(:,1)*ones(1,5)))
hold on, plot(1:5,Tf(1,:)/Tf(1,1),'go-')
hold on, plot(1:5,Tf(2,:)/Tf(2,1),'co-')
hold on, plot(1:5,Tf(3,:)/Tf(3,1),'bo-')
hold on, plot(1:5,Tf(4,:)/Tf(4,1),'mo-')
hold on, plot(1:5,Tf(5,:)/Tf(5,1),'ro-')
hold on, title('Mouse 243 - Delta Wave Frequency')

PlotErrorBarN(Mf2./(Mf2(:,1)*ones(1,5)))
hold on, plot(1:5,Tf2(1,:)/Tf2(1,1),'go-')
hold on, plot(1:5,Tf2(2,:)/Tf2(2,1),'co-')
hold on, plot(1:5,Tf2(3,:)/Tf2(3,1),'bo-')
hold on, plot(1:5,Tf2(4,:)/Tf2(4,1),'mo-')
hold on, plot(1:5,Tf2(5,:)/Tf2(5,1),'ro-')
hold on, title('Mouse 244 - Delta Wave Frequency')
...
hold on, plot(1:5,t(4,:),'mo-')
hold on, plot(1:5,t(5,:),'ro-')
hold on, title('Both Mice - Delta Wave Frequency')


figure('color',[1 1 1]), 
subplot(3,2,1)
plot(Tf(:,1),Tf(:,2)-Tf(:,1),'ko','markerfacecolor','k')
hold on, plot(Tf2(:,1),Tf2(:,2)-Tf2(:,1),'ro','markerfacecolor','r')
%xlim([0 1]), ylim([-0.5 0.5])
subplot(3,2,2)
plot(Mf(:,1),Mf(:,2)-Mf(:,1),'bo','markerfacecolor','b')
hold on, plot(Mf2(:,1),Mf2(:,2)-Mf2(:,1),'mo','markerfacecolor','m')
%xlim([0 1]), ylim([-0.5 0.5])
subplot(3,2,3), plot(Tf(:,3),Tf(:,4)-Tf(:,3),'ko','markerfacecolor','k')
hold on, plot(Tf2(:,3),Tf2(:,4)-Tf2(:,3),'ro','markerfacecolor','r')
%xlim([0 1]), ylim([-0.5 0.5])
subplot(3,2,4)
plot(Mf(:,3),Mf(:,4)-Mf(:,3),'bo','markerfacecolor','b')
hold on, plot(Mf2(:,3),Mf2(:,4)-Mf2(:,3),'mo','markerfacecolor','m')
%xlim([0 1]), ylim([-0.5 0.5])
subplot(3,2,5), plot(Tf(:,1),Tf(:,4)-Tf(:,3),'ko','markerfacecolor','k')
hold on, plot(Tf2(:,1),Tf2(:,4)-Tf2(:,3),'ro','markerfacecolor','r')
%xlim([0 1]), ylim([-0.5 0.5])
subplot(3,2,6)
plot(Mf(:,1),Mf(:,4)-Mf(:,3),'bo','markerfacecolor','b')
hold on, plot(Mf2(:,1),Mf2(:,4)-Mf2(:,3),'mo','markerfacecolor','m')
%xlim([0 1]), ylim([-0.5 0.5])
