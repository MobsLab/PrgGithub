function [Down,Q,B,h1,b1,h2,b2,h3,b3,h4,b4,th]=FindDownGL(S,neu,SWSEpoch,siz,th,ch,smo,fac,plo,num)

%[Down,Q,B,h1,b1,h2,b2,h3,b3,h4,b4]=FindDown(S,neu,SWSEpoch,siz,th,ch,smo,fac,plo,num)
%[Down,Q,B,h1,b1,h2,b2,h3,b3,h4,b4]=FindDown(S,number,SWSEpoch,10,0.01,1,0,[0 85],1);
% siz=10;
% th=0.01;
% smo=3;
% ch=1;
...
subplot(num(1),num(2),num(3):num(3)+2), plot(Range(Q,'s'),Data(Q),'k')
hold on, plot(Range(Restrict(Q,SWSEpoch),'s'),Data(Restrict(Q,SWSEpoch)),'r')

xl=xlim; line(xl,[th th])

DurTsd=tsd(Start(Down),End(Down,'ms')-Start(Down,'ms'));

[h1,b1]=hist(Data(Restrict(DurTsd,SWSEpoch)),[0:floor(siz/10):2000]); % duration of Down during Sleep Epoch
[h2,b2]=hist(Data(Restrict(DurTsd,TotEpoch-SWSEpoch)),[0:floor(siz/10):2000]); % duration of Down during Wake Epoch
title(['bin size: ', num2str(siz/10), 'ms, Th: ', num2str(th), ', smo: ', num2str(smo),', Nb: ',num2str(length(neu))])


subplot(num(1),num(2),num(3)+3), hold on
plot(b1,h1,'r','linewidth',2) % duration of Down during Sleep Epoch
plot(b2,h2,'k','linewidth',2) % duration of Down during Wake Epoch
set(gca,'yscale','log')
set(gca,'xscale','log')
...
if fac(2)>0
    ...
    if fac(1)>0
        Down=mergeCloseIntervals(Down,fac(1)); % restreint les Down states selon leur durée
    end
...
%     if length(st)>5000
%     [MQ,TQ]=PlotRipRaw(Q,st(1:5000),800);close
%     dur=en(1:5000)-st(1:5000);
%     else
%     [MQ,TQ]=PlotRipRaw(Q,st,800);close
%     dur=en-st;
%     end
