% ParcoursCompareDeltaToneNoTone
...
%             [BE,dpfcBurst]=Restrict(Dpfc,BurstDeltaEpoch);
            ...
            DurDownNoTone(a)=nanmean(enn(doNoTone)/1E4-do(doNoTone)/1E4);
            DurDownTone(a)=nanmean(enn(doTone)/1E4-do(doTone)/1E4);
            [hNoTone(a,:),bi]=hist(enn(doNoTone)/1E4-do(doNoTone)/1E4,[0:0.005:0.8]);
            [hTone(a,:),bi]=hist(enn(doTone)/1E4-do(doTone)/1E4,[0:0.005:0.8]);           
...
cd /media/DataMOBsRAID/ProjetBreathDeltaFeedback    
eval(['save DataParcoursCompareDeltaToneNoTone',exp])

else
    
cd /media/DataMOBsRAID/ProjetBreathDeltaFeedback  
    eval(['load DataParcoursCompareDeltaToneNoTone',exp])
end
...
PlotErrorBar2(DurDownNoTone,DurDownTone), ylabel('Duration Down states'), xlabel('No Tone vs Tone')
