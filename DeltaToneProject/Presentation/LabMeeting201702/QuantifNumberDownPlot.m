% QuantifNumberDownPlot
% 18.02.2017 KJ
%
% Graphs quantifying number/density of down states for many conditions and
% hours/sessions
%
%
%%%%%
% INPUT :
%
%   density:        bool - 1 to plot the density, 0 to plot the numbers of delta waves
%
...
%
% See
%   QuantitySleepDelta QuantifNumberDeltaPlot QuantifNumberDeltaPlot QuantitySleepDeltaDown
%   
%


function data = QuantifNumberDownPlot(density, timescale, moment, varargin)
...
if density
    maintitle = 'Down states density';
    y_label = 'down states per hour';
else
    maintitle = 'Number Down states';
    y_label = 'number of down states';
end
...
all_downs = cell2mat(quantity_res.downs.(timescale));
...
if paired==0
    for cond=1:length(conditions)
        ...
        for p=1:length(path_cond)
            data{cond}(p) =  sum(all_downs(path_cond(p),idx_plot));
    ...
    for p=1:length(path_cond)
        data{length(conditions)+1}(p) =  sum(all_downs(path_cond(p),idx_plot));
        if density && strcmpi(timescale,'hours')
            duration = length(idx_plot);
    ...
    for m=1:length(animals)
        for cond=1:length(conditions)
            ...
            for p=1:length(selected_paths)
                mouse_data(p) =  sum(all_downs(selected_paths(p),idx_plot));
                if density && strcmpi(timescale,'hours')
                    duration = length(idx_plot);
