% QuantitySleepDeltaDown
% 17.02.2017 KJ
%
% duration of each sleep substages
% 
% 
%   see QuantitySleepDelta QuantifNumberDelta 
%
...
for p=1:length(Dir.path)
    ...
    Down = intervalSet(Start(Down) + start_time, End(Down) + start_time);
    start_down = Start(Down);
    start_down = Restrict(ts(start_down),SWSEpoch);
    ...
    quantity_res.deltas.total{p} = length(start_deltas); %delta
    quantity_res.downs.total{p} = length(start_down); %down
    ...
    for s=1:length(sessions)
        ...
        %down
        quantity_res.downs.session{p,s} = length(Restrict(start_down,sessions{s}));
    end
    ...
    for h=1:length(hours_epoch)
        ...
        %down
        quantity_res.downs.hours{p,h} = length(Restrict(start_down,hours_epoch{h}));
    end
...
save QuantitySleepDeltaDown.mat quantity_res hours_expe substages_ind
