% AnalysesERPDownDeltaTone2
% 20.02.2017 KJ
%
%
% Data to analyse tone effect on signals:
%   - show MUA raster synchronized on tones
%   - show PFC averaged LFP synchronized on tones (many depth)
%   - show also HPC and Bulb
...
for p=1:length(Dir.path)
    ...
    %% Raster and save
    down_tone.pfc.deep{p} = RasterMatrixKJ(LFPdeep, ToneEvent, t_before, t_after);
    down_tone.pfc.sup{p} = RasterMatrixKJ(LFPsup, ToneEvent, t_before, t_after);
    down_tone.pfc.d1{p} = RasterMatrixKJ(PFC1, ToneEvent, t_before, t_after);
    down_tone.pfc.d2{p} = RasterMatrixKJ(PFC2, ToneEvent, t_before, t_after);
    down_tone.pfc.d3{p} = RasterMatrixKJ(PFC3, ToneEvent, t_before, t_after);
    
    down_tone.hpc.rip{p} = RasterMatrixKJ(HPCrip, ToneEvent, t_before, t_after);
    down_tone.bulb.deep{p} = RasterMatrixKJ(Bulb_deep, ToneEvent, t_before, t_after);
    down_tone.bulb.sup{p} = RasterMatrixKJ(Bulb_sup, ToneEvent, t_before, t_after);
    down_tone.pacx.deep{p} = RasterMatrixKJ(PaCx_deep, ToneEvent, t_before, t_after);
    down_tone.pacx.sup{p} = RasterMatrixKJ(PaCx_sup, ToneEvent, t_before, t_after);
...
save AnalysesERPDownDeltaTone2 -v7.3 down_tone substage_ind effect_period
