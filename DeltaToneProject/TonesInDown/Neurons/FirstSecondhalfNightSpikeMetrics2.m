...
for p=1:length(transit_res.path)
    ...
    Matfr{1} = firingrates(excited_neurons);
    Matfr{2} = firingrates(neutral_neurons);
    Matfr{3} = firingrates(inhibit_neurons);
    ...
    halfsize = floor(size(mean_endo,1)/2);

    for i=1:size(mean_endo,2)
        begin.first(i) = nanmean(mean_endo(1:halfsize,i));
        ending.first(i) = nanmean(mean_endo(halfsize:end,i));    
        indu.first(i) = nanmean(mean_indu(:,i));
    end
    ...
    plot(180:300,180:300,'color',[0.7 0.7 0.7]),
    xlabel('mean spike / 1st half'),
    ylabel('mean spike / 2nd half'),
