...
%
%
%   Look at the response of neurons to ripples with no down before - PETH Cross-Corr (PLOT)
%
% see
%   ParcoursRipplesNoDownbefNeuronCrossCorr ParcoursRipplesNeuronCrossCorrPlot
...
for p=1:length(ripcor_res.path)
    ...
    AllPethStart = [AllPethStart ; ripcor_res.start.MatRipples{p}];
end
...
line([0 0], get(gca,'ylim'), 'linewidth',2),
caxis([-3.3 9.4]),
title('Neurons on ripples with no down before (all records)')
...
line([0 0], get(gca,'ylim'), 'linewidth',2),
caxis([-3.3 9.4]),
title('Neurons on ripples start with no down before (all records)')
