%%AssessToneEffectMUApopulation
% 19.04.2018 KJ
%
%
% see
%   ParcoursToneEffectOnNeurons AssessToneEffectNeurons
%
...
for p=1:length(Dir.path)
    ...
    ToneOut  = Restrict(ToneEvent, CleanUpEpoch(SWSEpoch-aroundDown));
    ...
    %Endogeneous Down
    intv_post_tones = [tones_tmp tones_tmp+intv_success];
    ...
    for n=1:length(neurons_pop)
        ...
        % In down
        [m,sem,tps] = mETAverage(Range(ToneIn), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_inside.(npop){p}(:,1) = tps; assess_res.met_inside.(npop){p}(:,2) = m; assess_res.met_inside.(npop){p}(:,3) = sem;
        
        % out of down or nearby
        [m,sem,tps] = mETAverage(Range(ToneOut), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_out.(npop){p}(:,1) = tps; assess_res.met_out.(npop){p}(:,2) = m; assess_res.met_out.(npop){p}(:,3) = sem;
        
        % N2/N3, out of down
        [m,sem,tps] = mETAverage(Range(ToneNREM), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_nrem.(npop){p}(:,1) = tps; assess_res.met_nrem.(npop){p}(:,2) = m; assess_res.met_nrem.(npop){p}(:,3) = sem;
        
        %sham in
        [m,sem,tps] = mETAverage(Range(ShamIn), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_shamin.(npop){p}(:,1) = tps; assess_res.met_shamin.(npop){p}(:,2) = m; assess_res.met_shamin.(npop){p}(:,3) = sem;
        

        %% MUA response to the end of down states
        % with
        [m,sem,tps] = mETAverage(End(DownTone), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_with.(npop){p}(:,1) = tps; assess_res.met_with.(npop){p}(:,2) = m; assess_res.met_with.(npop){p}(:,3) = sem;
        
        % without
        [m,sem,tps] = mETAverage(End(DownNo), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_without.(npop){p}(:,1) = tps; assess_res.met_without.(npop){p}(:,2) = m; assess_res.met_without.(npop){p}(:,3) = sem;
        
        % to the end of up states
        [m,sem,tps] = mETAverage(Start(DownEndo), Range(MUA.(npop)), Data(MUA.(npop)), binsize_met, nbBins_met);
        assess_res.met_endo.(npop){p}(:,1) = tps; assess_res.met_endo.(npop){p}(:,2) = m; assess_res.met_endo.(npop){p}(:,3) = sem;
...
if sham_distrib
    save AssessToneEffectMUApopulation_2.mat assess_res sham_distrib binsize_met nbBins_met binsize_mua minDuration intwindow nb_sham neurons_pop
else
    save AssessToneEffectMUApopulation.mat assess_res sham_distrib binsize_met nbBins_met binsize_mua minDuration intwindow nb_sham neurons_pop
end
