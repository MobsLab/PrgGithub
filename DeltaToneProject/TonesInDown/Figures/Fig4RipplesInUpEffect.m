...
% 30.05.2018 KJ
%
% effect of ripples around up states
...
% load(fullfile(FolderDeltaDataKJ,'RipplesShamInUpRasterNeuron.mat'))
...
for p=1:length(figure_res.path)
    ...
    norm_ripples = figure_res.ripples_bef{p} ./ (figure_res.ripples_bef{p} + figure_res.ripples_aft{p});
    [mouse.d_norm.ripples{p}, mouse.x_norm.ripples{p}] = histcounts(norm_ripples, edges_norm, 'Normalization','probability');
...
for p=1:length(ripples_res.path)
    ...
    try
        ...
        m = Data(raster_tsd)';
        m = [m zeros(size(m,1), size(MatMUA.sham,2)-size(m,2))];
...
%Ripples - Distrib of scatter plot
clear h
...
%Distrib end-event (sham and ripples)
clear h
...
ylim([0 0.05]),
line([0 0],get(gca,'ylim'),'color',[0.7 0.7 0.7]),
xlabel('time relative to sham (s)'),
