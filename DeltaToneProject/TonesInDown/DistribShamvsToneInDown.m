...
for p=1:length(Dir.path)
    ...
    tones_bef{p} = nan(length(tonesin_tmp), 1); % between start down and tones
    tones_aft{p} = nan(length(tonesin_tmp), 1); % between end down and tones
    ...
    %sham in 
    sham_bef{p} = nan(length(shamin_tmp), 1); % between start down and sham
    sham_aft{p} = nan(length(shamin_tmp), 1); % between end down and sham
