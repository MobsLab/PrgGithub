...
for m=1:length(animals)
    ...
    proba.end.sham   = [proba.end.sham mean(probend_mouse)];
end
...
for m=1:length(animals)
    ...
    for p=1:length(sham_res.path)
        if strcmpi(sham_res.name{p},animals{m})
            ...
            mouse_y.begin.sham  = [mouse_y.begin.sham sham_res.begin.met_mua{p}(:,2)/normfact.begin];
            mouse_y.end.sham    = [mouse_y.end.sham sham_res.end.met_mua{p}(:,2)/normfact.end];
...
set(gca,'YLim', [0 2.3],'XLim',[-400 400],'Fontsize',fontsize);
...
errorbar(mua_mean.begin.sham.x, mua_mean.begin.sham.y, mua_mean.begin.sham.std, 'color', color_begin,'CapSize',1)
errorbar(mua_mean.end.sham.x, mua_mean.end.sham.y, mua_mean.end.sham.std, 'color', color_end,'CapSize',1)
h(1) = plot(mua_mean.begin.sham.x, mua_mean.begin.sham.y, 'color', color_begin, 'linewidth', 2);
h(2) = plot(mua_mean.end.sham.x, mua_mean.end.sham.y, 'color', color_end, 'linewidth', 2);
set(gca,'YLim', [0 2.3],'XLim',[-400 400],'Fontsize',fontsize);
line([0 0],get(gca,'ylim'),'linewidth',2,'color',[0.7 0.7 0.7]);
lgd = legend(h, 'begin', 'end'); lgd.Location='northwest';
xlabel('time from tones (ms)'), ylabel('Averaged MUA'),
...
PlotErrorBarN_KJ([proba.begin.tones' proba.end.tones'], 'newfig',0, 'barcolors',{color_begin, color_end}, 'paired',paired, 'optiontest',sigtest, 'showPoints',showPoints,'ShowSigstar',showsig);
