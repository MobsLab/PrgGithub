...
%
%   see 
%       FigTonesInDownN2N3 FigTonesOutDeltaN2N3
%

%load
clear

load(fullfile(FolderDeltaDataKJ,'TonesInDeltaN2N3Effect.mat'))
load(fullfile(FolderDeltaDataKJ,'ShamInDeltaN2N3Effect.mat'))

load(fullfile(FolderDeltaDataKJ,'TonesInDeltaN2N3Raster.mat'))
load(fullfile(FolderDeltaDataKJ,'ShamInDeltaN2N3Raster.mat'))
...
for m=1:length(animals)
    ...
    d_before.n2.tones    = [d_before.n2.tones ; nanmean(n2_before_mouse,1)];
    d_before.n3.tones    = [d_before.n3.tones ; nanmean(n3_before_mouse,1)];
    d_before.nrem.tones  = [d_before.nrem.tones ; nanmean(nrem_before_mouse,1)];
    
    d_after.n2.tones     = [d_after.n2.tones ; nanmean(n2_after_mouse,1)];
    d_after.n3.tones     = [d_after.n3.tones ; nanmean(n3_after_mouse,1)];
    d_after.nrem.tones   = [d_after.nrem.tones ; nanmean(nrem_after_mouse,1)];
    
    d_norm.n2.tones      = [d_norm.n2.tones ; nanmean(n2_norm_mouse,1)];
    d_norm.n3.tones      = [d_norm.n3.tones ; nanmean(n3_norm_mouse,1)];
    d_norm.nrem.tones    = [d_norm.nrem.tones ; nanmean(nrem_norm_mouse,1)];
end
...
d_before.n2.tones     = nanmean(d_before.n2.tones,1);
d_after.n2.tones      = nanmean(d_after.n2.tones,1);
d_norm.n2.tones       = nanmean(d_norm.n2.tones,1);
...
d_before.n3.tones     = nanmean(d_before.n3.tones,1);
d_after.n3.tones      = nanmean(d_after.n3.tones,1);
d_norm.n3.tones       = nanmean(d_norm.n3.tones,1);
...
d_before.nrem.tones     = nanmean(d_before.nrem.tones,1);
d_after.nrem.tones      = nanmean(d_after.nrem.tones,1);
d_norm.nrem.tones       = nanmean(d_norm.nrem.tones,1);
...
MatLFP.n2.tones  = []; MatLFP.n2.sham   = [];
ibefore.n2.tones = []; ibefore.n2.sham  = [];
MatLFP.n3.tones  = []; MatLFP.n3.sham   = [];
ibefore.n3.tones = []; ibefore.n3.sham  = [];
MatLFP.nrem.tones  = []; MatLFP.nrem.sham   = [];
...
for c=1:5
    ...
    for m=1:length(animals)
        ...
        y_mouse_tone.nrem = [];
        for p=1:length(tones_res.path)
            if strcmpi(tones_res.name{p},animals{m}) && ismember(c,tones_res.clusters{p})
                y_mouse_tone.n2   = [y_mouse_tone.n2 tones_res.n2.met_lfp{p,c}(:,2)];
                y_mouse_tone.n3   = [y_mouse_tone.n3 tones_res.n3.met_lfp{p,c}(:,2)];
                y_mouse_tone.nrem = [y_mouse_tone.nrem tones_res.nrem.met_lfp{p,c}(:,2)];
                
                if ~isempty(tones_res.n2.met_lfp{p,c}(:,1))
                    x_met = tones_res.n2.met_lfp{p,c}(:,1);
                end
                
            end
        end
        
        if ~isempty(y_mouse_tone.n2)
            y_mua_tones.n2{c}   = [y_mua_tones.n2{c} mean(y_mouse_tone.n2,2)];
            y_mua_tones.n3{c}   = [y_mua_tones.n3{c} mean(y_mouse_tone.n3,2)];
            y_mua_tones.nrem{c} = [y_mua_tones.nrem{c} mean(y_mouse_tone.nrem,2)];
        end
    end
    
    y_mua_tones.n2{c}   = mean(y_mua_tones.n2{c},2);       
    y_mua_tones.n3{c}   = mean(y_mua_tones.n3{c},2);
    y_mua_tones.nrem{c} = mean(y_mua_tones.nrem{c},2);
...
for c=1:5
    ...
    for m=1:length(animals) 
        ...
        y_mouse_sham.nrem = [];
        for p=1:length(sham_res.path)
            if strcmpi(sham_res.name{p},animals{m}) && ismember(c,sham_res.clusters{p})
                y_mouse_sham.n2   = [y_mouse_sham.n2 sham_res.n2.met_lfp{p,c}(:,2)];
                y_mouse_sham.n3   = [y_mouse_sham.n3 sham_res.n3.met_lfp{p,c}(:,2)];
                y_mouse_sham.nrem = [y_mouse_sham.nrem sham_res.nrem.met_lfp{p,c}(:,2)];
            end
        end
        
        if ~isempty(y_mouse_sham.n2)
            y_mua_sham.n2{c}   = [y_mua_sham.n2{c} mean(y_mouse_sham.n2,2)];
            y_mua_sham.n3{c}   = [y_mua_sham.n3{c} mean(y_mouse_sham.n3,2)];
            y_mua_sham.nrem{c} = [y_mua_sham.nrem{c} mean(y_mouse_sham.nrem,2)];
        end
    end
    
    y_mua_sham.n2{c}   = mean(y_mua_sham.n2{c},2);  
    y_mua_sham.n3{c}   = mean(y_mua_sham.n3{c},2);
    y_mua_sham.nrem{c} = mean(y_mua_sham.nrem{c},2);
end



% %mean response in N2
% y_mua_tones.n2 = mean(MatLFP.n2.tones,1);
...
title('Tones inside delta waves - N2')
...
line([40 40], ylim,'linewidth',1,'color',[0.7 0.7 0.7]), hold on
caxis([-2000 3000]),
set(gca,'YLim', [0 size(MatLFP.n2.sham,1)], 'XLim',[-400 400],'Fontsize',fontsize);
xlabel('time relative to sham (ms)'), ylabel('#sham'),
title('Sham inside delta waves - N2')

% TONES - LFP by clusters
clear h
subtightplot(4,4,2,gap), hold on
for c=1:5
    h(c) = plot(x_met, y_mua_tones.n2{c}, 'color', colori_cluster{c});
    lgd_clu{c} = num2str(c);
end
...
title('Mean LFP on tones - N2'),

% SHAM - LFP by clusters
clear h
subtightplot(4,4,6,gap), hold on
for c=1:5
    h(c) = plot(x_met, y_mua_sham.n2{c}, 'color', colori_cluster{c});
end
set(gca,'XLim',[-400 400],'Fontsize',fontsize);
line([0 0],get(gca,'ylim'),'linewidth',2,'color',[0.7 0.7 0.7]);
...
%Distrib of start and end of delta waves
clear h
...
set(gca,'ylim', [0 0.25], 'ytick',0:0.1:0.3, 'Fontsize',fontsize), 
...
title('occurence of start/end of deltas - N2'),
...
% lgd = legend(h, 'tones', 'sham'); lgd.Location='northwest';
title({'Occurence of tones/sham' , 'in Delta wave (normalized) - N2'}),
...
xlabel('time relative to tones (s)'), ylabel('#tones'),
title('Tones inside delta waves - N3')
...
line([40 40], ylim,'linewidth',1,'color',[0.7 0.7 0.7]), hold on
caxis([-2000 3000]),
set(gca,'YLim', [0 size(MatLFP.n3.sham,1)], 'XLim',[-400 400],'Fontsize',fontsize);
xlabel('time relative to sham (ms)'), ylabel('#sham'),
title('Sham inside delta waves - N3')

% TONES - LFP by clusters
clear h
subtightplot(4,4,2,gap), hold on
for c=1:5
    h(c) = plot(x_met, y_mua_tones.n3{c}, 'color', colori_cluster{c});
    lgd_clu{c} = num2str(c);
end
set(gca,'XLim',[-400 400],'Fontsize',fontsize);
line([0 0],get(gca,'ylim'),'linewidth',2,'color',[0.7 0.7 0.7]);
lgd = legend(h,lgd_clu); lgd.Location='northwest';
xlabel('time from tones (s)'),
title('Mean LFP on tones - N3'),

% SHAM - LFP by clusters
clear h
subtightplot(4,4,6,gap), hold on
for c=1:5
    h(c) = plot(x_met, y_mua_sham.n3{c}, 'color', colori_cluster{c});
end
set(gca,'XLim',[-400 400],'Fontsize',fontsize);
line([0 0],get(gca,'ylim'),'linewidth',2,'color',[0.7 0.7 0.7]);
xlabel('time from sham (s)'),
...
set(gca, 'ytick',0:0.02:0.08, 'Fontsize',fontsize),
% lgd = legend(h, 'tones', 'sham'); lgd.Location='northwest';
title({'Occurence of tones/sham', 'in Delta waves (normalized) - N3'}),
...
%TONES - raster plot
subtightplot(4,4,1,gap), hold on
imagesc(x_lfp/10, 1:size(MatLFP.nrem.tones,1), MatLFP.nrem.tones), hold on
axis xy, hold on
line([0 0], ylim,'linewidth',2,'color','w'), hold on
line([40 40], ylim,'linewidth',1,'color',[0.7 0.7 0.7]), hold on
caxis([-2000 3000]),
set(gca,'YLim', [0 size(MatLFP.nrem.tones,1)], 'XLim',[-400 400],'Fontsize',fontsize);
xlabel('time relative to tones (s)'), ylabel('#tones'),
title('Tones inside delta waves - NREM')
...
line([40 40], ylim,'linewidth',1,'color',[0.7 0.7 0.7]), hold on
caxis([-2000 3000]),
set(gca,'YLim', [0 size(MatLFP.nrem.sham,1)], 'XLim',[-400 400],'Fontsize',fontsize);
xlabel('time relative to sham (ms)'), ylabel('#sham'),
title('Sham inside delta waves - NREM')

% TONES - LFP by clusters
clear h
subtightplot(4,4,2,gap), hold on
for c=1:5
    h(c) = plot(x_met, y_mua_tones.nrem{c}, 'color', colori_cluster{c});
    lgd_clu{c} = num2str(c);
end
set(gca,'XLim',[-400 400],'Fontsize',fontsize);
line([0 0],get(gca,'ylim'),'linewidth',2,'color',[0.7 0.7 0.7]);
lgd = legend(h,lgd_clu); lgd.Location='northwest';
xlabel('time from tones (s)'),
title('Mean LFP on tones - NREM'),

% SHAM - LFP by clusters
clear h
subtightplot(4,4,6,gap), hold on
for c=1:5
    h(c) = plot(x_met, y_mua_sham.nrem{c}, 'color', colori_cluster{c});
end
set(gca,'XLim',[-400 400],'Fontsize',fontsize);
line([0 0],get(gca,'ylim'),'linewidth',2,'color',[0.7 0.7 0.7]);
xlabel('time from sham (s)'),
...
set(gca, 'ytick',0:0.02:0.08, 'Fontsize',fontsize),
% lgd = legend(h, 'tones', 'sham'); lgd.Location='northwest';
title({'Occurence of tones/sham', 'in Delta wave (normalized) - NREM'}),
...
PlotErrorBarN_KJ({proba.nrem.tones proba.nrem.sham}, 'newfig',0, 'barcolors',{'b','r'}, 'paired',paired, 'optiontest',optiontest, 'showPoints',1,'ShowSigstar','sig');
set(gca,'ylim',[0 1.2],'ytick',0:0.2:1,'xtick',1:2,'XtickLabel',{'tones','sham'},'Fontsize',fontsize),
title('Tones in NREM')
