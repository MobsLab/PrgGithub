%%FailedTonesEffectUpDownMousePlot
% 31.05.2018 KJ
%
...
for p=1:4%length(failed_res.path)
    ...
    tones.x_corr = failed_res.tones.corr.x{p};
    tones.y_corr = failed_res.tones.corr.y{p};
    
    sham.x_corr = failed_res.sham.corr.x{p};
    sham.y_corr = failed_res.sham.corr.y{p};
    ...
    title(failed_res.name{p})
    legend(h,'Tones','Sham')
    
end
suplabel('Correlogram on FAILED tones/sham in Up state vs down states','t');
...
for p=1:4%length(failed_res.path)
    ...
    tones.x_mua = failed_res.tones.mua{p}(:,1);
    tones.y_mua = Smooth(failed_res.tones.mua{p}(:,2), smoothing);
    tones.sem_mua = failed_res.tones.mua{p}(:,4);
    
    sham.x_mua = failed_res.sham.mua{p}(:,1);
    sham.y_mua = Smooth(failed_res.sham.mua{p}(:,2), smoothing);
    sham.sem_mua = failed_res.sham.mua{p}(:,4); 
...
suplabel('Mean FR on FAILED tones/sham in Up state','t');
