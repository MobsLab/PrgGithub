...
%
% see
%   RipplesInDownN2N3Effect TonesInUpN2N3Effect RipplesInUpN2N3Raster
%
...
effect_periods = GetEffectPeriodDownRipples(Dir);
...
for p=1:length(Dir.path)
    ...
    binsize_met = 10;
    nbBins_met  = 80;
    range_up = effect_periods(p,:);    % [30-100ms] after tone in Up
    range_up = [40 100]*10;
    binsize_mua = 2;
    ...
    %N2
    [m,~,tps] = mETAverage(Range(RipplesUpN2), Range(MUA), Data(MUA), binsize_met, nbBins_met);
    ripples_res.n2.met_ripples{p}(:,1) = tps; ripples_res.n2.met_ripples{p}(:,2) = m;
    
    %N3
    [m,~,tps] = mETAverage(Range(RipplesUpN3), Range(MUA), Data(MUA), binsize_met, nbBins_met);
    ripples_res.n3.met_ripples{p}(:,1) = tps; ripples_res.n3.met_ripples{p}(:,2) = m;
    
    %NREM
    [m,~,tps] = mETAverage(Range(RipplesUpNREM), Range(MUA), Data(MUA), binsize_met, nbBins_met);
...
save RipplesInUpN2N3Effect.mat ripples_res binsize_met nbBins_met binsize_mua minDurationDown range_up
