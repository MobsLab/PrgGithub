%%MeanCurvesMUAFakeDeltaDeepPlot
% 05.09.2019 KJ
%
% Infos
...
%
% see
%    MeanCurvesMUAFakeDeltaDeep MeanCurvesMUAFakeDeltaSupPlot
...
for m=1:length(animals)
    ...
    for p=1:length(muadelta_res.path)
        if strcmpi(muadelta_res.name{p},animals{m})
            ...
            %MUA
            y_all  = CenterFiringRateCurve(muadelta_res.met_mua.all{p}(:,1),muadelta_res.met_mua.all{p}(:,2), 1);
            y_good = CenterFiringRateCurve(muadelta_res.met_mua.good{p}(:,1),muadelta_res.met_mua.good{p}(:,2), 1);
            y_fake = CenterFiringRateCurve(muadelta_res.met_mua.fake{p}(:,1),muadelta_res.met_mua.fake{p}(:,2), 1);
...
xlabel('time from deep slow wave detection (ms)'),
ylabel('Normalized MUA'), 


% percentage of false positive
subplot(1,3,3), hold on
PlotErrorBarN_KJ([precision.good precision.fake]*100, 'newfig',0, 'barcolors',{color_good, color_fake}, 'paired',1, 'optiontest','ranksum', 'showPoints',1,'ShowSigstar','sig');
