...
ripplesup_tmp = Range(Restrict(Restrict(tRipples, NREM), up_PFCx));
ripplesdown_tmp = Range(Restrict(Restrict(tRipples, NREM), down_PFCx));
...
intv = [ripplesup_tmp+range_up(1) ripplesup_tmp+range_up(2)];
[~,intervals,~] = InIntervals(Start(big_down), intv);
intervals(intervals==0)=[];
intervals = unique(intervals);
success_down = ripplesup_tmp(intervals);

%Down>Up
intv = [ripplesdown_tmp+range_down(1) ripplesdown_tmp+range_down(2)];
[~,intervals,~] = InIntervals(Start(down_PFCx), intv);
intervals(intervals==0)=[];
intervals = unique(intervals);
success_up = ripplesdown_tmp(intervals);
