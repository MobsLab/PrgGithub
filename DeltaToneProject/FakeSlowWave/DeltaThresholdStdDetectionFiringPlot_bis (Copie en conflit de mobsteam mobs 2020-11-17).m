...
for p=1:length(thresh_res.path)
    ...
    % firing rate at 2 SD
    fr2SD.diff = [fr2SD.diff ; thresh_res.diff.firing_rate{p}(thD_list==2)];
    fr2SD.deep = [fr2SD.deep ; thresh_res.deep.firing_rate{p}(thD_list==2)];
    if ~isempty(thresh_res.sup.firing_rate{p})
        fr2SD.sup  = [fr2SD.sup ; thresh_res.sup.firing_rate{p}(thD_list==2)];
    end
...
for p=1:length(thresh_res.path)
    ...
    plot(thD_list, thresh_res.diff.firing_rate{p}, 'color', 'k'),
end
...
for p=1:length(thresh_res.path)
    ...
    plot(thD_list, thresh_res.deep.firing_rate{p}, 'color', 'r'),
end
...
for p=1:length(thresh_res.path)
    ...
    if ~isempty(thresh_res.sup.firing_rate{p})
        plot(thD_list, thresh_res.sup.firing_rate{p}, 'color', 'b'),
    end
end
...
b(2).CData = [1 0 0];
b(3).CData = [0 0 1];
set(gca,'xtick',1:2,'xticklabel',{'below 0.1Hz','below 0.3Hz'},'ytick',0:20:100),
ylabel('%'), ylim([0 110])
