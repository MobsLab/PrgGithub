%%OccurenceRipplesFakeDeltaDeepPlot
% 09.09.2019 KJ
%
% Infos
...
%
% see
%    OccurenceRipplesFakeDeltaSup FigureExampleFakeSlowWaveSup
...
for m=1:length(animals)
    ...
    for p=1:length(ripples_res.path)
        if strcmpi(ripples_res.name{p},animals{m})
            ...
            cc_mouse.down.global.x  = ripples_res.cc.down.global{p}(:,1);
            cc_mouse.down.local.x   = ripples_res.cc.down.local{p}(:,1);
            cc_mouse.delta.global.x = ripples_res.cc.delta.global{p}(:,1);
            cc_mouse.delta.local.x  = ripples_res.cc.delta.local{p}(:,1);
            cc_mouse.delta.fake.x   = ripples_res.cc.delta.fake{p}(:,1);
            
            if normalized
                x_norm = cc_mouse.down.global.x<-800;
                normfact.down.global  = mean(ripples_res.cc.down.global{p}(x_norm,2));
                normfact.down.local   = mean(ripples_res.cc.down.local{p}(x_norm,2));
                normfact.delta.global = mean(ripples_res.cc.delta.global{p}(x_norm,2));
                normfact.delta.local  = mean(ripples_res.cc.delta.local{p}(x_norm,2));
                normfact.delta.fake   = mean(ripples_res.cc.delta.fake{p}(x_norm,2));
            else
            ...
            cc_mouse.down.global.y  = [cc_mouse.down.global.y Smooth(ripples_res.cc.down.global{p}(:,2)/normfact.down.global, smoothing)];
            cc_mouse.down.local.y   = [cc_mouse.down.local.y Smooth(ripples_res.cc.down.local{p}(:,2)/normfact.down.local, smoothing)];
            cc_mouse.delta.global.y = [cc_mouse.delta.global.y Smooth(ripples_res.cc.delta.global{p}(:,2)/normfact.delta.global, smoothing)];
            cc_mouse.delta.local.y  = [cc_mouse.delta.local.y Smooth(ripples_res.cc.delta.local{p}(:,2)/normfact.delta.local, smoothing)];
            cc_mouse.delta.fake.y   = [cc_mouse.delta.fake.y Smooth(ripples_res.cc.delta.fake{p}(:,2)/normfact.delta.fake, smoothing)];
