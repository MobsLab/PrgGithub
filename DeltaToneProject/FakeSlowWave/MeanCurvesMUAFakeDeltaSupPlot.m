...
for m=1:length(animals)
    ...
    %recall precision
    recall.all(m,1) = mean(recall_mouse.all);
    recall.good(m,1) = mean(recall_mouse.good);
    recall.fake(m,1) = mean(recall_mouse.fake);
    
    precision.all(m,1) = mean(precision_mouse.all);
    precision.good(m,1) = mean(precision_mouse.good);
    precision.fake(m,1) = mean(precision_mouse.fake);
...
%error shadow
Hsall = shadedErrorBar(mua_mean.all.x, mua_mean.all.y, mua_mean.all.std,{},0.4);
...
Hsall.patch.FaceColor = color_all;
Hsgood.patch.FaceColor = color_good;
Hsfake.patch.FaceColor = color_fake;
%mean curves
h(1) = plot(mua_mean.all.x, mua_mean.all.y, 'color', color_all, 'linewidth',2);
h(2) = plot(mua_mean.good.x, mua_mean.good.y, 'color', color_good, 'linewidth',2);
h(3) = plot(mua_mean.fake.x, mua_mean.fake.y, 'color', color_fake, 'linewidth',2);
%properties
set(gca,'YLim', [0 1.8],'XLim',[-600 600],'Fontsize',fontsize);

line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
legend(h, 'all deltas', 'good deltas (25% best)', 'fake deltas (25% worst)');
xlabel('time from delta wave detection (ms)'),
ylabel('Normalized MUA'), 


%Recall
subplot(2,3,3), hold on
PlotErrorBarN_KJ([recall.all recall.good recall.fake]*100, 'newfig',0, 'barcolors',{color_all, color_good, color_fake}, 'paired',1, 'optiontest','ranksum', 'showPoints',1,'ShowSigstar','sig');
set(gca,'ylim',[0 100],'xtick',1:3,'XtickLabel',{'all', 'good','fake'},'Fontsize',fontsize),
ylabel('%'),
title('Recall')
%Recall
subplot(2,3,6), hold on
PlotErrorBarN_KJ([precision.all precision.good precision.fake]*100, 'newfig',0, 'barcolors',{color_all, color_good, color_fake}, 'paired',1, 'optiontest','ranksum', 'showPoints',1,'ShowSigstar','sig');
set(gca,'ylim',[0 100],'xtick',1:3,'XtickLabel',{'all', 'good','fake'},'Fontsize',fontsize),
ylabel('%'),
title('Precision')
