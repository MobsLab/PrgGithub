...
for p=1:length(Dir.path)
    ...
    %global down
    [crosscorr_res.down.global{p}(:,2), crosscorr_res.down.global{p}(:,1)] = CrossCorr(Range(tRipples), Start(GlobalDown), binsize_cc, nb_binscc);
    ...
    for tt=1:nb_tetrodes
        ...
        %global delta
        [crosscorr_res.delta.global{p,tt}(:,2), crosscorr_res.delta.global{p,tt}(:,1)] = CrossCorr(Range(tRipples), Start(GlobalDelta{tt}), binsize_cc, nb_binscc);
        %other delta
        [crosscorr_res.delta.other{p,tt}(:,2), crosscorr_res.delta.other{p,tt}(:,1)] = CrossCorr(Range(tRipples), Start(OtherDelta{tt}), binsize_cc, nb_binscc);
        %local delta
        if ~isempty(Start(LocalDelta{tt}))
            [crosscorr_res.delta.local{p,tt}(:,2), crosscorr_res.delta.local{p,tt}(:,1)] = CrossCorr(Range(tRipples), Start(LocalDelta{tt}), binsize_cc, nb_binscc);
        else
            crosscorr_res.delta.local{p,tt} = nan(size(crosscorr_res.delta.global{p,tt}));
        end
        
        %fake delta
        if ~isempty(Start(FakeDelta{tt}))
            [crosscorr_res.delta.fake{p,tt}(:,2), crosscorr_res.delta.fake{p,tt}(:,1)] = CrossCorr(Range(tRipples), Start(FakeDelta{tt}), binsize_cc, nb_binscc);
        else
            crosscorr_res.delta.fake{p,tt} = nan(size(crosscorr_res.delta.fake{p,tt}));
        end
        ...
        crosscorr_res.meandown{p,tt}(:,1) = tps; crosscorr_res.meandown{p,tt}(:,2) = m;
        
        %ripples
        [m,~,tps] = mETAverage(Range(tRipples), Range(LFP), Data(LFP), binsize_met, nbBins_met);
        crosscorr_res.meanripples{p,tt}(:,1) = tps; crosscorr_res.meanripples{p,tt}(:,2) = m;
        ...
        crosscorr_res.textbox_str{p,tt} = {Dir.name{p}, Dir.date{p}, ['Tetrode ' num2str(Dir.tetrodes{p}(tt)) '(tt=' num2str(tt) ')'], ['channel ' num2str(tetrodeChannelsCell{tt})], ...
        ...
        crosscorr_res.idtetrode{p,tt} = Dir.tetrodes{p}(tt);
        crosscorr_res.tetrodeChannels{p,tt} = tetrodeChannelsCell{tt};
        crosscorr_res.mainchannel{p,tt} = tetrodeChannels(tt);
...
% save ScriptOccurenceRipplesFakeDeltaDeep.mat crosscorr_res
% 
