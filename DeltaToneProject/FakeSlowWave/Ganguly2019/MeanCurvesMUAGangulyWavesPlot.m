...
recall.diff = []; precision.diff = []; fscore.diff = [];
recall.so = []; precision.so = []; fscore.so = [];
recall.delta = []; precision.delta = []; fscore.delta = [];
...
for m=1:length(animals)
    ...
    for p=1:length(muawaves_res.path)
        if strcmpi(muawaves_res.name{p},animals{m})
            ...
            %recall precision
            recall_mouse.diff = [recall_mouse.diff muawaves_res.diff.recall(p)];
            recall_mouse.so = [recall_mouse.so muawaves_res.so.recall(p)];
            recall_mouse.delta = [recall_mouse.delta muawaves_res.delta.recall(p)];
            
            precision_mouse.diff = [precision_mouse.diff muawaves_res.diff.precision(p)];
            precision_mouse.so = [precision_mouse.so muawaves_res.so.precision(p)];
            precision_mouse.delta = [precision_mouse.delta muawaves_res.delta.precision(p)];
    ...
    fscore.diff(m,1) = 2*recall_mouse.diff.*precision_mouse.diff / (recall_mouse.diff+precision_mouse.diff);
    fscore.so(m,1)   = 2*recall_mouse.so.*precision_mouse.so / (recall_mouse.so+precision_mouse.so);
    fscore.delta(m,1) = 2*recall_mouse.delta.*precision_mouse.delta / (recall_mouse.delta+precision_mouse.delta);
...
PlotErrorBarN_KJ([recall.diff recall.so recall.delta]*100, 'newfig',0, 'barcolors',{color_diff, color_so, color_delta}, 'paired',1, 'optiontest','ranksum', 'showPoints',1,'ShowSigstar','sig');
...
PlotErrorBarN_KJ([fscore.diff fscore.so fscore.delta]*100, 'newfig',0, 'barcolors',{color_diff, color_so, color_delta}, 'paired',1, 'optiontest','ranksum', 'showPoints',1,'ShowSigstar','sig');
set(gca,'ylim',[0 100],'xtick',1:3,'XtickLabel',{'Diff','SO','delta'},'Fontsize',fontsize),
xtickangle(30),
ylabel('fscore'),
...
errorbar(mua_mean.diff.x, deep_mean.diff.y, deep_mean.diff.std, 'color', color_diff,'CapSize',1)
errorbar(mua_mean.diff.x, sup_mean.diff.y, sup_mean.diff.std, 'color', color_diff,'CapSize',1)
h(1) = plot(mua_mean.diff.x, deep_mean.diff.y, 'color', color_diff, 'linewidth',3);
h(2) = plot(mua_mean.diff.x, sup_mean.diff.y, 'color', color_diff, 'linewidth',1);
set(gca,'YLim', [-1000 2000],'XLim',[-800 800],'Fontsize',fontsize);
line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
legend(h,'Deep','Sup');
xlabel('time from detection (ms)'),
...
errorbar(mua_mean.so.x, deep_mean.so.y, deep_mean.so.std, 'color', color_so,'CapSize',1)
errorbar(mua_mean.so.x, sup_mean.so.y, sup_mean.so.std, 'color', color_so,'CapSize',1)
h(1) = plot(mua_mean.so.x, deep_mean.so.y, 'color', color_so, 'linewidth',3);
h(2) = plot(mua_mean.so.x, sup_mean.so.y, 'color', color_so, 'linewidth',1);
set(gca,'YLim', [-1000 2000],'XLim',[-800 800],'Fontsize',fontsize);
line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
legend(h,'Deep','Sup');
xlabel('time from detection (ms)'),
...
errorbar(mua_mean.delta.x, deep_mean.delta.y, deep_mean.delta.std, 'color', color_delta,'CapSize',1)
errorbar(mua_mean.delta.x, sup_mean.delta.y, sup_mean.delta.std, 'color', color_delta,'CapSize',1)
h(1) = plot(mua_mean.delta.x, deep_mean.delta.y, 'color', color_delta, 'linewidth',3);
h(2) = plot(mua_mean.delta.x, sup_mean.delta.y, 'color', color_delta, 'linewidth',1);
set(gca,'YLim', [-1000 2000],'XLim',[-800 800],'Fontsize',fontsize);
line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
legend(h,'Deep','Sup');
xlabel('time from detection (ms)'),
