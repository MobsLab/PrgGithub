%%CharacterisationGagulyWavesGroupPlot_defense
% 23.11.2019 KJ
%
% Infos
%
...
for c=1:length(cludeep)
    data_recall{c}    = [average_so.recall{c} average_deltag.recall{c}];
    data_precision{c} = [average_so.precision{c} average_deltag.precision{c}];
end
...
fontsize = 18;
titles = {'group 4','group 5-6'};
labels = {'SO','\delta'};
showpoints = 0;

color_so = [0.7 0.7 0.7];
color_delta = [1 0.3 0.7];
colori = {[0.7 0.7 0.7], [1 0.3 0.7]};
...
for c=1:length(cludeep)
    ...
    PlotErrorBarN_KJ(data_precision{c}, 'newfig',0, 'barcolors', colori, 'paired',1, 'ShowSigstar','sig','showpoints',showpoints); hold on
    set(gca, 'ylim',[0 100], 'ytick',0:20:100, 'xlim', [0 3], 'xtick', 1:2, 'XTickLabel',labels,'fontsize',fontsize), 
    ylabel('% false postive'), xtickangle(30)
    title(titles{c}),
    
    subplot(length(cludeep),3,2+(c-1)*3), hold on
    PlotErrorBarN_KJ(data_recall{c}, 'newfig',0, 'barcolors', colori, 'paired',1, 'ShowSigstar','sig','showpoints',showpoints); hold on
    set(gca, 'ylim',[0 120], 'ytick',0:20:100, 'xlim', [0 3], 'xtick', 1:2, 'XTickLabel',labels,'fontsize',fontsize), 
...
for c=1:length(cludeep)
    ...
    %mean curves
    h(1) = plot(lfp_mean.so_up.x{c}, lfp_mean.so_up.y{c}, 'color', color_so, 'linewidth',2);
    h(2) = plot(lfp_mean.delta_up.x{c}, lfp_mean.delta_up.y{c}, 'color', color_delta, 'linewidth',2);
    %properties
    set(gca,'XLim',[-400 400],'yLim',[-400 400],'Fontsize',fontsize);
...
for c=1:length(cludeep)
    ...
    %mean curves
    h(1) = plot(mua_mean.so.x{c}, mua_mean.so.y{c}, 'color', color_so, 'linewidth',2);
    h(2) = plot(mua_mean.delta.x{c}, mua_mean.delta.y{c}, 'color', color_delta, 'linewidth',2);
    %properties
