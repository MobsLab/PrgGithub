...
% load
clear
load(fullfile(FolderDeltaDataKJ,'OccurenceRipplesFakeDeltaSup.mat'))
...
%             norm_factor.fake   = ripples_res.nb.fake{p};
    ...
    %mouse average
    cc_animal.down.y{m}   = mean(cc_mouse.down.y,2);
    cc_animal.deltas.y{m} = mean(cc_mouse.deltas.y,2);
    cc_animal.good.y{m}   = mean(cc_mouse.good.y,2);
    cc_animal.fake.y{m}   = mean(cc_mouse.fake.y,2);
...
for m=1:length(animals)
    plot(Cc.down.X, cc_animal.down.y{m}, 'color', col_curve, 'linewidth',1);
end
plot(Cc.down.X, Cc.down.meanY, 'color',col_mean, 'linewidth',2),
...
for m=1:length(animals)
    plot(Cc.deltas.X, cc_animal.deltas.y{m}, 'color', col_curve, 'linewidth',1);
end
plot(Cc.deltas.X, Cc.deltas.meanY, 'color',col_mean, 'linewidth',2),
line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
xlabel('time from ripples (ms)'),
...
for m=1:length(animals)
    plot(Cc.good.X, cc_animal.good.y{m}, 'color', col_curve, 'linewidth',1);
end
plot(Cc.good.X, Cc.good.meanY, 'color',col_mean, 'linewidth',2),
line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
xlabel('time from ripples (ms)'),
ylabel('Occurence of slow waves'), 
title('Slow waves (25% high inversion)'),

%fake
subplot(2,2,4), hold on
for m=1:length(animals)
    plot(Cc.fake.X, cc_animal.fake.y{m}, 'color', col_curve, 'linewidth',1);
end
plot(Cc.fake.X, Cc.fake.meanY, 'color',col_mean, 'linewidth',2),
line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
xlabel('time from ripples (ms)'),
ylabel('Occurence of slow waves'), 
title('Slow waves (25% low inversion)'),
