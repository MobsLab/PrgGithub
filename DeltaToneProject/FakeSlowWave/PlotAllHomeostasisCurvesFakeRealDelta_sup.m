%%PlotAllHomeostasisCurvesFakeRealDelta_sup
% 24.09.2019 KJ
%
% Infos
...
color_expfit = {'k','b','r', [0.6 0.6 0.6], [0.3 0.2 0.3]};
...
ylabel('Down Occupancy (% of NREM average)'), xlabel('Time (h)')
title('Down states')
...
set(gca, 'xlim',[8 21], 'ylim', [0 350],'fontsize',fontsize),
ylabel('Real SW occupancy (% of NREM average)'), xlabel('Time (h)')
title({'Slow Wave', '(real delta - with down)'})
...
set(gca, 'xlim',[8 21], 'ylim', [0 350],'fontsize',fontsize),
ylabel('Fake SW occupancy (% of NREM average)'), xlabel('Time (h)')
title({'Slow Wave', '(fake delta no down)'})
...
for p=1:length(homeo_res.path)
    ...
    Hstat = homeo_res.swa.rescaled.Hstat{p};
    h(1) = plot(Hstat.x_peaks, Hstat.y_peaks*100,'o','MarkerSize',sz,'MarkerEdgeColor',color_peaks, 'MarkerFaceColor',color_peaks);    
end
for p=1:length(homeo_res.path)
    if ismember(p,path_exclude)
    ...
    Hstat = homeo_res.swa.rescaled.Hstat{p};
    %fit
    h(2) = plot(Hstat.x_intervals, Hstat.reg_exp*100,'color',color_expfit{5},'linewidth',linewidt);
end


set(gca, 'xlim',[8 21], 'ylim', [0 350],'fontsize',fontsize),
legend(h, 'Occupancy', 'exponential fit')
ylabel('SWA (% of NREM average)'), xlabel('Time (h)')
