...
for p=1:length(homeo_res.path)
    ...
    for tt=1:length(homeo_res.tetrodes{p})
        Hstat = homeo_res.local.rescaled.Hstat{p,tt};
        if any(Hstat.reg0*100>250) || any(Hstat.reg_exp==0)
            continue
        end
        %fit
...
for p=1:length(homeo_res.path)
    ...
    %fit
    h(2) = plot(Hstat.x_intervals, Hstat.reg0*100,'color',color_expfit{3},'linewidth',linewidt);
end
...
for p=1:length(homeo_res.path)
    ...
    for tt=1:length(homeo_res.tetrodes{p})
        ...
        %fit
        h(2) = plot(Hstat.x_intervals, Hstat.reg0*100,'color',color_expfit{4},'linewidth',linewidt);
    end
end
set(gca, 'xlim',[8 21], 'ylim', [0 350],'fontsize',fontsize),
xlabel('Time (h)'),
