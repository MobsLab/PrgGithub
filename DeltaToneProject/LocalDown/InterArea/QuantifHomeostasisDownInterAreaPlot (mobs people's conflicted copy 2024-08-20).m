...
%data to plot
data_intra  = [perc_intra.pfc perc_intra.pa perc_intra.mo];
...
for p=1:length(local_res.path)
    ...
    %fit
    if any(Hstat.reg_exp*100>350)
        h(2) = plot(Hstat.x_intervals, (Hstat.reg_exp*100)/2,'color',color_expfit,'linewidth',linewidt);
    else
        h(2) = plot(Hstat.x_intervals, Hstat.reg_exp*100,'color',color_expfit,'linewidth',linewidt);
    end
end
%properties
set(gca, 'xlim',[8 21], 'ylim', [0 450],'fontsize',fontsize),
legend(h, 'Delta density', 'exponential fit')
ylabel('Delta waves density (% of NREM average)'), xlabel('Time (hour)')
title('Deltas in 3 areas')
...
set(gca, 'xlim',[8 21], 'ylim', [0 450],'fontsize',fontsize),
legend(h, 'Delta density', 'exponential fit')
ylabel('Intra Delta waves density (% of NREM average)'), xlabel('Time (hour)')
title('Intra delta waves')
