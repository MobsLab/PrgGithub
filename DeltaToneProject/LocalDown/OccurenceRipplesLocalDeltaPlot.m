%%OccurenceRipplesLocalDeltaPlot
% 09.09.2019 KJ
%
% Infos
...
for p=1:length(crosscorr_res.path)
    ...
    for tt=1:length(crosscorr_res.tetrodes{p})
        ...
        plot(crosscorr_res.delta.local{p,tt}(:,1), crosscorr_res.delta.local{p,tt}(:,2), 'color', col_down, 'linewidth',2);
        line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
        xlabel('time from ripples (ms)'),
        ...
        plot(crosscorr_res.delta.fake{p,tt}(:,1), crosscorr_res.delta.fake{p,tt}(:,2), 'color', col_down, 'linewidth',2);
        line([0 0], ylim,'Linewidth',1,'color',[0.5 0.5 0.5]), hold on
        xlabel('time from ripples (ms)'),
        ylabel('occurence of delta'), 
        title('Fake delta'),
