% ParcoursCompareDeltaToneNoTone2
...
for i=1:length(Dir.path)
    try
        ...
        try
            ...
            LFPs=LFP;
            clear LFP
            [MPEaverageDeltaPFCxDeep,TPEaverageDeltaPFCxDeep]=PlotRipRaw(LFPd,deltas/1E4,1000);close
            [MPEaverageDeltaPFCxSup,TPEaverageDeltaPFCxSup]=PlotRipRaw(LFPs,deltas/1E4,1000);close
        ...
        plot(tps,nanmean(TPEaverageDeltaPFCxDeep(dpfc_notone,:)),'k')
        plot(tps,nanmean(TPEaverageDeltaPFCxSup(dpfc_notone,:)),'r')
        plot(tps,nanmean(TPEaverageDeltaPFCxDeep(dpfc_tone,:)),'b')
        plot(tps,nanmean(TPEaverageDeltaPFCxSup(dpfc_tone,:)),'m')
        title([Dir.name{i},' No Tone vs Tone'])
        
        TPEaverageDeltaPFCxDeepNoTone(a,:) = nanmean(TPEaverageDeltaPFCxDeep(dpfc_notone,:));
        TPEaverageDeltaPFCxSupNoTone(a,:)  = nanmean(TPEaverageDeltaPFCxSup(dpfc_notone,:));
        TPEaverageDeltaPFCxDeepTone(a,:)   = nanmean(TPEaverageDeltaPFCxDeep(dpfc_tone,:));
        TPEaverageDeltaPFCxSupTone(a,:)    = nanmean(TPEaverageDeltaPFCxSup(dpfc_tone,:));
...
[~, p1] = ttest2(max(temp1'),max(temp2'));
[~, p2] = ttest2(max(temp3'),max(temp4'));
[~, p3] = ttest2(min(temp1'),min(temp2'));
[~, p4] = ttest2(min(temp3'),min(temp4'));
