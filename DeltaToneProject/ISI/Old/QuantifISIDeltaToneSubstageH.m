% QuantifISIDeltaToneSubstageH
% 15.12.2016 KJ
%
% horizontal bar plot (median, mode) for several conditions and substages.
...
%
%   EXAMPLE : 
%       QuantifISIDeltaToneSubstageH(1, 'show_sig','none');       
% 
%
% See
...
function QuantifISIDeltaToneSubstageH(figure_numbers, varargin)
    ...
    for conditions=pos_delays
        ...
        for sbp=1:length(substages_plot)
            ...
            subplot(1,length(substages_plot),sbp), hold on,
                PlotErrorBarN_KJ(data,'newfig',0,'horizontal',1,'barcolors',bar_color,'NonColumnTest',noncolumntest,'ShowSigstar',show_sig, 'optiontest',optiontest);
            title([NameSubstages{sub}]), xlabel('ms'), hold on,
            set(gca, 'YTickLabel',labels, 'YTick',1:numel(labels)), hold on,
    ...
    for conditions=pos_delays
        ...
        for sbp=1:length(substages_plot)
            ...
            subplot(1,length(substages_plot),sbp), hold on,
                PlotErrorBarN_KJ(data,'newfig',0,'horizontal',1,'barcolors',bar_color,'NonColumnTest',noncolumntest,'ShowSigstar',show_sig, 'optiontest',optiontest);
            title([NameSubstages{sub}]), xlabel('ms'), hold on,
            set(gca, 'YTickLabel',labels, 'YTick',1:numel(labels), 'YTickLabelRotation', 30), hold on,
