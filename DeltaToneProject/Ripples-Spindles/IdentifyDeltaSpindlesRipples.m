function [tDeltaT2,tDeltaP2,spiPeaks,ripts,ma,sa,tpsa,mb,sb,tpsb,mc,sc,tpsc,md,sd,tpsd,me,se,tpse]=IdentifyDeltaSpindlesRipples(LFP,ch,ch2,th,plo,absTh)

% ch  channel in superficial layers or EcoG/EEG
% ch2 channel in deep layers
...
if ch2>0
FilRip=FilterLFP(Restrict(LFP{ch2},Epoch),[130 250],96);

else
    
    FilRip=FilterLFP(Restrict(LFP{ch},Epoch),[130 250],96);
end
...
if vnoise>0
        ...
        NoiseRip=FilterLFP(Restrict(LFP{vnoise},Epoch),[130 250],96);
        ...
        [ripples,stdev,noise] = FindRipples(filtered,'thresholds',paramRip,'noise',filteredNoise,'show','off','durations',[30 30 100]);
else  
...
if i==ch|i==ch2
    plot(tpsa{i},smooth(ma{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)],'linewidth',2)
else
plot(tpsa{i},smooth(ma{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)])
end
...
if i==ch|i==ch2
    plot(tpsb{i},smooth(mb{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)],'linewidth',2)
else
plot(tpsb{i},smooth(mb{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)])
end
end
...
if i==ch|i==ch2
    plot(tpsc{i},smooth(mc{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)],'linewidth',2)
else
plot(tpsc{i},smooth(mc{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)])
end
end
...
tbins=4;nbbins=300;
for i=1:length(LFP)
[md{i},sd{i},tpsd{i}]=mETAverage(Range(ripts), Range(LFP{i}),Data(LFP{i}),tbins,nbbins);
if i==ch|i==ch2
    plot(tpsd{i},smooth(md{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)],'linewidth',2)
else
plot(tpsd{i},smooth(md{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)])
end
end
...
tbins=1;nbbins=300;
for i=1:length(LFP)
[me{i},se{i},tpse{i}]=mETAverage(Range(ripts), Range(LFP{i}),Data(LFP{i}),tbins,nbbins);
if i==ch|i==ch2
    plot(tpse{i},smooth(me{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)],'linewidth',2)
else
plot(tpse{i},smooth(me{i},3),'color',[i/length(LFP) 0 (length(LFP)-i)/length(LFP)])
end
end
