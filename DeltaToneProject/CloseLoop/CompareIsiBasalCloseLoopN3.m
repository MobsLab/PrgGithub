...
for p=1:length(Dir_basal.path)
    ...
    %N3
    [~, ~, N3] = GetSubstages('scoring','ob');
    
    %Restrict
    DeltaN3 = dropShortIntervals(and(delta_PFCx, N3),750);
    st_deltas = Start(DeltaN3);
...
for p=1:length(Dir_bci.path)
    ...
    %Restrict
    ToneN3 = Restrict(ToneEvent, N3);
    nb_tones = length(ToneN3);
    ...
    TonesTriggeredIntv = [Range(ToneN3) - (delay+pre_period),  Range(ToneN3) - (delay-pre_period)];
    ...
    %induced
    TonesSuccessIntv = [Range(ToneN3) + effect_periods(p,1), Range(ToneN3) + effect_periods(p,2)];
...
save CompareIsiBasalCloseLoopN3.mat basal_res bci_res effect_periods delays
