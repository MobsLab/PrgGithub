...
%
%   see 
%       LFPlayerInfluenceOnDetection LFPonDownStatesClustering3 LFPlayerInfluenceOnSpectra2        
%
...
for k=1:nb_clusters
    ...
    for sub=1:5 
        subplot(ny,nx,substageplot_idx(sub)), hold on
        hold on, plot(freq_sub{sub}, mean_spec{sub}, 'color', colori_cluster{k})
    end
...
for sub=1:5 
    subplot(ny,nx,substageplot_idx(sub)), hold on
    legend(legd), xlim([0 30]), 
    
    title(['Norm Spectrum in ' NameEpoch{sub}]), hold on
