...
function analyserGroupe_NormMax(enregistrements, groupe_nom, couleur_moyenne)
    ...
    individual_spectra = [];
    for i = 1:length(enregistrements)
        % Calculer le spectre Wake sans normalisation ni lissage pour vérification
        spectra_wake = mean(Data(Restrict(StsdSomCxdeepHM{i}, periods(i).Wake)));
        
        % Normaliser par le maximum
        spectra_wake = spectra_wake / max(spectra_wake);
        
        % Appliquer le lissage
        spectra_wake = smoothdata(spectra_wake, 'movmean', 5);  % Lissage sur 5 points
        
        % Sauvegarde du spectre lissé et normalisé pour le calcul de la moyenne
        individual_spectra(i, :) = spectra_wake; 
        
        % Tracé pour le spectre lissé et normalisé
    ...
    individual_spectra = [];
    for i = 1:length(enregistrements)
        % Spectre Wake normalisé et lissé
    ...
    xlabel('Fréquence (Hz)');
    ylabel('Puissance normalisée (dB)');
    legend show;
    ...
    ylabel('log10(Puissance normalisée)');
    legend show;
