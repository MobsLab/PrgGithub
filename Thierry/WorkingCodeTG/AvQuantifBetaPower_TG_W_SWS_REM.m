...
figure('color',[1 1 1]),
subplot(3,6,[1,2]), imagesc(temps,freqB, avdataSpREM), caxis([0 4E3]),axis xy,colormap(jet)
...
colorbar
title('OB REM ')
subplot(3,6,[3,4]), imagesc(temps,freqB, avdataSpSWS), axis xy, caxis([0 4E3]), colormap(jet)
line([0 0], ylim,'color','w','linestyle',':')
xlim([-30 +30])
xlabel('Time (s)')
ylabel('Frequency (Hz)')
colorbar
title('HPC NREM')
subplot(3,6,[5,6]), imagesc(temps,freqB, avdataSpWake), axis xy, caxis([0 6E3]), colormap(jet)
line([0 0], ylim,'color','w','linestyle','-')
xlim([-30 +30])
xlabel('Time (s)')
ylabel('Frequency (Hz)')
...
title('REM')
subplot(3,6,[15,16]),PlotErrorBarN_KJ({mean(AvBetaREM(:,beforeidx)/avFacSWS,2), mean(AvBetaSWS(:,duringidx)/avFacSWS,2)},'newfig',0,'paired',1,'ShowSigstar','sig');
ylim([0 1.8])
ylabel('theta power')
xticks(1:2)
xticklabels({'Before','During'});
