function [SpectroREM,SpectroSWS,SpectroWake,facREM,facSWS,facWake,valThetaREM,valThetaSWS,valThetaWake,freq,temps] = PlotThetaPowerOverTime_PFC_SingleMouse_TG(plo)
...
%if plo
    ...
    figure('color',[1 1 1]),
    subplot(231), imagesc(temps,freq, SpectroREM), axis xy, caxis([0 5]), colormap(jet)
    ...
    subplot(232), imagesc(temps,freq, SpectroSWS), axis xy, caxis([0 5]), colormap(jet)
    line([0 0], ylim,'color','w','linestyle',':')
    xlim([-30 +30])
    xlabel('Time (s)')
    ylabel('Frequency (Hz)')
    ...
    subplot(233), imagesc(temps,freq, SpectroWake), axis xy, caxis([0 5]), colormap(jet)
    line([0 0], ylim,'color','w','linestyle',':')
    xlim([-30 +30])
    xlabel('Time (s)')
    ylabel('Frequency (Hz)')
