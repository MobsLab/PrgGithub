...
%events=REM20_30s*1E4;
events=Ordered_REM(:,2)*1E4
events(isnan(events))=[]
%events=(Start(REMEpoch)/1E4)+10

%events=StartStim_dansREM;

fq=find(SpectroH2{3}>6.5&SpectroH2{3}<9.5);
fq2=find(SpectroH2{3}>3&SpectroH2{3}<5);
thetaPower2=10*log10(mean(SpectroH2{1}(:,fq),2));
thetaDeltaPower=10*log10(mean(SpectroH2{1}(:,fq),2))./(10*log10(mean(SpectroH2{1}(:,fq2),2)));
vv2=tsd(SpectroH2{2}*1E4,thetaPower2);
vv=tsd(SpectroH2{2}*1E4,thetaDeltaPower);
