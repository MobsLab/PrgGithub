function [ Nb, States, Latence, Result] = LatenceTransition_test()
...
for k=1:Nb_Stim
    ...
    if CandidateNextWakeTrans(1) > 0
        PreviousWakeTrans(k) = NaN;
    else
        PreviousWakeTrans(k) = (-1).*max(CandidateNextWakeTrans(CandidateNextWakeTrans<=0));
    end
...
for k=1:length(Time_Stim)
    ...
    if CandidateNextSWSTrans(1) > 0
        PreviousSWSTrans(k) = NaN;
    else
        PreviousSWSTrans(k) = (-1).*max(CandidateNextSWSTrans(CandidateNextSWSTrans<=0));
    end
...
for k=1:length(Time_Stim)
    ...
    if CandidateNextREMTrans(1) > 0
        PreviousREMTrans(k) = NaN;
    else
        PreviousREMTrans(k) = (-1).*max(CandidateNextREMTrans(CandidateNextREMTrans<=0));
    end
...
for k3=1:Nb_Stim
    ...
    if min([NextWakeTrans(k3),NextSWSTrans(k3),NextREMTrans(k3)])==NextWakeTrans(k3)
        Result(k3,3) = 0;
        Result(k3,4) = NextWakeTrans(k3);
        if Result(k3,2) == 1
            Nb_Trans_SWS_Wake = Nb_Trans_SWS_Wake + 1;
            Latence(Nb_Trans_SWS_Wake,8) = NextWakeTrans(k3);
            Latence(Nb_Trans_SWS_Wake,7) = k3;
    ...
    elseif min([NextWakeTrans(k3),NextSWSTrans(k3),NextREMTrans(k3)])==NextSWSTrans(k3)
        Result(k3,3) = 1;
        ...
        if Result(k3,2) == 1
            ...
            Latence(Nb_Trans_SWS_REM,6) = NextREMTrans(k3);
            Latence(Nb_Trans_SWS_REM,5) = k3;
        elseif Result(k3,2) == 2
            error = error + 1;
        else
            Nb_Stim_Wake = Nb_Stim_Wake +1;
...
Latence = Latence(1:max([Nb_Trans_REM_SWS, Nb_Trans_REM_Wake, Nb_Trans_SWS_REM, Nb_Trans_SWS_Wake]),:);
Nb = [[Nb_Stim; Nb_Trans_REM_SWS; Nb_Trans_REM_Wake; Nb_Trans_SWS_REM; Nb_Trans_SWS_Wake; Nb_Stim_Wake; error],zeros(7,2)];
Legend_Nb = ["Nb_Stim"; "Nb_Trans_REM_SWS"; "Nb_Trans_REM_Wake"; "Nb_Trans_SWS_REM"; "Nb_Trans_SWS_Wake"; "Nb_Stim_Wake"; "error"];
...
Nb(5,2) = mean(Latence(1:Nb_Trans_SWS_REM,6));
Nb(6,2) = mean(Latence(1:Nb_Trans_SWS_Wake,8));

Nb(2,3) = std(Latence(1:Nb_Trans_REM_SWS,2));
Nb(3,3) = std(Latence(1:Nb_Trans_REM_Wake,4));
Nb(5,3) = std(Latence(1:Nb_Trans_SWS_REM,6));
Nb(6,3) = std(Latence(1:Nb_Trans_SWS_Wake,8));
