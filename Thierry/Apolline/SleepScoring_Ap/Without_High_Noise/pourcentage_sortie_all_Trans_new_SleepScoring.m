function [] = pourcentage_sortie_all_Trans_new_SleepScoring(DataLocation, result, plot)
%pourcentage_sortie_all_Trans_new_SleepScoring Summary of this function goes here
...
%     plot = 'total'; % 'all' if you want the graphs per night, 'total' if you want only the sum graph
    ...
    for i = 1:length(DataLocation)
         DataLocation{i}=[DataLocation{i} '/Without_High_Noise'];
    end
    ...
    for l = 1:2:length(DataLocation)
        ...
        if strcmp(plot,'all') == 1
            ...
            suplabel([Mouse_Nb ' - Baseline ' night_Baseline ' - Stim ' night_Stim ' - ground Noise'],'t')
    ...
    suplabel('all 6 mices together - groundNoise','t')
