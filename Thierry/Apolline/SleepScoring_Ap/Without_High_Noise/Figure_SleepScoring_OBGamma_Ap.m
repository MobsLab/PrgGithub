...
function Figure_SleepScoring_OBGamma_Ap(foldername)
...
load([foldername 'SleepScoring_OBGamma_Ap'])

% load OB spectrum
if exist([foldername,'B_High_Spectrum_Ap.mat'])>0
    load([foldername,'B_High_Spectrum_Ap.mat']);
...
% load HPC spectrum
if exist([foldername,'H_Low_Spectrum_Ap.mat'])>0
    load([foldername,'H_Low_Spectrum_Ap.mat']);
...
try    
    res=pwd;
    cd(foldername);
    saveas(SleepScoringFigureOB, 'SleepScoringOB_Ap.png', 'png');
    cd(res);
catch    
    res=pwd;
    cd(foldername);
    saveas(SleepScoringFigureOB.Number, 'SleepScoringOB_Ap.png', 'png');
    cd(res);
end
