...
% 31.05.2018 AF
%
% SleepScoringOBGamma_Ap('PlotFigure',1)
%
% !! MODIFIED FONCTION WHITHOUT HIGH NOISE !!
%
...
function SleepScoringOBGamma_Ap(varargin)
...
if ~recompute
    if exist(fullfile(cd,'SleepScoring_OBGamma_Ap.mat'),'file')==2
...
save('SleepScoring_OBGamma_Ap','Info')
clear Info
...
disp('NoiseEpochs')
[Epoch,TotalNoiseEpoch,SubNoiseEpoch,Info_temp] = FindNoiseEpoch_SleepScoring_Ap(channel_hpc, 'foldername', foldername, 'user_confirmation', user_confirmation);
Info=Info_temp; clear Info_temp;
save('SleepScoring_OBGamma_Ap','Epoch','SubNoiseEpoch','TotalNoiseEpoch','-append')

%% Find theta and gamma epochs

% Gamma epoch
disp('Gamma Epochs')
[Sleep,SmoothGamma,Info_temp]=FindGammaEpoch_SleepScoring_Ap(Epoch, channel_bulb, minduration, 'foldername', foldername, 'user_confirmation', user_confirmation);
Info=ConCatStruct(Info,Info_temp); clear Info_temp;
save('SleepScoring_OBGamma_Ap','Sleep','SmoothGamma','-append')

% Theta epoch
disp('Theta Epochs')
[ThetaEpoch,SmoothTheta, ~, Info_temp] = FindThetaEpoch_SleepScoring_Ap(Sleep, channel_hpc, minduration, 'foldername', foldername, 'user_confirmation', user_confirmation);
Info=ConCatStruct(Info,Info_temp); clear Info_temp;
save('SleepScoring_OBGamma_Ap','ThetaEpoch','SmoothTheta','-append')
...
save('SleepScoring_OBGamma_Ap','REMEpoch','SWSEpoch','Wake','-append')


%% save Info 
save('SleepScoring_OBGamma_Ap','Info','-append')


%% Make sleep scoring figure if PlotFigure is 1
if PlotFigure==1
    % Calculate spectra if they don't alread exist
    if ~(exist('H_Low_Spectrum_Ap.mat', 'file') == 2)
        LowSpectrumSB_Ap(foldername,channel_hpc,'H');
    end
    if ~(exist('B_High_Spectrum_Ap.mat', 'file') == 2)
        HighSpectrum_Ap(foldername,channel_bulb,'B');
    end
    
    % Make figure
    Figure_SleepScoring_OBGamma_Ap(foldername)
end
