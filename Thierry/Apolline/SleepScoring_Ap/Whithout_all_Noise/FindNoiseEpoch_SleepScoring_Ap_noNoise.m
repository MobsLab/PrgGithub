...
function [Epoch,TotalNoiseEpoch,SubNoiseEpoch,Info]=FindNoiseEpoch_SleepScoring_Ap_noNoise(channel_hpc, varargin)
...
% Get HPC spectrum
if ~(exist([foldername '/' 'H_Low_Spectrum_Ap_noNoise.mat'], 'file') == 2)
    LowSpectrumSB_Ap_noNoise(foldername, channel_hpc,'H');
end
load(strcat(foldername, '/H_Low_Spectrum_Ap_noNoise.mat'))
fH = Spectro{3};
tH = Spectro{2};
SpH = Spectro{1};
