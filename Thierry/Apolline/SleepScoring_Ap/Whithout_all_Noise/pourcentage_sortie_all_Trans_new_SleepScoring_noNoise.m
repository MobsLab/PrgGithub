function [] = pourcentage_sortie_all_Trans_new_SleepScoring_noNoise(DataLocation,result,plot)
...
%     plot = 'total'; % 'all' if you want the graphs per night, 'total' if you want only the sum graph
    ...
    for l = 1:2:length(DataLocation)
        ...
        for i= [l l+1]
            ...
            if i == l
                Result = Analyse_Stim_Ap_new_sleepScoring_noNoise();
            else
                Result = Analyse_Stim_Ap_new_sleepScoring_noNoise();
            end
    ...
    suplabel('all 6 mice together','t')
