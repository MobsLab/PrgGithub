...
function Figure_SleepScoring_OBGamma_Ap_noNoise(foldername)
...
load([foldername 'SleepScoring_OBGamma_Ap_noNoise'])

% load OB spectrum
if exist([foldername,'B_High_Spectrum_Ap_noNoise.mat'])>0
    load([foldername,'B_High_Spectrum_Ap_noNoise.mat']);
...
% load HPC spectrum
if exist([foldername,'H_Low_Spectrum_Ap_noNoise.mat'])>0
    load([foldername,'H_Low_Spectrum_Ap_noNoise.mat']);
...
try    
    res=pwd;
    cd(foldername);
    saveas(SleepScoringFigureOB, 'SleepScoringOB_Ap_noNoise.png', 'png');
    cd(res);
catch    
    res=pwd;
    cd(foldername);
    saveas(SleepScoringFigureOB.Number, 'SleepScoringOB_Ap_noNoise.png', 'png');
    cd(res);
end
