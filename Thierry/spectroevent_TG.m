...
ev=ts(events*1E4);
length(Range(Restrict(ev,REMEpochWiNoise)))
length(Range(Restrict(ev,SWSEpochWiNoise)))
length(Range(Restrict(ev,WakeWiNoise)))
evR=Range(Restrict(ev,REMEpochWiNoise),'s');
evS=Range(Restrict(ev,SWSEpochWiNoise),'s');
evW=Range(Restrict(ev,WakeWiNoise),'s');

n=n+1;evc=evR; 
subplot(3,1,1),xlim([evc(n)-80 evc(n)+50]), caxis([10 70]),
subplot(3,1,2),xlim([evc(n)-80 evc(n)+50]), caxis([10 70]),
subplot(3,1,3),xlim([evc(n)-80 evc(n)+50]), caxis([10 70])
