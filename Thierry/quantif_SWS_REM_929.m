...
%% Homecage CNO_Atropine
cd /media/nas5/Thierry_DATA/M929_processed/929_CNO_atropine_05092019_190905_095055_BIS
load('SleepScoring_OBGamma.mat', 'Wake','REMEpoch','SWSEpoch','Sleep')

%SWS CNO_Atropine
SWS929CNO_AtropineDurationTot=sum(End(SWSEpoch,'s')-Start(SWSEpoch,'s'));%durée total de SWS
SWS929CNO_AtropineStart=Start(SWSEpoch)./(1e4);
SWS929CNO_AtropineEnd=End(SWSEpoch)./(1e4);
SWS929CNO_AtropineDurationEpisods=SWS929CNO_AtropineEnd-SWS929CNO_AtropineStart;%durée de chaque épisode
Nb_SWSEpoch929CNO_Atropine = length(SWS929CNO_AtropineStart);%Nombre d'épisodes de SWS

%WAKE CNO_Atropine
Wake929CNO_AtropineDurationTot=sum(End(Wake,'s')-Start(Wake,'s'));
Wake929CNO_AtropineStart = Start(Wake)./(1e4);
Wake929CNO_AtropineEnd = End(Wake)./(1e4); 
Wake929CNO_AtropineDurationEpisods=Wake929CNO_AtropineEnd-Wake929CNO_AtropineStart;
Nb_WakeEpoch929CNO_Atropine = length(Wake929CNO_AtropineStart); 

%REM CNO_Atropine
REM929CNO_AtropineDurationTot=sum(End(REMEpoch,'s')-Start(REMEpoch,'s'));
REM929CNO_AtropineStart=Start(REMEpoch)./(1e4);
REM929CNO_AtropineEnd=End(REMEpoch)./(1e4);
REM929CNO_AtropineDurationEpisods=REM929CNO_AtropineEnd-REM929CNO_AtropineStart;
Nb_REMEpoch929CNO_Atropine = length(REM929CNO_AtropineStart);

%Pourcentage de REM/SWS/Wake sur tout le sommeil

%REM/Sleep
PercREM_Sleep_929_CNO_Atropine=(sum(Stop(REMEpoch,'s')-Start(REMEpoch,'s'))./sum(Stop(Sleep,'s')-Start(Sleep,'s'))).*100
%SWS/Sleep
PercSWS_Sleep_929_CNO_Atropine=(sum(Stop(SWSEpoch,'s')-Start(SWSEpoch,'s'))./sum(Stop(Sleep,'s')-Start(Sleep,'s'))).*100
%Wake/Total
PercWake_Total_929_CNO_Atropine=(sum(Stop(Wake,'s')-Start(Wake,'s'))./(sum(Stop(Wake,'s')-Start(Wake,'s'))+sum(Stop(Sleep,'s')-Start(Sleep,'s')))).*100
%SWS/Total
PercSWS_Total_929_CNO_Atropine=(sum(Stop(SWSEpoch,'s')-Start(SWSEpoch,'s'))./(sum(Stop(Wake,'s')-Start(Wake,'s'))+sum(Stop(Sleep,'s')-Start(Sleep,'s')))).*100
%REM/Total
PercREM_Total_929_CNO_Atropine=(sum(Stop(REMEpoch,'s')-Start(REMEpoch,'s')))/(sum(Stop(Wake,'s')-Start(Wake,'s'))+sum(Stop(Sleep,'s')-Start(Sleep,'s'))).*100

[PercSWS_Total_929_CNO_Atropine PercWake_Total_929_CNO_Atropine PercREM_Total_929_CNO_Atropine]
PlotErrorBarN_KJ({PercWake_Total_929_CNO_Atropine PercSWS_Total_929_CNO_Atropine PercREM_Total_929_CNO_Atropine})
ylabel('Percentage (%)')
xlabel('Conditions')
title('Sleep/Wake proportions (Mouse 929 CNO_Atropine)')
xticklabels({'Wake','SWS','REM'})
xticks([1 2 3])
set(gca,'FontSize',14)
...
%Nombre d'épisodes
[Nb_WakeEpoch929CNO_Atropine Nb_SWSEpoch929CNO_Atropine Nb_REMEpoch929CNO_Atropine] 
PlotErrorBarN_KJ({Nb_WakeEpoch929CNO_Atropine Nb_SWSEpoch929CNO_Atropine Nb_REMEpoch929CNO_Atropine})
ylabel('Number of events')
xlabel('Conditions')
title('Number of sate events (Mouse 929 CNO_Atropine)')
xticklabels({'Wake','SWS','REM'})
xticks([1 2 3])
set(gca,'FontSize',14)

%Total sleep duration CNO_Atropine
TotalSleep929CNO_Atropine=SWS929CNO_AtropineDurationTot+REM929CNO_AtropineDurationTot;
TotalSession929CNO_Atropine=Wake929CNO_AtropineDurationTot+SWS929CNO_AtropineDurationTot+REM929CNO_AtropineDurationTot;
...
%% Calcul REM 1/3 - 2/3 - 3/3 CNO_Atropine
...
maxlimCNO_Atropine=max([max(End(Wake)),max(End(REMEpoch)), max(End(SWSEpoch))]);

numpointsCNO_Atropine=floor(maxlimCNO_Atropine/pas/1E4)+1;

for i=1:numpointsCNO_Atropine
    perCNO_Atropine(i)=FindPercREM(REMEpoch,SWSEpoch, intervalSet((i-1)*pas*1E4,i*pas*1E4));
    tpsCNO_Atropine(i)=(i*pas*1E4+(i-1)*pas*1E4)/2;
end

% function [PercFirstHalf, PercSecHalf, PercFirstThird,PercSecThird,PercLastThird]=PercREMFirstSecHalf(REMEpoch,SWSEpoch,Wake,pas,plo)

PercFirstHalfCNO_Atropine_929=nanmean(perCNO_Atropine(1:length(perCNO_Atropine)/2));
PercSecHalfCNO_Atropine_929=nanmean(perCNO_Atropine(length(perCNO_Atropine)/2:end));

PercFirstThirdCNO_Atropine_929=nanmean(perCNO_Atropine(1:length(perCNO_Atropine)/3))
PercSecThirdCNO_Atropine_929=nanmean(perCNO_Atropine(length(perCNO_Atropine)/3:2*length(perCNO_Atropine)/3))
PercLastThirdCNO_Atropine_929=nanmean(perCNO_Atropine(2*length(perCNO_Atropine)/3:end))

PlotErrorBarN_KJ({PercFirstHalfCNO_Atropine_929 PercSecHalfCNO_Atropine_929 PercFirstThirdCNO_Atropine_929 PercSecThirdCNO_Atropine_929 PercLastThirdCNO_Atropine_929})
ylabel('%REM')
xlabel('Periods')
title('Percentage REM in different periods CNO_Atropine')
xticklabels({'FirstHalf','SecondHalf','FirstThird','SecondThird', 'LastThird'})
xticks([1 2 3 4 5])
set(gca,'FontSize',14)
...
% if plo
SleepStagesCNO_Atropine=PlotSleepStage(Wake,SWSEpoch,REMEpoch,1);
hold on, plot(tpsCNO_Atropine/1E4,rescale(perCNO_Atropine,-1,2),'ro-','LineWidth',2)
set(gca,'FontSize',14)
ylabel('%REM and stages')
xlabel('Time (s)')
title('Percentage REM over time')
...
load('SleepScoring_OBGamma.mat', 'Wake','REMEpoch','SWSEpoch','Sleep')
load('H_Low_Spectrum.mat')
Spectrotsd_CNO_Atropine=tsd(Spectro{2}*1E4,10*log10(Spectro{1}));
f=Spectro{3};
plot(f,mean(Data(Restrict(Spectrotsd_CNO_Atropine,REMEpoch))),'k','LineWidth',2)
hold on,
...
Spectrotsd_CNO_Atropine=tsd(Spectro{2}*1E4,10*log10(Spectro{1}));
f=Spectro{3};
plot(f,mean(Data(Restrict(Spectrotsd_CNO_Atropine,Wake))),'k','LineWidth',2)
hold on,
...
Spectrotsd_CNO_Atropine=tsd(Spectro{2}*1E4,10*log10(Spectro{1}));
f=Spectro{3};
plot(f,mean(Data(Restrict(Spectrotsd_CNO_Atropine,SWSEpoch))),'k','LineWidth',2)
hold on,
