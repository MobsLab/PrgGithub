 function [Q] = NWbootstrap(X, Y, bins, bandwidth, alpha, resamples)
...
%----------------------
%
%NWbootstrap(X, Y, BINS, BANDWIDTH, ALPHA, RESAMPLES)
%
%Produces the non-parametric Nadaraya-Watson estimate of the mean of Y
%conditional on X, using a normal kernal. It also estimates the 1-ALPHA 
%confidence interval via bootstrapping and graphs both. 
%
...
%
%The output is an array that is 4 by BINS.  The first row is the values of
...
plot(binsX,CI(1,:),'k--')
plot(binsX,CI(2,:),'k--')
first = ['normal kernal, bandwidth = ' num2str(h)];
