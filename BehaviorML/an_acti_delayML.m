...
if numBatch==1
    ...
    ID_Cond=[47 48 52 55 57,    61 63 65 67,    72 74 76 78,...
...
%         45 47 50 52]; % COND
    ...
    ID_Basal=[65 67 69 71 81 91];% 101];
    
    ID_Test0 = [66 68 70];% TEST 0
    ID_Test = [80 90 100];% 110];% TEST
    ID_TestF = [];% TESTFINAL
    
    ID_Cond=[72 74 76 78, 82 84 86 88, 92 94 96 98];% 102 104 106 108]; % COND
...
MAT(ind,:)=[];
%numSessions=[IDacq(IDacq(:,2)==0,1);IDacq(IDacq(:,2)==1 | IDacq(:,2)==2,1);IDacq(IDacq(:,2)==3,1)];
...
numSessions=IDacq(IDacq(:,2)==1 | IDacq(:,2)==2,1);
...
clear semW semS
for n=1:length(numSessions)
    semW(n,1)=sem(Mat_W(~isnan(Mat_W(:,n)) & gpMice'==0,n));
    semS(n,1)=sem(Mat_S(~isnan(Mat_S(:,n)) & gpMice'==0,n));
    semW(n,2)=sem(Mat_W(~isnan(Mat_W(:,n)) & gpMice'==1,n));
    semS(n,2)=sem(Mat_S(~isnan(Mat_S(:,n)) & gpMice'==1,n));
    
    
    semW(n,3)=sem(W(~isnan(W(:,n)) & gpMice'==0,n));
    semS(n,3)=sem(S(~isnan(S(:,n)) & gpMice'==0,n));
    semW(n,4)=sem(W(~isnan(W(:,n)) & gpMice'==1,n));
    semS(n,4)=sem(S(~isnan(S(:,n)) & gpMice'==1,n));
end
...
errorbar(1:length(numSessions),nanmean(Mat_S(gpMice==0,:),1),semS(:,1),'.-r','Linewidth',2)
errorbar(1:length(numSessions),nanmean(Mat_S(gpMice==1,:),1),semS(:,2),'.-b','Linewidth',2)
...
errorbar(1:length(numSessions),nanmean(Mat_W(gpMice==0,:),1),semW(:,1),'.-r','Linewidth',2)
errorbar(1:length(numSessions),nanmean(Mat_W(gpMice==1,:),1),semW(:,2),'.-b','Linewidth',2)
...
% errorbar(1:length(numSessions),nanmean(Mat_S(gpMice==0,:),1),std(Mat_S(gpMice==0,:)),'.-r','Linewidth',2)
% errorbar(1:length(numSessions),nanmean(Mat_S(gpMice==1,:),1),std(Mat_S(gpMice==1,:)),'.-b','Linewidth',2)
errorbar(1:length(numSessions),nanmean(S(gpMice==0,:),1),semS(:,3),'.-r','Linewidth',2)
errorbar(1:length(numSessions),nanmean(S(gpMice==1,:),1),semS(:,4),'.-b','Linewidth',2)
title('Delay snd -> sleep')
...
% errorbar(1:length(numSessions),nanmean(Mat_W(gpMice==0,:),1),std(Mat_W(gpMice==0,:)),'.-r','Linewidth',2)
% errorbar(1:length(numSessions),nanmean(Mat_W(gpMice==1,:),1),std(Mat_W(gpMice==1,:)),'.-b','Linewidth',2)
errorbar(1:length(numSessions),nanmean(W(gpMice==0,:),1),semW(:,3),'.-r','Linewidth',2)
errorbar(1:length(numSessions),nanmean(W(gpMice==1,:),1),semW(:,4),'.-b','Linewidth',2)
title('Delay snd -> wake')
ylabel('10*log(tdelta) NORM within mouse')
xlabel('# test trials') 
xlim([0 length(numSessions)])
...
legend({['Cond sleep (n=',num2str(length(find(gpMice==0))),', #',sprintf(' %d',channelID(gpMice==0)),')'],...
    ['Cond wake (n=',num2str(sum(gpMice)),', #',sprintf(' %d',channelID(gpMice==1)),')'],'TEST0','TEST'})
