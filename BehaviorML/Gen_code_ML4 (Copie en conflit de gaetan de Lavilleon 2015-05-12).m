            ...
            while(arreter_init == 0)
                ...
                if change_parameter==1
                    ...
                    if 0
                        ...
                        % set videoparams
                        nameParams1={'BacklightCompensation','ExposureMode','FocusMode',...
                            'FrameRate','HorizontalFlip','VerticalFlip','WhiteBalanceMode'};
                        nameParams2={'Brightness','Contrast','Exposure','Focus','Gain',...
                            'Pan','Saturation','Sharpness','Tilt','WhiteBalance','Zoom'};
                        defaultValues1={'on','auto','auto','30.0000','off','off','auto'};
                        defaultValues2=[128,32,-7,51,64,0,32,22,0,4000,1];
                        
                        % reloading previous values
                        try
                            tempInfo=load('InfoTrackingTemp.mat','VideoParams');
                            for dd=1:length(nameParams1)
                                eval(['defaultValues1{dd}=tempInfo.VideoParams.',nameParams1{dd},';']);
                            end
                            for dd=1:length(nameParams2)
                                eval(['defaultValues2(dd)=tempInfo.VideoParams.',nameParams2{dd},';']);
                            end
                            disp('Video parameters from last session has been reloaded');
                        end
                        
                        for dd=1:length(nameParams1)
                            eval(['set(src,''',nameParams1{dd},''',''',defaultValues1{dd},''');']);
                        end
                        for dd=1:length(nameParams2)
                            eval(['set(src,''',nameParams2{dd},''',',num2str(defaultValues2(dd)),');']);
                        end
    ...
    function get_reference_v3(obj,event)
        ...
        function save_ref(obj,event) % keep ref in memory
            ...
            try
                ...
                save('InfoTrackingTemp.mat','ref','vid');
            end
            pause(0.5); delete(msg_box)
