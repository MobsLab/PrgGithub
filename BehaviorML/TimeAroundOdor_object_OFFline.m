...
rad_obj=0.01; % radius circular objets
...
Fig_Odor=figure('units','normalized','position',[0.1 0.08 0.3 0.6],...
...
%% RunExpeAllNight
    function RunExpeAllNight(obj,event)
        ...
        try
            ...
            nmouseexpe=input('Enter number of mice in the experiment: ');
            
            for i = 1:nmouseexpe
                temp=input(['Enter phase 0 path of mouse #',num2str(i),', : '],'s');
                NameExpeToAnalyze{i}=temp;
            end
        ...
        for nam=1:length(NameExpeToAnalyze)
            disp(' ')
            folder_name=[res,mark,NameExpeToAnalyze{nam}];
            try
                Tempreload=load([folder_name,mark,'TempNightOFF.mat']);
                OdorInfo=Tempreload.OdorInfo;
                maskobj=Tempreload.maskobj;%error
                disp([folder_name(max(strfind(folder_name,mark))+1:end),mark,'TempNightOFF already define... skipping this step'])
            catch
                ...
                copyfile([res,mark,'TempAroundOdorOFF.mat'],[folder_name,mark,'TempNightOFF.mat'])
                
                %save([folder_name,mark,'TempNightOFF.mat'],'-append','ListOfPhases','nPhase','nCTRL','nVARI','nFIXE','d_obj','folder_name','OrderLabel','rad_obj','StepTime')
        ...
        for nam=1:length(NameExpeToAnalyze)
            ...
            for ph=1:nPhase
                ...
                WaitToSaveAndLoad=1; save([folder_name,mark,'TempNightOFF.mat'],'-append','WaitToSaveAndLoad');
                clear WaitToSaveAndLoad
                copyfile([folder_name,mark,'TempNightOFF.mat'],[res,mark,'TempAroundOdorOFF.mat'])
                
                while ~exist('WaitToSaveAndLoad','var')
                    pause(1)
                    try
                        Tempreload=load([res,mark,'TempAroundOdorOFF.mat']);
                        WaitToSaveAndLoad=Tempreload.WaitToSaveAndLoad;
                    end
                end
                ...
                try Track_sniff(obj,event); catch, disp('PROBLEM !!!!!!!!!!');keyboard;end
                % -------- wait for start_Phase to finish -----------
...
%% Interface of analysis
    function start_Phase(obj,event)
        ...
        while n<10 && ok==0
            ...
            filename2=filename;
            try filename2(length(filename)-length_end-1:length(filename)-length_end)=numRec; catch, disp('!!! NO InfoTracking.mat !!!');keyboard;end
...
%% track mouse
    function Track_sniff(obj,event)
        ...
        try
            ...
            try
                intermed=load([folderPhase,mark,'InfoTracking.mat']);
                intermed.BW_threshold=0.25;
            catch
                enableTrack=0;
                disp(['No ',filename])
                keyboard
        ...
        while fr~=length(list) && enableTrack
            ...
            try
                ...
                set(inputDisplay(8),'string',['Time in recording = ',num2str(floor(Timeframe)),'s - Frames = ',num2str(fr),mark,num2str(length(list))]);
...
%% display tracking and save figure for all phases
    function Genrat_figureALLEXPE(obj,event)
        ...
        % -----------------------------------------------------------------
        % ------------------------- INPUTS --------------------------------
        HardDriveLocation= uigetdir(res,'Choose Folder to save figure');
        %         HardDriveLocation='/media/DataMOBs20/OdorMOMdKO-5-7/';%'H:\OdorMOMdKO\';%'/media/DataMOBs16/ProjetBULB/';
        %         nameFolderAnaly='AnalyzeOdorRecognition-dKO';
        typeanalyname=' row';%   ' - abs(log())'
        ErasePreviousAnaly=1;
        StepTimeImposed=[60 120 180]; %time in sec
        StepInterval=30; %time in sec
        lengthRecording=360; % in sec
        d_objImposed=[1 2 3]; % distance to object (cm)
        PercExcluOnObj=50;% remove if 50% of the animal is inside OdorZone
        %nPhase=3;
        plotfigureIndiv=1;
        freqresample=10; %hz
        ThreshSNIFF=3; % threshold to count as a sniff (in sec)
        ...
        if exist('NameExpeToAnalyze','var')
            ...
            try
                tlo=load([HardDriveLocation,mark,'MatExpe.mat'],'MatExpe','MatExpeInterval');
                MatExpe=tlo.MatExpe;
                MatExpeInterval=tlo.MatExpeInterval;
                if ErasePreviousAnaly, error; end
            catch
                MatExpe={};
                MatExpeInterval={};
                for nam=1:length(NameExpeToAnalyze)
                    ...
                    for ph=1:nPhase
                        ...
                        copyfile([folderPhase,mark,'AroundOdor.mat'],[HardDriveLocation,mark,'Mouse',num2str(nMouse),'-Phase',num2str(nPh),'-AroundOdor.mat'])
                            ...
                            if 0
                                for n_step=1:length(StepTimeImposed)
                                    ...
                                    for od=1:length(OdorInfo(:,3))
                                        ...
                                        % Odor Loc
                                        MatExpeTemp(od,1:3)=[nMouse, nPh, OdorInfo(od,3)];
                                        
                                        % resample Sniff at freqresample Hz
                                        timeWanted=1:1/freqresample:max(CaractSniff(indexTS,1));
                                        try
                                            sniffresample=interp1(CaractSniff(indexTS,1),CaractSniff(indexTS,6),timeWanted);
                                            % mean Time In Sniff
                                            %tempTSD=tsd(1E4*timeWanted',sniffresample');
                                            tempTSD=tsd(1E4*timeWanted',smooth(sniffresample',10));
                                            SniffEpoch=thresholdIntervals(tempTSD,10^(-10),'Direction','Above');
                                        catch
                                        ...
                                        if ~isempty(Stop(SniffEpoch,'s')),
                                            MatExpeTemp(od,4)=nanmean(Stop(SniffEpoch,'s')-Start(SniffEpoch,'s'));
                                            MatExpeTemp(od,5)=nansum(Stop(SniffEpoch,'s')-Start(SniffEpoch,'s'));
                                        else
                                            MatExpeTemp(od,4)=0;
                                            MatExpeTemp(od,5)=0;
                                        end
                                        MatExpeTemp(od,6)=length(Stop(SniffEpoch,'s'));
                                ...
                                % preallocate
                                MatTemp1=NaN(length(OdorInfo(:,3)),lengthRecording/StepInterval+3);
                                MatTemp2=MatTemp1;MatTemp3=MatTemp1;
                                MatTemp1th=MatTemp1;MatTemp2th=MatTemp1;MatTemp3th=MatTemp1;
                                MatTemp1thAbov=MatTemp1;MatTemp2thAbov=MatTemp1;MatTemp3thAbov=MatTemp1;
                                
                                for n_sI=1:floor(max(CaractSniff(:,1))/StepInterval)
                                    for od=1:length(OdorInfo(:,3))
                                        %disp(num2str([dob, n_step, od]))
                                        % restrict to all conditions
                                        indexTS=find(CaractSniff(:,1)<n_sI*StepInterval & CaractSniff(:,1)>(n_sI-1)*StepInterval & CaractSniff(:,4)==OdorInfo(od,3) & CaractSniff(:,3)==d_objImposed(dob) & CaractSniff(:,6)<PercExcluOnObj);
                                        
                                        % Odor Loc
                                        MatTemp1(od,1:3)=[nMouse, nPh, OdorInfo(od,3)]; MatTemp1th(od,1:3)=[nMouse, nPh, OdorInfo(od,3)]; MatTemp1thAbov(od,1:3)=[nMouse, nPh, OdorInfo(od,3)];
                                        MatTemp2(od,1:3)=[nMouse, nPh, OdorInfo(od,3)]; MatTemp2th(od,1:3)=[nMouse, nPh, OdorInfo(od,3)]; MatTemp2thAbov(od,1:3)=[nMouse, nPh, OdorInfo(od,3)];
                                        MatTemp3(od,1:3)=[nMouse, nPh, OdorInfo(od,3)]; MatTemp3th(od,1:3)=[nMouse, nPh, OdorInfo(od,3)]; MatTemp3thAbov(od,1:3)=[nMouse, nPh, OdorInfo(od,3)];
                                        ...
                                        LengthSniff=Stop(SniffEpoch,'s')-Start(SniffEpoch,'s');
                                        if ~isempty(Stop(SniffEpoch,'s')),
                                            MatTemp1(od,n_sI+3)=nanmean(Stop(SniffEpoch,'s')-Start(SniffEpoch,'s'));
                                            MatTemp2(od,n_sI+3)=nansum(Stop(SniffEpoch,'s')-Start(SniffEpoch,'s'));
                                        else
                                            MatTemp1(od,n_sI+3)=0;MatTemp1th(od,n_sI+3)=0;MatTemp1thAbov(od,n_sI+3)=0;
                                            MatTemp2(od,n_sI+3)=0;MatTemp2th(od,n_sI+3)=0;MatTemp2thAbov(od,n_sI+3)=0;
                                            
                                        end
                                        
                                        % threshold on sniff length
                                        if ~isempty(find(LengthSniff<ThreshSNIFF))
                                            MatTemp1th(od,n_sI+3)=nanmean(LengthSniff(find(LengthSniff<ThreshSNIFF)));
                                            MatTemp2th(od,n_sI+3)=nansum(LengthSniff(find(LengthSniff<ThreshSNIFF)));
                                        else
                                            MatTemp1th(od,n_sI+3)=0;MatTemp2th(od,n_sI+3)=0;
                                        end
                                        if ~isempty(find(LengthSniff>=ThreshSNIFF))
                                            MatTemp1thAbov(od,n_sI+3)=nanmean(LengthSniff(find(LengthSniff>=ThreshSNIFF)));
                                            MatTemp2thAbov(od,n_sI+3)=nansum(LengthSniff(find(LengthSniff>=ThreshSNIFF)));
                                        else
                                            MatTemp1thAbov(od,n_sI+3)=0;MatTemp2thAbov(od,n_sI+3)=0;
                                        end
                                        
                                        MatTemp3(od,n_sI+3)=length(Stop(SniffEpoch,'s'));
                                        MatTemp3th(od,n_sI+3)=length(find(LengthSniff<ThreshSNIFF));
                                        MatTemp3thAbov(od,n_sI+3)=length(find(LengthSniff>=ThreshSNIFF));
                                    end
                                end
                                try tempMat=MatExpeInterval{dob,1}; catch, tempMat=[];end
                                MatExpeInterval{dob,1}=[tempMat; MatTemp1(:,1:(lengthRecording/StepInterval+3))];
                                
                                try tempMat=MatExpeInterval{dob,2}; catch, tempMat=[];end
                                MatExpeInterval{dob,2}=[tempMat; MatTemp2(:,1:(lengthRecording/StepInterval+3))];
                                
                                try tempMat=MatExpeInterval{dob,3}; catch, tempMat=[];end
                                MatExpeInterval{dob,3}=[tempMat; MatTemp3(:,1:(lengthRecording/StepInterval+3))];
                                
                                % threshold on sniff length
                                try tempMat=MatExpeInterval{dob,4}; catch, tempMat=[];end
                                MatExpeInterval{dob,4}=[tempMat; MatTemp1th(:,1:(lengthRecording/StepInterval+3))];
                                
                                try tempMat=MatExpeInterval{dob,5}; catch, tempMat=[];end
                                MatExpeInterval{dob,5}=[tempMat; MatTemp2th(:,1:(lengthRecording/StepInterval+3))];
                                
                                try tempMat=MatExpeInterval{dob,6}; catch, tempMat=[];end
                                MatExpeInterval{dob,6}=[tempMat; MatTemp3th(:,1:(lengthRecording/StepInterval+3))];
                                
                                % Above threshold on sniff length
                                try tempMat=MatExpeInterval{dob,7}; catch, tempMat=[];end
                                MatExpeInterval{dob,7}=[tempMat; MatTemp1thAbov(:,1:(lengthRecording/StepInterval+3))];
                                
                                try tempMat=MatExpeInterval{dob,8}; catch, tempMat=[];end
                                MatExpeInterval{dob,8}=[tempMat; MatTemp2thAbov(:,1:(lengthRecording/StepInterval+3))];
                                
                                try tempMat=MatExpeInterval{dob,9}; catch, tempMat=[];end
                                MatExpeInterval{dob,9}=[tempMat; MatTemp3thAbov(:,1:(lengthRecording/StepInterval+3))];
                                
                                
                                for n_step=1:length(StepTimeImposed)
                                    % Mouse - Phase - OdorLoc -
                                    % meanTimeInSniff  - sum in sniff- sumPresence -
                                    indexStepI=floor(StepTimeImposed(n_step)/StepInterval);
                                    MatExpeTemp=[];
                                    for od=1:length(OdorInfo(:,3))
                                        MatExpeTemp(od,1:3)=[nMouse, nPh, OdorInfo(od,3)];
                                        
                                        MatExpeTemp(od,4)=nanmean(MatTemp1(od,4:indexStepI+3));
                                        MatExpeTemp(od,5)=nansum(MatTemp2(od,4:indexStepI+3));
                                        MatExpeTemp(od,6)=nansum(MatTemp3(od,4:indexStepI+3));
                                        
                                        % threshold on sniff length
                                        MatExpeTemp(od,7)=nanmean(MatTemp1th(od,4:indexStepI+3));
                                        MatExpeTemp(od,8)=nansum(MatTemp2th(od,4:indexStepI+3));
                                        MatExpeTemp(od,9)=nansum(MatTemp3th(od,4:indexStepI+3));
                                        
                                        % threshold on sniff length
                                        MatExpeTemp(od,10)=nanmean(MatTemp1thAbov(od,4:indexStepI+3));
                                        MatExpeTemp(od,11)=nansum(MatTemp2thAbov(od,4:indexStepI+3));
                                        MatExpeTemp(od,12)=nansum(MatTemp3thAbov(od,4:indexStepI+3));
                                    end
                ...
                disp(['-> saving in ',HardDriveLocation,mark,'MatExpe.mat'])
                save([HardDriveLocation,mark,'MatExpe.mat'],'MatExpe','MatExpeInterval')
                
            end
            
            % --------------------------------------------------
            % ----------- Display Time in sniff in odor zone -----------
            nbparam=size(MatExpe{1,1},2)-3;
            NameColumn(1:3)={'Mean sniff duration','Tot time in sniff','Nb sniff'};
            NameColumn(4:6)={['Mean sniff duration <',num2str(ThreshSNIFF),'s'],['Tot time in sniff <',num2str(ThreshSNIFF),'s'],['Nb sniff <',num2str(ThreshSNIFF),'s']};
            NameColumn(7:9)={['Mean sniff duration >=',num2str(ThreshSNIFF),'s'],['Tot time in sniff >=',num2str(ThreshSNIFF),'s'],['Nb sniff >=',num2str(ThreshSNIFF),'s']};
            a=-1;
            a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='VAR+FIX';end
            %a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='VAR/FIX';end
            a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='VAR-FIX';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='VAR/FIX  norm ph0';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='VAR+FIX norm ph0';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='VAR-FIX norm ph0';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='%VAR';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='%VAR norm ph0';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='abs(VAR-FIX)';end
            %             a=a+1; for aa=a*nbparam+1:(a+1)*nbparam, ParamFigures{aa}='abs(VAR-FIX) norm ph0';end
            
            numFig=1:length(ParamFigures);  % numFig=[19:length(ParamFigures)];
            for fig=numFig, figure('Color',[1 1 1],'Position',[scrsz(1:2),scrsz(3)/4, scrsz(4)/3]), numF(fig)=gcf;end
            if plotfigureIndiv
                for fig=1:nbparam*length(StepTimeImposed), figure('Color',[1 1 1],'Position',[scrsz(1:2),scrsz(3)/5, scrsz(4)/3]), numG(fig)=gcf;end;
                for fig=1:nbparam, figure('Color',[1 1 1],'Position',[scrsz(1:2),scrsz(3)/2, scrsz(4)/4]), numFI(fig)=gcf;end;
            end
            ...
            for dob=1:length(d_objImposed)
                for n_step=1:length(StepTimeImposed)
                    Mtemp=MatExpe{dob,n_step};
                    Umice=unique(Mtemp(:,1));
                    
                    for fig=numFig
                        ValVARvsFIX=[];tempValVARvsFIX=[];
                        Ncolumn=abs(rem(fig-1,nbparam))+4;
                        fprintf('%d',Ncolumn);
                        
                        for mi=1:length(Umice)
                            for ph=1:nPhase
                                indexVAR=find(Mtemp(:,1)==Umice(mi) & Mtemp(:,2)==ph-1 & Mtemp(:,3)==2);
                                indexFIX=find(Mtemp(:,1)==Umice(mi) & Mtemp(:,2)==ph-1 & Mtemp(:,3)==3);
                                
                                try
                                    ValVAR(mi,ph)=nanmean(Mtemp(indexVAR,Ncolumn),1);
                                    ValFIX(mi,ph)=nanmean(Mtemp(indexFIX,Ncolumn),1);
                                    
                                    if ~isempty(strfind(ParamFigures{fig},'VAR+FIX'))
                                        tempValVARvsFIX(mi,ph)=ValVAR(mi,ph)+ValFIX(mi,ph);
                                        
                                    elseif ~isempty(strfind(ParamFigures{fig},'VAR-FIX'))
                                        tempValVARvsFIX(mi,ph)=ValVAR(mi,ph)-ValFIX(mi,ph);
                                        
                                    elseif ~isempty(strfind(ParamFigures{fig},'VAR/FIX'))
                                        tempValVARvsFIX(mi,ph)=ValVAR(mi,ph)/ValFIX(mi,ph);
                                        
                                    elseif ~isempty(strfind(ParamFigures{fig},'%VAR'))
                                        tempValVARvsFIX(mi,ph)=100*ValVAR(mi,ph)/(ValVAR(mi,ph)+ValFIX(mi,ph));
                                    end
                                    
                                    if ~isempty(strfind(ParamFigures{fig},'abs'))
                                        tempValVARvsFIX(mi,ph)=abs(tempValVARvsFIX(mi,ph));
                                    end
                                    
                                    if ~isempty(strfind(ParamFigures{fig},'norm ph0'))
                                        ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph)/tempValVARvsFIX(mi,1);
                                    else
                                        ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph);
                                    end
                            ...
                            % data single animal
                            if fig<=nbparam && plotfigureIndiv
                                figure(numG((n_step-1)*length(StepTimeImposed)+Ncolumn-3)),
                                subplot(2,ceil(length(Umice)/2),mi),
                                bar([ValVAR(mi,:)',ValFIX(mi,:)'])
                                set(gca,'XTick',1:3)
                                set(gca,'XTickLabel',{'0','1','2'})
                                try
                                    templim=ylimG(Ncolumn-3,n_step);
                                    ylimG(Ncolumn-3,n_step)=max(templim,max(ylim));
                                catch
                                    ylimG(Ncolumn-3,n_step)=max(ylim);
                                end
                                
                                
                                xlabel('Phase')
                                ylabel(NameColumn{Ncolumn-3})
                                title(['Mouse',num2str(Umice(mi)),'; t=0-',num2str(StepTimeImposed(n_step)),'s ;  rZone=',num2str(d_objImposed(dob)),'cm'])
                                
                                if n_step==1 && ~isempty(MatExpeInterval)
                                    figure(numFI(Ncolumn-3))
                                    MItemp=MatExpeInterval{dob,Ncolumn-3};
                                    indexVAR=find(MItemp(:,1)==Umice(mi) & MItemp(:,3)==2);
                                    indexFIX=find(MItemp(:,1)==Umice(mi) & MItemp(:,3)==3);
                                    
                                    if Ncolumn-3==1, ylimi=[0,15]; elseif Ncolumn-3==2, ylimi=[0,30];else, ylimi=[0,8];end
                                    
                                    subplot(2,length(Umice),mi),
                                    bar(MItemp(indexFIX,4:end)); title(['Mouse ',num2str(Umice(mi)),' - FIXED'])
                                    ylim(ylimi); set(gca,'XTick',1:3); set(gca,'XTickLabel',{'0','1','2'})
                                    ylabel(NameColumn{Ncolumn-3})
                                    
                                    subplot(2,length(Umice),length(Umice)+mi),
                                    bar(MItemp(indexVAR,4:end)); title(['Mouse ',num2str(Umice(mi)),' - VARIANT'])
                                    ylim(ylimi); axis ij; xlabel('Phase')
                                    set(gca,'XTick',1:3); set(gca,'XTickLabel',{'0','1','2'})
                                    ylabel([num2str(StepInterval),'s steps'])
                                    
                                end
                            end
                            
                        end
                        if plotfigureIndiv && fig<=nbparam 
                            nameFigureIndiv(numG((n_step-1)*length(StepTimeImposed)+Ncolumn-3))={['FigureAnalyseOdorIndiv_0-',num2str(StepTimeImposed(n_step)),'s_',NameColumn{Ncolumn-3}]};
                            figure(numG((n_step-1)*length(StepTimeImposed)+Ncolumn-3)),
                            legend({'VARIANT','FIXED'})
                            for sub=1:length(Umice)
                                subplot(2,ceil(length(Umice)/2),sub),
                                ylim([0 ylimG(Ncolumn-3,n_step)]);
                            end
                        end
                        ...
                        % display
                        figure(numF(fig)),
                        subplot(length(d_objImposed),length(StepTimeImposed),(dob-1)*length(StepTimeImposed)+n_step)
                        
                        if strcmp(typeanalyname,' - abs(log())')
                            % remove inf values
                            tempValVARvsFIX=abs(log(ValVARvsFIX));
                            tempValVARvsFIX(isinf(tempValVARvsFIX))=NaN;
                            
                            MeanbarMat=nanmean(tempValVARvsFIX,1);
                            stdbarMat=stdError(abs(log(ValVARvsFIX)));
                            PlotErrorBar3(tempValVARvsFIX(:,1),tempValVARvsFIX(:,2),tempValVARvsFIX(:,3),0,2)
                            
                            pval(1)=ranksum(tempValVARvsFIX(:,1),tempValVARvsFIX(:,2));npval(1,1:2)=[sum(~isnan(ValVARvsFIX(:,1))),sum(~isnan(ValVARvsFIX(:,2)))];
                            if nPhase>2
                                pval(2)=ranksum(tempValVARvsFIX(:,1),tempValVARvsFIX(:,3));npval(2,1:2)=[sum(~isnan(ValVARvsFIX(:,1))),sum(~isnan(ValVARvsFIX(:,3)))];
                                pval(3)=ranksum(tempValVARvsFIX(:,2),tempValVARvsFIX(:,3));npval(3,1:2)=[sum(~isnan(ValVARvsFIX(:,2))),sum(~isnan(ValVARvsFIX(:,3)))];
                            end
                            
                        elseif strcmp(typeanalyname,' row')
                            MeanbarMat=nanmean(ValVARvsFIX,1);
                            stdbarMat=stdError(ValVARvsFIX);
                            PlotErrorBar3(ValVARvsFIX(:,1),ValVARvsFIX(:,2),ValVARvsFIX(:,3),0,2)
                            
                            pval(1)=ranksum(ValVARvsFIX(:,1),ValVARvsFIX(:,2));npval(1,1:2)=[sum(~isnan(ValVARvsFIX(:,1))),sum(~isnan(ValVARvsFIX(:,2)))];
                            if nPhase>2
                                pval(2)=ranksum(ValVARvsFIX(:,1),ValVARvsFIX(:,3));npval(2,1:2)=[sum(~isnan(ValVARvsFIX(:,1))),sum(~isnan(ValVARvsFIX(:,3)))];
                                pval(3)=ranksum(ValVARvsFIX(:,2),ValVARvsFIX(:,3));npval(3,1:2)=[sum(~isnan(ValVARvsFIX(:,2))),sum(~isnan(ValVARvsFIX(:,3)))];
                            end
                        end
                        %bar([0:nPhase-1],MeanbarMat);
                        %hold on, errorbar(0:1:length(MeanbarMat)-1,MeanbarMat,stdbarMat,'+k')
                        
%                         hold on, errorbar(1:length(MeanbarMat),MeanbarMat,stdbarMat,'+k')
                        set(gca,'XTick',1:3)
                        set(gca,'XTickLabel',{'0','1','2'})
                        title(['Duration = ',num2str(StepTimeImposed(n_step)),'s ;  Odor zone = ',num2str(d_objImposed(dob)),'cm'])
                        
                        % display significance
                        positionPv=[0.85 0.9 0.95];
                        ipvname={'01','02','12'};
                        ipvname=ipvname(1:length(pval));
                        for ipv=1:length(ipvname);
                            
                            line([str2num(ipvname{ipv}(1))+1 str2num(ipvname{ipv}(2))+1],[1 1]*max(ylim)*positionPv(ipv),'Color',colori{ipv+3});
                            pvaltemp=pval(ipv);
                            npvaltemp=[num2str(npval(ipv,1)),'-',num2str(npval(ipv,2))];
                            if pvaltemp<0.05,
                                text(mean([str2num(ipvname{ipv}(1))+1 str2num(ipvname{ipv}(2))+1]),max(ylim)*(positionPv(ipv)+0.03),['* ',npvaltemp],'Color',colori{ipv+3});
                            elseif pvaltemp<0.01,
                                text(mean([str2num(ipvname{ipv}(1))+1 str2num(ipvname{ipv}(2))+1]),max(ylim)*(positionPv(ipv)+0.03),['** ',npvaltemp],'Color',colori{ipv+3});
                            else
                                text(mean([str2num(ipvname{ipv}(1))+1 str2num(ipvname{ipv}(2))+1]),max(ylim)*(positionPv(ipv)+0.03),['NS ',npvaltemp],'Color',colori{ipv+3});
                            end
                        end
                        
                        ylimax(fig,(dob-1)*length(StepTimeImposed)+n_step)=max(ylim);
                        ylimin(fig,(dob-1)*length(StepTimeImposed)+n_step)=min(ylim);
                    end
            ...
            for fig=numFig
                figure(numF(fig)),
                for sub=1:length(d_objImposed)*length(StepTimeImposed)
                    subplot(length(d_objImposed),length(StepTimeImposed),sub)
                    ylim([min(ylimin(fig,:)),max(ylimax(fig,:))]);
                    xlabel(['Phase',typeanalyname])
                    if rem(sub,length(StepTimeImposed))==1
                        ylabel([NameColumn{abs(rem(fig-1,nbparam))+1},'  ',ParamFigures{fig}])
                    end
                end
            end
            ...
            disp('Saving figures...')
            for fig=numFig,
                nametemp=[NameColumn{abs(rem(fig-1,nbparam))+1},'_',ParamFigures{fig}]; 
                nametemp(strfind(nametemp,' '))='_'; nametemp(strfind(nametemp,'/'))='o';
                if ~isempty(strfind(nametemp,'>=')), nametemp=[nametemp(1:strfind(nametemp,'>=')-1),'Sup',nametemp(strfind(nametemp,'>=')+2:end)];end
                if ~isempty(strfind(nametemp,'<')), nametemp=[nametemp(1:strfind(nametemp,'<')-1),'Inf',nametemp(strfind(nametemp,'<')+1:end)];end
                saveFigure(numF(fig),['FigureAnalyseOdor_',nametemp],[HardDriveLocation]);
            end
            if plotfigureIndiv
                try
                    figure(numG((n_step-1)*length(StepTimeImposed)+Ncolumn-3))
                    for fig=numG
                        nametemp=nameFigureIndiv{fig}; nametemp(strfind(nametemp,' '))=[];
                        saveFigure(fig,nametemp,[HardDriveLocation]);
                    end
                    for fig=numFI
                        NCtemp=NameColumn{abs(fig-max(numFI)+3)};NCtemp(strfind(NCtemp,' '))='';
                        nametemp=['FigureAnalyseOdorIndiv_Multistep_',NCtemp];
                        saveFigure(fig,nametemp,[HardDriveLocation]);
                    end
                end
            end
