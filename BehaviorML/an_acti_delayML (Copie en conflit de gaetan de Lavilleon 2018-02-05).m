...
analyname=['an_acti_delay_Expe',num2str(numBatch),'.mat'];
if doBasal
    analyname = [analyname(1:end-4),'_Basal.mat'];
end
...
try
    ...
    for i=1:length(IDacq)
        ...
        for id =1:length(channelID)
            ...
            % Load data
            if strcmp(mark,'/')
                a = Activity_linux(numBatch,IDacq(1,i), channelID(id));
            else
                a = Activity(IDacq(1,i), channelID(id));
            end
            ...
            ind1=min(find(mod(hr_startRec+timeStamps/3600,24)> PeriodDay(1) & mod(hr_startRec+timeStamps/3600,24)< PeriodDay(2) ));
            ind2=min(find(timeStamps>ind1 & mod(hr_startRec+timeStamps/3600,24)> PeriodDay(2) ));
            a = restrict(a, ind1,ind2);
            
            %<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            % get time of sound stim
            % discard lost mice
            if channelID(id)==11 && IDacq(1,i)>95 || channelID(id)==1 && IDacq(1,i)>103 ...
                    || channelID(id)==2 && IDacq(1,i)>107
                evt_stimTimeStamps =[];
...
legend([{['Cond sleep (n=',num2str(length(find(gpMice==0))),', #',sprintf(' %d',channelID(gpMice==0)),')']},'Expe BASAL',...
