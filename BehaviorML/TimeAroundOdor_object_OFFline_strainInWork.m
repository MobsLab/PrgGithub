...
%% display tracking and save figure for all phases
 ...
 %% display tracking and save figure for all phases
    function Genrat_figureALLEXPE(obj,event)
        ...
        % -----------------------------------------------------------------
        % ------------------------- INPUTS --------------------------------
        nameFolderAnaly='AnalyzeOdorRecognitionALL-26062014';
        typeanalyname=' - abs(log())';% ' row'    
        ErasePreviousAnaly=0;
        StepTimeImposed=[60 120 180]; %time in sec
        d_objImposed=2; % distance to object (cm)
        PercExcluOnObj=50;% remove if 50% of the animal is inside OdorZone
        %nPhase=3;
        plotfigureIndiv=1;
        freqresample=10; %hz
        NameStrain={'WT','dKO'};
        % create folder of analysis
        if ~exist(['/media/DataMOBs16/ProjetBULB/',nameFolderAnaly],'dir')
            mkdir(['/media/DataMOBs16/ProjetBULB/',nameFolderAnaly])
        end
        % --------------------------------------------------
        % -------------- get all experiments --------------
        try
        ...
        if exist('NameExpeToAnalyze','var')
            ...
            try
                tlo=load('/media/DataMOBs16/ProjetBULB/',nameFolderAnaly,'/MatExpe.mat','MatExpe');
                MatExpe=tlo.MatExpe;
                if ErasePreviousAnaly, error; end
            catch
                MatExpeNames={'Mouse','strain','Phase','OdorLoc','meanTimeInSniff','sum in sniff','sumPresence'};
                MatExpe={};
                for nam=1:length(NameExpeToAnalyze)
                    ...
                    for ph=1:nPhase
                        ...
                        copyfile([folderPhase,mark,'AroundOdor.mat'],['/media/DataMOBs16/ProjetBULB/',nameFolderAnaly,'/Mouse',num2str(nMouse),'-Phase',num2str(nPh),'-AroundOdor.mat'])
                        ...
                        try strain=Tempreload.strain;catch, strain='WT';end
                        nstrain=find(strcmp(strain,NameStrain));
                        ...
                        for dob=1:length(d_objImposed)
                            ...
                            for n_step=1:length(StepTimeImposed)
                                ...
                                for od=1:length(OdorInfo(:,3))
                                    ...
                                    % Odor Loc
                                    MatExpeTemp(od,1:4)=[nMouse, nstrain, nPh, OdorInfo(od,3)];
                                    ...
                                    if ~isempty(Stop(SniffEpoch,'s')),
                                        MatExpeTemp(od,5)=nanmean(Stop(SniffEpoch,'s')-Start(SniffEpoch,'s'));
                                        MatExpeTemp(od,6)=nansum(Stop(SniffEpoch,'s')-Start(SniffEpoch,'s'));
                                    else
                                        MatExpeTemp(od,5)=0;
                                        MatExpeTemp(od,6)=0;
                                    end
                                    MatExpeTemp(od,7)=length(Stop(SniffEpoch,'s'));
                                    
                                    % sumPresence
                                    %MatExpeTemp(od,7)=nansum(CaractSniff(indexTS,5));
                                end
                                
                                % Mouse - strain - Phase - OdorLoc - meanTimeInSniff  - sum in sniff- sumPresence
                ...
                disp(['-> saving in ProjetBULB/',nameFolderAnaly,'/MatExpe.mat'])
                save(['/media/DataMOBs16/ProjetBULB/',nameFolderAnaly,'/MatExpe.mat'],'MatExpe','MatExpeNames','NameStrain')
                
            end
            
            % --------------------------------------------------
            % ----------- Display Time in sniff in odor zone -----------
            nbparam=size(MatExpe{1,1},2)-4;
            NameColumn={'Mean time sniff','Tot time in sniff','Nb sniff'};
            a=0; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean time sniff VAR/FIX','Tot time in sniff VAR/FIX','Nb sniff VAR/FIX'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff VAR+FIX','Tot time in sniff VAR+FIX','Nb sniff VAR+FIX'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff VAR-FIX','Tot time in sniff VAR-FIX','Nb sniff VAR-FIX'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff (VAR/FIX) norm ph0','Tot time in sniff (VAR/FIX) norm ph0','Nb sniff (VAR/FIX) norm ph0'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff (VAR+FIX) norm ph0','Tot time in sniff (VAR+FIX) norm ph0','Nb sniff (VAR+FIX) norm ph0'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff (VAR-FIX) norm ph0','Tot time in sniff (VAR-FIX) norm ph0','Nb sniff (VAR-FIX) norm ph0'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff %VAR','Tot time in sniff %VAR','Nb sniff %VAR'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff %VAR norm ph0','Tot time in sniff %VAR norm ph0','Nb sniff %VAR norm ph0'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff abs(var-fix)','Tot time in sniff abs(var-fix)','Nb sniff abs(var-fix)'};
            a=a+1; ParamFigures(a*nbparam+1:(a+1)*nbparam)={'Mean duration sniff abs(var-fix) norm ph0','Tot time in sniff abs(var-fix) norm ph0','Nb sniff abs(var-fix) norm ph0'};
            
            numFig=1:length(ParamFigures);  % numFig=[19:length(ParamFigures)];
            
            for fig=numFig, figure('Color',[1 1 1],'Position',[scrsz(1:2),scrsz(3)/2, scrsz(4)/3]), numF(fig)=gcf;end
            if plotfigureIndiv, for fig=1:nbparam*length(StepTimeImposed), figure('Color',[1 1 1],'Position',scrsz/2), numG(fig)=gcf;end;end
            ...
            for dob=1:length(d_objImposed)
                 for n_step=1:length(StepTimeImposed)
                        ...
                        for mi=1:length(Umice), Ustrain(mi)=unique(Mtemp(Mtemp(:,1)==Umice(mi),2));end
                        
                        for fig=numFig
                            ValVARvsFIX=[];tempValVARvsFIX=[];
                            Ncolumn=abs(rem(fig-1,nbparam))+5;
                            %fprintf('%d',Ncolumn);
                            
                            for mi=1:length(Umice)
                                for ph=1:nPhase
                                    indexVAR=find(Mtemp(:,1)==Umice(mi) & Mtemp(:,3)==ph-1 & Mtemp(:,4)==2);
                                    indexFIX=find(Mtemp(:,1)==Umice(mi) & Mtemp(:,3)==ph-1 & Mtemp(:,4)==3);
                                    ...
                                    try
                                        ...
                                        if fig<=nbparam
                                            ValVARvsFIX(mi,ph)=ValVAR(mi,ph)/ValFIX(mi,ph);
                                        
                                        elseif fig>nbparam && fig<=2*nbparam
                                            ValVARvsFIX(mi,ph)=ValVAR(mi,ph)+ValFIX(mi,ph);
                                        
                                        elseif fig>2*nbparam && fig<=3*nbparam
                                            ValVARvsFIX(mi,ph)=ValVAR(mi,ph)-ValFIX(mi,ph);
                                        
                                        elseif fig>3*nbparam && fig<=4*nbparam
                                            tempValVARvsFIX(mi,ph)=ValVAR(mi,ph)/ValFIX(mi,ph);
                                            ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph)/tempValVARvsFIX(mi,1);
                                        
                                        elseif fig>4*nbparam && fig<=5*nbparam
                                            tempValVARvsFIX(mi,ph)=ValVAR(mi,ph)+ValFIX(mi,ph);
                                            ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph)/tempValVARvsFIX(mi,1);
                                        
                                        elseif fig>5*nbparam && fig<=6*nbparam
                                            tempValVARvsFIX(mi,ph)=ValVAR(mi,ph)-ValFIX(mi,ph);
                                            ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph)/tempValVARvsFIX(mi,1);
                                            
                                        elseif fig>6*nbparam && fig<=7*nbparam
                                            ValVARvsFIX(mi,ph)=100*ValVAR(mi,ph)/(ValVAR(mi,ph)+ValFIX(mi,ph));
                                            
                                        elseif fig>7*nbparam && fig<=8*nbparam
                                            tempValVARvsFIX(mi,ph)=100*ValVAR(mi,ph)/(ValVAR(mi,ph)+ValFIX(mi,ph));
                                            ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph)/tempValVARvsFIX(mi,1);
                                       
                                        elseif fig>8*nbparam && fig<=9*nbparam
                                            ValVARvsFIX(mi,ph)=abs(ValVAR(mi,ph)-ValFIX(mi,ph));
                                            
                                        elseif fig>9*nbparam && fig<=10*nbparam
                                            tempValVARvsFIX(mi,ph)=abs(ValVAR(mi,ph)-ValFIX(mi,ph));
                                            ValVARvsFIX(mi,ph)=tempValVARvsFIX(mi,ph)/tempValVARvsFIX(mi,1);
                                        end
                                    catch
                                        keyboard;
                                ...
                                if fig<=nbparam && plotfigureIndiv
                                    figure(numG((n_step-1)*length(StepTimeImposed)+Ncolumn-4)), 
                                    subplot(2,ceil(length(Umice)/2),mi),
                                    bar([ValVAR(mi,:)',ValFIX(mi,:)'])
                                    set(gca,'XTick',1:3)
                                    set(gca,'XTickLabel',{'0','1','2'})
                                    ylimG(Ncolumn-4,n_step)=max(ylim);
                                    xlabel('Phase')
                                    ylabel(NameColumn{Ncolumn-4})
                            ...
                            if fig<=nbparam && plotfigureIndiv
                                ...
                                for sub=1:length(Umice)
                                    ...
                                    ylim([0 max(ylimG(Ncolumn-4,n_step))]);
                                end
                            end
                            ...
                            elseif strcmp(typeanalyname,' row')
                                tempValVARvsFIX=ValVARvsFIX;
                            end
                            tempValVARvsFIX(isinf(tempValVARvsFIX))=NaN;
                            
                            for us=unique(Ustrain)
                                MeanbarMat{us}=nanmean(tempValVARvsFIX(find(Ustrain==us),:),1);
                                if strcmp(typeanalyname,' - abs(log())')
                                    stdbarMat{us}=stdError(abs(log(ValVARvsFIX(find(Ustrain==us),:))));
                                else
                                    stdbarMat{us}=stdError(ValVARvsFIX(find(Ustrain==us),:));
                                end
                                
                                pval(1,us)=ranksum(tempValVARvsFIX(find(Ustrain==us),1),tempValVARvsFIX(find(Ustrain==us),2));
                                npval{1,us}=[sum(~isnan(ValVARvsFIX(find(Ustrain==us),1))),sum(~isnan(ValVARvsFIX(find(Ustrain==us),2)))];
                                if nPhase>2
                                    pval(2,us)=ranksum(tempValVARvsFIX(:,1),tempValVARvsFIX(:,3));
                                    npval{2,us}=[sum(~isnan(ValVARvsFIX(:,1))),sum(~isnan(ValVARvsFIX(:,3)))];
                                    pval(3,us)=ranksum(tempValVARvsFIX(:,2),tempValVARvsFIX(:,3));
                                    npval{3,us}=[sum(~isnan(ValVARvsFIX(:,2))),sum(~isnan(ValVARvsFIX(:,3)))];
                                end
                            end
                            ...
                            for ipv=1:length(ipvname);
                                ...
                                line([str2num(ipvname{ipv}(1)) str2num(ipvname{ipv}(2))],[1 1]*max(ylim)*positionPv(ipv),'Color',colori{ipv+3});
                                pvaltemp=pval(ipv);
                                npvaltemp=[num2str(npval(ipv,1)),'-',num2str(npval(ipv,2))];
                                if pvaltemp<0.05,
                                    text(mean([str2num(ipvname{ipv}(1)) str2num(ipvname{ipv}(2))]),max(ylim)*(positionPv(ipv)+0.03),['* ',npvaltemp],'Color',colori{ipv+3});
                                elseif pvaltemp<0.01,
                                    text(mean([str2num(ipvname{ipv}(1)) str2num(ipvname{ipv}(2))]),max(ylim)*(positionPv(ipv)+0.03),['** ',npvaltemp],'Color',colori{ipv+3});
                                else
                                    text(mean([str2num(ipvname{ipv}(1)) str2num(ipvname{ipv}(2))]),max(ylim)*(positionPv(ipv)+0.03),['NS ',npvaltemp],'Color',colori{ipv+3});
                                end
                            end
