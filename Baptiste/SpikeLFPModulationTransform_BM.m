function [PhasesSpikes,mu,Kappa,pval]=SpikeLFPModulationTransform_BM(S,fil,epoch,nbin,plo,FiltOrPhase)
...
if sum(Stop(epoch,'s')-Start(epoch,'s'))==0 | length(Range(Restrict(S,epoch)))==0
    ...
    [mu.Nontransf, Kappa.Nontransf, pval.Nontransf, Rmean, delta, sigma,confDw,confUp] = CircularMean_SBSB(PhasesSpikes.Nontransf);
    [mu.Transf, Kappa.Transf, pval.Transf, Rmean, delta, sigma,confDw,confUp] = CircularMean_SBSB(PhasesSpikes.Transf);
    ...
    if plo
        ...
        subplot(1,2,2),
        bar([XSpike.NonTransf,XSpike.NonTransf+2*pi],[DistSpike.NonTransf,DistSpike.NonTransf]), hold on
