function RipplesEpoch = CreateRipplesSleep_BM(varargin)

%% only changes brought by BM
% line 319 FindRipples_abs_BM instead of FindRipples_abs
% line 334 Info.Restrict instead of Info.Epoch
...
if rmvnoise
    try
        ...
        Mouse_name={['M' num2str(ExpeInfo.nmouse)]};
        for sess=1:length(Sess.(Mouse_name{1}))
            cd(Sess.(Mouse_name{1}){sess})
...
if ExpeInfo.SleepSession==0
    load('behavResources_SB.mat', 'Behav' ,'TTLInfo')
    try; Info.Epoch = Info.Epoch-TTLInfo.StimEpoch; end
    try; Info.Epoch = Info.Epoch-TTLInfo.VHC_Stim_Epoch; end % for mice with 2nd stim
    if restrict
        Info.Restrict = and(Info.Epoch , Behav.FreezeAccEpoch);
    else
        Info.Restrict = Info.Epoch;
    end
else
    if restrict
        Info.Restrict = SWSEpoch-TotalNoiseEpoch;
...
disp(' ')
if ExpeInfo.SleepSession==0
    [ripples_abs, meanVal, stdVal] = FindRipples_abs_BM(HPCrip, HPCnonRip, ...
        Info.Epoch, Info.Restrict,'frequency_band',Info.frequency_band, ...
        'threshold',Info.threshold(1,:),'durations',Info.durations,'stim',stim,'mean_std_values',Info.MeanStdVals,'restrict',restrict);
else
    [ripples_abs, meanVal, stdVal] = FindRipples_abs_BM(HPCrip, HPCnonRip, ...
        Info.Epoch, Info.Restrict,'frequency_band',Info.frequency_band, ...
...
if ExpeInfo.SleepSession==0
    if restrict
        % first detect on restricted epoch, then on total epoch (differents thresholds, then merge)
        [ripples_sqrt1,stdev] = FindRipples_sqrt_BM(HPCrip, HPCnonRip, Info.Epoch, Info.threshold(2,:), ...
            'clean',1 , 'stdev' , stdev , 'restrict' , Info.Restrict);
        [ripples_sqrt2,stdev] = FindRipples_sqrt_BM(HPCrip, HPCnonRip, Info.Epoch, Info.threshold(2,:), ...
            'clean',1 , 'stdev' , stdev);
        
        Epoch1 = intervalSet(ripples_sqrt1(:,1) , ripples_sqrt1(:,3));
        Epoch2 = intervalSet(ripples_sqrt2(:,1) , ripples_sqrt2(:,3));
        for i=1:length(Start(Epoch1))
            ind_to_use(i)=sum(Stop(and(subset(Epoch1,i),Epoch2)) - Start(and(subset(Epoch1,i),Epoch2))) > 0;
        end
        ripples_sqrt = [ripples_sqrt1(~ind_to_use,:) ; ripples_sqrt2];
        [~,sort_ind] = sort(ripples_sqrt(:,1));
        ripples_sqrt = ripples_sqrt(sort_ind,:);
    else
        [ripples_sqrt,stdev] = FindRipples_sqrt_BM(HPCrip, HPCnonRip, Info.Epoch, Info.threshold(2,:), ...
            'clean',1 , 'stdev' , Info.MeanStdVals);
    end
else
    if restrict
        [ripples_sqrt,stdev] = FindRipples_sqrt_BM(HPCrip, HPCnonRip, Info.Epoch, Info.threshold(2,:), ...
