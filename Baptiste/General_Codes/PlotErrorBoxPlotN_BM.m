function [pval,hb,eb] = PlotErrorBoxPlotN_BM(A,varargin)
...
for i = 1:2:length(varargin)
    ...
    switch(lower(varargin{i}))
        ...
        case 'compactstyle'
            compactstyle = varargin{i+1};
            if compactstyle~=0 && compactstyle ~=1
                error('Incorrect value for property ''compactstyle''.');
            end
        otherwise
...
if compactstyle==0
    ...
    if isempty(barcolors)
        hb = boxplot(B);
        % 'FaceColor',[0.3 0.3 0.3],bar_argin{:});
    elseif ~iscell(barcolors)
        hb = boxplot(R,'FaceColor',barcolors,bar_argin{:});
    elseif length(barcolors)==1
        hb = boxplot(R,'FaceColor',barcolors{1},bar_argin{:});
    else
        for i=1:length(R)
            hb = boxplot(i, R(i), 'FaceColor', barcolors{i},bar_argin{:});
        end
    end
    ...
    if isempty(barcolors)
        hb = boxplot(B,'BoxStyle','filled');
        % 'FaceColor',[0.3 0.3 0.3],bar_argin{:});
    elseif ~iscell(barcolors)
        hb = boxplot(R,'FaceColor',barcolors,bar_argin{:},'BoxStyle','filled');
    elseif length(barcolors)==1
        hb = boxplot(R,'FaceColor',barcolors{1},bar_argin{:},'BoxStyle','filled');
    else
        for i=1:length(R)
            hb = boxplot(i, R(i), 'FaceColor', barcolors{i},bar_argin{:},'BoxStyle','filled');
        end
    end
end
