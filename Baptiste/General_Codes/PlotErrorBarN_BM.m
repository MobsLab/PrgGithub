function [pval,eb,stats_out,b] = PlotErrorBarN_BM(A,varargin)
...
if ~horizontal
    ...
    if isempty(barcolors)
        b=bar(x,R,'FaceColor',[0.3 0.3 0.3],bar_argin{:});
    elseif ~iscell(barcolors)
        b=bar(x,R,'FaceColor',barcolors,bar_argin{:});
    elseif length(barcolors)==1
            b=bar(x,R,'FaceColor',barcolors{1},bar_argin{:});
    else
        for i=1:length(R)
            b(i)=bar(x(i), R(i), 'FaceColor', barcolors{i},bar_argin{:});
        end
    end

    %plot the bars with the errors
    eb = errorbar(x,R,E,'+','Color','k');
%     xlim([0 N+1])
    if exist('y_lim','var')
        ylim(y_lim)
    end
...
%horizontal
    ...
    %bar colors
    if isempty(barcolors)
        b=barh(x,R,'FaceColor',[0.3 0.3 0.3],'EdgeColor','k')
    elseif length(barcolors)==1
        b=barh(x,R,'FaceColor',barcolors{1},'EdgeColor','k')
    else
        for i=1:length(R)
            b(i)=barh(x(i), R(i), 'FaceColor', barcolors{i},'EdgeColor','k');
        end
    end
    
    %plot the bars with the errors
    eb = herrorbar(x,R,1:length(E),E,'.k');
    ...
    if paired
        ...
        for k=1:nb_individual
            ...
            for i=1:length(idx)-1
                try 
                    line(0.1+[x(idx(i)) x(idx(i+1))]', [M(k,idx(i)) M(k,idx(i+1))]', 'color',[0.6 0.6 0.6]); 
                end
            end
        end
    end
