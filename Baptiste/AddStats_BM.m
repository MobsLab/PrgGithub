function [pval , stats_out]=AddStats_BM(A,varargin)
% Input variables
...
for i = 1:2:length(varargin)
    ...
    switch(lower(varargin{i}))
        ...
        case 'no_check_distrib'
            no_check_distrib = varargin{i+1};
        otherwise
...
if ~isempty(column_test)
    ...
    elseif strcmp(optiontest,'ttest')
        ...
        for c=1:length(column_test)
            i = column_test{c}(1);
            j = column_test{c}(2);
            if or(hnorm(i)+hnorm(j)==0 && hvar(min(i,j),max(i,j))==0 , no_check_distrib)
                if paired
