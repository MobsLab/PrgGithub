...
clear HistData_FR FR_mov_corrected FiringRATE FiringRATE_hist HistData_FR
        ...
        for neur=Place_Cells{3}{mm}%1:length(stats{sess}{mm})%
            if and(and(~isempty(mapNS_mov{1}{mm}{neur}) , ~isempty(mapNS_mov{2}{mm}{neur})) , and(~isempty(mapNS_mov{3}{mm}{neur}) , ~isempty(mapNS_mov{4}{mm}{neur})))
                ...
                if and(and(~isempty(U1) , ~isempty(U2)) , and(~isempty(U3) , ~isempty(U4)))
                    ...
                    %% In its current state the code doesn't discriminate between 0 firing rate because the mouse doesn't go there and because the cell didn't dire
                    % Get map and cut of parts outside the maze and set to
                    % nan regions where the mouse never went
                    ...
                    for j=1:length(C)
                        FiringRATE{sess}{mm}{neur}(j,:) = [C(j) nanmean(map_temp(U(IC==j)))];
                    end
                    [~,order]=sort(FiringRATE{sess}{mm}{neur}(:,1));
                    FiringRATE{sess}{mm}{neur}=FiringRATE{sess}{mm}{neur}(order,:);
                
                    % bin regularly thorughout the maze
                    for bin=1:100
                        ind=find(and(FiringRATE{sess}{mm}{neur}(:,1)>=((bin-1)/100) , FiringRATE{sess}{mm}{neur}(:,1)<(bin/100)));
                        if ~isempty(ind)
                            FiringRATE_hist{sess}{mm}{neur}(bin) = nanmean(FiringRATE{sess}{mm}{neur}(ind,2));
                            HistData_FR{sess}(m,bin) = nanmean(FiringRATE{sess}{mm}{neur}(ind,2));
                        end
                    end
...
clear PlaceCellpeak PlaceCellOrder
for sess=1:4%length(Session_type)
    [~,PlaceCellpeak{sess}]=max(HistData_FR{sess}'); [~,PlaceCellOrder{sess}]=sort(PlaceCellpeak{sess});
end
...
for mm=1:length(stats{3})
    for neur = Place_Cells{3}{mm}
            clf
            try
            subplot(3,1,1:2)
            imagesc(mapNS_mov{sess}{mm}{neur}.rate), axis xy
            ...
            try
                ...
                hold on
                plot([0.01:0.01:1],FiringRATE_hist{sess}{mm}{neur})
                
                FiringRATE_hist{sess}{mm}{neur}
            end
...
figure
subplot(141), sess=1;
imagesc( linspace(0,1,100) , [1:size(HistData_FR{sess},1)] ,[HistData_FR{sess}(PlaceCellOrder{sess},:)])
caxis([0 .05])
colormap viridis
...
subplot(142), sess=2;
imagesc( linspace(0,1,100) , [1:size(HistData_FR{sess},1)] ,[HistData_FR{sess}(PlaceCellOrder{1},:)])
caxis([0 .05])
...
subplot(243)
PlotCorrelations_BM(PlaceCellpeak{1}/100,PlaceCellpeak{2}/100)
axis square
...
subplot(247)
imagesc(linspace(0,1,100) , linspace(0,1,100) , SmoothDec(HistData_FR{1}(PlaceCellOrder{1},:).*HistData_FR{2}(PlaceCellOrder{1},:),1)), caxis([0 5e-4]), colormap viridis
axis square
line([0 1],[0 1],'LineStyle','--','Color','r','LineWidth',2)
...
subplot(144)
imagesc(linspace(0,1,100) , [1:size(HistData_FR{2},1)] ,[HistData_FR{2}(PlaceCellOrder{2},:)])
caxis([0 .05])
xlabel('linear UMaze distance'), yticklabels({''})
vline(.2,'--r')
makepretty
...
figure
subplot(141), sess=3;
imagesc( linspace(0,1,100) , [1:size(HistData_FR{sess},1)] ,runmean([HistData_FR{sess}(PlaceCellOrder{sess},:)]',2)')
caxis([0 .05])
colormap viridis
ylabel('HPC neurons no'), xlabel('linear UMaze distance')
vline(.2,'--r')
...
subplot(142), sess=4;
imagesc( linspace(0,1,100) , [1:size(HistData_FR{sess},1)] ,runmean([HistData_FR{sess}(PlaceCellOrder{3},:)]',2)')
caxis([0 .05])
xlabel('linear UMaze distance'), yticklabels({''})
vline(.2,'--r')
...
subplot(243)
PlotCorrelations_BM(PlaceCellpeak{3}/100,PlaceCellpeak{4}/100)
axis square
line([0 1],[0 1],'LineStyle','--','Color','r','LineWidth',2)
xlabel('Place cells peak, Hab (linear dist)'), ylabel('Place cells peak, Cond (linear dist)')

subplot(247)
imagesc(linspace(0,1,100) , linspace(0,1,100) , SmoothDec(HistData_FR{3}(PlaceCellOrder{3},:).*HistData_FR{4}(PlaceCellOrder{3},:),1)), caxis([0 5e-4]), colormap viridis
axis square
line([0 1],[0 1],'LineStyle','--','Color','r','LineWidth',2)
...
subplot(144)
imagesc(linspace(0,1,100) , [1:size(HistData_FR{4},1)] ,runmean([HistData_FR{4}(PlaceCellOrder{4},:)]',2)')
caxis([0 .05])
xlabel('linear UMaze distance'), yticklabels({''})
vline(.2,'--r')
...
subplot(162)
plot(FR_mov_corrected{2}(PlaceCellOrder{3})-FR_mov_corrected{1}(PlaceCellOrder{3}) , 'Color' , 'k' , 'LineWidth' , 2)
box off
...
subplot(164)
plot(FR_mov_corrected{4}(PlaceCellOrder{3})-FR_mov_corrected{3}(PlaceCellOrder{3}) , 'Color' , 'k' , 'LineWidth' , 2)
box off
...
subplot(165)
plot(FR_Ripples_corrected(PlaceCellOrder{3})-FR_mov_corrected{4}(PlaceCellOrder{3}) , 'Color' , 'k' , 'LineWidth' , 2)
box off
...
subplot(166)
plot(FR_Ripples_corrected(PlaceCellOrder{3})-FR_PreRipples_corrected(PlaceCellOrder{3}) , 'Color' , 'k' , 'LineWidth' , 2)
box off
...
MakeSpreadAndBoxPlot3_SB({FR_mov_corrected{1} FR_mov_corrected{2} FR_mov_corrected{3} FR_mov_corrected{4} FR_PreRipples_corrected FR_Ripples_corrected} ,...
    Cols,X_plo,Legends,'showpoints',0,'paired',0)
...
MakeSpreadAndBoxPlot3_SB({FR_Ripples_corrected(ind_shock_hab)./FR_PreRipples_corrected(ind_shock_hab) FR_Ripples_corrected(ind_safe_hab)./FR_PreRipples_corrected(ind_safe_hab)},...
    Cols2,X2_plo,Legends2,'showpoints',1,'paired',0)
set(gca,'Yscale','log')
hline(1,'--r')
...
figure
MakeSpreadAndBoxPlot3_SB({FR_Ripples_corrected(ind_shock_cond)./FR_PreRipples_corrected(ind_shock_cond) FR_Ripples_corrected(ind_safe_cond)./FR_PreRipples_corrected(ind_safe_cond)},...
    Cols2,X2_plo,Legends2,'showpoints',1,'paired',0)
set(gca,'Yscale','log')
hline(1,'--r')
ylabel('Ratio reactivation Ripples/PreRipples')
...
subplot(323)
plot(linspace(0,1,length(HistData_Spikes_all{3})) , HistData_Spikes_all{3}(PlaceCellOrder{3})./HistData_Spikes_all{5}(PlaceCellOrder{3}))
set(gca,'Yscale','log')
hline(1,'--r')
...
subplot(324)
plot(linspace(0,1,length(HistData_Spikes_all{4})) , HistData_Spikes_all{4}(PlaceCellOrder{3})./HistData_Spikes_all{5}(PlaceCellOrder{3}))
hold on
plot(linspace(0,1,length(HistData_Spikes_Ripples)) , HistData_Spikes_Ripples(PlaceCellOrder{3})./HistData_Spikes_all{5}(PlaceCellOrder{3}))
plot(linspace(0,1,length(HistData_Spikes_Ripples)) , HistData_Spikes_PreRipples(PlaceCellOrder{3})./HistData_Spikes_all{5}(PlaceCellOrder{3}))
plot(linspace(0,1,length(HistData_Spikes_Ripples)) , HistData_Spikes_Ripples(PlaceCellOrder{3})./HistData_Spikes_PreRipples(PlaceCellOrder{3}))
set(gca,'Yscale','log')
hline(1,'--r')
...
figure
clear A B
A = HistData_Spikes_all{4}(PlaceCellOrder{3}(1:40))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:40));
B = HistData_Spikes_all{4}(PlaceCellOrder{3}(end-80:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-80:end));
...
clear A B
A = HistData_Spikes_Ripples(PlaceCellOrder{3}(1:40))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:40)); sum(A==0)/length(A), A(A==0)=NaN;
B = HistData_Spikes_Ripples(PlaceCellOrder{3}(end-80:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-80:end)); sum(B==0)/length(B), B(B==0)=NaN;
...
clear A B C D
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{3}(1:40))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:40)));
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{3}(1:40))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:40))); B(B==-Inf)=NaN;
C = log10(HistData_Spikes_all{4}(PlaceCellOrder{3}(end-80:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-80:end)));
D = log10(HistData_Spikes_Ripples(PlaceCellOrder{3}(end-80:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-80:end))); D(D==-Inf)=NaN;
...
subplot(122)
PlotCorrelations_BM(HistData_Spikes_all_allcells{4}(PlaceCellOrder{3})./HistData_Spikes_all_allcells{5}(PlaceCellOrder{3}) , HistData_Spikes_Ripples_allcells(PlaceCellOrder{3})./HistData_Spikes_all_allcells{5}(PlaceCellOrder{3}))
hold on
...
clear A B
A = log(HistData_Spikes_Ripples(PlaceCellOrder{3}(1:40))./HistData_Spikes_PreRipples(PlaceCellOrder{3}(1:40)));
B = log(HistData_Spikes_Ripples(PlaceCellOrder{3}(end-80:end))./HistData_Spikes_PreRipples(PlaceCellOrder{3}(end-80:end)));
...
clear A B
A = (HistData_Spikes_Ripples(PlaceCellOrder{3}(1:40))-HistData_Spikes_PreRipples(PlaceCellOrder{3}(1:40)))./(HistData_Spikes_Ripples(PlaceCellOrder{3}(1:40))+HistData_Spikes_PreRipples(PlaceCellOrder{3}(1:40)));
B = (HistData_Spikes_Ripples(PlaceCellOrder{3}(end-80:end))-HistData_Spikes_PreRipples(PlaceCellOrder{3}(end-80:end)))./(HistData_Spikes_Ripples(PlaceCellOrder{3}(end-80:end))+HistData_Spikes_PreRipples(PlaceCellOrder{3}(end-80:end)));
...
clear A B
A = log(HistData_Spikes_PreRipples(PlaceCellOrder{3}(1:40)));
B = log(HistData_Spikes_Ripples(PlaceCellOrder{3}(1:40)));
...
clear A B
A = log(HistData_Spikes_PreRipples(PlaceCellOrder{3}(end-80:end)));
B = log(HistData_Spikes_Ripples(PlaceCellOrder{3}(end-80:end)));
...
subplot(133), sess=4;
clear A, A = log10(HistData_Spikes_Ripples(PlaceCellOrder{3})./HistData_Spikes_PreRipples(PlaceCellOrder{3})); A(or(A==Inf,A==-Inf))=[];
plot(runmean_BM(A,10) , 'Color' , [.4 .5 .6]), hold on
clear A, A = log10(HistData_Spikes_Ripples(PlaceCellOrder{3})./HistData_Spikes_all{5}(PlaceCellOrder{3})); A(or(A==Inf,A==-Inf))=[];
...
figure
subplot(131), sess=3;
imagesc( linspace(0,1,100) , [1:size(HistData_FR{sess},1)] ,runmean([HistData_FR{sess}(PlaceCellOrder{4},:)]',2)')
caxis([0 .05])
colormap viridis
ylabel('HPC neurons no'), xlabel('linear UMaze distance')
vline(.2,'--r')
...
subplot(133), sess=4;
clear A, A = log10(HistData_Spikes_Ripples(PlaceCellOrder{4})./HistData_Spikes_PreRipples(PlaceCellOrder{4})); A(or(A==Inf,A==-Inf))=[];
plot(runmean_BM(A,10) , 'Color' , [.4 .5 .6]), hold on
clear A, A = log10(HistData_Spikes_Ripples(PlaceCellOrder{4})./HistData_Spikes_all{5}(PlaceCellOrder{4})); A(or(A==Inf,A==-Inf))=[];
plot(runmean_BM(A,10) , 'Color' , [.6 .5 .4])
makepretty_BM
ylabel('firing rate during safe side ripples (log scale)'), hline(0,'--r')
xlim([0 size(FR_mov_corrected{1},2)]), ylim([-.5 1])
...
clear A B
A = log10(HistData_Spikes_Ripples(PlaceCellOrder{4}(1:20))./HistData_Spikes_PreRipples(PlaceCellOrder{4}(1:20))); A(A==or(Inf,-Inf))=NaN;
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{4}(end-30:end))./HistData_Spikes_PreRipples(PlaceCellOrder{4}(end-30:end))); B(B==or(Inf,-Inf))=NaN;
...
clear A B
A = log10(HistData_Spikes_Ripples(PlaceCellOrder{4}(1:20))./HistData_Spikes_all{5}(PlaceCellOrder{4}(1:20))); A(A==or(Inf,-Inf))=NaN;
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{4}(end-30:end))./HistData_Spikes_all{5}(PlaceCellOrder{4}(end-30:end))); B(B==or(Inf,-Inf))=NaN;
...
subplot(131)
clear A B
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{4})./HistData_Spikes_all{5}(PlaceCellOrder{4}));
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{4})./HistData_Spikes_all{5}(PlaceCellOrder{4}));
...
subplot(232)
clear A B
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{3}(1:20))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:20)));
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{3}(1:20))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:20)));
ind = or(or(A==Inf,A==-Inf) , or(B==Inf,B==-Inf));
A(ind)=[];
B(ind)=[];
...
clear A B
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{3}(end-40:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-40:end)));
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{3}(end-40:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-40:end)));
ind = or(or(A==Inf,A==-Inf) , or(B==Inf,B==-Inf));
A(ind)=[];
B(ind)=[];
...
subplot(235)
clear A B
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{4}(1:20))./HistData_Spikes_all{5}(PlaceCellOrder{4}(1:20)));
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{4}(1:20))./HistData_Spikes_all{5}(PlaceCellOrder{4}(1:20)));
ind = or(or(A==Inf,A==-Inf) , or(B==Inf,B==-Inf));
A(ind)=[];
B(ind)=[];
...
clear A B
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{4}(end-40:end))./HistData_Spikes_all{5}(PlaceCellOrder{4}(end-40:end)));
B = log10(HistData_Spikes_Ripples(PlaceCellOrder{4}(end-40:end))./HistData_Spikes_all{5}(PlaceCellOrder{4}(end-40:end)));
ind = or(or(A==Inf,A==-Inf) , or(B==Inf,B==-Inf));
A(ind)=[];
B(ind)=[];
...
subplot(255)
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{3}(1:40))./HistData_Spikes_all{5}(PlaceCellOrder{3}(1:40)));
B = log10(HistData_Spikes_all{4}(PlaceCellOrder{3}(end-80:end))./HistData_Spikes_all{5}(PlaceCellOrder{3}(end-80:end)));
MakeSpreadAndBoxPlot3_SB({A B},Cols2,X2,Legends2,'showpoints',1,'paired',0)
ylabel('firing rate during moving')

subplot(2,5,10)
A = log10(HistData_Spikes_all{4}(PlaceCellOrder{4}(1:40))./HistData_Spikes_all{5}(PlaceCellOrder{4}(1:40)));
B = log10(HistData_Spikes_all{4}(PlaceCellOrder{4}(end-80:end))./HistData_Spikes_all{5}(PlaceCellOrder{4}(end-80:end)));
MakeSpreadAndBoxPlot3_SB({A B},Cols2,X2,Legends2,'showpoints',1,'paired',0)
ylabel('firing rate during moving')
...
%%
Lims = [30,70]; % definition of shock and safe zone
ShockCells = 1: sum(PlaceCellpeak{4}<Lims(1));
SafeCells = length(PlaceCellOrder{4})-sum(PlaceCellpeak{4}>Lims(2)):length(PlaceCellOrder{4});
...
subplot(152)
imagesc(linspace(-250,250,size(MeanFR_AroundRip_all,2)) , [1:size(MeanFR_AroundRip_all,1)] , MeanFR_AroundRip_all(PlaceCellOrder{4},190:207))
...
subplot(253)
Data_to_use = MeanFR_AroundRip_all(PlaceCellOrder{4}(ShockCells),180:220);
Conf_Inter=nanstd(Data_to_use)/sqrt(size(Data_to_use,1));
Mean_All_Sp = nanmean(Data_to_use);
...
Data_to_use = MeanFR_AroundRip_all(PlaceCellOrder{4}(SafeCells),180:220);
Conf_Inter=nanstd(Data_to_use)/sqrt(size(Data_to_use,1));
Mean_All_Sp = nanmean(Data_to_use);
h=shadedErrorBar(linspace(-.2,.2,41) , runmean(Mean_All_Sp,5) , runmean(Conf_Inter,5) ,'-r',1); hold on;
...
subplot(258)
makepretty_BM
MakeSpreadAndBoxPlot3_SB({nanmean(MeanFR_AroundRip_all(PlaceCellOrder{4}(ShockCells),[190:210])') nanmean(MeanFR_AroundRip_all(PlaceCellOrder{4}(SafeCells),[190:210])')},Cols2,X2_plo,Legends2,'showpoints',1,'paired',0)
...
[f,g]=max(HistData_FR{sess}(PlaceCellOrder{4},:)');
[c,e]=max(zscore(MeanFR_AroundRip_all(PlaceCellOrder{4},[190:207])'));
for i=[ShockCells, SafeCells]%size(MeanFR_AroundRip_all,1)
    plot((e(i)/18-.5)*100 ,1-g(i)/100 , '.' , 'MarkerSize' , 50 , 'Color' , [col(i,:)]), hold on
end
AllCells = 1:length(PlaceCellOrder{4});
AllCells([ShockCells, SafeCells]) = [];
for i=AllCells%size(MeanFR_AroundRip_all,1)
