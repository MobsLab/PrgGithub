...
for m=1:8
    ...
    for states=1:3
        ...
        if m<5
            if states==1
                Sp_ByState_f{m}{states} = CleanSpectro(Sp_ByState_f{m}{states} , RANGE , 8);
...
for m=1:8
    ...
    for states=1:3
        ...
        if m<5, xlabel('Frequency (Hz)'), end
        if m==1; ylabel('Power (a.u.)'), legend('Wake','NREM ','REM'), elseif m==5, ylabel('power (log scale)'), end
        if m==1, title('OB'), end
...
for ferret=1:2
    for sess=1:length(SleepInfo.path{ferret})
        ...
        Trans_Wake_SWS = Start(bef_cell{1,2}); % beginning of all SWS that is preceded by Wake
        Trans_SWS_Wake = Start(bef_cell{2,1}); % beginning of all Wake  that is preceded by SWS
        TRANSITIONS{ferret}(sess,1) = length(Trans_Wake_SWS)./TotDur;
        TRANSITIONS{ferret}(sess,2) = length(Trans_SWS_Wake)./TotDur;
        ...
        TRANSITIONS{ferret}(sess,3) = length(Trans_Wake_REM)./TotDur;
        TRANSITIONS{ferret}(sess,4) = length(Trans_REM_Wake)./TotDur;
        
        clear Trans_REM_SWS Trans_SWS_REM
        [aft_cell,bef_cell]=transEpoch(SWSEpoch,REMEpoch);
        Trans_REM_SWS = Start(bef_cell{1,2}); % beginning of all SWS that is preceded by REM
        Trans_SWS_REM = Start(bef_cell{2,1}); % beginning of all REM  that is preceded by SWS
        TRANSITIONS{ferret}(sess,5) = length(Trans_REM_SWS)./TotDur;
        TRANSITIONS{ferret}(sess,6) = length(Trans_SWS_REM)./TotDur;
...
Cols = {[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3]};
X = 1:6;
Legends = {'Wake-SWS','SWS-Wake','Wake-REM','REM-Wake','REM-SWS','SWS-REM'};

figure
for ferret=1:2
    subplot(1,2,ferret)
    MakeSpreadAndBoxPlot3_SB({TRANSITIONS{ferret}(:,1) TRANSITIONS{ferret}(:,2) TRANSITIONS{ferret}(:,3)...
        TRANSITIONS{ferret}(:,4) TRANSITIONS{ferret}(:,5) TRANSITIONS{ferret}(:,6)}...
        ,Cols,X,Legends,'showpoints',1,'paired',0,'showsigstar','none');
    if ferret==1, ylabel('transitions (#/recording hour)'), end
...
for m=1:8
    ...
    for states=1:4
        ...
        Sp_ByState_f{m}{states} = Restrict(Sptsd_f{m} , State-TotalNoiseEpoch);
        if m<5
            if states==1
                Sp_ByState_f{m}{states} = CleanSpectro(Sp_ByState_f{m}{states} , RANGE , 8);
                Mean_Spec_f{m}(states,:) = nanmean(Data(Sp_ByState_f{m}{states}));
...
for m=1:8
    ...
    for states=1:4
        if m<5
            %             plot(Range_Low , Mean_Spec_f{m}(states,:) , Cols{states} , 'LineWidth' , 2), hold on, box off
            plot(Range_Low , runmean(Mean_Spec_f{m}(states,:),5) , 'Color' , Cols{states} , 'LineWidth' , 2), hold on, box off
        else
            plot(Range_Middle , Mean_Spec_f{m}(states,:) , 'Color' , Cols{states} , 'LineWidth' , 2), hold on, box off
        end
        xlabel('Frequency (Hz)')
        if m>4, ylabel('Power (a.u.)'), end
        if m==1; legend('Wake','NREM1','NREM2','REM'), elseif m==4, ylabel('power (log scale)'), end
    if m==1, title('OB'), end
    if m==2, title('PFC'), end
    if m==3, title('HPC'), end
    if m==4, title('AuCx'), end
...
figure
for states=1:4
    plot(Range_ULow , runmean(Mean_Spec_f{m}(states,:),5) , 'Color' , Cols{states} , 'LineWidth' , 2), hold on, box off
end
...
TRANSITIONS{1}(8,:)=NaN;
TRANSITIONS{2}(7:end,:)=NaN;

Cols = {[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3],[.3 .3 .3]};
X = 1:10;
Legends = {'Wake-NREM1','NREM1-Wake','Wake-NREM2','NREM2-Wake','Wake-REM','REM-Wake','REM-NREM1','NREM1-REM','REM-NREM2','NREM2-REM'};
...
for ferret=1:2
    ...
    MakeSpreadAndBoxPlot3_SB({TRANSITIONS{ferret}(:,1) TRANSITIONS{ferret}(:,2) TRANSITIONS{ferret}(:,3)...
        ...
        TRANSITIONS{ferret}(:,8) TRANSITIONS{ferret}(:,9) TRANSITIONS{ferret}(:,10)}...
        ,Cols,X,Legends,'showpoints',1,'paired',0,'showsigstar','none');
    if ferret==1, ylabel('transitions (#/recording hour)'), end
    title(['Ferret ' num2str(ferret)])
    ylim([0 16])
