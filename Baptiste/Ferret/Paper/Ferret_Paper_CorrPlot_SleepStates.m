...
ylabel('PDF'), xlabel('HPC theta / delta power (a.u.)')
makepretty
...
plot(X(1:5e3:end) , Y(1:5e3:end) , '.g','MarkerSize',10), hold on
X = log10(Data(Restrict(SmoothTheta , N1))); Y = log10(Data(Restrict(SmoothDelta_OB , N1))); 
plot(X(1:1e3:end) , Y(1:1e3:end) , '.','MarkerSize',10,'Color',[.8 .5 .2]), hold on
X = log10(Data(Restrict(SmoothTheta , N2))); Y = log10(Data(Restrict(SmoothDelta_OB , N2))); 
plot(X(1:5e3:end) , Y(1:5e3:end) , '.','MarkerSize',10,'Color',[1 0 0]), hold on
