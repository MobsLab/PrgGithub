...
function Figure_SleepScoring_OB_Ferret_BM(foldername)
...
% load Ref spectrum
if ~exist([foldername,'Ref_Low_Spectrum.mat'])>0
    LowSpectrumSB([cd filesep],1,'Ref')
end

if exist([foldername,'Ref_Low_Spectrum.mat'])>0
    load([foldername,'Ref_Low_Spectrum.mat']);
...
SmoothGammaNew = Restrict(smooth_ghi,Epoch);
SmoothThetaNew = Restrict(smooth_Theta,Epoch);
...
[~, rawN, ~] = nhist(log(Data(Restrict(smooth_Theta,Sleep))), 'maxx',max(log(Data(Restrict(smooth_Theta,Sleep)))), 'noerror', 'xlabel','Theta power', 'ylabel',[]); 
axis xy,
view(90,-90),
