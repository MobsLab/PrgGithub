...
clear FigureTemp
subplot(1,6,6)
FigureTemp=[Before_Freezing.Shock'  Before_Freezing.Safe'  Freezing_Epoch_Values.Shock'  Freezing_Epoch_Values.Safe'];
FigureTemp(8,:)=NaN;
...
xticklabels({'Active shock','Active safe','Freezing shock','Freezing safe'}) 
makepretty
xtickangle(45)
set(gca,'FontSize',20)
...
for zones=2:length(Zones)
    for mouse=1:length(Mouse_names)
        Before_Freezing.(Zones{zones})(mouse)=nanmean(all_mice.(Zones{zones}).Respi(mouse,1:margins_bef*10));
        Freezing_Epoch_Values.(Zones{zones})(mouse)=nanmean(AllMiceRespiratoryFrequency.(Zones{zones})(mouse,margins_bef*10+1:margins_bef*20))/12;
