...
[EMG_TSD.Fear , Epoch1 , NameEpoch]=MeanValuesPhysiologicalParameters_BM(Mouse,'fear','emg','accelero','ob_low');

for mouse=1:length(Mouse)
    Mouse_names{mouse}=['M' num2str(Mouse(mouse))];
end

ind=[4 3 5 6];  % indices for corresponding Epoch, cf NameEpoch and NameEpoch2
% Calculating data for each mouse
for mouse=1:length(Mouse_names)
    for sess=2:3;
        for state=1:length(State1)
            DataEMG.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})=Data(EMG_TSD.(Session_type{sess}).emg.tsd{mouse,ind(state)}); DataEMG.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})(DataEMG.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})==0)=NaN;
        end
    end
...
for mouse=1:length(Mouse_names)
    for sess=2:3
        for state=1:length(State1)
            try
                EMG_Filtered_Pre.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})=FilterLFP(EMG_TSD.(Session_type{sess}).emg.tsd{mouse,ind(state)},[50 300],1024);
...
%% Figures
Cols = {[0.3 0.3 0.3],[0 0 0],[1 0.5 0.5],[0.5 0.5 1]};
X = [1,2,3,4];
Legends = {'Active','Freezing','Shock','Safe'};
NoLegends = {'','','',''};
%Mean
figure
subplot(221)
MakeSpreadAndBoxPlot2_SB({MeanEMG_Unfiltered.Cond.Active ,MeanEMG_Unfiltered.Cond.Fz , MeanEMG_Unfiltered.Cond.Fz_Shock , MeanEMG_Unfiltered.Cond.Fz_Safe},Cols,X,NoLegends,'showpoints',0,'paired',1); makepretty; xtickangle(45);
...
subplot(222)
MakeSpreadAndBoxPlot2_SB({MeanEMG_Unfiltered.Ext.Active ,MeanEMG_Unfiltered.Ext.Fz , MeanEMG_Unfiltered.Ext.Fz_Shock , MeanEMG_Unfiltered.Ext.Fz_Safe},Cols,X,NoLegends,'showpoints',0,'paired',1); makepretty; xtickangle(45);
ylabel('EMG power (a.u.)'); a=text(-1,2e5,'Raw data','FontSize',20,'FontWeight','bold'); set(a,'Rotation',90)
set(gca, 'YScale', 'log'); ylim([5e4 4e6]); title('UMaze analysis')
...
%% Distrib
for sess=2:3; % correct where respi is nan
    for mouse=1:length(Mouse)
        for state=2:length(State1)
            DataEMG_Filtered.Fear.(State1{state}).(Mouse_names{mouse})=DataEMG_Filtered.Fear.(State1{state}).(Mouse_names{mouse});
        end
    end
end
...
for mouse=1:length(Mouse) % if you want beautiful distrib figures make state=3:4
    ...
    for sess=2:3; % correct where respi is nan
        subplot(2,1,sess-1)
        Y1 = prctile(log10(DataEMG_Filtered.(Session_type{sess}).Fz.(Mouse_names{mouse})),Prc);
        Y2 = prctile(log10(DataEMG_Filtered.(Session_type{sess}).Fz.(Mouse_names{mouse})),100-Prc);
        try
            for state=2:length(State1)
                h=histogram(log10(DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})),'BinLimits',[Y1 Y2],'NumBins',100); hold on
                HistData.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})=h.Values;
                Logical_to_use = and(log10(DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse}))<Y2 , log10(DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse}))>Y1) ;
                
                Clean_DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})=DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})(Logical_to_use);
            end
        end
    end
end
...
figure
subplot(121)
Conf_Inter=nanstd(HistData_All.Cond.Fz)/sqrt(size(HistData_All.Cond.Fz,1));
shadedErrorBar([1:100],runmean(nanmean(HistData_All.Cond.Fz),3)*2,Conf_Inter,'k',1); hold on;
Conf_Inter=nanstd(HistData_All.Cond.Fz_Shock)/sqrt(size(HistData_All.Cond.Fz_Shock,1));
shadedErrorBar([1:100],runmean(nanmean(HistData_All.Cond.Fz_Shock),3),Conf_Inter,'r',1);
Conf_Inter=nanstd(HistData_All.Cond.Fz_Safe)/sqrt(size(HistData_All.Cond.Fz_Safe,1));
shadedErrorBar([1:100],runmean(nanmean(HistData_All.Cond.Fz_Safe),3),Conf_Inter,'b',1);
makepretty;
f=get(gca,'Children'); legend([f(9),f(5),f(1)],'All fz','Fz shock','Fz safe');
xlabel('EMG values (a.u.)'); ylabel('%');
title('Cond')
subplot(122)
Conf_Inter=nanstd(HistData_All.Ext.Fz)/sqrt(size(HistData_All.Ext.Fz,1));
shadedErrorBar([1:100],runmean(nanmean(HistData_All.Ext.Fz),3)*2,Conf_Inter,'k',1); hold on;
Conf_Inter=nanstd(HistData_All.Ext.Fz_Shock)/sqrt(size(HistData_All.Ext.Fz_Shock,1));
shadedErrorBar([1:100],runmean(nanmean(HistData_All.Ext.Fz_Shock),3),Conf_Inter,'r',1);
Conf_Inter=nanstd(HistData_All.Ext.Fz_Safe)/sqrt(size(HistData_All.Ext.Fz_Safe,1));
shadedErrorBar([1:100],runmean(nanmean(HistData_All.Ext.Fz_Safe),3),Conf_Inter,'b',1);
makepretty; title('Ext')
xlabel('EMG values (a.u.)');
a=suptitle('EMG values distribution, UMaze, cond sessions, BM mice, n=7'); a.FontSize=20;
...
for mouse=1:length(Mouse) % zscoring from data all freezing
    for sess=2:3; % correct where respi is nan
        for state=2:4
            LogZscoredClean_DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse}) = (log10(Clean_DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse}))-nanmean(log10(Clean_DataEMG_Filtered.(Session_type{sess}).(State1{2}).(Mouse_names{mouse}))))/nanstd(log10(Clean_DataEMG_Filtered.(Session_type{sess}).(State1{2}).(Mouse_names{mouse})));
        end
    end
end
...
for mouse=1:length(Mouse)
    ...
    %figure
    for state=2:4
        X_to_use = LogZscoredClean_DataEMG_Filtered.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})(bin:bin:end);
        Y_to_use = Clean_Respi_Rythm2.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})(bin:bin:end);
        %plot(X_to_use,Y_to_use,colo{state-2})
        hold on
        Ellipse.(Session_type{sess}).(State1{state}).(Mouse_names{mouse}) = Ellipse_From_Data_BM(X_to_use,Y_to_use);
    end
%     makepretty
...
for mouse=1:length(Mouse)
    for sess=2:3; % correct where respi is nan
        for state=2:4
            ...
            hold on
            EllipseX_All.(Session_type{sess}).(State1{state})(mouse,:) = Ellipse.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})(:,1);
            EllipseY_All.(Session_type{sess}).(State1{state})(mouse,:) = Ellipse.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})(:,2);
        end
        %     makepretty
        %     title(Mouse_names{mouse})
        %     xlabel('EMG power (a.u.)'); %ylim([0 8])
...
Mouse=[1184 1205 1224 1225 1226 1227];
[EMG_Cond_TSD , Epoch1 , NameEpoch]=MeanValuesPhysiologicalParameters_BM(Mouse,'cond','emg');
[EMG_Ext_TSD , Epoch1 , NameEpoch]=MeanValuesPhysiologicalParameters_BM(Mouse,'exti','emg');
...
for mouse=1:length(Mouse) % if you want beautiful distrib figures make state=3:4
    ...
    for state=2:length(State1)
        ...
        HistData.(Session_type{sess}).(State1{state}).(Mouse_names{mouse})=h.Values;
        Logical_to_use = and(log10(DataEMG_Filtered.Fear.(State1{state}).(Mouse_names{mouse}))<Y2 , log10(DataEMG_Filtered.Fear.(State1{state}).(Mouse_names{mouse}))>Y1);
