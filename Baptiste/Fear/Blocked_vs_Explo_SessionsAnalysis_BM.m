...
for group=1
    ...
    for mouse=21:length(Mouse)
        ...
        try
            ...
            Respi_unbl_shock(mouse) = nanmean(Data(Restrict(OutPutData.(Session_type{sess}).(Drug_Group{group}).respi_freq_bm.tsd{mouse,5} , Blocked_Epoch.(Mouse_names{mouse}))));
            Respi_unbl_safe(mouse) = nanmean(Data(Restrict(OutPutData.(Session_type{sess}).(Drug_Group{group}).respi_freq_bm.tsd{mouse,6} , Blocked_Epoch.(Mouse_names{mouse}))));
