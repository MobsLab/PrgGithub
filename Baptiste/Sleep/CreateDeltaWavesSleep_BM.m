...
% Do on all Sleep and not only SWS + save differently dependy on the structure

function DeltaOffline = CreateDeltaWavesSleep_BM(varargin)
...
if strcmpi(scoring,'accelero')
    try
        load SleepScoring_Accelero TotalNoiseEpoch Sleep
    catch
        load StateEpoch TotalNoiseEpoch Sleep
...
elseif strcmpi(scoring,'ob')
    try
        load SleepScoring_OBGamma TotalNoiseEpoch Sleep
    catch
        load StateEpochSB TotalNoiseEpoch Sleep
    end
    
end

InputInfo.Epoch=Sleep-TotalNoiseEpoch;
...
if ~isempty(InputInfo.channel_deep) && ~isempty(InputInfo.channel_sup)
    ...
    EEGsleepDiff = Restrict(EEGsleepDiff,Sleep-TotalNoiseEpoch);
    ...
    if InputInfo.SaveDelta
        if exist('DeltaWaves.mat', 'file') == 2
            save(['DeltaWaves_' structure '.mat'], ['deltas_' InputInfo.structure suffixe], ['alldeltas_' InputInfo.structure suffixe], ['deltas_' InputInfo.structure suffixe '_Info'],'-append')
        else
            save(['DeltaWaves_' structure '.mat'], ['deltas_' InputInfo.structure suffixe], ['alldeltas_' InputInfo.structure suffixe], ['deltas_' InputInfo.structure suffixe '_Info'])
        end
        ...
        for i=1:length(evt.time)
            evt.description{i}= ['delta_waves_' InputInfo.structure suffixe];
        end
