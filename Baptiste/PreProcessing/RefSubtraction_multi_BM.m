...
function RefSubtraction_multi_BM(filename,nChannels,nSouris,varargin)
...
% global DATA
% DATA.nChannels = 32;
% Ref_Channel = GetWideBandData(20);
% [ Low_Passed_Ref, OUT ] = LowPassFilter(Ref_Channel(:,2), 500 , 2e4);
% 
    ...
    for j = 1:nChunks,
        ...
        for i=1:nSouris %preceed individually every mouse data and write with preceeded data and data to keep into a same file
            if length(chs_ref{i})~=0 %if length===0, means no treatement needed
                if sum(d(:,chs_ref{i})>1e4)>1
%                     keyboard
                    d(:,chs_to_preceed{i})=d(:,chs_to_preceed{i})-d(:,chs_ref{i});
                else
                % choose your method:
                % 1) Low and High pass
                Low_Passed_Ref = real(LowPassFilter( d(:,chs_ref{i}) , 10 , frequency )) ; % add by BM, 03/2021
                High_Passed_Ref = real(HighPassFilter_BM(d(:,chs_ref{i}) , 250 , frequency ));
                CombinedRef = Low_Passed_Ref + High_Passed_Ref;
                d(:,chs_to_preceed{i})=d(:,chs_to_preceed{i})-CombinedRef;
                end
                % 2) pima_logKern
                %                 d(:,chs_to_preceed{i})=d(:,chs_to_preceed{i})-pima_logKernSmooth_BM(d(:,chs_ref{i}),1,800)*ones(1,length(chs_to_preceed{i})); % to have the same dimension by using a multiplication of ones
                % 3) Runmean
                %d(:,chs_to_preceed{i})=d(:,chs_to_preceed{i})-runmean(d(:,chs_ref{i}),1000)*ones(1,length(chs_to_preceed{i}));
                %                 length_to_use = length_to_use + size(d,1);
                %                 d(:,chs_to_preceed{i}) = d(:,chs_to_preceed{i})-Low_Passed_Ref(1:length_to_use)*ones(1,length(chs_to_preceed{i})); % to have the same dimension by using a multiplication of ones
                %
        ...
        if m ~= 0,
            for i=1:nSouris %preceed individually every mouse data and write with preceeded data and data to keep into a same file
                if length(chs_ref{i})~=0 %if length===0, means no treatement needed
                    ...
                    [ out, OUT ] = LowPassFilter( d(:,chs_ref{i}), 2, 20 ) ;
                    
                    d(:,chs_to_preceed{i})=d(:,chs_to_preceed{i})-out*ones(1,length(chs_to_preceed{i})); % to have the same dimension by using a multiplication of ones
                    towrite=d(:,sort([chs_to_preceed{i},chs_to_keep{i}]));
                else
                    towrite=d(:,chs_to_keep{i});
                end
