...
if restrict
    ...
    nonLFP = Restrict(nonLFP , SWSEpoch);
    Epoch = SWSEpoch;
end
...
if stim
    try
        load('behavResources.mat','StimEpoch'); % supposed for eyelid, 0.3 ms before, 0.5 ms after
        st = Start(StimEpoch);
        time = Range(Filsp_tmp);
        TotalEpoch = intervalSet(time(1), time(end));
        for istim=1:length(st)
            sti(istim) = st(istim)-3000;
        ...
        try % add by BM, optimized for VHC stims
            ...
            load('behavResources.mat','StimEpoch2'); % supposed for VHC, 0.1 ms before, 0.2 ms after
            st = Start(StimEpoch2);
            time = Range(Filsp_tmp);
