...
%    wvranges        = zeros(length(spktimes),nElec);
    ...
    fid         = fopen(spkname,'w'); 
    %chunkSize   = 100;
    %nbChunk     = floor(length(spktimes))/chunkSize;
    %remChunk    = length(spktimes) - floor(length(spktimes));
    
    for j=1:length(spktimes)
    %for j=1:nbChunk
        %spkT    = double(spktimes((j-1)*chunkSize+1:j*chinkSize));
        %ix      = (spkT(:)'-sbefore)*nElec;
        %ix      = repmat(ix,[(sbefore+safter)*nElec 1])+repmat([1:(sbefore+safter)*nElec]',[1 nbChunk]);
        %ix      = ix(:);
        try
        ...
        %wvranges(j,:) = range(wvaswv); %Modified by Adrien
