function [headers, ldtimes, idcell, fidpos] = rwheadermod(fpath1, fpath2)
%  This function reads in only the header of the binary file created
%  by the MouseRec data collection program with path and filename in
%  string FPATH1 and writes it to new file in FPATH2.
%
%      [headers, ldtimes, idcell, fidpos] = readheadermod(fpath1, fpath2)
%
...
fid = fopen(fpath1,'r','ieee-be');  % Binary raw data file to read
fid1=fopen(fpath2,'w','ieee-be');  % Feature file to create
...
if fid == -1  %  Set to empty output arrays
    ...
    fidpos = -1;
    errordlg(['Cannot open file ' fpath1])
