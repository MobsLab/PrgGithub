function sleepwakepsd(fnam, pnam)
    ...
    for ct=40:c
     ...
     % Read in the Data channels
        ...
        if  dtlen > winlen %  If it was greater than one window size, process file
               ...
               [p,fax] = pwelch(datach-mean(datach),hamming(fs*8),fs*4,fs*16,fs);
               semilogx(fax,10*log10(p));
               title(['Channel ' int2str(ct) ' up to ' num2str(lim(2)/(60*60)) ' Hours'])
               pause
