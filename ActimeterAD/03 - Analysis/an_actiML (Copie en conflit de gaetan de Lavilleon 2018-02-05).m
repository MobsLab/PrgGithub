...
for id =1:length(orderID)
    hold on,
    plot([1:size(sleepWakeDuration,1)]*4-4,sleepWakeDuration(:,orderID(id)),...
        '.-','Color',colori(5*id,:),'Linewidth',2)
    leg=[leg,{['Mouse #',num2str(orderID(id))]} ];
end
for i=1:n_block
    if mod(i,24/block)==1
        line([1 1]*(i-1)*block,[0 100],'Color',[0.5 0.5 0.5])
    else
...
xlabel('# block'), xlim([-4 size(sleepWakeDuration,1)*4])
ylabel('% sleep')
...
legend(leg), xlabel('Time (h)')
set(gca,'Ytick',4*[0:1:length(orderID)-1])
...
colorbar;caxis([0 80]); line(floor(length(orderID)/2)*[1 1]+0.5,ylim,'Color','w')
title('Nb of auditory Stim per block (black lines = midnight)')
xlabel('#Mouse WAKE GROUP                  #Mouse SLEEP GROUP'); ylabel('# block')
