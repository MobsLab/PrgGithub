function [numF,filename]=TrackThreeMiceOnline(~,~)
...
inputDisplay(3)=uicontrol(FigDouble,'style','text','units','normalized','position',[0.15 0.85 0.4 0.02],'string','Filename3 = TO DEFINE','FontSize',12);
inputDisplay(4)=uicontrol(FigDouble,'style','text','units','normalized','position',[0.2 0.07 0.07 0.02],'string','OFF','FontSize',12);
...
%% Ask for all inputs and display
    function giv_inputs(obj,event)
        ...
        % DEFINE INPUTS FOR THE EXPE
            ...
            defAns={'BULB-','Mouse-158','Mouse-161','Mouse-184','-Rest-tracking','5'};
        
        prompt = {'pefixe','mouse 1','mouse 2','mouse3','suffixe','CountDown (s)'};
        ...
        name3=answer{4};
        suffixe = answer{5};
        CDdur= str2num(answer{6});
        
        set(inputDisplay(1),'string',['Mouse 1 = ',name1]);
        set(inputDisplay(2),'string',['Mouse 2 = ',name2]);
        set(inputDisplay(3),'string',['Mouse 3 = ',name3]);
        set(figbutton(2),'enable','on','FontWeight','bold')
        disp(' ');disp('-------------------- New Expe ---------------------');
        try
            save([res,mark,'TempTripleTrack.mat'],'-append','pefixe','name1','name2','suffixe','imageRef','CDdur');
        catch
            save([res,mark,'TempTripleTrack.mat'],'pefixe','name1','name2','name3','suffixe','imageRef','CDdur');
        end
    end
...
%% Ask for all inputs and display
    function Real_distance(obj,event)
        ...
        save([res,mark,'TempTripleTrack.mat'],'-append','Ratio_IMAonREAL');
    end
...
%% Ask for 2 masks
    function Get_masks(obj,event)
        ...
        function save_mask1(obj,event) % keep ref in memory
            mask1=single(Mmask);
            save('TempTripleTrack.mat','mask1','-append');
            disp('mask 1 saved');
        ...
        function save_mask2(obj,event) % keep ref in memory
            mask2=single(Mmask);
            save('TempTripleTrack.mat','mask2','-append');
            disp('mask 2 saved');
        ...
        function save_mask3(obj,event) % keep ref in memory
            mask3=single(Mmask);
            save('TempTripleTrack.mat','mask3','-append');
            disp('mask 3 saved');
            msg_box=msgbox('Mask 3 saved','save','modal');
            pause(0.4); close(msg_box);
            Reset_mask;
        end
...
%% track mouse
    function Track_Double(obj,event)
        ...
        % -------------------
        % reload everything
        tempLoad=load([res,mark,'TempTripleTrack.mat']);
        ...
        mask3=tempLoad.mask3;
        
        prefac=num2str(num_exp);
        if length(prefac)==1, prefac=['0',prefac];end
        name_folder=[tempLoad.pefixe,'-',tempLoad.name1,'-',tempLoad.name2,'-',tempLoad.name3,'-',TodayIs,'-',prefac,tempLoad.suffixe];
        ...
        name_folder3=[tempLoad.pefixe,'-',tempLoad.name3,'-',TodayIs,'-',prefac,tempLoad.suffixe];
        ...
        figure(FigDouble), subplot(2,4,3), hold off
        htrack2 = imagesc(zeros(size((imageRef))));axis image;caxis([0 1])
        title('tracking mouse 1','Color','w'); axis ij
        
        figure(FigDouble), subplot(2,4,4), hold off
        htrack3 = imagesc(zeros(size((imageRef))));axis image;caxis([0 1])
        title('tracking mouse 2','Color','w'); axis ij
        
        figure(FigDouble), subplot(2,4,7), hold off
        htrack4 = imagesc(zeros(size((imageRef))));axis image;caxis([0 1])
        title('tracking mouse 3','Color','w'); axis ij
        ...
        while enableTrack
            ...
            % ---------------------------------------------------------
            % --------------------- FIND MOUSE 3, ---------------------
            subimage3 = uint8(double(subimage).*double(mask3));
            % Convert the resulting grayscale image into a binary image.
            diff_im = im2bw(subimage3,BW_threshold2);
            % Remove all the objects less large than smaller_object_size
            diff_im = bwareaopen(diff_im,smaller_object_size2);
            
            %immobility
            try MovIM3=(diff_im-tempIM3); catch, MovIM3=single(zeros(size(diff_im)));end
            tempIM3=diff_im;
            immob_val3(num_fr)=sqrt(sum(sum(((MovIM3).*(MovIM3)))))/12000/2*100;
            ...
            % ---------------------------------------------------------
            % display video, mouse position and save in posmat
            set(htrack4,'Cdata',bw);
            if size(centroids) == [1 2]
                set(g3,'Xdata',centroids(1),'YData',centroids(2))
                PosMat3(num_fr,1)=chrono;
                PosMat3(num_fr,2)=centroids(1);
                PosMat3(num_fr,3)=centroids(2);
                PosMat3(num_fr,4)=NaN;
            else
                set(g3,'Xdata',0,'YData',0)
                PosMat3(num_fr,:)=[chrono;NaN;NaN;NaN];
            end
        ...
        try
            ...
            save([res,mark,'TempTripleTrack.mat'],'-append','PosMat')
            pause(0.5)
            save([res,mark,name_folder1,mark,'PosMat.mat'],'PosMat','PosMat1');
            try copyfile([res,mark,'TempTripleTrack.mat'],[name_folder1,mark,'DoubleTrack.mat']); catch; keyboard;end
        catch
        ...
        try
            ...
            try copyfile([res,mark,'TempTripleTrack.mat'],[name_folder2,mark,'DoubleTrack.mat']); catch; keyboard;end
        ...
        % ---------------------------------------------------------
        try
            PosMat=PosMat3; ref=imageRef; mask=mask3;
            timeImposed=[min(PosMat(:,1)):1/15:max(PosMat(:,1))]; % at 15Hz
            disp(['... Saving ',name_folder3,'.mat']); indexpos=find(~isnan(PosMat(:,2)));
            try
                Pos=[timeImposed',interp1(PosMat(indexpos,1),PosMat(indexpos,2:3),timeImposed)];
            catch
                disp('ATTENTION !!!!! Pos mouse 2 is composed of NaN only!');
            ...
            Movtsd=tsd(PosMat3(:,1)*1E4',SmoothDec(immob_val3',1));
            save(name_folder3,'Pos','PosMat','Movtsd','ref','mask','shape_ratio2','BW_threshold2','smaller_object_size2','frame_rate','Ratio_IMAonREAL');
            save([res,mark,'TempTripleTrack.mat'],'-append','PosMat')
            pause(0.5)
            save([res,mark,name_folder3,mark,'PosMat.mat'],'PosMat','PosMat2');
            try copyfile([res,mark,'TempTripleTrack.mat'],[name_folder3,mark,'DoubleTrack.mat']); catch; keyboard;end
            
            disp('Done')
            
        catch
            disp('Problem saving mouse 3...');
            keyboard;
        end
        ...
        hold on, plot(PosMat3(:,2),PosMat3(:,3),'r'); ylabel(name_folder3);
        ...
        hold on, plot(PosMat3(:,1),SmoothDec(immob_val3',1),'r');
        
        legend({tempLoad.name1,tempLoad.name2,tempLoad.name3});
...
%% Count_Down before Track_sniff
    function Count_Down(obj,event)
        templ=load([res,mark,'TempTripleTrack.mat'],'CDdur');
        CDdur=templ.CDdur;
        chronoshow=uicontrol('style','edit', 'units','normalized','position',[0.065 0.26 0.04 0.05],...
            'string',num2str(floor(0)),'ForegroundColor','g','BackgroundColor','k');
        tdeb = clock;
