function result = f_disprof_clust(Y,dis,link,mc,iter,alpha,tol)
% - disimilarity profile analysis (DISPROF) of a cluster analysis dendrogram
%
% USAGE: result = f_disprof_cluster(Y,'dis',link,mc,iter,alpha,tol)
%
...
%         1: 'average'  (UPGMA)                                         
%         2: 'centroid' (UPGMC)
%         3: 'complete' (Furthest Distance)
%         4: 'median'   (WPGMC)
%         5: 'single'   (Shortest Distance)
%         6: 'ward'     (Inner Squared Distance)
%         7: 'weighted' (WPGMA)
...
%
% SEE ALSO: f_cluster, f_dis, f_disprof, f_disprof_clustPlot
...
if (~isequal(dis,'euc') && ismember(link,[2 4 6]))
   error('DIS must be ''euc'' when link = 2, 4, or 6!')
end
...
switch link
   ...
   case 7 
      ...
      linkType = 'WPGMA';
   otherwise
      error('LINK must be value from 1 to 7!');
end
% ---------------------------------------
...
% Remove any links where distances are zero (in case duplicate row occur in Y):
cv(cv==0) = [];
