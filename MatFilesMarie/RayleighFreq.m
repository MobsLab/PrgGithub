...
% [H,HS,Ph,ModTheta]=RayleighFreq(lfp,S,sig,fre,fi)) plot the distribution of phases for
...
% figure('color',[1 1 1]),
imagesc([0:4*pi],ab,HS), axis xy
ylabel('Frequence (Hz)')
xlabel('Phase (rad)')

ModTheta=[mu', Kappa', pval',Rmean',ab'];

FigArrow([ModTheta(ModTheta(:,3)<0.05,1),pas*find(ModTheta(:,3)<0.05)], ModTheta(ModTheta(:,3)<0.05,2)/fac,ModTheta(ModTheta(:,3)<0.05,1),sig)
FigArrow([ModTheta(ModTheta(:,3)<0.05,1)+2*pi,pas*find(ModTheta(:,3)<0.05)], ModTheta(ModTheta(:,3)<0.05,2)/fac,ModTheta(ModTheta(:,3)<0.05,1),sig)
