...
%   11/10/99  EAJ  Update e-mail address.k
...
ax = repmat(gca,narrows,1);%ax = o * gca;
...
while (any(axnotdone)),
	...
	[azA,elA] = view(curax);curT = viewmtx(azA,elA); %curT = get(curax,'Xform');
...
if ~isempty(ii),
	...
	%	transform back to 3-D
		if (oneax), threeD=invT*tmp1;
            ...
            tmp1=[tmp1;tmp1;tmp1;tmp1]; tmp1=invT(:,ii).*tmp1;
		      tmp2=zeros(4,4*length(ii)); tmp2(:)=tmp1(:);
...
%	transform Start points
	...
	if (oneax), X0=T*tmp1;
        ...
        tmp1=[tmp1;tmp1;tmp1;tmp1]; tmp1=T.*tmp1;
...
%	compute stoppoints
	...
	if (oneax), tmp3=invT*tmp1;
        ...
        tmp1=[tmp1;tmp1;tmp1;tmp1]; tmp1=invT.*tmp1;
	      tmp2=zeros(4,4*narrows); tmp2(:)=tmp1(:);
...
if ~isempty(ii),
	...
	%	transform start points
		...
		if (oneax)
            ...
            tmp1=[tmp1;tmp1;tmp1;tmp1]; tmp1=T(:,[ii ii ii ii]).*tmp1;
	...
	%	inverse transform the cross points
		if (oneax)
            ...
            tmp1=[Xp;Xp;Xp;Xp]; tmp1=invT(:,[ii ii ii ii]).*tmp1;
		      tmp2=zeros(4,16*numii); tmp2(:)=tmp1(:);
...
%	compute start points
	...
	if (oneax)
        ...
        tmp1=[tmp1;tmp1;tmp1;tmp1]; tmp1=[T T T T T T T T T T T].*tmp1;
...
%	inverse transform
	if (oneax)
        ...
        tmp1=[tmp1;tmp1;tmp1;tmp1]; tmp1=[invT invT invT invT invT invT invT invT invT invT invT].*tmp1;
	      tmp2=zeros(4,44*narrows); tmp2(:)=tmp1(:);
...
%	% make or modify the arrows
	for k=1:narrows,
		if all(isnan(ud(k,[3 6])))&arrow_is2DXY(ax(k))
            zz=[];
        else
            zz=z(:,k);
        end
