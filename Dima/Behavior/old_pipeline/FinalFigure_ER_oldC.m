...
dir_out = '/home/mobsrick/Dropbox/MOBS_workingON/Dima/Ongoing results/Behavior/Mouse828/'; % Where?
...
calibdir = '/media/mobsrick/DataMOBS87/Mouse-828/20190220/Calib/';
...
Intensities = [0 0.5 1 1.5 2 2.5 3.0];
...
Dir_Pre = PathForExperimentsERC_Dima('TestPre');
Dir_Pre = RestrictPathForExperiment(Dir_Pre,'nMice',nmouse);
Dir_Cond = PathForExperimentsERC_Dima('Cond');
Dir_Cond = RestrictPathForExperiment(Dir_Cond,'nMice',nmouse);
Dir_Post = PathForExperimentsERC_Dima('TestPost');
Dir_Post = RestrictPathForExperiment(Dir_Post,'nMice',nmouse);
if h24
...
if Avoidance
    % PreTests
    for i = 1:length(Dir_Pre.path{1})
        a = load([Dir_Pre.path{1}{i} 'behavResources.mat'], 'Occup', 'PosMat','Xtsd', 'Ytsd', 'Vtsd', 'mask', 'Zone',...
            'ZoneIndices', 'Ratio_IMAonREAL');
        Pre_Xtsd{i} = a.Xtsd;
        Pre_Ytsd{i} = a.Ytsd;
        Pre_Vtsd{i} = a.Vtsd;
    ...
    for i = 1:length(Dir_Cond.path{1})
        b = load([Dir_Cond.path{1}{i} 'behavResources.mat'], 'Occup', 'PosMat','Xtsd', 'Ytsd', 'Vtsd', 'mask', 'Zone',...
            'ZoneIndices', 'Ratio_IMAonREAL');
        Cond_Xtsd{i} = b.Xtsd;
        Cond_Ytsd{i} = b.Ytsd;
        Cond_Vtsd{i} = b.Vtsd;
	...
	% PostTests
    for i=1:length(Dir_Post.path{1})
        c = load([Dir_Post.path{1}{i} 'behavResources.mat'],'Occup', 'PosMat','Xtsd', 'Ytsd', 'Vtsd', 'mask', 'Zone',...
            'ZoneIndices', 'Ratio_IMAonREAL');
        Post_Xtsd{i} = c.Xtsd;
        Post_Ytsd{i} = c.Ytsd;
        Post_Vtsd{i} = c.Vtsd;
...
for u = 1:length(Dir_Post.path{1})
    if Avoidance
...
for m = 1:length(Dir_Post.path{1})
    if Avoidance
...
for r=1:length(Dir_Post.path{1})
    if Avoidance
...
if Avoidance
    ...
    hold on
    for p=1:1:length(Dir_Pre.path{1})
...
if Avoidance
    ...
    hold on
    for p=1:1:length(Dir_Cond.path{1})
    ...
    set(gca, 'XTickLabel', []);
    set(gca, 'YTickLabel', []);
    for p=1:length(Dir_Cond.path{1})
...
if Avoidance
    ...
    hold on
    for l=1:1:length(Dir_Post.path{1})
...
if Avoidance
    ...
    hold on
    for k = 1:length(Dir_Post.path{1})
    ...
    hold on
    for g = 1:length(Dir_Post.path{1})
