%MakeArtefactsZeros_DB - for binary file(.dat): the values that coincide with
%                    noise (defined with FindNoiseEpoch_SleepScoring)
%                    are changed to zeros. Copy of the original file will be saved by
%                    default. This will help with Kilosort spike sorting.
%
%  USAGE
%
%    MakeArtefactsZeros_DB(filename, nChannels, suffix, <options>)
%
...
% Dima Bryzgalov, Laboratoires MOBS, ESPCI-ParisTech 25.04.2019

function MakeArtefactsZeros_DB(filename,nChannels,suffix,varargin)
...
% Load and treat noise epoch
if exist('behavResources.mat', 'var') == 2
    if ~exist('SleepScoring_Accelero.mat')
        error('File with noise does not exist');
    else
        load('SleepScoring_Accelero.mat',NoiseVarName);
    end
else
    load('SleepScoring_OBGamma.mat',NoiseVarName);
end
NoiseIntervals = [Start(TotalNoiseEpoch)/1e4 End(TotalNoiseEpoch)/1e4];
...
if nSamples > maxSamplesPerChunk,
    ...
    for j = 1:nChunks,
        ...
        st = find(InIntervals(ttt,NoiseIntervals));
