function [pval,hb,eb] = PlotErrorBarN_DB(A,varargin)

% function pval= PlotErrorBarN_DB(A,varargin)
%
...
% - Distribution (optional) = 1 if sample is drawn from normal distribution (ttest will be done), 
%                           0 if from any other distribution (default 0, -> non-parametric tests)
...
if ~horizontal
    ...
    if isempty(barcolors)
        hb = bar(R,'FaceColor',[0.3 0.3 0.3], 'FaceAlpha', 0.45, bar_argin{:});
    elseif ~iscell(barcolors)
        hb = bar(R,'FaceColor',barcolors, 'FaceAlpha', 0.45, bar_argin{:});
    elseif length(barcolors)==1
            hb = bar(R,'FaceColor',barcolors{1}, 'FaceAlpha', 0.45, bar_argin{:});
    else
        for i=1:length(R)
            hb = bar(i, R(i), 'FaceColor', barcolors{i}, 'FaceAlpha', 0.45, bar_argin{:});
        end
    end
...
%horizontal
    ...
    %bar colors
    if isempty(barcolors)
        barh(R,'FaceColor',[0.3 0.3 0.3],'EdgeColor','k', 'FaceAlpha', 0.45)
    elseif length(barcolors)==1
        barh(R,'FaceColor',barcolors{1},'EdgeColor','k', 'FaceAlpha', 0.45)
    else
        for i=1:length(R)
            barh(i, R(i), 'FaceColor', barcolors{i},'EdgeColor','k', 'FaceAlpha', 0.45);
        end
    end
...
if showpoints && ~horizontal
    ...
    for i=1:N
        temp=A{i}(~isnan(A{i}));
        plot(i*ones(length(temp),1)+0.1,temp,'ko','markerfacecolor','w', 'LineStyle', 'None')
    end
    ...
    elseif strcmp(optiontest,'ttest')
        ...
        for c=1:length(column_test)
            ...
            if hnorm(i)+hnorm(j)==0 && hvar(min(i,j),max(i,j))==0
                ...
                warning('The data are not distributed normally. Please consider using non-parametric tests')
            end
        end
        stats(stats>thresh_signif)=nan;
        if ~horizontal
