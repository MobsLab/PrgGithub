%RefSubtraction_GroupByGroup - for binary file(.dat): the values of specified channels will
...
%    chs_to_treat       Cell with your electrode groups to rereference - vector of matrices divided by semicolons;
%    chs_ref            Cell with your reference electrode groups - vector of matrices divided by semicolons;
%               chs_to_treat and chs_ref should be the same size and
%               the first cell in chs_to_treat corresponds to the first cell in chs_ref
%    chs_to_keep        Vector of channels that shouldn't be processed
...
% This programme is based on the 'LoadBinary' wrote by Michaël Zugaro
% Kejian, Laboratoires MOBS, ESPCI-ParisTech 08.04.2015
% Dima Bryzgalov, Laboratoires MOBS, ESPCI-ParisTech 25.02.2019

function RefSubtraction_GroupByGroup(filename,nChannels,nSouris,varargin)
...
for i=1:4:nSouris*4
    ...
    if ~isempty(varargin{i+1})
        if ~(isa(varargin{i+1},'cell'))
            error(['format error for chs_to_treat, for mouse ',num2str(indice) '. Should be a cell']);
        end
        for j=1:size(varargin{i+1},1)
            if ~(round(varargin{i+1}{j})==varargin{i+1}{j})
                error(['format error for chs_to_treat, for mouse ',num2str(indice) '. Channels must be integers']);
            end
        end
    end
    
    if ~isempty(varargin{i+2})
        if ~(isa(varargin{i+2},'cell'))
            error(['format error for chs_ref, for mouse ',num2str(indice) '. Should be a cell']);
        end
        for j=1:size(varargin{i+2},1)
            if ~(round(varargin{i+2}{j})==varargin{i+2}{j})
                error(['format error for chs_ref, for mouse ',num2str(indice) '. Channels must be integers']);
            end
        end
    end
    
    if ~(size(varargin{i+1},1)==size(varargin{i+2},1))
        error('Number of electrode groups in chs_to_treat and chs_ref are not equal, please verify');
    end
    ...
    if ~((isempty(varargin{i+1}) && isempty(varargin{i+2}) && ~isempty(varargin{i+3}~=0)) || (~isempty(varargin{i+1}) && ~isempty(varargin{i+2})))
    ...
    for k=1:size(varargin{i+1},1)
        chs_to_treat{indice}{k}=varargin{i+1}{k}+1;
    end
    
    for k=1:size(varargin{i+2},1)
        chs_ref{indice}{k}=varargin{i+2}{k}+1;
    end
...
if nSamples > maxSamplesPerChunk,
    ...
    for j = 1:nChunks,
        ...
        for i=1:nSouris %treat individually every mouse data and write with treated data and data to keep into a same file
            if ~isempty(chs_ref{i}) %if empty, means no treatement needed
                for k=1:size(chs_ref{i},2)
                    
             d(:,chs_to_treat{i}{k})=d(:,chs_to_treat{i}{k})-mean(d(:,chs_ref{i}{k}),2)*ones(1,length(chs_to_treat{i}{k})); % to have the same dimension by using a multiplication of ones

                end
                towrite=d(:,1:nChannels);
    ...
    if remainder ~= 0,
        ...
        [m,n] = size(d);
        if m ~= 0,
            for i=1:nSouris %treat individually each mouse data and write with treated data and data to keep into a same file
                if ~isempty(chs_ref{i}) %if empty, means no treatement needed
                    for k=1:size(chs_ref{i},2)
                        d(:,chs_to_treat{i}{k})=d(:,chs_to_treat{i}{k})-mean(d(:,chs_ref{i}{k}),2)*ones(1,length(chs_to_treat{i}{k})); % to have the same dimension by using a multiplication of ones
                    end
...
for i=1:nSouris
    movefile(fullfile(pathstr,[name,'_',output_name{i},'.new']),fullfile(pathstr,[name,'_',output_name{i},ext])); % new treated files renamed to original file's name
end
