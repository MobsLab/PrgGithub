...
function [AlignedXtsd,AlignedYtsd,ZoneEpochAligned,XYOutput] = MorphMazeToSingleShape_EmbReact_SB(Xtsd,Ytsd,ShockZone,Ref,Ratio_IMAonREAL,XYInput,ZoneDef)
...
%% For zones, remove nans
idx = isnan(Data(AlignedXtsd));
idy = isnan(Data(AlignedYtsd));
time = Range(AlignedXtsd);
datX = Data(AlignedXtsd);
datY = Data(AlignedYtsd);

time(idx) = [];
datX(idy) = [];
datY(idy) = [];
Xtsd_zones = tsd(time, datX);
Ytsd_zones = tsd(time, datY);

%% give us a look at the result
figure
hold on
for z = 1:5
    ZoneEpochAligned_X1 = thresholdIntervals(Xtsd_zones,ZoneDef{z}(1,1),'Direction','Above');
    ZoneEpochAligned_X2 = thresholdIntervals(Xtsd_zones,ZoneDef{z}(1,2),'Direction','Below');
    ZoneEpochAligned_Y1 = thresholdIntervals(Ytsd_zones,ZoneDef{z}(2,1),'Direction','Above');
    ZoneEpochAligned_Y2 = thresholdIntervals(Ytsd_zones,ZoneDef{z}(2,2),'Direction','Below');
    ZoneEpochAligned{z} = and(and(and(ZoneEpochAligned_X1,ZoneEpochAligned_X2),ZoneEpochAligned_Y1),ZoneEpochAligned_Y2);
    plot(Data(Restrict(Xtsd_zones,ZoneEpochAligned{z})),Data(Restrict(Ytsd_zones,ZoneEpochAligned{z})),'color',colors{z})
end
line([0 0],[0 1])
line([1 1],[0 1])
line([0 1],[0 0])
