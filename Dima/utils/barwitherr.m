...
%   11/07/19    Dmitri Bryzgalov        the error bars made one-sided
%
%**************************************************************************
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        lowerErrors = zeros(length(errors(:,1)),1);
        for i=1:size(values,2)
            id = find(values(:,i)<0);
            if ~isempty(id)
                lowerErrors(id)=-upperErrors(id);
                upperErrors(id,i)= 0;
            end
        end
        
        hErrorbar(col) = errorbar(mean(x,1), values(xOrder,col),lowerErrors, upperErrors(xOrder, col), '.k','LineWidth',3);
    ...
    hErrorbar = errorbar(mean(x,1), values, lowerErrors, upperErrors, '.k', 'LineWidth',3);
