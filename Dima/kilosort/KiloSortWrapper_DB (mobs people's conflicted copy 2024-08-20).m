function savepath = KiloSortWrapper_DB(varargin)
...
addParameter(p,'nmouse',882,@isnumeric)                 % Number of mouse - used to create a channel map (script should be created before) - % Changed by Dima Bryzgalov 25/04/2019
...
eval(['[nb_groups, name_map] = channelmap_Mouse' num2str(nmouse) '(basepath);']) % which channel map?
...
%create config file
ops = MakeConfigKS_Mouse([basename '.dat'], basepath, 'TMult', 8, 'whiteningRange',4, 'ChanMapFile', name_map,'GPU_id',GPU_id);
