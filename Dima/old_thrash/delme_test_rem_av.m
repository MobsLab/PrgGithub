% RampOver1Day_quantifPlot_DB.m
% got it from Julie 15.06.2017
% source script here /home/mobsrick/Dropbox/Kteam/PrgMatlab/Julie/Opto/RampOver1Day_quantifPlot_OB_PFC_HPC_PiCx.m
...
%     - power before stim (dashed line) & during stim (solid line) 
...
%     - difference (during stim) - (before stim)
%     - percentage of increase during stim : (during-before) / during
%     - difference (during-before) normalized by power at 1Hz  out of stim    (impedance normalization) : "Diff_norm"
%     - difference (during-before) normalized by power during 20Hz stim ("infection" normalization)

% ratio PFC / bulb


% TODO: to code ratio for names ratrher than for el.
% numbers, explain figures in parameters, check
% ratio_plo
% !!!!!!!!!!!!!!!!!!! To fix mistake of taking std of std vector
...
% Do you want to save the pictures?
sav=1;
...
mousenames={'534'};
    ...
    % Load the data
    cd /media/mobsrick/DataMOBs71/Mouse-534/Results/062617_Plain_elec/theta
    load(['DataRampOver1DayQuantif_no_selection.mat']);
    disp(['Loading existing data from DataRampOver1DayQuantif_no_selection.mat']);

structlist={'dHPC_rip', 'PFCx_deep', 'PFCx_deep_left', 'dHPC_sup', 'dHPC_deep_left', 'vHPC_deep_right'};
rat_struct = {'PFCx_deep', 'dHPC_rip'};
% colori={'c','r','r','c','c','g','g'};
colori={[0 1 1],[1 0 0],[1 0 0],[0 1 1],[0 1 1],[1 0.6 0.2]};
res=pwd;
...
if allmice_plo       
    BefDurPlot=figure('Position',[  1346 -175 1855 1121]);          
    DiffPlot=figure('Position',[    1346 -175 1855 1121]);  
%     DiffPercPlot=figure('Position',[  1851          -8         467         846]);  
%     DiffNormPlot=figure('Position',[   1851          -8         467         846]);  
    BefDurRatioPlot=figure('Position',[    1346 -175 1855 1121]);  
    PFC_HPC_ratioplot=figure('Position',[   1346 -175 1855 1121]);  
end
...
try 
    ...
    for man=1:length(Dir.path), 
        ...
        % Power for normalization is taken as a pre-stimulation 30 sec before stimulation on 1Hz
            fq=1;
            ...
            for fq=1:nb_fq
                ...
                bbb=squeeze(LFPpower_Before(1,i,man,id_high,:,fq));% Power before the stimulation on the frequency requested
                ddd=squeeze(LFPpower(1,i,man,id_high,:,fq));% Power during the stimulation on the frequency requested
                ...
                % Percentage of difference of during and before normalized by before (during-before/before*100)
                ...
                % Difference of during and before normalized by power at 1Hz (before)
...
% Average across days within one mouse
for man=1:length(Dir.path), 
...
%             if ind_2remov_by_stt(man,i), text(0.3,0.5,'bad LFP','units','normalized','fontsize',15),end
    ...
    if ratio_PFC_HPC_plo
         figure(PFC_HPC_ratioplot)
        subplot(2,length(Dir.path),man)
        plot(fq_list, ddd_mean(2,:)./ddd_mean(1,:),'Color','k', 'LineWidth', 2)
       ylabel(' ratio PFC / HPC righ ')

        subplot(2,length(Dir.path),length(Dir.path)+man)
        plot(fq_list, ddd_mean(3,:)./ddd_mean(5,:),'Color','k', 'LineWidth', 2)
        ylabel(' ratio PFC / HPC left ')
    end
% 
%     if i==1, title([Dir.path{man}(end-11:end-9) '-' Dir.path{man}(end-7:end)]), end
%     if man==1, ylabel(structlist{i}), end
...
ind_not_efficient= [ind_right_2remov ind_left_2remov ind_right_2remov ind_left_2remov ind_right_2remov ind_right_2remov ind_left_2remov];
    ...
    ind_not_efficient= zeros(length(Dir.path),length(structlist));
    ampl_th=[];
end
...
for i=1:length(structlist)
    ...
    errorbar(fq_list', nanmean(mm_dur,1), nanstd(mm_dur,1)/sqrt(sum(~isnan(mm_dur(:,1)))),'Color', colori{i}, 'LineWidth',2);
    hold on
    errorbar(fq_list', nanmean(mm_bef,1), nanstd(mm_bef,1)/sqrt(sum(~isnan(mm_bef(:,1)))),'Color', colori{i},'LineStyle','--')
...
%     errorbar(fq_list, nanmean(mm_dur./mm_bef,1), nanstd(mm_dur./mm_bef,1)/sqrt(sum(~isnan(mm_bef(:,1)))),'Color', colori{i},'LineWidth',2)
...
for i=1:length(structlist)
    ...
    subplot(length(structlist),1,i)
    errorbar(fq_list, nanmean(mm_ratio,1), nanstd(mm_ratio,1)/sqrt(sum(~isnan(mm_ratio(:,1)))),'Color',colori{i},'LineWidth',2), hold on
...
for i=1:length(structlist)
    ...
    subplot(length(structlist),1,i)
    plot(fq_list, mm_dur-mm_bef), hold on
    errorbar(fq_list, nanmean(mm_dur-mm_bef,1), nanstd(mm_dur-mm_bef,1)/sqrt(sum(~isnan(mm_dur(:,1)))), 'Color', colori{i},'LineWidth',2),
...
for i=1:length(structlist)
    ...
    errorbar(fq_list, nanmean(mm_perc,1), nanstd(mm_perc,1)/sqrt(sum(~isnan(mm_perc(:,1)))),'Color',colori{i},'LineWidth',2),
    YL=ylim;
...
mm_Mean_right=squeeze(Mx_mean_during(:,2,:)) ./ squeeze(Mx_mean_during(:,1,:));
subplot(211); 
...
% Left
mm_Mean_left=squeeze(Mx_mean_during(:,3,:)) ./ squeeze(Mx_mean_during(:,5,:));
subplot(212); 
plot(fq_list,mm_Mean_left),hold on
%errorbar(fq_list, nanmean(mm_Mean_left,1), nanstd(mm_Mean_left,1),'k','LineWidth',2),
errorbar(fq_list, nanmean(mm_Mean_left,1), nanstd(mm_Mean_left,1)/sqrt(sum(~isnan(mm_Mean_left(:,5)))),'k','LineWidth',2),
