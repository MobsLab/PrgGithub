...
% Parameters
Dir = '/media/nas5/ProjetERC2/Mouse-828/20190308/BaselineSleep-DecodingBoxes/_Concatenated/';
numclust = 10;
dir_out= '/home/mobsrick/Dropbox/MOBS_workingON/Dima/Ongoing results/Spikes/Thibault_test/';
...
% Do
StimSent = thresholdIntervals(StimTSD,0.99,'Direction','Above');
...
id_found = zeros(1,length(StimSentArraySamples));
for i = 1:length(SpikesDetectedArraySamples)
    [m(i),id] = min(abs((StimSentArraySamples+14)-SpikesDetectedArraySamples(i)));
    if m(i)<4
        id_found(i) = id;
    end
end
...
Nchan = 4;
nt0=32;
fbinary = [Dir 'M828_20190308_SleepDecoding.fil'];
d = dir(fbinary);
ForceMaxRAMforDat   = 10000000000;
memallocated = ForceMaxRAMforDat;
nint16s      = memallocated/2;

ExtractArray = round(StimSentArray(StimsWOSpikesIDX)/1e4*20000);
...
for ibatch = 1:Nbatch
    ...
    if nsampcurr<NTbuff
        ...
        for i = 1:length(indicesTokeep)
            ...
            X = X(1:end-1);
            waveforms_all(:,:,temp) = reshape(DATA(X,indicesTokeep)',size(indicesTokeep,1),nt0,[]);
        end
...
% Saving
saveas(f, [dir_out 'Boxes1_M828_SpikeShapes.fig']);
saveFigure(f,'Boxes1_M828_SpikeShapes',dir_out);

%% Cross correlograms
f2 = figure('units', 'normalized', 'outerposition', [0 0 0.65 0.65]);
for i=2:length(S)
    [C,B]=CrossCorr((Start(StimSent)+6),Range(S{i}),10,50);
    subplot(5,2,i-1)
    bar(B/1E3,C,1,'k')
    ylim([0 100])
...
% Saving
saveas(f2, [dir_out 'Boxes1_M828_CrossCorrect.fig']);
saveFigure(f2,'Boxes1_M828_CrossCorrect',dir_out);

f3 = figure('units', 'normalized', 'outerposition', [0 0 0.65 0.65]);
for i=2:length(S)
    [C,B]=CrossCorr((StimSentArray(StimsWOSpikesIDX)+6),Range(S{i}),10,50);
    subplot(5,2,i-1)
    bar(B/1E3,C,1,'k')
    ylim([0 100])
    line([0 0], ylim,'color','r')
...
% Saving
saveas(f3, [dir_out 'Boxes1_M828_CrossNotFound.fig']);
saveFigure(f3,'Boxes1_M828_CrossNotFound',dir_out);
...
% Saving
saveas(f, [dir_out 'Boxes1_M828_SpikeShapesOriginal.fig']);
saveFigure(f,'Boxes1_M828_SpikeShapes_Original',dir_out);
...
[C,B]=CrossCorr(Range(S{10}),Start(StimSent),1,100);
...
saveas(f, [dir_out 'Boxes1_M828_Karim.fig']);
saveFigure(f,'Boxes1_M828_Karim',dir_out);