...
for i = 1:length(SpikesDetectedCorrArraySamples)
    [m(i),id] = min(abs((StimSentArraySamples+14)-SpikesDetectedCorrArraySamples(i)));
    if m(i)<4
        id_found(i) = id;
    end
end
...
DetectionWOSpikesIDX = setdiff([1:length(StimSentArraySamples)],id_found);
...
ExtractArray = round(StimSentArray(DetectionWOSpikesIDX)/1e4*20000);
...
for elec = 1:size(W,2)
    ...
    if elec == 1
        line(xlim,[-1041 -1041],'Color','r');
        title([num2str(round(length(DetectionWOSpikesIDX)/length(StimSentArray)*100))...
...
[C,B]=CrossCorr(StimSentArray(DetectionWOSpikesIDX),StimSentArray(DetectionWOSpikesIDX),1,50);C(B==0)=0;
bar(B,C,'FaceColor','k','EdgeColor','k')
set(gca,'FontWeight','bold','FontSize',14,'LineWidth',3);
xlabel('time (ms)')
ylim([0 20])
...
for i=2:length(S)
    [C,B]=CrossCorr((StimSentArray(DetectionWOSpikesIDX)+6),Range(S{i}),10,50);
    subplot(5,2,i-1)
    bar(B/1E3,C,1,'k')
    ylim([0 100])
    line([0 0], ylim,'color','r')
