...
% example :
% [ph,mu, Kappa,pval,B,C]=ModulationThetaCorrection(S{numNeurons(1)},fil,Epoch{9},30,1);, JustPoltMod(Data(ph{1}),30);
...
subplot(1,3,2),
[ph,mu, Kappa, pval,B,C]=ModulationTheta(S,fil,epoch,nbin,2);
...
subplot(1,3,3),
hold on
bar(B,2*nmbBin*Cc/sum(Cc),1, 'facecolor','k')
bar(B+2*pi,2*nmbBin*Cc/sum(Cc),1, 'facecolor','k')
