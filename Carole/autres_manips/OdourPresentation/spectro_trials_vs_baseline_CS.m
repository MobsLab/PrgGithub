...
supertit = strcat('mouse ', num2str(mouse_num), ', spectro for each trial');
figure('Color',[1 1 1], 'rend','painters','pos',[10 10 1400 700],'Name',supertit, 'NumberTitle','off')
    si = 0;
    
path_hab=strcat('/media/mobschapeau/2D373EF5372BA96B/Mouse', num2str(mouse_num), '/20190221')
path_od=strcat('/media/mobschapeau/2D373EF5372BA96B/Mouse', num2str(mouse_num), '/20190222')
...
for k = 1:length(Start(OdorEpoch))
%     plot(Spectro{3},mean(Data(Restrict(Spectsd,subset(OdorEpoch,k)))),'color',cols(k,:),'linewidth',2)
plot(f,mean(Data(Restrict(Spectsd,subset(OdorEpoch,k)))),'color',cols(7-k,:),'linewidth',2)
hold on
end
%colormap jet
...
% title(strcat('mouse ', num2str(mouse_num), ', mean power between 8 and 12Hz'))
% xlabel('trials')
% ylabel('mean power')
%%
    ...
    %save
    if sav
        print([dirout strcat('/media/mobschapeau/2D373EF5372BA96B/Mouse', num2str(mouse_num))], '-dpng', '-r300');
    end 