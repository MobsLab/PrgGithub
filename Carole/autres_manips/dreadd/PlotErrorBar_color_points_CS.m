function [pval,eb] = PlotErrorBar_color_points_CS(A,varargin)
...
% - pointscolors (optional) = default : white
...
if showpoints && ~horizontal
    ...
    for i=1:N
        temp=A{i}(~isnan(A{i}));
        if isempty(pointscolors)
            plot(i*ones(length(temp),1)+0.1,temp,'ko','markerfacecolor','w')
        else
            for k=1:length(temp);
                plot(i*ones(length(temp),1)+0.1,temp(k),'ko','markerfacecolor',pointscolors(k));
            end
        end
    end
