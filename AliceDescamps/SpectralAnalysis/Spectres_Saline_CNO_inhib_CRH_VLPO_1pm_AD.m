%% Input Dir inhib DREADD
DirSaline = PathForExperiments_DREADD_MC('inhibDREADD_CRH_VLPO_SalineInjection_1pm');
DirCNO = PathForExperiments_DREADD_MC('inhibDREADD_CRH_VLPO_CNOInjection_1pm');
...
for j=1:length(sp_cno)
    ...
    if isempty(sp_cno{j})==0        
        ...
        SpectreCNO_SWS_AfterInj_3h_max_norm(j,:)= SpectreCNO_SWS_AfterInj_3h_mean(j,:)./max(SpectreCNO_SWS_AfterInj_3h_mean(j,:)); SpectreCNO_SWS_AfterInj__3h_mean(SpectreCNO_SWS_AfterInj_3h_mean==0)=NaN;
