...
function Figure_SleepScoring_Accelero_newgamma(coeff_display, foldername)
...
try
    load([foldername 'SleepScoring_Accelero_newgamma'])
catch
...
%% Final display
SleepScoringFigure_newgamma = figure;
set(SleepScoringFigure_newgamma,'color',[1 1 1],'Position',[1 1 1600 600])
...
try
    ...
    try
        res=pwd;
        cd(foldername);
        saveas(SleepScoringFigure_newgamma, 'SleepScoringAccelero_newgamma.png', 'png');
        cd(res);
    catch
        res=pwd;
        cd(foldername);
        saveas(SleepScoringFigure_newgamma.Number, 'SleepScoringAccelero_newgamma.png', 'png');
        cd(res);
    end
