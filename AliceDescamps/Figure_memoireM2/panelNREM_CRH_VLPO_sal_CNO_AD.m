...
%         idx_same_en_basal{j} = find(VecTimeDay_basal{j}>t_end,1,'last');
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_basal{j}.Wake,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'sws',tempbin,same_st_basal{j},same_en_basal{j});
...
%         idx_same_en_SD{k} = find(VecTimeDay_SD{k}>t_end,1,'last');
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SD{k}.Wake,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'sws',tempbin,same_st_SD{k},same_en_SD{k});
...
subplot(4,6,[13,14]), hold on % NREM bouts number overtime
plot(nanmean(data_num_SWS_basal),'linestyle','-','marker','o','markerfacecolor',col_sal,'color',col_sal)
errorbar(nanmean(data_num_SWS_basal), stdError(data_num_SWS_basal),'color',col_sal)
plot(nanmean(data_num_SWS_SD),'linestyle','-','marker','o','markerfacecolor',col_cno,'color',col_cno)
errorbar(nanmean(data_num_SWS_SD), stdError(data_num_SWS_SD),'color',col_cno)
...
subplot(4,6,[15,16]), hold on % NREM bouts mean duraion overtime
plot(nanmean(data_dur_SWS_basal),'linestyle','-','marker','o','markerfacecolor',col_sal,'color',col_sal)
errorbar(nanmean(data_dur_SWS_basal), stdError(data_dur_SWS_basal),'color',col_sal)
plot(nanmean(data_dur_SWS_SD),'linestyle','-','marker','o','markerfacecolor',col_cno,'color',col_cno)
errorbar(nanmean(data_dur_SWS_SD), stdError(data_dur_SWS_SD),'color',col_cno)
xlim([0 9])
makepretty
title('Sommeil lent')
...
subplot(4,6,[21,22]) % NREM bouts mean duraion quantif barplot
PlotErrorBarN_KJ({...
...
xticks([1.5 4.5]); xticklabels({'1-4','5-9'}); xtickangle(0)
xticks([1.5 4.5]); xticklabels({'Pré','Post'}); xtickangle(0)
ylabel('Durée ép. SL (s)')
xlabel('Période')
makepretty


timebin=1:4;
% [p_basal_SD,h] = signrank(nanmean(data_dur_SWS_basal(:,timebin),2),nanmean(data_dur_SWS_SD(:,timebin),2));
...
subplot(4,6,[17,23]) %new FI = 1 - proba stay nrem (5:9h)
PlotErrorBarN_KJ({...
    (nanmean(data_SWS_REM_basal(:,5:9),2)+nanmean(data_SWS_WAKE_basal(:,5:9),2))*100,...
    (nanmean(data_SWS_REM_SD(:,5:9),2)+nanmean(data_SWS_WAKE_SD(:,5:9),2))*100},...
...
timebin=5:9;
[p_basal_SD,h]=signrank((nanmean(data_SWS_REM_basal(:,timebin),2)+nanmean(data_SWS_WAKE_basal(:,timebin),2)), (nanmean(data_SWS_REM_SD(:,timebin),2)+nanmean(data_SWS_WAKE_SD(:,timebin),2)));
% [h,p_basal_SD]=ttest((nanmean(data_SWS_REM_basal(:,timebin),2)+nanmean(data_SWS_WAKE_basal(:,timebin),2)), (nanmean(data_SWS_REM_SD(:,timebin),2)+nanmean(data_SWS_WAKE_SD(:,timebin),2)));
if p_basal_SD<0.05; sigstar_DB({[1 2]},p_basal_SD,0,'LineWigth',16,'StarSize',24);end
makepretty

subplot(4,6,[18,24]) %new CI = proba stay nrem (5:9h)
PlotErrorBarN_KJ({...
    (nanmean(data_SWS_SWS_basal(:,5:9),2)),...
    (nanmean(data_SWS_SWS_SD(:,5:9),2))},...
    'newfig',0,'paired',0,'showsigstar','none','x_data',[1:2],'barcolors',{col_sal,col_cno});
xticks([1.5]); xticklabels({'5-9'}); xtickangle(0)
xlabel('Temps (h)')
ylim([.98 1])
...
timebin=5:9;
[p_basal_SD,h]=signrank((nanmean(data_SWS_SWS_basal(:,timebin),2)), (nanmean(data_SWS_SWS_SD(:,timebin),2)));
% [h,p_basal_SD]=ttest((nanmean(data_SWS_SWS_basal(:,timebin),2), (nanmean(data_SWS_SWS_SD(:,timebin),2)));
if p_basal_SD<0.05; sigstar_DB({[1 2]},p_basal_SD,0,'LineWigth',16,'StarSize',24);end
makepretty
