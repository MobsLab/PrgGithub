...
%%Basal sleep W/ injection at 9am
t_inj = 10;
t_start = 10;
t_end = 18;
...
%         idx_same_en_basal{j} = find(VecTimeDay_basal{j}>t_end,1,'last');
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_basal{j}.Wake,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'rem',tempbin,same_st_basal{j},same_en_basal{j});
...
%         idx_same_en_SD{k} = find(VecTimeDay_SD{k}>t_end,1,'last');
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SD{k}.Wake,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'rem',tempbin,same_st_SD{k},same_en_SD{k});
...
subplot(4,6,[13,14]), hold on % REM bouts number overtime
...
subplot(4,6,[17,23]) %new FI = 1 - proba stay rem (5:9h)
PlotErrorBarN_KJ({...
    (nanmean(data_REM_SWS_basal(:,5:9),2)+nanmean(data_REM_WAKE_basal(:,5:9),2))*100,...
    (nanmean(data_REM_SWS_SD(:,5:9),2)+nanmean(data_REM_WAKE_SD(:,5:9),2))*100},...
    'newfig',0,'paired',0,'showsigstar','none','x_data',[1:2],'barcolors',{col_sal,col_cno});
xticks([1.5]); xticklabels({'Post'}); xtickangle(0)
xlabel('Période')
ylabel('Indice de fragmentation ')

timebin=5:9;
% [p_basal_SD,h]=signrank((nanmean(data_REM_SWS_basal(:,timebin),2)+nanmean(data_REM_WAKE_basal(:,timebin),2)), (nanmean(data_REM_SWS_SD(:,timebin),2)+nanmean(data_REM_WAKE_SD(:,timebin),2)));
[h,p_basal_SD]=ttest((nanmean(data_REM_SWS_basal(:,timebin),2)+nanmean(data_REM_WAKE_basal(:,timebin),2)), (nanmean(data_REM_SWS_SD(:,timebin),2)+nanmean(data_REM_WAKE_SD(:,timebin),2)));
if p_basal_SD<0.05; sigstar_DB({[1 2]},p_basal_SD,0,'LineWigth',16,'StarSize',24);end
makepretty

subplot(4,6,[18,24]) %new CI = proba stay rem (5:9h)
PlotErrorBarN_KJ({...
    (nanmean(data_REM_REM_basal(:,5:9),2)),...
    (nanmean(data_REM_REM_SD(:,5:9),2))},...
    'newfig',0,'paired',0,'showsigstar','none','x_data',[1:2],'barcolors',{col_sal,col_cno});
...
timebin=5:9;
% [p_basal_SD,h]=signrank((nanmean(data_REM_REM_basal(:,timebin),2)), (nanmean(data_REM_REM_SD(:,timebin),2)));
[h,p_basal_SD]=ttest((nanmean(data_REM_REM_basal(:,timebin),2)), (nanmean(data_REM_REM_SD(:,timebin),2)));
if p_basal_SD<0.05; sigstar_DB({[1 2]},p_basal_SD,0,'LineWigth',16,'StarSize',24);end
makepretty
