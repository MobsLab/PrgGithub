...
for j=1:length(Dir1.path)
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_basal{j}.WakeWiNoise,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'sws',tempbin,time_st,time_end);
...
for k=1:length(Dir2.path)
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SD{k}.WakeWiNoise,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'sws',tempbin,time_st,time_end);
...
for k=1:length(Dir3.path)
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SDsafe{k}.WakeWiNoise,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SD{k}.REMEpoch,same_epoch_SDsafe{k}),'sws',tempbin,time_st,time_end);
...
PlotErrorBarN_KJ({...
    (1-nanmean(data_SWS_SWS_basal(:,4:8),2))*100,...
    (1-nanmean(data_SWS_SWS_SDsafe(:,4:8),2))*100,...
    (1-nanmean(data_SWS_SWS_SD(:,4:8),2))*100},...
...
timebin=4:8;
[p_basal_SD,h]=ranksum(1-nanmean(data_SWS_SWS_basal(:,timebin),2),1-nanmean(data_SWS_SWS_SD(:,timebin),2));
[p_SD_ctrlInhib,h]=ranksum(1-nanmean(data_SWS_SWS_SD(:,timebin),2), 1-nanmean(data_SWS_SWS_SDsafe(:,timebin),2));
[p_basal_ctrlInhib,h]=ranksum(1-nanmean(data_SWS_SWS_basal(:,timebin),2),1-nanmean(data_SWS_SWS_SDsafe(:,timebin),2));
