...
for j=1:length(Dir1.path)
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_basal{j}.WakeWiNoise,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'rem',tempbin,time_st,time_end);
...
for k=1:length(Dir2.path)
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SD{k}.WakeWiNoise,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'rem',tempbin,time_st,time_end);
...
for k=1:length(Dir3.path)
    ...
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SDsafe{k}.WakeWiNoise,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SD{k}.REMEpoch,same_epoch_SDsafe{k}),'rem',tempbin,time_st,time_end);
...
if p_basal_ctrlInhib<0.05; sigstar_DB({[7 8]},p_basal_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
...
PlotErrorBarN_KJ({...
    (1-nanmean(data_REM_REM_basal(:,4:8),2)),...
    (1-nanmean(data_REM_REM_SDsafe(:,4:8),2)),...
    (1-nanmean(data_REM_REM_SD(:,4:8),2))},...
