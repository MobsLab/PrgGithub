...
for j=1:length(Dir1.path)
    ...
    [dur_moyenne_ep_WAKE, num_moyen_ep_WAKE, perc_moyen_WAKE, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_basal{j}.Wake,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'wake',tempbin,time_end);
    dur_WAKE_basal{j}=dur_moyenne_ep_WAKE;
    num_WAKE_basal{j}=num_moyen_ep_WAKE;
    perc_WAKE_basal{j}=perc_moyen_WAKE;
    
    [dur_moyenne_ep_SWS, num_moyen_ep_SWS,perc_moyen_SWS, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_basal{j}.Wake,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'sws',tempbin,time_end);
    dur_SWS_basal{j}=dur_moyenne_ep_SWS;
    num_SWS_basal{j}=num_moyen_ep_SWS;
    perc_SWS_basal{j}=perc_moyen_SWS;
    
    [dur_moyenne_ep_REM, num_moyen_ep_REM,perc_moyen_REM, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_basal{j}.Wake,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'rem',tempbin,time_end);
    dur_REM_basal{j}=dur_moyenne_ep_REM;
    num_REM_basal{j}=num_moyen_ep_REM;
    perc_REM_basal{j}=perc_moyen_REM;
    
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_basal{j}.Wake,same_epoch_basal{j}),and(stages_basal{j}.SWSEpoch,same_epoch_basal{j}),and(stages_basal{j}.REMEpoch,same_epoch_basal{j}),'rem',tempbin,time_st,time_end);
...
for k=1:length(Dir2.path)
    ...
    [dur_moyenne_ep_WAKE, num_moyen_ep_WAKE,perc_moyen_WAKE, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_SD{k}.Wake,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'wake',tempbin,time_end);
    dur_WAKE_SD{k}=dur_moyenne_ep_WAKE;
    num_WAKE_SD{k}=num_moyen_ep_WAKE;
    perc_WAKE_SD{k}=perc_moyen_WAKE;
    
    [dur_moyenne_ep_SWS, num_moyen_ep_SWS,perc_moyen_SWS, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_SD{k}.Wake,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'sws',tempbin,time_end);
    dur_SWS_SD{k}=dur_moyenne_ep_SWS;
    num_SWS_SD{k}=num_moyen_ep_SWS;
    perc_SWS_SD{k}=perc_moyen_SWS;
    
    [dur_moyenne_ep_REM, num_moyen_ep_REM,perc_moyen_REM, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_SD{k}.Wake,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'rem',tempbin,time_end);
    dur_REM_SD{k}=dur_moyenne_ep_REM;
    num_REM_SD{k}=num_moyen_ep_REM;
    perc_REM_SD{k}=perc_moyen_REM;
    
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SD{k}.Wake,same_epoch_SD{k}),and(stages_SD{k}.SWSEpoch,same_epoch_SD{k}),and(stages_SD{k}.REMEpoch,same_epoch_SD{k}),'rem',tempbin,time_st,time_end);
...
for k=1:length(Dir3.path)
    ...
    %%load sleep scoring
    if exist('SleepScoring_Accelero.mat')
        stages_SDsafe{k} = load('SleepScoring_Accelero', 'REMEpoch', 'SWSEpoch', 'Wake','tsdMovement');
    elseif exist('SleepScoring_OBGamma.mat')
        stages_SDsafe{k} = load('SleepScoring_OBGamma', 'REMEpoch', 'SWSEpoch', 'Wake','tsdMovement');
    else
    end
    ...
    %%compute percentage mean duration and number of bouts
    [dur_moyenne_ep_WAKE, num_moyen_ep_WAKE,perc_moyen_WAKE, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_SDsafe{k}.Wake,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.REMEpoch,same_epoch_SDsafe{k}),'wake',tempbin,time_end);
    dur_WAKE_SDsafe{k}=dur_moyenne_ep_WAKE;
    num_WAKE_SDsafe{k}=num_moyen_ep_WAKE;
    perc_WAKE_SDsafe{k}=perc_moyen_WAKE;
    
    [dur_moyenne_ep_SWS, num_moyen_ep_SWS,perc_moyen_SWS, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_SDsafe{k}.Wake,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.REMEpoch,same_epoch_SDsafe{k}),'sws',tempbin,time_end);
    dur_SWS_SDsafe{k}=dur_moyenne_ep_SWS;
    num_SWS_SDsafe{k}=num_moyen_ep_SWS;
    perc_SWS_SDsafe{k}=perc_moyen_SWS;
    
    [dur_moyenne_ep_REM, num_moyen_ep_REM,perc_moyen_REM, rg]=Get_Mean_Dur_Num_Overtime_MC(and(stages_SDsafe{k}.Wake,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.REMEpoch,same_epoch_SDsafe{k}),'rem',tempbin,time_end);
    dur_REM_SDsafe{k}=dur_moyenne_ep_REM;
    num_REM_SDsafe{k}=num_moyen_ep_REM;
    perc_REM_SDsafe{k}=perc_moyen_REM;
    
    [dur_TOT_moyenne_ep, rg]=Get_Mean_Dur_TOTALE_Overtime_MC(and(stages_SDsafe{k}.Wake,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SD{k}.REMEpoch,same_epoch_SDsafe{k}),'rem',tempbin,time_st,time_end);
    dur_TOT_REM_SDsafe{k}=dur_TOT_moyenne_ep;

    
    %%compute transition probabilities
    [trans_REM_to_REM, trans_REM_to_SWS, trans_REM_to_WAKE, trans_SWS_to_REM, trans_SWS_to_SWS, trans_SWS_to_WAKE, trans_WAKE_to_REM, trans_WAKE_to_SWS, trans_WAKE_to_WAKE]=...
        Get_proba_timebins_Overtime_MC_VF(and(stages_SDsafe{k}.Wake,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.SWSEpoch,same_epoch_SDsafe{k}),and(stages_SDsafe{k}.REMEpoch,same_epoch_SDsafe{k}),tempbin,time_end);
...
timebin=1:3;
[p_base_SD,h] = ranksum(nanmean(data_perc_WAKE_basal(:,timebin),2),nanmean(data_perc_WAKE_SD(:,timebin),2));
[p_SD_ctrlInhib,h] = ranksum(nanmean(data_perc_WAKE_SD(:,timebin),2), nanmean(data_perc_WAKE_SDsafe(:,timebin),2));
[p_base_ctrlInhib,h] = ranksum(nanmean(data_perc_WAKE_basal(:,timebin),2), nanmean(data_perc_WAKE_SDsafe(:,timebin),2));
if p_base_SD<0.05; sigstar_DB({[1 3]},p_base_SD,0,'LineWigth',16,'StarSize',24);end
if p_SD_ctrlInhib<0.05; sigstar_DB({[2 3]},p_SD_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
if p_base_ctrlInhib<0.05; sigstar_DB({[1 2]},p_base_ctrlInhib,0,'LineWigth',16,'StarSize',24);end

timebin=4:8;
[p_base_SD,h] = ranksum(nanmean(data_perc_WAKE_basal(:,timebin),2),nanmean(data_perc_WAKE_SD(:,timebin),2));
[p_SD_ctrlInhib,h] = ranksum(nanmean(data_perc_WAKE_SD(:,timebin),2), nanmean(data_perc_WAKE_SDsafe(:,timebin),2));
[p_base_ctrlInhib,h] = ranksum(nanmean(data_perc_WAKE_basal(:,timebin),2), nanmean(data_perc_WAKE_SDsafe(:,timebin),2));
if p_base_SD<0.05; sigstar_DB({[5 7]},p_base_SD,0,'LineWigth',16,'StarSize',24);end
if p_SD_ctrlInhib<0.05; sigstar_DB({[6 7]},p_SD_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
if p_base_ctrlInhib<0.05; sigstar_DB({[5 6]},p_base_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
...
timebin=1:3;
[p_base_SD,h] = ranksum(nanmean(data_perc_SWS_basal(:,timebin),2),nanmean(data_perc_SWS_SD(:,timebin),2));
[p_SD_ctrlInhib,h] = ranksum(nanmean(data_perc_SWS_SD(:,timebin),2), nanmean(data_perc_SWS_SDsafe(:,timebin),2));
[p_base_ctrlInhib,h] = ranksum(nanmean(data_perc_SWS_basal(:,timebin),2), nanmean(data_perc_SWS_SDsafe(:,timebin),2));
if p_base_SD<0.05; sigstar_DB({[1 3]},p_base_SD,0,'LineWigth',16,'StarSize',24);end
if p_SD_ctrlInhib<0.05; sigstar_DB({[2 3]},p_SD_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
if p_base_ctrlInhib<0.05; sigstar_DB({[1 2]},p_base_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
...
timebin=1:3;
[p_base_SD,h] = ranksum(nanmean(data_perc_REM_basal(:,timebin),2),nanmean(data_perc_REM_SD(:,timebin),2));
[p_SD_ctrlInhib,h] = ranksum(nanmean(data_perc_REM_SD(:,timebin),2), nanmean(data_perc_REM_SDsafe(:,timebin),2));
[p_base_ctrlInhib,h] = ranksum(nanmean(data_perc_REM_basal(:,timebin),2), nanmean(data_perc_REM_SDsafe(:,timebin),2));
if p_base_SD<0.05; sigstar_DB({[1 3]},p_base_SD,0,'LineWigth',16,'StarSize',24);end
if p_SD_ctrlInhib<0.05; sigstar_DB({[2 3]},p_SD_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
if p_base_ctrlInhib<0.05; sigstar_DB({[1 2]},p_base_ctrlInhib,0,'LineWigth',16,'StarSize',24);end
...
plot(nanmean(data_num_REM_basal),'linestyle','-','marker','o','markerfacecolor',col_basal,'color',col_basal), hold on
errorbar(nanmean(data_num_REM_basal), stdError(data_num_REM_basal),'color',col_basal)
...
plot(nanmean(data_num_REM_SDsafe),'linestyle','-','marker','o','markerfacecolor',col_SDsafe,'color',col_SDsafe)
errorbar(nanmean(data_num_REM_SDsafe), stdError(data_num_REM_SDsafe),'color',col_SDsafe)
xlim([0 8])
makepretty
title('Sommeil paradoxal')

subplot(4,6,[15,16]) % REM bouts mean duraion overtime
plot(nanmean(data_dur_REM_basal),'linestyle','-','marker','o','markerfacecolor',col_basal,'color',col_basal), hold on
errorbar(nanmean(data_dur_REM_basal), stdError(data_dur_REM_basal),'color',col_basal)
...
plot(nanmean(data_dur_REM_SDsafe),'linestyle','-','marker','o','markerfacecolor',col_SDsafe,'color',col_SDsafe)
errorbar(nanmean(data_dur_REM_SDsafe), stdError(data_dur_REM_SDsafe),'color',col_SDsafe)
xlim([0 8])
makepretty
title('Sommeil paradoxal')
...
subplot(4,6,[17,23]) % new FI = 1 - proba stay rem (4-8h)
PlotErrorBarN_KJ({...
    (nanmean(data_REM_SWS_basal(:,4:8),2)+nanmean(data_REM_WAKE_basal(:,4:8),2))*100,...
    (nanmean(data_REM_SWS_SDsafe(:,4:8),2)+nanmean(data_REM_WAKE_SDsafe(:,4:8),2))*100,...
    (nanmean(data_REM_SWS_SD(:,4:8),2)+nanmean(data_REM_WAKE_SD(:,4:8),2))*100},...
...
timebin=4:8;
[p_basal_SD,h]=ranksum((nanmean(data_REM_SWS_basal(:,timebin),2)+nanmean(data_REM_WAKE_basal(:,timebin),2)),(nanmean(data_REM_SWS_SD(:,timebin),2)+nanmean(data_REM_WAKE_SD(:,timebin),2)));
[p_SD_ctrlInhib,h]=ranksum((nanmean(data_REM_SWS_SD(:,timebin),2)+nanmean(data_REM_WAKE_SD(:,timebin),2)), (nanmean(data_REM_SWS_SDsafe(:,timebin),2)+nanmean(data_REM_WAKE_SDsafe(:,timebin),2)));
[p_basal_ctrlInhib,h]=ranksum((nanmean(data_REM_SWS_basal(:,timebin),2)+nanmean(data_REM_WAKE_basal(:,timebin),2)), (nanmean(data_REM_SWS_SDsafe(:,timebin),2)+nanmean(data_REM_WAKE_SDsafe(:,timebin),2)));
