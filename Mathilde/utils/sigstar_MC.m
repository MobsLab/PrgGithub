function varargout=sigstar_MC(groups,stats,nosort,varargin)
...
%          bar.
%
%%%% Optional arguments (Dmitri Bryzgalov, 2019)
%
% plotbar controls whether you want to plot a bar, or only stars (default = 1)
%
% LineWidth is a linewidth of a bar (default = 1.5)
%
% StarSize is the size of the stars (default = 10)
%
% Outputs
...
for i = 1:2:length(varargin)
    if ~ischar(varargin{i})
        error(['Parameter ' num2str(i+2) ' is not a property (type ''help <a href="matlab:help sigstar_DB">sigstar_DB</a>'' for details).']);
    end
    switch(lower(varargin{i}))
        case 'linewidth'
            linw = varargin{i+1};
            if isdscalar(linw,'<0')
                error('Incorrect value for property ''LineWidth'' (type ''help <a href="matlab:help sigstar_DB">sigstar_DB</a>'' for details).');
            end
        case 'starsize'
            starsize = varargin{i+1};
            if isdscalar(starsize,'<0')
                error('Incorrect value for property ''StarSize'' (type ''help <a href="matlab:help sigstar_DB">sigstar_DB</a>'' for details).');
            end
        case 'plotbar'
            plotbar = varargin{i+1};
            if isdscalar(plotbar,'==0','==1')
                error('Incorrect value for property ''PlotBar'' (type ''help <a href="matlab:help sigstar_DB">sigstar_DB</a>'' for details).');
            end
    end
end
...
if ~exist('starsize', 'var')
    starsize=10;
end
if ~exist('plotbar', 'var')
    plotbar=1;
end
...
for ii=1:length(groups)
    ...
    H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),plotbar,linw,starsize);
end
%-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,plotbar,linw,starsize)
...
if plotbar
    H(1)=plot(x(:),y,'-k','LineWidth',linw,'Tag','sigstar_bar');
end
...
starY=mean(y)+myRange(ylim)*0.03;
H(2)=text(mean(x(:)),starY,stars,...
    'HorizontalAlignment','Center',...
    'BackGroundColor','none',...
    'Tag','sigstar_stars','FontSize',starsize,'FontWeight','bold');
