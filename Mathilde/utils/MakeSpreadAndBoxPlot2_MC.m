function [pval , stats_out]=MakeSpreadAndBoxPlot2_MC(A,Cols,X,Legends,varargin)
% Input variables
...
%         hold on
        ...
        a=iosr.statistics.boxPlot(X(k),A{k}(:),'boxColor',Cols{k},'lineColor',[.5 .5 .5],'medianColor',Cols{k},'boxWidth',0.6,'showOutliers',false);
        a.handles.upperWhiskers.Visible='off';a.handles.upperWhiskerTips.Visible='off';
        a.handles.lowerWhiskers.Visible='off';a.handles.lowerWhiskerTips.Visible='off';
        ...
        if showpoints
            ...
            handlesplot=plotSpread(A{k}(:),'distributionColors','w','xValues',X(k),'spreadWidth',0.8); hold on; %%Cols{k}*0.5
...
%         
%         handlesplot=plot(k+0.1,temp,'ko','markerfacecolor',Cols{k}*0.5,'MarkerSize',5)
%        
%     end
...
%                 line(0.1+[idx(i) idx(i+1)]', [M(k,idx(i)) M(k,idx(i+1))]', 'color',[0.6 0.6 0.6]);
                line([-0.01+idx(i) 0.05+idx(i+1)]', [M(k,idx(i)) M(k,idx(i+1))]', 'color',[0.6 0.6 0.6]);
