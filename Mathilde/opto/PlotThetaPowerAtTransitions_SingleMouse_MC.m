function [SpREM,SpSWS,SpWake, temps] = PlotThetaPowerAtTransitions_SingleMouse_MC(WakeWiNoise,SWSEpochWiNoise,REMEpochWiNoise,event, plo)
...
% compute average spectro (in all states), triggered on the END of the
% event
if strcmp(lower(event),'end')
[MH_REM,SH_REM,tps]=AverageSpectrogram(sptsdH,freqH,ts(End(REMEpochWiNoise)),500,500,0);
[MH_SWS,SH_SWS,tps]=AverageSpectrogram(sptsdH,freqH,ts(End(SWSEpochWiNoise)),500,500,0);
[MH_wake,SH_wake,tps]=AverageSpectrogram(sptsdH,freqH,ts(End(WakeWiNoise)),500,500,0);

% compute average spectro (in all states), triggered on the START of the
% event
elseif strcmp(lower(event),'start')
[MH_REM,SH_REM,tps]=AverageSpectrogram(sptsdH,freqH,ts(Start(REMEpochWiNoise)),500,500,0);
 [MH_SWS,SH_SWS,tps]=AverageSpectrogram(sptsdH,freqH,ts(Start(SWSEpochWiNoise)),500,500,0);
[MH_wake,SH_wake,tps]=AverageSpectrogram(sptsdH,freqH,ts(Start(WakeWiNoise)),500,500,0);
end
