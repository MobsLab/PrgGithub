...
%
%%INPUTS
% scoring (optional):   method used to distinguish sleep from WakeWiNoise
...
% scoring:
%
% see FindNREMEpochWiNoisesML
%

function [features, Namesfeatures, EpochSleep, NoiseEpoch, scoring] = FindNREMfeatures_ForOptoMice_MC(varargin)
...
%params
Namesfeatures = {'PFsupOsci','PFdeepOsci','BurstDelta','REM','WakeWiNoise','SWS','SIEpochSleep','PFswa','OBswa'};
mergelim = 5;
mergesleep = 3;
...
if strcmpi(scoring,'accelero')
    load SleepScoring_Accelero SWSEpochWiNoise REMEpochWiNoise WakeWiNoise ImmobilityEpoch TotalNoiseEpoch
    NoiseEpoch = TotalNoiseEpoch;
    EpochSleep = ImmobilityEpoch - TotalNoiseEpoch;
elseif strcmpi(scoring,'ob')
    try
        load SleepScoring_OBGamma SWSEpochWiNoise REMEpochWiNoise WakeWiNoise SleepWiNoise TotalNoiseEpoch
    catch
        load StateEpochSB SWSEpochWiNoise SWSEpochWiNoise REMEpochWiNoise WakeWiNoise SleepWiNoise TotalNoiseEpoch
    end
    NoiseEpoch = TotalNoiseEpoch;
%   EpochSleep = SleepWiNoise - TotalNoiseEpoch;
    EpochSleep = SleepWiNoise;
end
...
if exist('EpochSleep','var')
    ...
    features{5} = REMEpochWiNoise;
    features{6} = WakeWiNoise;
    features{7} = SWSEpochWiNoise;
