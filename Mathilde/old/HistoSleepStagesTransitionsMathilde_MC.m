function [h,rg,vec]=HistoSleepStagesTransitionsMathilde_MC(SleepStages,stim,vec,plo)
...
q=length(find(Data(SleepStages)==1.5));
...
if q==0
    h(a,1)=length(find(Data(Restrict(SleepStages,epoch))==4))/length(Data(Restrict(SleepStages,epoch)))*100;
    h(a,2)=length(find(Data(Restrict(SleepStages,epoch))==3))/length(Data(Restrict(SleepStages,epoch)))*100;
    h(a,3)=length(find(Data(Restrict(SleepStages,epoch))==1))/length(Data(Restrict(SleepStages,epoch)))*100;
    a=a+1;
else
    h(a,1)=length(find(Data(Restrict(SleepStages,epoch))==4))/length(Data(Restrict(SleepStages,epoch)))*100;
    h(a,2)=length(find(Data(Restrict(SleepStages,epoch))==3))/length(Data(Restrict(SleepStages,epoch)))*100;
    h(a,3)=length(find(Data(Restrict(SleepStages,epoch))==2))/length(Data(Restrict(SleepStages,epoch)))*100;
    h(a,4)=length(find(Data(Restrict(SleepStages,epoch))==1.5))/length(Data(Restrict(SleepStages,epoch)))*100;
    h(a,5)=length(find(Data(Restrict(SleepStages,epoch))==1))/length(Data(Restrict(SleepStages,epoch)))*100;
    a=a+1;
end
...
if q>0
    for n=1:5
        th(n,1)=mean(h(find(vec<0),n));th(n,2)=std(h(find(vec<0),n));
%         line(xlim,[th(n,1) th(n,1)],'color',co(n,:),'linewidth',2)
%         line(xlim,[th(n,1)+th(n,2) th(n,1)+th(n,2)],'color',co(n,:),'linewidth',1,'linestyle',':')
%         line(xlim,[th(n,1)-th(n,2) th(n,1)-th(n,2)],'color',co(n,:),'linewidth',1,'linestyle',':')
    end
    legend({'Wake','REM','N1','N2','N3'})
else
    for n=1:3
        th(n,1)=mean(h(find(vec<0),n));th(1,2)=std(h(find(vec<0),n));
%         line(xlim,[th(n,1) th(n,1)],'color',co(n,:),'linewidth',2)
%         line(xlim,[th(n,1)+th(n,2) th(n,1)+th(n,2)],'color',co(n,:),'linewidth',1,'linestyle',':')
%         line(xlim,[th(n,1)-th(n,2) th(n,1)-th(n,2)],'color',co(n,:),'linewidth',1,'linestyle',':')
    end
