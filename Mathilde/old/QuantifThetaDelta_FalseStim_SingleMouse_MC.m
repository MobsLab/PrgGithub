function [MSws,MRem,MWake,ThetaSWS,ThetaREM,ThetaWake,DeltaSWS, DeltaREM, DeltaWake,freqH,tps] = QuantifThetaDelta_FalseStim_SingleMouse_MC(Wake,REMEpoch,SWSEpoch,plo)
% outputs : HPC spectrogram values matrices during REM,NREM,Wake
%           theta power REM,NREM,Wake & theta/delta ratio
...
StimSWS=Range(SwsStim);
StimREM=Range(RemStim);
StimWake=Range(WakeStim);

load H_Low_Spectrum
SpectroH= Spectro;
freqH=SpectroH{3};
sptsd= tsd(SpectroH{2}*1e4, SpectroH{1});

[MSws,SSws,TPS]=AverageSpectrogram(sptsd,freqH,(Restrict(SwsStim,SWSEpoch)),500,500,0);
[MRem,SRem,TPS]=AverageSpectrogram(sptsd,freqH,(Restrict(RemStim,REMEpoch)),500,500,0);
[MWake,SWake,TPS]=AverageSpectrogram(sptsd,freqH,(Restrict(WakeStim,Wake)),500,500,0);
...
ThetaSWS=squeeze(nanmean(MatSWS(find(freqH<10&freqH>5),:,:),1));   % frequency band average to get the theta
ThetaREM=squeeze(nanmean(MatREM(find(freqH<10&freqH>5),:,:),1));
ThetaWake=squeeze(nanmean(MatWake(find(freqH<10&freqH>5),:,:),1));

DeltaSWS=squeeze(nanmean(MatSWS(find(freqH<4&freqH>1),:,:)));      % frequency band average  to get the delta
DeltaREM=squeeze(nanmean(MatREM(find(freqH<4&freqH>1),:,:)));
DeltaWake=squeeze(nanmean(MatWake(find(freqH<4&freqH>1),:,:)));      % frequency band average  to get the delta
...
if plo
    ...
    figure, subplot(6,5,[2,4]), imagesc(TPS/1E3,freqH,MRem), axis xy
    colormap(jet)
    caxis([0 5e04])
    ...
    subplot(6,5,[7,9]), imagesc(TPS/1E3,freqH,MSws), axis xy
    colormap(jet)
    caxis([0 5e04])
    line([0 0], ylim,'color','w','linestyle',':')
    xlim([-50 +50])
    ...
    subplot(6,5,[12,14]), imagesc(TPS/1E3,freqH,MWake), axis xy
    colormap(jet)
    caxis([0 5e04])
    line([0 0], ylim,'color','w','linestyle',':')
    xlim([-50 +50])
