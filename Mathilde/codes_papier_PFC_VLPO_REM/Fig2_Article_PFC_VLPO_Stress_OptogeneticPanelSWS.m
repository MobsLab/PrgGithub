...
imagesc(time/1e3, freq{2}, SpectroP_SWS_Ctrl{5}), axis xy, xlim([-30 30]), caxis([1 5])
...
imagesc(time/1e3, freq{2}, SpectroP_SWS_opto{5}), axis xy, xlim([-30 30]), caxis([1 5])
hold on, line([0 0], ylim, 'color','w')
colormap(jet), colorbar
ylabel('Frequency (Hz)')
set(gca,'fontsize',17,'fontname','Lato Semibold')
...
PlotErrorBarN_MC({nanmean(av_Sp_PFC_deltaBandSWS_Before_ctrl,2), nanmean(av_Sp_PFC_deltaBandSWS_During_ctrl,2),...
    ...
    'newfig',0,'paired',0,'showsigstar','none','x_data',[1:2,4:5],'barcolors',{col_ctrl_off,col_ctrl_on,col_opto_off,col_opto_on});
