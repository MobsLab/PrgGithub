...
% colors
clrs = {[0 0.2 0.6], [1 0 0], [0.2 0.6 0], [0.4 0 0.6], [1 0.4 0]};
%% Get the data
...
%% Calculate occupamcy maps
% PreTest sham
cnt=1;
for p = 1:length(DirPreSham.path)
    for q = 1:1:ntest
        Pre_Xtsd_Sham_full{cnt} = Pre_Xtsd_Sham{p}{q};
        Pre_Ytsd_Sham_full{cnt} = Pre_Ytsd_Sham{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_pre_sham = BehavOccupationMap(Pre_Xtsd_Sham_full, Pre_Ytsd_Sham_full);
% CondPAG sham
cnt=1;
for p = 1:length(DirCondPAGSham.path)
    for q = 1:1:3
        CondPAG_Xtsd_Sham_full{cnt} = CondPAG_Xtsd_Sham{p}{q};
        CondPAG_Ytsd_Sham_full{cnt} = CondPAG_Ytsd_Sham{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_condPAG_sham = BehavOccupationMap(CondPAG_Xtsd_Sham_full, CondPAG_Ytsd_Sham_full);
% PostPAGTest sham
cnt=1;
for p = 1:length(DirPostPAGSham.path)
    for q = 1:1:ntest
        PostPAG_Xtsd_Sham_full{cnt} = PostPAG_Xtsd_Sham{p}{q};
        PostPAG_Ytsd_Sham_full{cnt} = PostPAG_Ytsd_Sham{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_postPAG_sham = BehavOccupationMap(PostPAG_Xtsd_Sham_full, PostPAG_Ytsd_Sham_full);
% CondMFB sham
cnt=1;
for p = 1:length(DirCondMFBSham.path)
    for q = 1:1:3
        CondMFB_Xtsd_Sham_full{cnt} = CondMFB_Xtsd_Sham{p}{q};
        CondMFB_Ytsd_Sham_full{cnt} = CondMFB_Ytsd_Sham{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_condMFB_sham = BehavOccupationMap(CondMFB_Xtsd_Sham_full, CondMFB_Ytsd_Sham_full);
% PostMFB sham
cnt=1;
for p = 1:length(DirPostMFBSham.path)
    for q = 1:1:ntest
        PostMFB_Xtsd_Sham_full{cnt} = PostMFB_Xtsd_Sham{p}{q};
        PostMFB_Ytsd_Sham_full{cnt} = PostMFB_Ytsd_Sham{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_postMFB_sham = BehavOccupationMap(PostMFB_Xtsd_Sham_full, PostMFB_Ytsd_Sham_full);
...
for p = 1:length(DirPreExp.path)
    for q = 1:1:ntest
        Pre_Xtsd_Exp_full{cnt} = Pre_Xtsd_Exp{p}{q};
        Pre_Ytsd_Exp_full{cnt} = Pre_Ytsd_Exp{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_pre_exp = BehavOccupationMap(Pre_Xtsd_Exp_full, Pre_Ytsd_Exp_full);
% CondPAG exp
cnt=1;
for p = 1:length(DirCondPAGExp.path)
    for q = 1:1:3
        CondPAG_Xtsd_Exp_full{cnt} = CondPAG_Xtsd_Exp{p}{q};
        CondPAG_Ytsd_Exp_full{cnt} = CondPAG_Ytsd_Exp{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_condPAG_exp = BehavOccupationMap(CondPAG_Xtsd_Exp_full, CondPAG_Ytsd_Exp_full);
% PostPAGTest exp
cnt=1;
for p = 1:length(DirPostPAGExp.path)
    for q = 1:1:ntest
        PostPAG_Xtsd_Exp_full{cnt} = PostPAG_Xtsd_Exp{p}{q};
        PostPAG_Ytsd_Exp_full{cnt} = PostPAG_Ytsd_Exp{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_postPAG_exp = BehavOccupationMap(PostPAG_Xtsd_Exp_full, PostPAG_Ytsd_Exp_full);
% CondMFB exp
cnt=1;
for p = 1:length(DirCondMFBExp.path)
    for q = 1:1:3
        CondMFB_Xtsd_Exp_full{cnt} = CondMFB_Xtsd_Exp{p}{q};
        CondMFB_Ytsd_Exp_full{cnt} = CondMFB_Ytsd_Exp{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_condMFB_exp = BehavOccupationMap(CondMFB_Xtsd_Exp_full, CondMFB_Ytsd_Exp_full);
% PostMFB exp
cnt=1;
for p = 1:length(DirPostMFBExp.path)
    for q = 1:1:ntest
        PostMFB_Xtsd_Exp_full{cnt} = PostMFB_Xtsd_Exp{p}{q};
        PostMFB_Ytsd_Exp_full{cnt} = PostMFB_Ytsd_Exp{p}{q};
        cnt=cnt+1;
    end
end
OccupMap_postMFB_exp = BehavOccupationMap(PostMFB_Xtsd_Exp_full, PostMFB_Ytsd_Exp_full);

%% plot figures
%% Occupancies for all mice
...
makepretty_DB
subplot(259)
imagesc(OccupMap_condMFB_sham)
axis xy
hold on
plot(mazeMap(:,1),mazeMap(:,2),'w','LineWidth',3)
% plot(ShockZoneMap(:,1),ShockZoneMap(:,2),'k','LineWidth',3)
set(gca,'XTick', [], 'XTickLabel',{},'YTick', [], 'YTickLabel',{});
colormap hot
caxis([0 .08])
makepretty_DB
subplot(2,5,10)
imagesc(OccupMap_postMFB_sham)
axis xy
hold on
plot(mazeMap(:,1),mazeMap(:,2),'w','LineWidth',3)
% plot(ShockZoneMap(:,1),ShockZoneMap(:,2),'k','LineWidth',3)
...
figure
subplot(231), PlotErrorBarN_KJ({Pre_Occup_Exp_mean(:,:,1), Pre_Occup_Exp_mean(:,:,4), Pre_Occup_Exp_mean(:,:,6), Pre_Occup_Exp_mean(:,:,3), Pre_Occup_Exp_mean(:,:,7), Pre_Occup_Exp_mean(:,:,5), Pre_Occup_Exp_mean(:,:,2)},'newfig',0,'paired',1, 'ShowPoints',0);
set(gca,'Xtick',[1:7],'XtickLabel',{'Stim', 'FarStim', 'CenterStim', 'Center', 'CenterNoStim', 'FarNoStim', 'NoStim'});
xtickangle(45)
ylim([0 100])
ylabel('% time spent')
title('TestPre expe')
makepretty
% Test post PAG
subplot(232), PlotErrorBarN_KJ({PostPAG_Occup_Exp_mean(:,:,1), PostPAG_Occup_Exp_mean(:,:,4), PostPAG_Occup_Exp_mean(:,:,6), PostPAG_Occup_Exp_mean(:,:,3), PostPAG_Occup_Exp_mean(:,:,7), PostPAG_Occup_Exp_mean(:,:,5), PostPAG_Occup_Exp_mean(:,:,2)},'newfig',0,'paired',1, 'ShowPoints',0);
set(gca,'Xtick',[1:7],'XtickLabel',{'Stim', 'FarStim', 'CenterStim', 'Center', 'CenterNoStim', 'FarNoStim', 'NoStim'});
xtickangle(45)
ylim([0 100])
ylabel('% time spent')
...
makepretty
% Test post MFB
subplot(233), PlotErrorBarN_KJ({PostMFB_Occup_Exp_mean(:,:,1), PostMFB_Occup_Exp_mean(:,:,4), PostMFB_Occup_Exp_mean(:,:,6), PostMFB_Occup_Exp_mean(:,:,3), PostMFB_Occup_Exp_mean(:,:,7), PostMFB_Occup_Exp_mean(:,:,5), PostMFB_Occup_Exp_mean(:,:,2)},'newfig',0,'paired',1, 'ShowPoints',0);
set(gca,'Xtick',[1:7],'XtickLabel',{'Stim', 'FarStim', 'CenterStim', 'Center', 'CenterNoStim', 'FarNoStim', 'NoStim'});
xtickangle(45)
ylim([0 100])
ylabel('% time spent')
...
subplot(234), PlotErrorBarN_KJ({Pre_Occup_Sham_mean(:,:,1), Pre_Occup_Sham_mean(:,:,4), Pre_Occup_Sham_mean(:,:,6), Pre_Occup_Sham_mean(:,:,3), Pre_Occup_Sham_mean(:,:,7), Pre_Occup_Sham_mean(:,:,5), Pre_Occup_Sham_mean(:,:,2)},'newfig',0,'paired',1,'barcolors',[1 1 1 ], 'ShowPoints',0);
set(gca,'Xtick',[1:7],'XtickLabel',{'Stim', 'FarStim', 'CenterStim', 'Center', 'CenterNoStim', 'FarNoStim', 'NoStim'});
xtickangle(45)
ylim([0 100])
ylabel('% time spent')
...
makepretty
% Test post PAG
subplot(235), PlotErrorBarN_KJ({PostPAG_Occup_Sham_mean(:,:,1), PostPAG_Occup_Sham_mean(:,:,4), PostPAG_Occup_Sham_mean(:,:,6), PostPAG_Occup_Sham_mean(:,:,3), PostPAG_Occup_Sham_mean(:,:,7), PostPAG_Occup_Sham_mean(:,:,5),PostPAG_Occup_Sham_mean(:,:,2)},'newfig',0,'paired',1,'barcolors',[1 1 1 ], 'ShowPoints',0);
set(gca,'Xtick',[1:7],'XtickLabel',{'Stim', 'FarStim', 'CenterStim', 'Center', 'CenterNoStim', 'FarNoStim', 'NoStim'});
xtickangle(45)
ylim([0 100])
ylabel('% time spent')
...
makepretty
% Test post MFB
subplot(236), PlotErrorBarN_KJ({PostMFB_Occup_Sham_mean(:,:,1), PostMFB_Occup_Sham_mean(:,:,4), PostMFB_Occup_Sham_mean(:,:,6), PostMFB_Occup_Sham_mean(:,:,3), PostMFB_Occup_Sham_mean(:,:,7), PostMFB_Occup_Sham_mean(:,:,5), PostMFB_Occup_Sham_mean(:,:,2)},'newfig',0,'paired',1,'barcolors',[1 1 1 ], 'ShowPoints',0);
set(gca,'Xtick',[1:7],'XtickLabel',{'Stim', 'FarStim', 'CenterStim', 'Center', 'CenterNoStim', 'FarNoStim', 'NoStim'});
xtickangle(45)
ylim([0 100])
ylabel('% time spent')
...
makepretty
% averaged speed in shock zone
subplot(224),hb = PlotBarMC(nanmean(Pre_speed_Sham_mean,2), nanmean(Pre_speed_Exp_mean,2), nanmean(PostPAG_speed_Sham_mean,2), nanmean(PostPAG_speed_Exp_mean,2), nanmean(PostMFB_speed_Sham_mean,2), nanmean(PostMFB_speed_Exp_mean,2));
set(gca,'Xtick',[1:3],'XtickLabel',{'TestPre', 'TestPostPAG', 'TestPostMFB'});
ylabel('Speed (cm/s)');
xlim([0.10 4])
...
makepretty
% Rank sum test
p = ranksum(comp_post_mfb,comp_post_pag);
if p<0.05
