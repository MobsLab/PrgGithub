function [SpREM,SpSWS,SpWake,temps] = PlotBetaPowerOverTime_SingleMouse_MC(plo)
...
[MB_wake,SB_wake,tps]=AverageSpectrogram(sptsdB,freqB,Restrict(ts(events*1E4),and(WakeWiNoise,LowThetaEpochMC)),500,500,0);
...
SpREM=MB_REM/median(MB_REM(:)); % normalisation
SpSWS=MB_SWS/median(MB_SWS(:));
SpWake=MB_wake/median(MB_wake(:));

runfac=4;
temps=tps/1E3;

freq=[1:size(SpREM,1)]/size(SpREM,1)*100;
% freqS=[1:size(SpectroSWS,1)]/size(SpectroSWS,1)*20;
% freqW=[1:size(SpectroWake,1)]/size(SpectroWake,1)*20;

idx1=find(freq>40&freq<60);
tpsidx=find(temps>-15&temps<0);

valBetaREM=mean(SpREM(idx1,:),1);
valBetaSWS=mean(SpSWS(idx1,:),1);
valBetaWake=mean(SpWake(idx1,:),1);
