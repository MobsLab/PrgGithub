...
ShortEpoch=Epoch1-subset(Epoch1,lengthind);
bef2=Stop(Epoch2);
aft2=Start(Epoch2);
bef1=Stop(ShortEpoch);
aft1=Start(ShortEpoch);
NewEpoch=intervalSet([],[]);
for j=1:length(Start(ShortEpoch))
  if(sum(aft1(j)==bef2) +sum(bef1(j)==aft2))==2
    ind1=find((aft1(j)==bef2));
    ind2=find(bef1(j)==aft2);
    long1=(Stop(subset(Epoch2,ind1))-Start(subset(Epoch2,ind1)))>minout;
    long2=(Stop(subset(Epoch2,ind2))-Start(subset(Epoch2,ind2)))>minout;

    if long1 & long2
        if size(Start(NewEpoch),1)==0 & size(Start(NewEpoch),2)==0
            NewEpoch=intervalSet(aft1(j),bef1(j));
        else
