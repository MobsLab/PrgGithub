...
if clean
    ...
    if logical(exist('TTLInfo'))
        if isfield(TTLInfo, 'StimEpoch') && stim
    ...
    GoodTime=Range(Restrict(LFPf,TotalEpoch-SCleanEpoch));
...
disp(['  Step 1: After detection by thresholding: ' num2str(length(Start(PotentialSpindleEpochs))) ' events.']);
...
disp(['  Step 2: After merging close interval spindles: ' num2str(length(Start(PotentialSpindleEpochs))) ' events.']);

% Get rid of ripples that are too short
PotentialSpindleEpochs = dropShortIntervals(PotentialSpindleEpochs, minSpindleDuration);
disp(['  Step 3: After removing too short events: ' num2str(length(Start(PotentialSpindleEpochs))) ' events.']);

% Get rid of ripples that are too long
PotentialSpindleEpochs = dropLongIntervals(PotentialSpindleEpochs, maxSpindleDuration);
disp(['  Step 4: After removing too long events: ' num2str(length(Start(PotentialSpindleEpochs))) ' events.']);
...
disp(['  Step 5: After second thresholding: ' num2str(length(Start(FinalSpindlesEpoch))) ' events.']);
