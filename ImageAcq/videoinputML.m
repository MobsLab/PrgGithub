function obj = videoinputML(varargin)
...
%    $Revision: 1.1.6.19 $  $Date: 2010/12/27 01:14:20 $
...
try
    ...
    uddchildren = [];
    for s = 1:nSrcs,
        uddchildren = [uddchildren eval(['imaq.' schemaName, '_source(uddParent)'])]; %#ok<AGROW>
    end
...
if isDiskLogger
    % If the user set the disk logger to a VideoWriter, configure the
    % callback properties.
    uddobj = imaqgate('privateGetField', obj, 'uddobject');
    
    if isa(newValue, 'VideoWriter')
        callback = imaqgate('privateGetVideoWriterLoggerFcn');
        set(uddobj, 'ZZZVideoWriterLoggingFcn', callback);
    else
