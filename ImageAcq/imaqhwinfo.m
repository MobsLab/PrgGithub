...
%    $Revision: 1.1.6.7 $  $Date: 2010/12/27 01:13:55 $

% Error checking.
if ~isa(obj, 'imaqdevice')
    error(message('imaq:imaqhwinfo:invalidType'));
elseif ~isvalid(obj)
    error(message('imaq:imaqhwinfo:invalidOBJ'));
end
...
if nargin==1,
    ...
    if ~(ischar(field) || iscellstr(field)),
        error(message('imaq:imaqhwinfo:invalidFieldName'));
    end
    ...
    for i=1:Nobjects,
        ...
        for j = 1:nFields
            try
                ...
                elseif length(ind)>1,
                    % Ambiguous property
                    error(message('imaq:imaqhwinfo:invalidField', field{j}))
                else
                    % Found a match.
