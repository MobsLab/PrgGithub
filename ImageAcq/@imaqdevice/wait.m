...
%    $Revision: 1.1.6.8 $  $Date: 2010/12/27 01:14:13 $
...
if ~isa(objects, 'imaqdevice'),
    error(message('imaq:wait:invalidType'));
elseif ~all(isvalid(objects)),
    ...
    % object found.
    error(message('imaq:wait:invalidOBJ', inval_OBJ_indexes(1)));
end
...
if (nargin == 3)
    ...
    if strmatch(state, {'running', 'logging'})
        ...
        error(message('imaq:wait:invalidSTATE'));
    end
end
% -----End of Error Checks-----
