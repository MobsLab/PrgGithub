...
%    $Revision: 1.1.6.9 $  $Date: 2010/12/27 01:13:50 $

% Error checking.
if ~isa(obj, 'imaqdevice')
    error(message('imaq:getdata:invalidType'));
elseif (length(obj) > 1)
    error(message('imaq:getdata:OBJ1x1'));
elseif ~isvalid(obj)
...
if nin>1 && ~isnumeric(varargin{1}),
    error(message('imaq:getdata:invalidNInput'));
end

if nin>2 && ~ischar(varargin{2}),
    error(message('imaq:getdata:invalidTypeInput'));
end

if nin>3 && ~ischar(varargin{3}),
    error(message('imaq:getdata:invalidFormatInput'));
end

if nin>4
    error(message('imaq:getdata:tooManyInputs'));
end
