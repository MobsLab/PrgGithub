...
%   same name as the code file containing the video input object code.
%
%   To recreate OBJ, execute the code file by calling its filename.  
%
%   Example:
...
%   $Revision: 1.1.6.14 $  $Date: 2010/12/27 01:14:02 $
...
if (fid==-1)
    error(message('imaq:obj2mfile:fileopen', filename));
end

% Convert array to udd objects
objects=imaqgate('privateGetField',objects, 'uddobject');
...
objPropNamesAndValues = cell(numVideoInputObjects, 1);
objPropInfo = cell(numVideoInputObjects, 1);
for i = 1:numVideoInputObjects
    ...
    for j = 1:length(srcObjects{i})
        srcPropNamesAndValues{i}{j} = get(srcObjects{i}(j)); %#ok<AGROW>
        srcPropInfo{i}{j} = propinfo(srcObjects{i}(j)); %#ok<AGROW>
    end
end
...
try
    ...
    [~, functionName]=fileparts(file); %#ok<NASGU,NASGU>
    fprintf(fid, 'function out = %s\n', functionName);
catch 
    % Could not write to the output file.
    error(message('imaq:obj2mfile:fileWrite', upper(filename)));
end %try

% Write general function help comment.
localSetMCodeHelpComment(fid, file, 'vidobj');

try
    % String for checking that the image acquisition toolbox exist.
    ...
    % Write the code for checking the Image Acquisition Toolbox.
...
% Code for loading the MAT-file.
if (createMAT)
    % A MAT-file was created.
    try
        % Code string for loading the MAT-file.
        ...
        % Write the code for loading the MAT-file.
...
for i = 1:numVideoInputObjects 
    ...
    try
        % String defining an object's AdaptorName, DeviceID,
        ...
        % Write the code for defining an object's AdaptorName, DeviceID,
        % VideoFormat, and Tag values.
    ...
    if strcmp(reuse, 'reuse')
        try
            % String for reusing an existing video input object.
            ...
            % Write the code for reusing an existing video input object.
    ...
    % Code for configuring an object's properties.  
    ...
    % be saved. 
    [objProps{i}, objMATProps{i}] = localGetPropertiesToConfigure(objPropNamesAndValues{i}, objPropInfo{i}, mode); %#ok<AGROW,AGROW>

    if (~isempty(objMATProps{i}))
        try
            % String for configuring properties whose values were
            % saved in a MAT-file.
    ...
    if(~isempty(objProps{i}))
        ...
        try
            ...
            % Write the code for configuring the video input object's
            % properties.
    ...
    trigConfigToWrite = localGetTriggerConfiguration(objPropNamesAndValues{i}, objPropInfo{i}, mode);
    
    if ~isempty(trigConfigToWrite)
        try
            configTrigCommentMcodeStr = sprintf('%% Configure %s triggering.\n', objName);
            fprintf(fid, '%s', configTrigCommentMcodeStr);
            
            configTrigMcodeStr= sprintf('triggerconfig(%s, ''%s'', ''%s'', ''%s'');\n\n', ...
                objName, trigConfigToWrite.TriggerType, ...
                trigConfigToWrite.TriggerCondition, ...
                trigConfigToWrite.TriggerSource);
            fprintf(fid, '%s', configTrigMcodeStr);
        catch %#ok<CTCH>
            % Could not write to the output file.
            error(message('imaq:obj2mfile:fileWrite', upper(filename)));     
        end
    ...
    % Code for accessing an object's video source.
    try
    ...
    % Write the code for configuring an object's video source properties.
    try
        for j = 1:length(srcObjects{i})
            ...
            if (~isempty(srcMATProps))
                % Write the code to configure video source properties values that
                ...
                % String for configuring properties whose values
    ...
    outObjStr = [outObjStr sprintf('vidObj%d ', i)]; %#ok<AGROW> 
end

% Code for returning objects.
try
    if (i == 1)
        % String for returning a single object.
        % Do not concatenate the object with brackets for performance.
        returnObjStr = sprintf('out = %s;\n', outObjStr);
    else
        % String for returning multiple objects.
        % Must concatenate the objects with brackets.
        returnObjStr = sprintf('out = [%s];\n', outObjStr);
    end %if
    
    % Write the code for returning the video input objects.
...
if ~isa(objects, 'imaqdevice')
    ...
    error(message('imaq:obj2mfile:invalidType'));
end 

% Check if OBJ is an image acquisition object.
if ~all(isvalid(objects))
    error(message('imaq:obj2mfile:invalidOBJ'));
end %if
...
if ~ischar(filename)
    error(message('imaq:obj2mfile:nonStringFILENAME'));
end 
...
if ~iscellstr(varargin)
    error(message('imaq:obj2mfile:nonStringSetting'));
end
...
elseif (length(varargin) == 1)
    ...
    if isempty(strmatch(lower(varargin{1}), validSettings.All));
       ...
       error(message('imaq:obj2mfile:invalidSetting'));
    end %if
...
elseif (length(varargin) == 3)
    ...
    elseif isempty(strmatch(lower(varargin{3}), validSettings.Reuse))
        ...
        error(message('imaq:obj2mfile:invalidREUSE'));
    end %if
else 
    % More than three settings specified.
    error(message('imaq:obj2mfile:tooManySettings'));
end
...
% Converts the specified FILENAME to a file that has a valid MATLAB
% extension. If FILENAME contains no extension or if FILENAME contains an
% extension other than a MATLAB extension (.m), append a (.m) to the end of
% filename.
...
vidObjs = cell(numObjects, 1);
sourceObjs = cell(numObjects, 1);
for i = 1:numObjects
    vidObjs{i} = objects(i);
...
switch mode
    case 'modified'
        ...
        % names whose values need to be explicitly configured in the code.
        ...
        for i = 1:numPropFields
            ...
            if (iscell(currentPropValue) || isa(currentPropValue, 'function_handle'))
                matPropertyNames{end+1} = currentPropName; %#ok<AGROW>
                continue;
            end %if
            ...
            if ~isequal(currentPropValue, defaultPropValue)
                ...
                if ~isequal(info.(allPropertyFields{i}).ReadOnly, 'always')
                    ...
                    % to be configured.
                    nonDefaultPropertyNames{end+1} = currentPropName; %#ok<AGROW>
                end %if
            end %if
        end %for
    ...
    case 'all'
        ...
        for j = 1:numPropFields
            ...
            if ~isequal(propertyReadOnlyStatus, 'always')
                ...
                % configure.
                nonReadOnlyPropertyFields{end+1} = currentPropName; %#ok<AGROW>
            end %if
        end %for
...
% -------------------------------------------------------------------------
function trigConfig = localGetTriggerConfiguration(currentValue, info, mode)
% Determine the triggering configuration that needs to be written out.
trigConfig.TriggerType = currentValue.TriggerType;
trigConfig.TriggerCondition = currentValue.TriggerCondition;
trigConfig.TriggerSource = currentValue.TriggerSource;

defaultConfig.TriggerType = info.TriggerType.DefaultValue;
defaultConfig.TriggerCondition = info.TriggerCondition.DefaultValue;
defaultConfig.TriggerSource = info.TriggerSource.DefaultValue;

if isequal(trigConfig, defaultConfig) && ~strcmpi(mode, 'all')
    trigConfig = [];
end
...
% Query through an object's properties and write the code for configuring
...
for i = 1:length(prop)
    ...
    if strcmp(propName, 'UserData')
        ...
        if (MATLoaded),
            ...
            % Value cannot be loaded from the MAT-file. Use the
            % default value.
            if strcmpi(syntax, 'set')
...
% ------------------------------------------------------------------
function writeMAT = localSaveToMATFile(file, writeMAT, varName, value) %#ok<INUSD>
...
% Create the comments for the beginning of the generated code file.

[~, functionName]=fileparts(file); %#ok<NASGU,NASGU>
fcnNameUpper = upper(functionName);

try
    % String for the function helper comments.
    fcnHelpCommentMcodeStr = sprintf('%%%s Code for creating a video input object.\n', fcnNameUpper);
    ...
    fcnHelpCommentMcodeStr = sprintf('%s%%   This MATLAB code file, %s.M, was generated from the OBJ2MFILE function.\n', fcnHelpCommentMcodeStr, fcnNameUpper);
    ...
    fcnHelpCommentMcodeStr = sprintf('%s%%   code file but with a .MAT extension. To recreate this video input object,\n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%   type the name of the code file, %s, at the MATLAB command prompt.\n', fcnHelpCommentMcodeStr, functionName);
    fcnHelpCommentMcodeStr = sprintf('%s%%   \n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%   The code file, %s.M and its associated MAT-file, %s.MAT (if\n', fcnHelpCommentMcodeStr, fcnNameUpper, fcnNameUpper);
    fcnHelpCommentMcodeStr = sprintf('%s%%   it exists) must be on your MATLAB path.\n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%   \n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%   Example: \n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%       %s = %s;\n', fcnHelpCommentMcodeStr, name, functionName);
    fcnHelpCommentMcodeStr = sprintf('%s%%   \n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%   See also VIDEOINPUT, IMAQDEVICE/PROPINFO, IMAQHELP, PATH.\n', fcnHelpCommentMcodeStr);
    fcnHelpCommentMcodeStr = sprintf('%s%%   \n\n', fcnHelpCommentMcodeStr);

    % Write the code for the function helper comments.
