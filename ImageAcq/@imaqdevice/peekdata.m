...
%    $Revision: 1.1.6.6 $  $Date: 2010/12/27 01:14:04 $

% Error checking.
if ~isa(obj, 'imaqdevice')
    error(message('imaq:peekdata:invalidType'));
elseif (length(obj) > 1)
    error(message('imaq:peekdata:OBJ1x1'));
elseif ~isvalid(obj)
    error(message('imaq:peekdata:invalidOBJ'));
elseif (nargin==1) || ~isnumeric(frames) || (length(frames)~=1)
    error(message('imaq:peekdata:invalidFrames'));
end
