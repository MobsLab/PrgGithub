function vidfile = Behav_react_movie(experiment, nmice)
%
...
%% Manage inputs - check '*' before run
tic
...
%% Get paths of each individual mouse - check '*' before run
...
vidfile = VideoWriter([micenames{isession} '_' epoch_name],'MPEG-4'); % * For windows and mac
...
for k = 1:length(x_traj) 
    ...
    fig1 = subplot(3, 7, 1);
    plot(x_traj(k), y_traj(k), 'o', 'LineWidth', 5, 'MarkerSize', 5, 'MarkerEdgeColor', [0.1 0.5 0.4])
    ...
    if strcmp(epoch_name, 'Cond1') || strcmp(epoch_name, 'Cond2') || strcmp(epoch_name, 'Cond3') || strcmp(epoch_name, 'Cond4')
        if nonzeros((times(k) == shocks_aligned)) == 1
            plot(x_traj(k), y_traj(k), '*', 'LineWidth', 3, 'MarkerSize', 4, 'MarkerEdgeColor', [1 0 0])
            x_shock(ishock) = x_traj(k);
            y_shock(ishock) = y_traj(k);
        end
        if x_shock(1) ~= 0
    ...
    xline(x_axis(3), ':', 'Color', [0 0 0], 'LineWidth', 1);
    grid on;
    ...
    for i = 1:length(rips_onfig)
        xline(ripples_aligned(rips_onfig(i)), '--', 'Color', [0 0.2 0.5], 'LineWidth', 1.5);
    end
    clear i    
        
    % Plot shocks
    for i = 1:length(shocks_onfig)
        xline(shocks_aligned(shocks_onfig(i)), '--', 'Color', [0.8 0 0], 'LineWidth', 1.5);
    end
    clear i
