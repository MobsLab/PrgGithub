function[PCtemplates, s, per_explained] = react_pca_ica_MM(nmice, experiment, TemplatePeriod, numtemplates, multi_U, calc, interN, issaveweight, varargin)
%%
...
%%Path
addpath(genpath('D:\TINS\ERC_data'))
addpath(genpath('C:\Users\mario\Desktop\TINS'))
addpath(genpath('C:\Users\mario\Dropbox\Kteam\PrgMatlab'))
...
experiment = "PAG";
TemplatePeriod = 'cond';
numtemplates = 5; %Number of PCs. 2-3 max.  
%}
...
binsize = 0.1*1e4; %0.1*1e4 = 100 ms
%}
IsPlotInd = false;
...
Dir = PathForExperimentsERC_MM(fetchpaths);
...
for isession = 1:numsessions
    ...
    if calc == "PCA"
        ...
        EV = sum(per_explained{isession}(1:numtemplates));
        ...
        % Elbow rule
        figure 
        plot([1:numtemplates],L([1:numtemplates]), 'r-o',[numtemplates+1:size(L,1)], L([numtemplates+1:size(L,1)]),'b-o');
        xlabel('Number of PC');
        ylabel('Eigenvalue');
        %yline(percentile_shuff, 'LineStyle','--', 'Color',[0.9290 0.6940 0.1250]);
        yline(lambda_m, 'LineStyle','--', 'Color',[0.9290 0.6940 0.1250]);
        title([num2str(EV) ' % of variance explained by ' num2str(numtemplates) ' templates']);
        ...
        if IsSaveFig == 1
            ...
            saveas(gcf, ['PCA_ElbowRule_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
        end

    elseif calc == "ICA"
        [A, PCtemplates{isession}] = fast_ica_MM(Qtemplate, numtemplates);
        if issaveweight == 1
            filename = ['ICA_W_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.xlsx'];
...
sign_N = strings(6,numtemplates);
for itemp = 1:numtemplates
    figure('color',[1 1 1], 'units', 'normalized', 'outerposition', [0 0 0.5 0.5]);
    ...
    if IsSaveFig == 1
        if calc == "PCA"
            ...
            saveas(gcf, ['PCA_W_PC' num2str(itemp) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.svg']);
        elseif calc == "ICA"
            saveas(gcf, ['ICA_W_PC' num2str(itemp) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.png']);
            saveas(gcf, ['ICA_W_PC' num2str(itemp) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.svg']);
        end
    end
    %}
end
...
if IsSaveFig == 1
    if calc == "PCA"
        filename = ['PCA_Repartition_SignN_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.xlsx'];
        writematrix(sign_N,filename,'Sheet',1,'Range','A1')
     elseif calc == "ICA"
         filename = ['ICA_Repartition_SignN_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.xlsx'];
        writematrix(sign_N,filename,'Sheet',1,'Range','A1')
    end
end
    ...
    for isession = 1:numsessions
        for itemplate = 1:numtemplates
            finv{isession}{itemplate} = plotIndividFigure_react_pca(tRipples{isession}, b{isession}.SessionEpoch,...
                ...
                squeeze(meanRSripPo(isession, itemplate, :)), tPr, tCo, tPo, micenames{isession}, itemplate, sign{isession}{itemplate});
            
            if IsSaveFig == 1
                if calc == "PCA"
                    saveas(gcf, ['PCA_PC' num2str(itemplate) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.png']);
                    saveas(gcf, ['PCA_PC' num2str(itemplate) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.svg']);
                elseif calc == "ICA"
                    saveas(gcf, ['ICA_PC' num2str(itemplate) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.png']);
                    saveas(gcf, ['ICA_PC' num2str(itemplate) '_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.svg']);
                end
            end
...
if IsSaveFig == 1
    if calc == "PCA"
        ...
        saveas(gcf, ['PCA_ReactStrength_Ripples_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    elseif calc == "ICA"
        ...
        saveas(gcf, ['ICA_ReactStrength_Ripples_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    end
end
...
if IsSaveFig == 1
    if calc == "PCA"
        ...
        saveas(gcf, ['PCA_ScoreCorrel_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    elseif calc == "ICA"
        ...
        saveas(gcf, ['ICA_ScoreCorrel_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    end
end
...
if IsSaveFig == 1
    if calc == "PCA"
        filename = ['PCA_Mean_over_epochs_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.xlsx'];
        writematrix(mean_export,filename,'Sheet',1,'Range','A1');
    elseif calc == "ICA"
        filename = ['ICA_Mean_over_epochs_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '.xlsx'];
        writematrix(mean_export,filename,'Sheet',1,'Range','A1');  
    end
end
...
if IsSaveFig == 1
    if calc == "PCA"
        ...
        saveas(gcf, ['PCA_PCScore_Epochs_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    elseif calc == "ICA"
        ...
        saveas(gcf, ['ICA_PCScore_Epochs_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);      
    end
end
...
[~,h2] = PlotErrorBarN_DB([meanPr_plot meanCo_plot meanCoFreeze_plot meanCoRipples_plot meanCoNoRipples_plot...
    meanPo_plot meanPoRipples_plot], 'barcolors', [1 1 1], 'barwidth', 0.6, 'newfig', 0, 'showpoints', 1);
...
if IsSaveFig == 1
    if calc == "PCA"
       ...
       saveas(gcf, ['PCA_PCScore_SleepEpochs_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    elseif calc == "ICA"
       ...
       saveas(gcf, ['ICA_PCScore_SleepEpochs_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    end
end
...
if IsSaveFig == 1
    if calc == "PCA"
        ...
        saveas(gcf, ['PCA_LocHabCond_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    elseif calc == "ICA"
        ...
        saveas(gcf, ['ICA_LocHabCond_param_' micenames{isession} '_templateEpoch_' TemplatePeriod '_numTemplates_' num2str(numtemplates) '.svg']);
    end
end
...
function fh = plotIndividFigure_react_pca(tRipples, SessionEpoch, HabEpoch, CondEpoch, SWSEpoch,...
    replayGtsd, LocPCHab, LocPCCond, SWRrate, mPr, mCo, mPo, tPr, tCo, tPo, mousename, itemplate, sign)
