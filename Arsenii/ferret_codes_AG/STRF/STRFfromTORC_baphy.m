...
% cd (WorkingFolder);
% load('A2.mat')
% twopR = binNinf_double';
% load('matlab.mat_Run2.mat')
% trialonset = C;
% strfCellArray = STRF2pA1(twopR,'Test_2019_03_12_TOR_2.m',[WorkingFolder '/TORCmfiles'], trialonset);
% %%
% WorkingFolder = '/home/yves/Dropbox (Equipe Audition)/myScripts_Yves/myMatlabscripts_Yves/2p_TORCstrf_Yves/20_01_30';
% cd (WorkingFolder);
% load('TORC_spikes_mean.mat')
% strfCellArray = STRF2pA1(TORC_spikes_mean,'mou008a03_p_TOR.m',WorkingFolder);
%%
rasterfs = 1000;
dur = 3;
cm = [[linspace(0,1,50)' linspace(0,1,50)' ones(50,1)] ; [ones(50,1) linspace(1,0,50)' linspace(1,0,50)']];
%% FROM strf_online.m (LINES 1-177)
...
if ~isfield(options,'usefirstcycle')
    options.usefirstcycle = 1;
end
if ~isfield(options,'usefirstcycle')
    options.dur = dur;
end
if ~isfield(options,'plotMe')
    options.plotMe = 1;
end
...
StimParam.mf        = options.rasterfs/rasterfs; % config('mf');
StimParam.ddur      = round(rasterfs*dur);
StimParam.stdur     = round(rasterfs*dur);
StimParam.stonset   = 0;
...
[waveParams,W,Omega,StimParam.nrips,StimParam.basep] = ...
    ...
    StimParam.a1rv,rasterfs);

StimParam.basep = round(rasterfs/min(abs(diff([0 unique(abs(W(find(W))))]))));
noct = round(1/min(abs(diff([0 unique(abs(Omega(find(Omega))))']))));
maxv = max(abs(W));
maxf = max(max(abs(Omega)));
saf = ceil(maxv*2 + rasterfs/StimParam.basep);
numcomp = ceil(maxf*2*noct + 1);

StStims = stimprofile(waveParams,W,Omega,StimParam.lfreq,...
    StimParam.hfreq,numcomp,StimParam.basep,saf,rasterfs);
%StStims = stimscale(StStims,'moddep',0.9,[],10*saf*StimParam.basep/rasterfs,...
%    10*numcomp/noct);
StStims = stimscale(StStims,'moddep',0.9,[],10*saf*StimParam.basep/rasterfs,...
    2*numcomp);
...
%%
for cellbatch = 0:floor(size(raster,1)/25)
if options.plotMe; figure; end
sn = 0;
for cnt = 1:min(25,size(raster,1)-cellbatch*25)
    r = zeros(size(raster,2),size(raster,3),size(raster,4));
    cn = cnt+cellbatch*25;
    strfest = zeros(stimX,stimT);
    sn = sn + 1;
    r_temp = squeeze(raster(cn,:,:,:));
    r = r_temp;
    [row,col] = find( isnan(squeeze(r_temp(1,:,:))) );
    ...
    if StimParam.ddur<StimParam.basep*2 || options.usefirstcycle,
...
%         disp('INCLUDING FIRST TORC CYCLE!!!');
    ...
    snr = get_snr(r,StStims,StimParam.basep,StimParam.mf,waveParams,StimParam.a1rv,rasterfs);
    if ~options.onlySNR
    for rep = 1:realrepcount
        reclst = 1:StimParam.numrecs;
        for rec = reclst
            spkdata = r(:,rep,rec);
            ...
            [dsum,cnorm] = makepsth(spkdata,rasterfs/StimParam.basep,FirstStimTime,...
                StimParam.stdur,StimParam.mf,rasterfs);
            ...
            if ~isempty(dsum)
                ...
                dsum = dsum*(rasterfs/binsize);  % Normalization by binsize
        ...
        if options.plotMe
            ...
            if options.compact
                if channel==1
                    ht=title([sprintf('E.%d Rep.%.f Sp.%d SNR %.2f',channel, realrepcount, nansum(r(:)),snr)],'FontSize',6);
                else
                    ht=title([sprintf('E.%.f Sp.%d SNR %.2f',channel,nansum(r(:)),snr)],'FontSize',6);
                    set(gca,'XTickLabel',[]);
                end
                set(gca,'FontSize',6);
                ytick=num2str(str2num(get(gca,'YTickLabel'))./rasterfs);
                set(gca,'YTickLabel',ytick);
            else
                xlabel(sprintf('%.1f sig, %.f sp, snr %.2f',options.sigthreshold,nansum(r(:)),snr));
...
%----------------------------------------------------------------
function  [waveParams,W,Omega,N,T,k] = makestimprofile(a1am,a1rf,a1ph,a1rv,rasterfs);
...
W = vels;
T = round(rasterfs/min(abs(diff([0 unique(abs(W(find(W))))]))));
%k = round(abs(cat(1,a1rv{:}))'*T/rasterfs + 1);
...
function [ststims,freqs]=stimprofile(waveParams,W,Omega,...
				 lfreq,hfreq,numcomp,T,saf,rasterfs);
...
% saf 	     : sampling frequency (default = rasterfs Hz)

[ap,ws,omegs,lr,numstims] = size(waveParams);
[a,b]=size(Omega);
[c,d]=size(W);

if nargin==7, saf = rasterfs; end
...
%----------------------------------------------------------------
function [wrapdata,cnorm] = makepsth(dsum,fhist,startime,endtime,mf,rasterfs);
...
if fhist==0, fhist=rasterfs/(endtime-startime); end
%if fhist==0, fhist=1; end
dsum = dsum(:);

period = rasterfs*(1/fhist)*mf;  % in samples
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function snr = get_snr(spdata,stim,basep,mf,waveParams,a1rv,rasterfs)
...
freqindx = round(abs(cat(1,a1rv{:}))'*basep/rasterfs + 1);
