function [data,cfg] = ft_read_oe_data_py(cfg,rebuild)
...
% 
% DJS 2021
if nargin<2, rebuild=0; end
if ~isfield(cfg,'numchans'), cfg.numchans = 32; end
if ~isfield(cfg,'path'), cfg.path = 'M:\Phoneme Discrim ECoG\ECoG_Data'; end
if ~isfield(cfg,'node'), cfg.node = '100'; end
if ~isfield(cfg,'channelmapfile'), cfg.channelmapfile = 'M:\Phoneme Discrim ECoG\ECoG_Data\ECoG_channel_map.mat'; end

if ~isfield(cfg,'recording')
    d = dir(fullfile(cfg.path,cfg.experiment,'Record Node*'));
    cfg.recording = d(1).name;
end
fn = fullfile(cfg.path,cfg.experiment,[cfg.experiment '.mat']);  %data file for store the raw data extrcated from OE data
...
[Y0,M0,D0,h0,m0]=datevec(cfg.date);
