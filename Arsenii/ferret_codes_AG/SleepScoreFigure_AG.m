function SleepScoreFigure_AG(directory,PlotEp)
...
try
remtheta=(Restrict(theta_new,And(PlotEp,S2_epoch)));
catch
    remtheta=(Restrict(theta_new,S2_epoch));
end
ghi_new_r=Restrict(ghi_new,ts(Range(remtheta)));
plot(log(Data(ghi_new_r)),log(Data(remtheta)),'.','color',[1 0.2 0.2],'MarkerSize',1);
hold on
try
sleeptheta=(Restrict(theta_new,And(PlotEp,S1_epoch)));
catch
    sleeptheta=(Restrict(theta_new,S1_epoch));
end
