...
%  USAGE
%
%    d = IsolationDistance(features,<options>)
%
%    features       features obtained using e.g. <a href="matlab:help LoadSpikeFeatures">LoadSpikeFeatures</a> (or a
%                   subset thereof)
...
%  OUTPUT
%
%    For each cluster, a measure of the separation between the cluster and all
%    other spikes. Namely, the Mahalanobis distance of the Nth closest spike
%    outside the cluster (where N is the number of spikes inside the cluster).
...
function [distance,LRatio] = IsolationDistance(features,varargin)
...
if ~isempty(units)
    for i = 1:size(units,1),
        unit = units(i,:);
        [di,LR] = ComputeOnce(features,unit,show);
        distance(i)=di;
        L(i)=LR;
    end
    LRatio = L/length(units);
else
    distance = [];
    LRatio = [];
end
...
function [distance,L] = ComputeOnce(features,unit,show)
...
if nSpikesInCluster < nFeatures,
	disp(['Not enough spikes for unit ' unitStr '.']);
	return
end
if nSpikesInCluster > nSpikesInElectrodeGroup/2,
	disp(['Too many spikes for unit ' unitStr '.']);
	return
end
