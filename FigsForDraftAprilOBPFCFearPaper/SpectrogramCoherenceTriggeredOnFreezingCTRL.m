...
% Get data
[Dir,KeepFirstSessionOnly]=GetRightSessionsFor4HzPaper('CtrlRipplesOnly');
...
for m=1:length(KeepFirstSessionOnly)
    ...
    for st=1:size(AllCombi,1)
        ...
        [ACoh.(StucCombi){m},S,t]=AverageSpectrogram(Angtsd,fC,ts(Stop(FreezeEpoch)),200,100,0,0,0);
        clear C
    end
...
close all
ClimsToUse={[0 0.00017];[0 0.00011];[0 0.00017]}
...
%     saveas(fig,[SaveFigFolder,'TriggeredSpectroStopFZ',Struc{sp},'.fig'])
%     saveas(fig,[SaveFigFolder,'TriggeredSpectroStopFZ',Struc{sp},'.png']), close all
end

ClimsToUse={[0.4 0.65];[0.4 0.9];[0.4 0.65]}
for st=1:size(AllCombi,1)
    fig=figure;
    clear AllM Power4hzBand
    StucCombi=[Struc{AllCombiNums(st,1)},'_',Struc{AllCombiNums(st,2)}];
    ...
    xlabel('time to FZ onset (s)')
    ylabel('3-6 power zscore')
%     saveas(fig,[SaveFigFolder,'TriggeredCohStopFZ',strrep(StucCombi,'_','-'),'.fig'])
%     saveas(fig,[SaveFigFolder,'TriggeredCohStopFZ',strrep(StucCombi,'_','-'),'.png']), close all
    
end


 ClimsToUse={[0 0.3];[0 0.3];[0 0.3]}
for st=1:size(AllCombi,1)
    fig=figure;
    clear AllM Power4hzBand
    StucCombi=[Struc{AllCombiNums(st,1)},'_',Struc{AllCombiNums(st,2)}];
    ...
    xlabel('time to FZ onset (s)')
    ylabel('3-6 power zscore')
    saveas(fig,[SaveFigFolder,'TriggeredImagCohStopFZ',strrep(StucCombi,'_','-'),'.fig'])
    saveas(fig,[SaveFigFolder,'TriggeredImagCohStopFZ',strrep(StucCombi,'_','-'),'.png']), close all
