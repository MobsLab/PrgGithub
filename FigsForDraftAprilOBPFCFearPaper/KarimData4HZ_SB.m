...
for k = 1:length(DirB.path)
    ...
    if exist('SpikeData.mat')>-1
        ...
        % No S by phase of course
        AllS{1} = S;
        
        for num = 1:length(S)
            for type = 1
                [StartResp_B{type}(neur,:), B] = CrossCorr(Start(FreezeEpoch),Range(AllS{type}{num}),40,125);
                [StopResp_B{type}(neur,:), B] = CrossCorr(Stop(FreezeEpoch),Range(AllS{type}{num}),40,125);
                
                
                [~,MeanNoFz_B(neur)] = FiringRateEpoch(S{num},TotEpoch-FreezeEpoch);
                [~,MeanFz_B(neur)] = FiringRateEpoch(S{num},FreezeEpoch);
                Q = MakeQfromS(AllS{type}(num),0.04*1E4);
                StdNeur_B(neur) = std(Data(Restrict(Q,TotEpoch-FreezeEpoch)));
                ...
                NormResp_B{type}(neur,:) = (nanmean(CorrRem,1));
