...
cd('/home/gruffalo/Dropbox/MOBS_workingON/ob4hz-manuscrit/Data')
load('PhaseMeasures.mat')

close all,
load(['/Users/sophiebagur/Dropbox/Kteam/PrgMatlab/WaveFormLibrary.mat'])
% Add library WF Data
...
for nn=1:num
    ...
    AllJit(1,nn)=PhaseMeasures.HPC.Fz{nn}.Jit*PhaseMeasures.FreqHPCFz(nn);
    AllJit(2,nn)=PhaseMeasures.HPC.NoFz{nn}.Jit*PhaseMeasures.FreqHPCNoFz(nn);
    AllJit(3,nn)=PhaseMeasures.OB.Fz{nn}.Jit*PhaseMeasures.FreqOBFz(nn);
    AllJit(4,nn)=PhaseMeasures.OB.NoFz{nn}.Jit*PhaseMeasures.FreqOBNoFz(nn);
...
clear p h stats
close all
% HPC 3 Ways of calculating
HPCSig=and(Allpval(1,:)<0.05,Allpval(2,:)<0.05);
X=AllKappa(1,HPCSig);
Y=AllKappa(2,HPCSig);
[p.HPCAND,h.HPCAND,stats.HPCAND]=ModIndexPlot(X,Y);
saveas(gcf,'HPCSigAnd_Mod.fig'), close all

HPCSig=or(Allpval(1,:)<0.05,Allpval(2,:)<0.05);
X=AllKappa(1,HPCSig);
Y=AllKappa(2,HPCSig);
[p.HPCOR,h.HPCOR,stats.HPCOR]=ModIndexPlot(X,Y);
saveas(gcf,'HPCSigOr_Mod.fig'), close all

HPCSig=or(Allpval(1,:)<100,Allpval(2,:)<100);
X=AllKappa(1,HPCSig);
Y=AllKappa(2,HPCSig);
[p.HPCALL,h.HPCALL,stats.HPCALL]=ModIndexPlot(X,Y);
saveas(gcf,'HPCAll_Mod.fig'), close all


% OB 3 Ways of calculating
OBSig=and(Allpval(3,:)<0.05,Allpval(4,:)<0.05);
X=AllKappa(3,OBSig);
Y=AllKappa(4,OBSig);
[p.OBAND,h.OBAND,stats.OBAND]=ModIndexPlot(X,Y);
saveas(gcf,'OBSigAnd_Mod.fig'), close all

OBSig=or(Allpval(3,:)<0.05,Allpval(4,:)<0.05);
X=AllKappa(3,OBSig);
Y=AllKappa(4,OBSig);
[p.OBOR,h.OBOR,stats.OBOR]=ModIndexPlot(X,Y);
saveas(gcf,'OBSigOr_Mod.fig'), close all

OBSig=and(Allpval(3,:)<100,Allpval(4,:)<100);
X=AllKappa(3,OBSig);
Y=AllKappa(4,OBSig);
[p.OBALL,h.OBALL,stats.OBALL]=ModIndexPlot(X,Y);
saveas(gcf,'OBAll_Mod.fig'), close all
save('ModIndexStats.mat','p','h','stats')

% pie chart OB
figure
alphaval=0.05;
Cols1=[0,109,219;240,73,73]/263;
A=[sum(Allpval(3,:)<=alphaval&Allpval(4,:)>alphaval),...
    sum(Allpval(3,:)<=alphaval&Allpval(4,:)<=alphaval),...
    sum(Allpval(3,:)>alphaval&Allpval(4,:)<=alphaval),...
    sum(Allpval(3,:)>alphaval&Allpval(4,:)>alphaval)];
h=pie(A)
...
set(h(7), 'FaceColor', 'w');
title('OB')
X1=(Allpval(3,:)<alphaval);
X2=(Allpval(4,:)<alphaval);
[table,chi2,p] = crosstab(X1,X2);
clear p h chi2stat
[h,p, chi2stat,df] =prop_test([sum(X1),sum(X2)],[length(X1),length(X2)],'true');
save('Chi2TestProportionsOB.mat','df','p','chi2stat')

% pie chart HPC
figure
alphaval=0.05;
Cols1=[0,109,219;240,73,73]/263;
A=[sum(Allpval(1,:)<=alphaval&Allpval(2,:)>alphaval),...
    sum(Allpval(1,:)<=alphaval&Allpval(2,:)<=alphaval),...
    sum(Allpval(1,:)>alphaval&Allpval(2,:)<=alphaval),...
    sum(Allpval(1,:)>alphaval&Allpval(2,:)>alphaval)];
h=pie(A)
set(h(1), 'FaceColor', Cols1(1,:));
...
set(h(7), 'FaceColor', 'w');
title('HPC')
X1=(Allpval(1,:)<alphaval);
X2=(Allpval(2,:)<alphaval);
[table,chi2,p] = crosstab(X1,X2);
clear p h chi2stat
[h,p, chi2stat,df] =prop_test([sum(X1),sum(X2)],[length(X1),length(X2)],'true');
save('Chi2TestProportionsHPC.mat','df','p','chi2stat')
...
figure
plot(AllSpk,(AllKappa(3,:)-AllKappa(4,:))./(AllKappa(3,:)+AllKappa(4,:)),'k.')
xlabel('FR mod'), ylabel('Kappa Mod')
box off
set(gca,'FontSize',15)
[rho,p]=corr(AllSpk',[(AllKappa(3,:)-AllKappa(4,:))./(AllKappa(3,:)+AllKappa(4,:))]','type','Spearman');


figure
plotSpread({AllSpk},'distributionColors',[0.4 0.4 0.4],'xValues',[1])
hold on
line([0.8 1.2],[1 1]*nanmedian(AllSpk),'color','k','linewidth',2)
...
%% Figrues not in paper
%% Look at all params for all units
figure
subplot(241)
...
% look at whosen params for all untis
figure
clf
%figure
...
% look at whosen params for units that are significant in both cases
figure
Cols2=[0,146,146;189,109,255]/263;
HPCSig=and(Allpval(1,:)<0.05,Allpval(2,:)<0.05);
OBSig=and(Allpval(3,:)<0.05,Allpval(4,:)<0.05);
subplot(221)
plot(AllKappa(1,HPCSig),AllKappa(2,HPCSig),'.','color',Cols2(1,:),'MarkerSize',10), hold on
plot(AllKappa(3,OBSig),AllKappa(4,OBSig),'.','color',Cols2(2,:),'MarkerSize',10)
line([0 1],[0 1],'color','k')
legend('HPC','OB')
xlabel('Fz'),ylabel('NoFz')
title('Kappa'),box off
...
set(gca,'FontSize',15)
subplot(223)
ModH=(AllKappa(1,HPCSig)-AllKappa(2,HPCSig))./(AllKappa(1,HPCSig)+AllKappa(2,HPCSig));
ModB=(AllKappa(3,OBSig)-AllKappa(4,OBSig))./(AllKappa(3,OBSig)+AllKappa(4,OBSig));
plotSpread({ModH,ModB},'distributionColors',[0.4 0.4 0.4],'xNames',{'HPC','OB'},'xValues',[1,2])
hold on
line([0.8 1.2],[1 1]*median(ModH),'color','k','linewidth',2)
line([0.8 1.2]+1,[1 1]*median(ModB),'color','k','linewidth',2)
line(xlim,[0 0],'color','k','linestyle',':')
[p(1),h]=signrank(ModH);
[p(2),h]=signrank(ModB);
for i=1:2
...
set(gca,'FontSize',15)
subplot(222)
plot(AllPwPC(1,HPCSig),AllPwPC(2,HPCSig),'.','color',Cols2(1,:),'MarkerSize',10), hold on
plot(AllPwPC(3,OBSig),AllPwPC(4,OBSig),'.','color',Cols2(2,:),'MarkerSize',10)
line([0 0.5],[0 0.5],'color','k')
legend('HPC','OB')
xlabel('Fz'),ylabel('NoFz')
title('PwPC'), box off
...
set(gca,'FontSize',15)
subplot(224)
ModH=(AllPwPC(1,HPCSig)-AllPwPC(2,HPCSig))./(AllPwPC(1,HPCSig)+AllPwPC(2,HPCSig));
ModB=(AllPwPC(3,OBSig)-AllPwPC(4,OBSig))./(AllPwPC(3,OBSig)+AllPwPC(4,OBSig));
plotSpread({ModH,ModB},'distributionColors',[0.4 0.4 0.4],'xNames',{'HPC','OB'},'xValues',[1,2])
hold on
line([0.8 1.2],[1 1]*median(ModH),'color','k','linewidth',2)
line([0.8 1.2]+1,[1 1]*median(ModB),'color','k','linewidth',2)
...
% PPI FigSH
X=(AllPwPC(3,:)-AllPwPC(4,:))./(AllPwPC(3,:)+AllPwPC(4,:));
figure
a=iosr.statistics.boxPlot(1,X','boxColor',[0.9 0.9 0.9],'lineColor',[0.9 0.9 0.9],'medianColor','k','boxWidth',0.5,'showOutliers',false);
a.handles.upperWhiskers.Visible='off';a.handles.upperWhiskerTips.Visible='off';
a.handles.lowerWhiskers.Visible='off';a.handles.lowerWhiskerTips.Visible='off';
