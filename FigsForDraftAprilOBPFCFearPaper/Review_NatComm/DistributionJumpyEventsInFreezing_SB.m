...
%         Times = Times - BeppEpoch;
        ...
        TimesQ =  MakeQfromS(tsdArray(ts(Times)),0.1*1E4);
        tps=[0.05:0.05:1];
        ...
        for ep=2:length(Start(FreezeAccEpoch))-2
            ...
            Triggered{mouse}(ep-1,:) = [TempDataPre,runmean(TempData,1),TempDataPost];
        end
                        mouse =mouse+1;
...
for mouse = 1:length(Triggered)
    try
AllTrig = [AllTrig,nanmean(Triggered{mouse},1)'/10];
    end
end

hold on
errorbar([-0.5:0.05:-0.05,tps,1.05:0.05:1.5],nanmean(AllTrig'),stdError(AllTrig'),'linewidth',3,'color','k')
xlim([-0.2 1.2])
set(gca,'XTick',[-0.2:0.2:1.2],'FontSize',20)
xlabel('Norm. time')
ylabel('Micromovement density (/s)')
