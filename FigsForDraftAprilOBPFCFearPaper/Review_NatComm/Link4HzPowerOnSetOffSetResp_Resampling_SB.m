...
for k = 1:length(Dir.path)
    ...
    if exist('SpikeData.mat')>0
        ...
        ZscrPow = zscore(PowRatio);
        PowerIdentity{1} = find(zscore(PowRatio)>0); % strong
        PowerIdentity{2} = find(zscore(PowRatio)<0); % weak
        DurResamp{1} = Dur(PowerIdentity{1});
        DurResamp{2} = Dur(PowerIdentity{2});
        
        [val,ind] = min([length(PowerIdentity{1}),length(PowerIdentity{2})]);
        if ind==1, otherone=2; else otherone=1;end
        EpisodesToUse{1} = [];
        EpisodesToUse{2} = [];
        
        for num = 1:val
            [timediff,whichone] = nanmin(abs(DurResamp{ind}(num)-DurResamp{otherone}));
            if timediff<3
                DurResamp{otherone}(whichone) = NaN;
                EpisodesToUse{otherone} = [EpisodesToUse{otherone},whichone]; % matching
                EpisodesToUse{ind} = [EpisodesToUse{ind},num]; % matched
            end
        end
        
        AllDur = [AllDur,Dur([PowerIdentity{1}(EpisodesToUse{1}),PowerIdentity{2}(EpisodesToUse{2})])];
        AllPow = [AllPow,ZscrPow([PowerIdentity{1}(EpisodesToUse{1}),PowerIdentity{2}(EpisodesToUse{2})])];

        MeanSpecStr = [MeanSpecStr;nanmean(Spec(PowerIdentity{1}(EpisodesToUse{1}),:))];
        MeanSpecWk = [MeanSpecWk;nanmean(Spec(PowerIdentity{2}(EpisodesToUse{2}),:))];

        for num = 1:length(S)
            [StartRespStr(neur,:), B] = CrossCorr(Start(subset(FreezeEpoch,sort(PowerIdentity{1}(EpisodesToUse{1})))) ,Range(S{num}),40,125);
            [StopRespStr(neur,:), B] = CrossCorr(Stop(subset(FreezeEpoch,sort(PowerIdentity{1}(EpisodesToUse{1})))),Range(S{num}),40,125);
            [StartRespWk(neur,:), B] = CrossCorr(Start(subset(FreezeEpoch,sort(PowerIdentity{2}(EpisodesToUse{2})))),Range(S{num}),40,125);
            [StopRespWk(neur,:), B] = CrossCorr(Stop(subset(FreezeEpoch,sort(PowerIdentity{2}(EpisodesToUse{2})))),Range(S{num}),40,125);
