...
%                     keyboard
% 
%                     [h,x1,x2] = hist2([ratio1,ratio2]);
%                     X = ones(50,1)*x1;
%                     Y = x2'*ones(1,50);  
% 
%                     figure
%                     plot(ratio1,ratio2,'k.')
%  
%                     figure,
%                     [ROI,xi,yi] = roipoly(mat2gray(h));
% 
%                      keyboard
%                     inIx = inpolygon(ratio1,ratio2,x1(floor(xi)+1),x2(floor(yi)+1));
%                     axes(gCheckEegStates.SubPlots(2))
%                     hold on
%                     gCheckEegStates.ThetaRatioPlot = plot(gCheckEegStates.Times(inIx),10,'w*','LineWidth',2);
%                     
% 
%                     
% % 
% %                     ratio = conv(ratio,hamming(10),'same');
% %                     ratio = 20*(ratio-min(ratio))/(max(ratio)-min(ratio)); %to be displayed on the [0 20] frequency range subplots
% 
%                     axes(gCheckEegStates.SubPlots(ch2use))
%                     hold on
%                     gCheckEegStates.ThetaRatioPlot = plot(gCheckEegStates.Times,ratio,'w','LineWidth',2);
                  
                case '2' % added by Adrien Peyrache
