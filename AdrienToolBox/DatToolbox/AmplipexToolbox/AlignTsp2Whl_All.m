...
if isempty(datFiles)
    xmldata = xml_load([mergename filesep  fbasename '.xml']);
...
for fix=1:length(datFiles)
    ...
    AlignTsp2Whl([fname filesep fname],colorIx,frommpg);    
end
...
if merge
    ...
    for fix=1:length(datFiles)
        fname = datFiles{fix};
        whl = load([fname filesep fname '.whl']);
        whlall = [whlall;whl];
    end
    dlmwrite([mergename filesep mergename '.whl'],whlall,'delimiter','\t');
end
