...
%  USAGE
%
%    ripples = FindRipplesAd(samples,<options>)
%
...
function ripples = FindRipplesAd(samples,varargin)
...
%  			end
        case 'highth',
			highThresholdFactor = varargin{i+1};
 			if ~isnumeric(highThresholdFactor),
 				error('Incorrect value for property ''highThresholdFactor'' (type ''help FindRipples'' for details).');
            end
        case 'lowth',
			lowThresholdFactor = varargin{i+1};
 			if ~isnumeric(lowThresholdFactor),
 				error('Incorrect value for property ''lowThresholdFactor'' (type ''help FindRipples'' for details).');
 			end            
		otherwise,
