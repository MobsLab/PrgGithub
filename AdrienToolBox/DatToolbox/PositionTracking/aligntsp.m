function aligntsp(file,keepchannels)
% get timestamps and positions from tsp file
tspdataall=load([file '.tsp']);

%get rid of columns
channels=sort([keepchannels.*2 (keepchannels.*2)-1]);
tspdata=tspdataall(:,[1 channels+1]);
...
if (LEDchannels>0)
    ...
    for j=1:LEDchannels/2
        ...
        for k=1:size(ranges,1)
                interpTsp1kHz(tspdata(ranges(k,1),1)-tspdata(1,1)+1:tspdata(ranges(k,2),1)-tspdata(1,1),j*2)=-1;
                interpTsp1kHz(tspdata(ranges(k,1),1)-tspdata(1,1)+1:tspdata(ranges(k,2),1)-tspdata(1,1),j*2+1)=-1;
        end
    end
...
%     end
    ...
    whldata=zeros(size(whldatatemp,1),LEDchannels)-1;
    whldata(:,1:size(whldatatemp,2))=whldatatemp;
    
    %save it
    fid=fopen([file '.whl'],'w');
    if (size(keepchannels,2)==3)
        for i=1:size(whldata,1)
            fprintf (fid,'%i %i %i %i %i %i\n',whldata(i,:));
        end
    end
    
    if (size(keepchannels,2)==2)
        for i=1:size(whldata,1)
            fprintf (fid,'%i %i %i %i\n',whldata(i,:));
        end
    end
     
     if (size(keepchannels,2)==1)
        for i=1:size(whldata,1)
            fprintf (fid,'%i %i\n',whldata(i,:));
        end
    end
