...
#     # Assign unique colors to each mouse
#     colors = plt.cm.get_cmap("tab10", len(mice))
...
#         x_positions = np.arange(len(parameters))  # Positions for "slope", "op_point", "tau"
#         plt.scatter(x_positions, mouse_data["Value"], color=colors(i), alpha=0.8, label=mouse)
#         plt.plot(x_positions, mouse_data["Value"], color=colors(i), linestyle="dashed", alpha=0.6)

#         # Store mouse color for legend
#         mouse_legend[mouse] = colors(i)
...
#     plt.show()
    ...
    # return mouse_legend  # Return dictionary of colors assigned to mice
