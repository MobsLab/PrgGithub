function [M1,M2,T1,T2,OscBurstEvt,OscBurst,maps1,data1,stats1,maps2,data2,stats2]=CompTwoChannelsOscBurst(LFP,v,Epoch,v2,freqBurst,durBurst,vnoise)
...
FilRip=FilterLFP(Restrict(LFP{v},Epoch),freqBurst,512);
rgFil=Range(FilRip,'s');
filtered=[rgFil-rgFil(1) Data(FilRip)];

FilRip2=FilterLFP(Restrict(LFP{v2},Epoch),freqBurst,512);
...
if vnoise>0
        ...
        NoiseRip=FilterLFP(Restrict(LFP{vnoise},Epoch),freqBurst,512);
        ...
        [OscBurst,stdev,noise] = FindRipples(filtered,'thresholds',paramRip,'durations',[40 durBurst] ,'noise',filteredNoise,'show','off');

else  
%         try
%         [ripples,stdev,noise] = FindRipples(filtered,'thresholds',paramRip,'restrict',[debPeriodRip finPeriodRip]);
%         catch
        [OscBurst,stdev,noise] = FindRipples(filtered,'thresholds',paramRip,'durations',[40 durBurst]);    
%         end
...
[maps1,data1,stats1] = RippleStats(filtered,OscBurst);
[maps2,data2,stats2] = RippleStats(filtered2,OscBurst);
if PloStat
PlotRippleStats(OscBurst,maps1,data1,stats1)
PlotRippleStats(OscBurst,maps2,data2,stats2)
...
OscBurst(:,1:3)=OscBurst(:,1:3)+rgFil(1);

[M1,T1]=PlotRipRaw(LFP{v},OscBurst,durBurst(2));title(['Voie LFP ',num2str(v)])
    subplot(1,2,1), ylabel(['Ripples channel ', num2str(v), ', detected channel ', num2str(v)]) 
    
[M2,T2]=PlotRipRaw(LFP{v2},OscBurst,durBurst(2));title(['Voie LFP ',num2str(v2)])
    subplot(1,2,1), ylabel(['Ripples channel ', num2str(v2), ', detected channel ', num2str(v)]) 

%----------------------------------------
%----------------------------------------

 OscBurstEvt=intervalSet((OscBurst(:,2)+rgFil(1)-0.1)*1E4,(OscBurst(:,2)+rgFil(1)+0.1)*1E4);    
