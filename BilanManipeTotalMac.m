...
filename='/Users/Bench/Documents/Data/';
cd(filename)
end
...
if Analy==1&allBefore==1&tr==4&LimiTrials==1&Protocol==4& renorm==1& renormAll==0& Bis==0
    Res1{1}=Res1{1}([2 1 3]);
    Res1{3}=Res1{3}([2 1 3]);

    Res2{1}=Res2{1}([2 1 3 4]);
    Res2{3}=Res2{3}([2 1 3 4]);

    Res3{1}=Res3{1}([2 1 3 4]);
    Res3{3}=Res3{3}([2 1 3 4]);

    Res4{1}=Res4{1}([2 3 1 4]);
    Res4{3}=Res4{3}([2 3 1 4]);
end
...
%
    ...
    group{1}=GA;
    group{2}=GB;
    [p,t,st] = anovan(X, group,'model','interaction', 'display','on');
    % [anvA, anvB, anvAB] = TwoWayAnova(X, GA, GB);
...
% 
% [p,h,stat]=ranksum(Res1{1}, Res1{2},'method','exact')
% [p,h,stat]=ranksum(Res2{1}, Res2{2},'method','exact')
% [p,h,stat]=ranksum(Res3{1}, Res3{2},'method','exact')
% [p,h,stat]=ranksum(Res4{1}, Res4{2},'method','exact')
...
GA=[ones(size(A,1),1); 2*ones(size(B,1),1); 3*ones(size(C,1),1)];
X=[A;C;D];

[p,t,st] = anova1(X, GA,'off');
disp(' ')
disp('Small')
disp(' ')
disp(['t value: ',t(2,5)])
disp(['p anova: ',num2str(p)])
[h,p,c,statt]=ttest2(A,B);disp(['p:',num2str(p),' t:',num2str(statt.tstat),' df:',num2str(statt.df)])
[h,p,c,statt]=ttest2(A,C);disp(['p:',num2str(p),' t:',num2str(statt.tstat),' df:',num2str(statt.df)])
[h,p,c,statt]=ttest2(B,C);disp(['p:',num2str(p),' t:',num2str(statt.tstat),' df:',num2str(statt.df)])
...
disp(' ')
[p,t,st] = anova1(X, GA,'off');
    disp('F one-way anova')
disp(['t value: ',t(2,5)])
disp(['p anova: ',num2str(p)])
[h,p,c,statt]=ttest2(A,C);disp(['p:',num2str(p),' t:',num2str(statt.tstat),' df:',num2str(statt.df)])
[h,p,c,statt]=ttest2(A,D);disp(['p:',num2str(p),' t:',num2str(statt.tstat),' df:',num2str(statt.df)])
[h,p,c,statt]=ttest2(C,D);disp(['p:',num2str(p),' t:',num2str(statt.tstat),' df:',num2str(statt.df)])
...
disp(' ')
  ...
  %  PlotErrorBar([A,B,C,D])

    group{1}=GA;
    group{2}=GB;
    [p,t,st] = anovan(X, group,'model','interaction', 'display','off');
    % [anvA, anvB, anvAB] = TwoWayAnova(X, GA, GB);
    disp('F anova interaction model')
disp(t(2,5))
disp(t(3,5))
disp(t(4,5))

    disp(['Small area, trial number effect: ',num2str(p(1)), ', Before/After effect: ', num2str(p(2)), ', Interaction: ',num2str(p(3))])
    disp(' ')
...
disp(t(4,5))
    ...
    disp(['Large area, trial number effect: ',num2str(p(1)), ', Before/After effect: ', num2str(p(2)), ', Interaction: ',num2str(p(3))])
    disp(' ')
    
figure('color',[1 1 1]), plot([1 1 1 1],AliM,'ko','markerfacecolor','k'), hold on, plot([2 2 2.05 2],BliM,'ko','markerfacecolor','k'),xlim([0 3]),ylim([0 1.4])
