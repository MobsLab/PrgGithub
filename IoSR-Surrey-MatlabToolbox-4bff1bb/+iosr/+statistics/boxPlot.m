...
%                             'auto' (see boxColor).
%       method              - The method used to calculate the quantile2s,
%                             labelled according to
%                             http://en.wikipedia.org/wiki/quantile2. The
%                             default is 'R-8' whereas the default for
%                             Matlab is 'R-5'. See
%                             IOSR.STATISTICS.quantile2.
...
%                             IOSR.STATISTICS.quantile2). The fields are:
...
% 
%   See also IOSR.STATISTICS.TAB2BOX, IOSR.STATISTICS.quantile2, COLORMAP,
...
%   Copyright 2016 University of Surrey.
    ...
    properties (AbortSet)
       ...
       method = 'R-8'               % The method used to calculate the quantile2s.
    ...
    methods
        ...
        %       ylabel('MPG')
        %
        %     Example 6: Box plot calculated from weighted quantile2s
        %
        %       % load data
        %       load carbig
        %       
    ...
    methods (Access = protected)
        ...
        %CALCULATESTATS calculate the statistic for the box plot
            ...
            for n = 1:prod(obj.outDims)
                ...
                subidxAll{1} = ':';
                if ~isempty(obj.addPrctiles)
                    obj.statistics.addPrctiles(subidxAll{:}) = iosr.statistics.quantile2(obj.y(subidxAll{:}),obj.addPrctiles(:)./100,[],obj.method,obj.weights(subidxAll{:}));
                end
                obj.statistics.PL(subidx{:}) = iosr.statistics.quantile2(obj.y(subidxAll{:}),min(obj.percentile)/100,[],obj.method,obj.weights(subidxAll{:}));
                obj.statistics.PU(subidx{:}) = iosr.statistics.quantile2(obj.y(subidxAll{:}),max(obj.percentile)/100,[],obj.method,obj.weights(subidxAll{:}));
            end
