...
%QQPLOT quantile2-quantile2 plot with patch option
%
%   IOSR.STATISTICS.QQPLOT(Y) displays a quantile2-quantile2 plot of the
%   sample quantile2s of Y versus theoretical quantile2s from a normal
%   distribution. If the distribution of Y is normal, the plot will be
%   close to linear.
% 
%   IOSR.STATISTICS.QQPLOT(X,Y) displays a quantile2-quantile2 plot of two
...
%   'R-8'. Type 'help iosr.statistics.quantile2' for more information. The
...
% 
%   See also IOSR.STATISTICS.quantile2, IOSR.STATISTICS.BOXPLOT.
...
%   Copyright 2016 University of Surrey.
    ...
    switch sum(IXn)
        ...
        case 1
            ...
            x_label = 'Standard normal quantile2s';
            y_label = 'Sample quantile2s';
        case 2
            ...
            assert(isequal(size(X),size(Y)), 'iosr:quantile2:invalidInput', 'Input data must be the same size')
            x_label = 'X quantile2s';
            y_label = 'Y quantile2s';
        otherwise
    ...
    % quartiles
    q1x = iosr.statistics.quantile2(X,.25,[],method);
    q3x = iosr.statistics.quantile2(X,.75,[],method);
    q1y = iosr.statistics.quantile2(Y,.25,[],method);
    q3y = iosr.statistics.quantile2(Y,.75,[],method);
