function varargout=sigstarh(groups,stats,nosort)
    % sigstarh - Add significance stars to HORIZONTAL bar charts, boxplots, line charts, etc,
    %
    % H = sigstarh(groups,stats,nsort)
    %
    % Purpose
    ...
    %          either as pairs of scalars indicating locations along the y axis or as 
    %          strings corresponding to y-tick labels. Groups can be a mixture of both 
    %          definition types.
    ...
    % 2. 
    % bar([5,2,1.5])
    % sigstarh({[2,3],[1,2], [1,3]},[nan,0.05,0.05])
    %
    ...
    % subplot(1,2,1)
    % bar(y)
    % sigstarh({[1,2], [2,3], [4,5]})
    % subplot(1,2,2)
    % bar(y)
    % sigstarh({[2,3],[1,2], [4,5]})
    %
    ...
    %1. A pair of indices.
    %2. A pair of strings (in cell array) referring to y-Tick labels
    ...
    ylocs=nan(length(groups),2); %matrix that will store the indices 
    ...
    for ii=1:length(groups)
        ...
        if isnumeric(grp)
            ylocs(ii,:)=grp; %Just store the indices if they're the right format already

        elseif iscell(grp) %Handle string pairs or string/index pairs

            if isstr(grp{1})
                a=strmatch(grp{1},ytl);
            elseif isnumeric(grp{1})
                a=grp{1};
            end
            if isstr(grp{2})
                b=strmatch(grp{2},ytl);
    ...
    if ~nosort
        ...
        ylocs=ylocs(ind,:);groups=groups(ind);
        stats=stats(ind);
    end
    ...
    x=xlim;
    xd=myRange(x)*0.05; %separate sig bars vertically by 5% 

    for ii=1:length(groups)
        thisX=findMinY(ylocs(ii,:))+xd;
        H(ii,:)=makeSignificanceBar(ylocs(ii,:),thisX,stats(ii));
    end
    %-----------------------------------------------------
    ...
    %being extra cautious and leaving this it to the end just in case the x limits
    %of the graph have changed as we add the highlights. The ticks are set as a
    %proportion of the x axis range and we want them all to be the same the same
    %for all bars.
    xd=myRange(xlim)*0.01; %Ticks are 1% of the x axis range
    for ii=1:length(groups)
        x=get(H(ii,1),'XData');
        x(1)=x(1)-xd;
        x(4)=x(4)-xd;   
...
function H=makeSignificanceBar(y,x,p)
    ...
    starX=mean(x)+myRange(xlim)*offset;
...
function x=findMinY(y)
    % The significance bar needs to be plotted a reasonable distance above all the data points
    % found over a particular range of y values. So we need to find these data and calculat the 
    % the minimum x value needed to clear all the plotted data present over this given range of 
    % y values. 
    %
    ...
    axis(gca,'tight')
    set(gca,'ylim',y) %Matlab automatically re-tightens x-axis

    xLim = get(gca,'XLim'); %Now have max x value of all elements within range.
    x = max(xLim);
