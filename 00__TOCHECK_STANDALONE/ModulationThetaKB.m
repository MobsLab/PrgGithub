...
function [ph,mu, Kappa, pval,B,C]=ModulationThetaKB(tsa,EEGf,Epoch,nmbBin,plo)

% [ph,mu, Kappa, pval,B,C]=ModulationTheta(tsa,EEGf,Epoch,nmbBin,plo)
% INPUT
...
if plo==1
    ...
    hold on
    bar(B,2*nmbBin*C/sum(C),1, 'facecolor','k')
    bar(B+2*pi,2*nmbBin*C/sum(C),1, 'facecolor','k')
