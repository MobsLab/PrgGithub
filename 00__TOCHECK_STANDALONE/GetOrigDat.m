...
%     Properties    Values
%    -------------------------------------------------------------------------
%     'intervals'   list of time intervals to read from the LFP file
...
%    lfp = GetLFP(5,'intervals',[0 120]);
%    % same, plus from 240.2 to 265.23 seconds
%    lfp = GetLFP(5,'intervals',[0 120;240.2 265.23]);
%    % multiple channels
%    lfp = GetLFP([1 2 3 4 10 17],'intervals',[0 120]);
%    % channel # 3 (= ID 2), from 0 to 120 seconds
%    lfp = GetLFP(3,'intervals',[0 120],'select','number');
...
function [lfp,indices] = GetOrigDat(channels,varargin)
...
for i = 1:nIntervals,
	...
	start = intervals(i,1);
	% Load data
	data = LoadBinary(filename,'duration',duration,'frequency',DATA.rates.dat,'nchannels',nChannels,'start',start,'channels',channels);
	t = start:(1/DATA.rates.dat):(start+(length(data)-1)/DATA.rates.dat);t=t';
	lfp = [lfp ; t data];
