function [map,mapS,stats,px,py,FR,sizeFinal,PrField,C,ScField,pfH,pf]=PlaceField(tsa,XS,YS,varargin)

         

%  OUTPUT
%    [map,mapS,stats,px,py,FR,sizeFinal,PrField,C,ScField,pfH,pf]
%    map.x          x bins
%    map.y          y bins
              ...
              switch(lower(varargin{i})),
                ...
                case 'video',
                  ...
                  if ~isa(freqVideo,'numeric'),
                    error('Incorrect value for property ''video'' (type ''help PlaceField'' for details).');
                  end
                ...
                case 'plotresults',
                  ...
                  if ~(plotresults == 1 || plotresults == 0)
                      error('Incorrect value for property ''plotresults'' (type ''help PlaceField'' for details).');
                  end
...
stats.sparsity=stat.sparsity;
            ...
            C=GravityCenter(stats.field.*map.rate);
            GC(i,1)=C(1);
            GC(i,2)=C(2);
...
valXmin=(max(Data(XS))-min(Data(XS)))/20;
valYmin=(max(Data(XS))-min(Data(XS)))/20;
subplot(3,2,4) , plot(Data(XS),Data(YS),'Color',[0.8 0.8 0.8])
hold on, plot(px,py,'r.')
xlim([min(Data(XS))-valXmin max(Data(XS))+valXmin])
ylim([min(Data(YS))-valYmin max(Data(YS))+valYmin])
rg=Range(XS,'s');
...
yl=ylim;
plot(Range(tsa,'s'),yl(2)*ones(length(Range(tsa,'s')),1),'r.')
