...
try
    ...
    for i=0.2:0.2:4
        ...
        [m(a,:),~,t]=mETAverage(tDelta,Range(Qt),Data(Qt),10,200);
        [l(a,:),~,tt]=mETAverage(tDelta,Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
        nb(a)=length(tDelta);
        a=a+1;
    end
    ...
    for i=0.2:0.2:4
        ...
        de=Range(tDelta);
        [m2(a,:),~,~]=mETAverage(de(2:end-1),Range(Qt),Data(Qt),10,200);
        [l2(a,:),~,~]=mETAverage(de(2:end-1),Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
        nb2(a)=length(tDelta);
        a=a+1;
    end
    ...
    if prof
        a=1;
        for i=0.2:0.2:4
            [tDelta,~]=FindDeltaWavesSingleChanGL('PFCx_sup',SWSEpoch,prof,0,i,75,[1 5]);
            de=Range(tDelta);
            [m3(a,:),~,~]=mETAverage(de(2:end-1),Range(Qt),Data(Qt),10,200);
            [l3(a,:),~,~]=mETAverage(de(2:end-1),Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
            nb3(a)=length(tDelta);
            a=a+1;
        end
    end
    ...
    [mD1,~,tD1]=mETAverage((Start(Down)+End(Down))/2,Range(Qt),Data(Qt),10,200);
    [mD2,~,~]=mETAverage(Start(Down),Range(Qt),Data(Qt),10,200);
    [mD3,~,~]=mETAverage(End(Down),Range(Qt),Data(Qt),10,200);

    [mDlfp1,~,~]=mETAverage((Start(Down)+End(Down))/2,Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
    [mDlfp2,~,~]=mETAverage(Start(Down),Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
    [mDlfp3,~,~]=mETAverage(End(Down),Range(LFPd),Data(LFPd)-Data(LFPs),10,200);


    [mD1b,~,tD1b]=mETAverage((Start(Down2)+End(Down2))/2,Range(Qt),Data(Qt),10,200);
    [mD2b,~,~]=mETAverage(Start(Down2),Range(Qt),Data(Qt),10,200);
    [mD3b,~,~]=mETAverage(End(Down2),Range(Qt),Data(Qt),10,200);

    [mDlfp1b,~,~]=mETAverage((Start(Down2)+End(Down2))/2,Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
    [mDlfp2b,~,~]=mETAverage(Start(Down2),Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
    [mDlfp3b,~,~]=mETAverage(End(Down2),Range(LFPd),Data(LFPd)-Data(LFPs),10,200);
...
if prof==0
    ...
    [~,NumNeurons]=GetSpikesFromStructure('PFCx');
    NbNumNeu=length(NumNeurons);
    [~,Qt,~,~,~,~,~,~,~,~,~]=FindDown(S,NumNeurons,SWSEpoch,10,0.01,1,1,[0 70],1);close
