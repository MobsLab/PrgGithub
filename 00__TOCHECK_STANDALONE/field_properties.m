function [center, fsize, m_fr, s_fr,rf] = field_properties (fr, probav,threshold)
...
if fsize > 0
    ...
    rf2 = rf{idmax};
    rf2 = sub2ind(size(fr), rf2(:,1), rf2(:,2));
    nbdim = ndims(fr);
    ind(nbdim).c = [];
    [ind.c] = ind2sub(size(fr), rf2);
    I = [ind.c] .* repmat(fr(rf2), 1, nbdim);
    center = sum(I) ./ sum (fr(rf2));
else
    center = [0, 0];
end
