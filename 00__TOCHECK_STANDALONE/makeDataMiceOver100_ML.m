...
%                                                
                                                ...
                                                %--------------------------
                                                % modified by Marie 2014-02-17:
%                 
%                                                 PosC2=[ones(3,1)*PosC(1,2:3); PosC(:,2:3); ones(3,1)*PosC(end,2:3)];
%                                                 PosT{i}=resample(PosC2(:,1:2),floor(FreqVideo/FreqObs{i}*1E4),1E4);                     
%                 
                                                if ~isempty(Postemp), tlpt=max(Postemp(:,1)); else tlpt=0;end
                                                Postemp=[Postemp;[PosC(21:end-20,1)+tlpt,PosC(21:end-20,2:3)]];


                                        end
                                        X_TSD=tsd(Postemp(:,1),Postemp(:,2));
                                        Y_TSD=tsd(Postemp(:,1),Postemp(:,3));
                                        
                                        N=floor(FreqVideo/FreqObs{i}*1E4)/1E4;
                                        t_TSD=ts(Postemp(1,1):(Postemp(end,1)-Postemp(1,1))/(N*size(Postemp,1)-1):Postemp(end,1));
                                        
                                        Postemp=[Data(t_TSD),Data(Restrict(X_TSD,t_TSD)),Data(Restrict(Y_TSD,t_TSD))];
