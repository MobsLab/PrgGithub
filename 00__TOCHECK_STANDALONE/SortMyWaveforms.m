function [UnitID,AllParams,WFInfo]=SortMyWaveforms(WFMat,DropBoxLocation,PlotOrNot)
...
AllParamsNew=AllParams(end-size(WFMat,2)+1:end,:);
DatNew=Dat(:,end-size(WFMat,2)+1:end);
...
if PlotOrNot
    ...
    legend({'Int','PN','ambig','ambig','Int new','PN new'})
    ...
    if ~isempty(PyrNeurons)
        plot(DatNew(:,UnitID==1),'c'),
        plot(DatNew(:,UnitID==0.5),'k'),
    end
