function BulbSleepScriptKB(RemovPreEpoch)
...
r=Spectro{2}*1E4;
    ...
    while ok==0|i==50
        try
            eval(['load(''LFPData/LFP',num2str(i),'.mat'');'])
            LFP;
            ok=1;
        catch
            i=i+1;
...
% try
%     eval(['load(''SpectrumDataH/Spectrum',num2str(chB)])
%     eval(['load(''SpectrumDataL/Spectrum',num2str(chH)])
% catch
    try
