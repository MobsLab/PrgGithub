%RefSubtraction_multi_AverageChans - for binary file(.dat): the values of specified channels will
...
function RefSubtraction_multi_AverageChans(filename,nChannels,nSouris,varargin)
...
for i=1:4:nSouris*4
    ...
    if length(varargin{i+2})~=0
    if ~(isa(varargin{i+2},'double') & round(varargin{i+2})==varargin{i+2})
        error(['format error for chs_ref, for mouse ',num2str(indice)]);
    end
    end
    
    if length(varargin{i+3})~=0
    ...
    if ~((length(varargin{i+1})==0 & length(varargin{i+2})==0 & length(varargin{i+3}~=0)) || (length(varargin{i+1})~=0 & length(varargin{i+2})>0))
    ...
    for j = 1:nChunks,
        ...
        for i=1:nSouris %preceed individually every mouse data and write with preceeded data and data to keep into a same file
            if length(chs_ref{i})~=0 %if length===0, means no treatement needed
                d(:,chs_to_preceed{i})=d(:,chs_to_preceed{i})-mean(d(:,chs_ref{i}),2)*ones(1,length(chs_to_preceed{i})); % to have the same dimension by using a multiplication of ones
