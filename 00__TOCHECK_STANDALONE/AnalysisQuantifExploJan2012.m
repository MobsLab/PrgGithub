...
smo=3;
sizeMap=50;
tpsTh=0.2*1E4; %0.75*1E4
Limdist=11; %11

Vth=20;
NumNeuron=nan; % 0 si pas de place field, 6 pr M17-20110622
...
if sav==1
        load xyMax xMaz yMaz
        save ParametersAnalyseICSS X Y S stim limMaz limM limMaze xMaz yMaz o M N varargin
end
%close all
...
if isnan(NumNeuron)
    ...
    if NEWanalysis
    E2=subset(ICSSEpoch,o);
    if length(o)>1
        st=Start(E2);
        en=End(E2);
        stTemp=Range(Restrict(stim,subset(E2,1)));
        st(1)=stTemp(1);
        E2=intervalSet(st,en);
    else
        st=Range(Restrict(stim,E2));
        en=End(E2);
        E2=intervalSet(st(1),en);
    end
    XS=Restrict(X,E2);
    YS=Restrict(Y,E2);
    stimS=Restrict(stim,E2);
    end
...
%     [mapS,mapSs,statsS,pxS,pyS,Fr,si,PF,centre]=PlaceField(stimS,XS,YS,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)],'smoothing',smo,'threshold',thPF);
    [mapS,mapSs,statsS,pxS,pyS,Fr,si,PF,centre]=PlaceField(stimS,XS,YS,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ],'smoothing',smo,'threshold',thPF);
...
%     YS=Y;
    ...
    spike=Restrict(stim,Epoch);
%     [mapS,mapSs,statsS,pxS,pyS,Fr,si,PF,centre]=PlaceField(spike,Restrict(X,Epoch),Restrict(Y,Epoch),'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)],'smoothing',smo,'threshold',thPF);
    [mapS,mapSs,statsS,pxS,pyS,Fr,si,PF,centre]=PlaceField(spike,Restrict(X,Epoch),Restrict(Y,Epoch),'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ],'smoothing',smo,'threshold',thPF);    
    hold on; title(['spike detection']);
...
%                     st=Start(E2);
%                     en=End(E2);
%                     stTemp=Range(Restrict(spike,subset(E2,1)));
%                     st(1)=stTemp(1);
%                     E2=intervalSet(st,en);
%                 else
%                     st=Range(Restrict(spike,E2));
%                     en=End(E2);
%                     E2=intervalSet(st(1),en);
%                 end
%                 Epoch=E2;
...
%[Oc1,OcS1,OcR1,OcRS1]=OccupancyMapKB(X1,Y1,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)]);close%
[Oc1,OcS1,OcR1,OcRS1]=OccupancyMapKB(X1,Y1,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ]);close

% [Oc2,OcS2,OcR2,OcRS2]=OccupancyMapKB(X2,Y2,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)]);close
% [OcS,OcSS,OcRS,OcRSS]=OccupancyMapKB(XS,YS,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)]);close

[Oc2,OcS2,OcR2,OcRS2]=OccupancyMapKB(X2,Y2,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ]);close
[OcS,OcSS,OcRS,OcRSS]=OccupancyMapKB(XS,YS,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ]);close

try
%     [OcT,OcST,OcRT,OcRST]=OccupancyMapKB(Restrict(X,TrackingEpoch),Restrict(Y,TrackingEpoch),'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz) ]);close
    [OcT,OcST,OcRT,OcRST]=OccupancyMapKB(Restrict(X,TrackingEpoch),Restrict(Y,TrackingEpoch),'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz)]);close    
catch
    
    [OcT,OcST,OcRT,OcRST]=OccupancyMapKB(X,Y,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ]);close
%     [OcT,OcST,OcRT,OcRST]=OccupancyMapKB(X,Y,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)]);close
end
...
dxm=rescale([min(xMaz) ;Data(X) ;max(xMaz)],0,si);
dym=rescale([min(yMaz) ;Data(Y) ;max(yMaz)],0,si);
...
%         [Oc,OcS,OcR,OcRS]=OccupancyMapKB(Xij,Yij,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(yMaz)-min(yMaz) 0 max(xMaz)-min(xMaz)]);close
       [Oc,OcS,OcR,OcRS]=OccupancyMapKB(Xij,Yij,'axis',[0 15],'smoothing',smo,'size',sizeMap,'limitmaze',[0 max(xMaz)-min(xMaz) 0 max(yMaz)-min(yMaz) ]);close
        ...
        try
            ...
            StdAngle(i,j)=nanstd(Data(Restrict(Angij,trajdir)));
            
            if j==1
                StdAnglePre(nnbPre)=nanstd(Data(Restrict(Angij,trajdir)));
               % nnbPre=nnbPre+1;
            else
                StdAnglePost(nnbPost)=nanstd(Data(Restrict(Angij,trajdir)));                
                %nnbPost=nnbPost+1;
            end
        ...
        try
        QuadrantTimePre(nnbPre,1)=length(Data(Restrict(Xij,zoneNE)))/30;
        catch
        QuadrantTimePre(nnbPre,1)=0;    
 %       QuadrantTimePre(nnbPre,(j-1)*4+1)=0;            
        end
        try
        QuadrantTimePre(nnbPre,(j-1)*4+2)=length(Data(Restrict(Xij,zoneSE)))/30;
        catch
        QuadrantTimePre(nnbPre,2)=0;    
%         QuadrantTimePre(nnbPre,(j-1)*4+2)=0;  
        end
        try
        QuadrantTimePre(nnbPre,3)=length(Data(Restrict(Xij,zoneNW)))/30;
        catch
        QuadrantTimePre(nnbPre,3)=0;    
        end
        try
        QuadrantTimePre(nnbPre,4)=length(Data(Restrict(Xij,zoneSW)))/30;
        catch
        QuadrantTimePre(nnbPre,4)=0;    
        end
        
        try
        QuadrantTimePost(nnbPost,1)=length(Data(Restrict(Xij,zoneNE)))/30;
        catch
        QuadrantTimePost(nnbPost,1)=0;    
        end
        try
        QuadrantTimePost(nnbPost,2)=length(Data(Restrict(Xij,zoneSE)))/30;
        catch
        QuadrantTimePost(nnbPost,2)=0;    
        end
        try
        QuadrantTimePost(nnbPost,3)=length(Data(Restrict(Xij,zoneNW)))/30;
        catch
        QuadrantTimePost(nnbPost,3)=0;    
        end
        try
        QuadrantTimePost(nnbPost,4)=length(Data(Restrict(Xij,zoneSW)))/30;
        catch
        QuadrantTimePost(nnbPost,4)=0;    
        end
