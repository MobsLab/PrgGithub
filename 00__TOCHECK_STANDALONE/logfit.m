%% function [slope, intercept,MSE, R2, S,P] = logfit(x,y,varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [graphType,slope, intercept,MSE, R2, S, P,extra] = logfit(x,y,varargin)
...
k=1;
if isempty(varargin)
    [graphType,slope, intercept,MSE, R2, S, P,extra] = findBestFit(x,y);
    return;
    ...
    switch lower(varargin{1}); % make all lowercase in case someone put in something different.
        ...
        case {'linear','lin'}
            graphType = 'linear';
        otherwise            
            [graphType,slope, intercept, MSE, R2, S, P,extra] = findBestFit(x,y,varargin{:});
            return;
    end
...
if isempty(x2fit) || isempty(y2fit)
    ...
    slope=NaN; intercept=NaN; MSE = NaN; R2= NaN; P=Nan;
...
% outputs shifted.
function [graphType, slope, intercept, MSE, R2, S,P,extra] = findBestFit(x,y,varargin)
...
for ii=1:4
    [a,b,c,MSE(ii),R2(ii)]=logfit(x,y,testList{ii},varargin{:});
end
warning('on')
...
[graphType,slope, intercept, MSE, R2, S,P,extra]=logfit(x,y,graphType,varargin{:});
