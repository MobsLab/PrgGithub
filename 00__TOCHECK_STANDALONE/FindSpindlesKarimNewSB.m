function [all_spindles, SWA, stdev, usedEpoch] = FindSpindlesKarimNewSB(LFP, freq, Epoch, showingORnot)
...
for i=1:length(Start(Epoch))
    ...
    try
        ...
        Filsp=FilterLFP(Restrict(LFP,subset(Epoch,i)),freq,512);
        ...
        [spindles,swa,stdev,~] = FindspindlesMarie(filtered,'durations',[100 350 200000],'stdev',vari,'thresholds',thfreq,'show',showingORnot);
        ...
        spindles(:,1:3)=spindles(:,1:3)+rgFilsp(1);
        all_spindles=[all_spindles;spindles];
        swa=swa+rgFilsp(1);
        SWA=[SWA;swa];
...
if length(all_spindles)<10
    ...
    for i=1:length(Start(Epoch))
        try
            ...
            %     [spiStartstemp, spiEndstemp, spiPeakstemp] = findSpindles(lfp, thspindles1, thspindles2);
            %[spiPeakstemp] = findSpindles2(lfp, thspindles1,thspindles2);
            Filsp=FilterLFP(Restrict(LFP,subset(Epoch,i)),freq,1024);
            rgFilsp=Range(Filsp,'s');
            filtered=[rgFilsp-rgFilsp(1) Data(Filsp)];
            % [ripples,stdev,noise] = FindRipples(filtered,'durations',[600 500
            % 6000],'thresholds',[2 7]);
            ...
            [spindles, swa, stdev, ~] = FindspindlesMarie(filtered,'durations',[100 350 200000],'thresholds',thfreq,'show',showingORnot);
            spindles(:,1:3)=spindles(:,1:3)+rgFilsp(1);
            all_spindles=[all_spindles;spindles];
            swa=swa+rgFilsp(1);
            SWA=[SWA;swa];
