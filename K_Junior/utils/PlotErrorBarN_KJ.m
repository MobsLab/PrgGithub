function [pval,eb,stats_out] = PlotErrorBarN_KJ(A,varargin)
...
if ~horizontal
    ...
    if isempty(barcolors)
        bar(x,R,'FaceColor',[0.3 0.3 0.3],bar_argin{:});
    elseif ~iscell(barcolors)
        bar(x,R,'FaceColor',barcolors,bar_argin{:});
    elseif length(barcolors)==1
            bar(x,R,'FaceColor',barcolors{1},bar_argin{:});
    else
        for i=1:length(R)
            bar(x(i), R(i), 'FaceColor', barcolors{i},bar_argin{:});
        end
    end
...
%horizontal
    ...
    %bar colors
    if isempty(barcolors)
        barh(x,R,'FaceColor',[0.3 0.3 0.3],'EdgeColor','k')
    elseif length(barcolors)==1
        barh(x,R,'FaceColor',barcolors{1},'EdgeColor','k')
    else
        for i=1:length(R)
            barh(x(i), R(i), 'FaceColor', barcolors{i},'EdgeColor','k');
        end
    end
