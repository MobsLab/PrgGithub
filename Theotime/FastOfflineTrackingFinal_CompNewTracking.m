function FastOfflineTrackingFinal_CompNewTracking(varargin)
    %% FastOfflineTrackingFinal_CompNewTracking
    % FastOfflineTrackingFinal_CompNewTracking is a GUI for tracking
    % animals in videos. It allows to track from Matlab generated frames
    % or from videos. It also allows to create a reference image and a
    % mask for the tracking. 

    % This one is fast compared to the original OfflineTrackingFinal_CompNewTracking because if 
    % you input an avi File and a duration, you then just have to create the ref, mask and let's go.

    % The GUI is divided into several sections:     
    % 1. Tracking: allows to track the animal in the video or frames
    % 2. Launch Batch: allows to launch a batch of videos for tracking
    % 3. Create Ref: allows to create a reference image and a mask for the
    %    tracking
    % Arguments:
    %   - varargin: optional arguments for the function
    %     'aviFile': the avi file to track
    %     'duration': the duration of the **recording** in seconds (comes from the amplifier.dat - caution the video is sped up by an unknown factor.)
...
% Parse the arguments
p = inputParser;
addParameter(p, 'aviFile', [], @(x) isempty(x) || ischar(x) || isstruct(x));
addParameter(p, 'duration', [], @(x) isempty(x) || isnumeric(x));
parse(p, varargin{:});
aviFile = p.Results.aviFile;
...
if ~isempty(aviFile) && ischar(aviFile)
    % Check if the aviFile is a valid .avi file
...
%                     end
        ...
        function stop_vid(~,~)
            ...
            if  length(choice)==13 %'Track offline'
                ...
                nanind=1E7;
                set(hand(7),'Visible','off');
                set(hand(8),'Visible','off');
                set(hand(9),'Visible','off');
            elseif length(choice)==2 % 'No'
                go=0;
                nanind=1E7;
                set(hand(7),'Visible','on');
                set(hand(9),'Visible','on');
        ...
        function GetVidInfo
            ...
            if ~isempty(aviFile)
                [PathName, nameFile, ext] = fileparts(aviFile);
            ...
            typical_size = 5*sqrt(l1*l2);
            if ~isempty(duration)
                durmax = inputdlg({'how long is recording in s'}, 'Prompt for duration' ,1, {num2str(duration)});
            else
                durmax = inputdlg({'how long is recording in s'}, 'Prompt for duration' ,1);
            end
