%BehaviorERC - Plot basic behavior comparisons of ERC experiment averaged across mice.
%
% Plot occupance in the shock zone in the PreTests vs PostTests
% Plot occupancies after reversal in the shock zone in the PreTests vs PostTests
...
clear all
dir_out = '/home/mickey/Dropbox/Mobs_member/Theotime_De_Charrin/Presentation/figures/';
...
clrs_MFB = {[0.466 0.674 0.188], [0 0.447 0.741], [0.85 0.325 0.098], [0.494 0.184 0.556],...
    [0.635 0.078 0.184], [0.3010 .745 .933], [0.929 .694 .125], 'r'};


% Axes
fh = figure('units', 'normalized', 'outerposition', [0 0 0.9 0.9]);
PreTest_Axes = axes('position', [0.03 0.07 0.14 0.38]);
Cond_Axes = axes('position', [0.19 0.07 0.14 0.38]);
PostTest_Axes = axes('position', [0.35 0.07 0.14 0.38]);
PreReversal_Axes = axes('position', [0.51 0.07 0.14 0.38]);
Reversal_Axes = axes('position', [0.67 0.07 0.14 0.38]);
PostReversal_Axes = axes('position', [0.83 0.07 0.14 0.38]);

PreTestTraj_Axes = axes('position', [0.03 0.54 0.14 0.38]);
CondTraj_Axes = axes('position', [0.19 0.54 0.14 0.38]);
PostTestTraj_Axes = axes('position', [0.35 0.54 0.14 0.38]);
PreReversalTraj_Axes = axes('position', [0.51 0.54 0.14 0.38]);
ReversalTraj_Axes = axes('position', [0.67 0.54 0.14 0.38]);
PostReversalTraj_Axes = axes('position', [0.83 0.54 0.14 0.38]);
...
a_MFB = load([Dir.path{2}{1} '/behavResources.mat'], 'behavResources');
...
numtesttmp = min(numtest, length(id_Pre));
id_Pre = id_Pre(1:numtesttmp);
numtesttmp = min(numtest, length(id_Cond));
id_Cond = id_Cond(1:numtesttmp);
numtesttmp = min(numtest, length(id_Post));
id_Post = id_Post(1:numtesttmp);
...
plot(maze(:,1),maze(:,2),'k','LineWidth',3)
plot(shockZone(:,1),shockZone(:,2),'r','LineWidth',3)
title('PAG Conditioning','FontSize',18,'FontWeight','bold');
xlim([0 1])
ylim([0 1])
% set(gca, 'FontSize', 18, 'FontWeight',  'bold');
...
%% SAME FOR MFB - PLOTTING REVERSAL NOW
%% Find indices of PreTests and PostTest session in the structure
id_Pre = zeros(1,numtest);
id_Cond = cell(1,numtest);
id_Post = cell(1,numtest);

id_Pre = zeros(1,length(a_MFB.behavResources));
id_Cond = zeros(1,length(a_MFB.behavResources));
id_Post = zeros(1,length(a_MFB.behavResources));
for k=1:length(a_MFB.behavResources)
    if ~isempty(strfind(a_MFB.behavResources(k).SessionName,'TestPre'))
        id_Pre(k) = 1;
    end
    if ~isempty(strfind(a_MFB.behavResources(k).SessionName,'Cond'))
        id_Cond(k) = 1;
    end
    if ~isempty(strfind(a_MFB.behavResources(k).SessionName,'TestPost'))
        id_Post(k) = 1;
    end
end
id_Pre=find(id_Pre);
...
% Calculate occupancy de novo
for k=1:length(id_Pre)
    for t=1:length(a_MFB.behavResources(id_Pre(k)).Zone)
        Pre_Occup(k,t)=size(a_MFB.behavResources(id_Pre(k)).ZoneIndices{t},1)./...
            size(Data(a_MFB.behavResources(id_Pre(k)).Xtsd),1);
    end
end
for k=1:length(id_Cond)
    for t=1:length(a_MFB.behavResources(id_Cond(k)).Zone)
        Cond_Occup(k,t)=size(a_MFB.behavResources(id_Cond(k)).ZoneIndices{t},1)./...
            size(Data(a_MFB.behavResources(id_Cond(k)).Xtsd),1);
    end
end
for k=1:length(id_Post)
    for t=1:length(a_MFB.behavResources(id_Post(k)).Zone)
        Post_Occup(k,t)=size(a_MFB.behavResources(id_Post(k)).ZoneIndices{t},1)./...
            size(Data(a_MFB.behavResources(id_Post(k)).Xtsd),1);
    end
end
Pre_Occup_Shock = squeeze(Pre_Occup(:,1));
Cond_Occup_Shock = squeeze(Cond_Occup(:,1));
...
%% Plot
% Trajectories
axes(PreReversalTraj_Axes);
for i=1:length(id_Pre)
    plot(Data(a_MFB.behavResources(id_Pre(i)).AlignedXtsd),Data(a_MFB.behavResources(id_Pre(i)).AlignedYtsd),...
        'LineWidth',3);
    hold on
end
box off
...
plot(maze(:,1),maze(:,2),'k','LineWidth',3)
plot(shockZone(:,1),shockZone(:,2),'r','LineWidth',3)
title('Pre Reversal','FontSize',18,'FontWeight','bold');
xlim([0 1])
ylim([0 1])
% set(gca, 'FontSize', 18, 'FontWeight',  'bold');

% Cond
axes(ReversalTraj_Axes);
for i=1:length(id_Cond)
    plot(Data(a_MFB.behavResources(id_Cond(i)).AlignedXtsd),Data(a_MFB.behavResources(id_Cond(i)).AlignedYtsd),...
        'LineWidth',3);
    hold on
    tempX = Data(a_MFB.behavResources(id_Cond(i)).AlignedXtsd);
    tempY = Data(a_MFB.behavResources(id_Cond(i)).AlignedYtsd);
    plot(tempX(a_MFB.behavResources(id_Cond(i)).PosMat(:,4)==1),tempY(a_MFB.behavResources(id_Cond(i)).PosMat(:,4)==1),...
        'p','Color','g','MarkerFaceColor','green','MarkerSize',16);
    clear tempX tempY
end
box off
set(gca,'XtickLabel',{},'YtickLabel',{});
...
title('MFB Conditioning','FontSize',18,'FontWeight','bold');
xlim([0 1])
ylim([0 1])
% set(gca, 'FontSize', 18, 'FontWeight',  'bold');

% PostTests
axes(PostReversalTraj_Axes);
for i=1:length(id_Post)
    plot(Data(a_MFB.behavResources(id_Post(i)).AlignedXtsd),Data(a_MFB.behavResources(id_Post(i)).AlignedYtsd),...
        'LineWidth',3);
    hold on
    if i <= length(id_Cond)
        tempX = Data(a_MFB.behavResources(id_Cond(i)).AlignedXtsd);
