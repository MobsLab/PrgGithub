...
try
    ...
    if restrict
        ...
        % LossPredTsd = Restrict(LossPredTsd, Moving);
        % V = Restrict(V, Moving);
    end
        ...
        LossPredTsd_list{imouse} = mean(Data(LossPredTsd));

end

%% Warning - this needs to be ran twice - you have to select the right version;
LossPredTsd_list
[LossNonMoving, VNonMoving] = MergeTsdFromCells(LossPredTsd_list, Vtsd_list);
[LossMoving, VMoving] = MergeTsdFromCells(LossPredTsd_list, Vtsd_list);

data = [LossMoving; LossNonMoving];
group = [ones(size(LossMoving)); 2*ones(size(LossNonMoving))];
% figure; boxplot(data, group, 'Labels', {'Moving', 'NonMoving'})

% Create the violin plot
figure;
violinplot(data, group); % Violin plot function
ax = gca; % Get current axis
ax.FontSize = 14; %
% Set group labels manually
xticks([1 2]);
xticklabels({'Moving', 'NonMoving'});
ylabel('LossPred (a.u.)', 'FontSize', 14)
mean_Moving = mean(LossMoving)
mean_NonMoving = mean(LossNonMoving)

hold on;
scatter(1, mean_Moving, 100, 'r', 'filled', 'd'); % Red diamond for x1 mean
scatter(2, mean_NonMoving, 100, 'r', 'filled', 'd'); % Red diamond for x2 mean
text(1, mean_Moving, sprintf('%.2f', mean_Moving), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center', 'FontSize', 12, 'Color', 'k');
text(2, mean_NonMoving, sprintf('%.2f', mean_NonMoving), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center', 'FontSize', 12, 'Color', 'k');
title('Distribution of Loss Prediction in Moving vs Non-Moving - no Sleep epochs')
hold off;
