...
%                   (default = [-0.5 0.5])
%     'saveplot'    Save both plots (default = 0)
%    =========================================================================
%
%   saveplot added by Dmitri Bryzgalov (13.04.18)
%
%  SEE
%
...
for i = 1:2:length(varargin),
	...
	switch(lower(varargin{i})),
        ...
        case 'saveplot',
			...
			if isdscalar(saveplot,'==0','==1'),
				error('Incorrect value for property ''saveplot'' (type ''help <a href="matlab:help PlotRippleStats">PlotRippleStats</a>'' for details).');
			end
		otherwise,
...
for electrode = 1:nElectrodes,
    ...
    if saveplot == 1
        saveas(f, [res '/Ripplelook.fig']);
        saveFigure(f,'Ripplelook',res);
        close(f);
    end
...
%  	b = bar(((0:nCorrBins)-nCorrBins/2)/1000,stats{electrode}.acg.data);xlim([-nCorrBins nCorrBins]/2000);set(b,'FaceColor',[0 0 0]);xlabel('Autocorrelogram');
    ...
    if saveplot == 1
        saveas(f, [res '/Ripplestats.fig']);
        saveFigure(f,'Ripplestats',res);
        close(f);
    end
end
