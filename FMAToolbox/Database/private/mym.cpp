...
void mexFunction(int nlhs, mxArray*plhs[], int nrhs, const mxArray*prhs[]) {
  ...
  if (!runonce) {
    runonce = true;
    mexPrintf("mYm v1.0.8, Copyright (C) 2006, Swiss Federal Institute of technology, Lausanne, CH\n");
    ...
    const unsigned nex = nrhs-jarg-1;	// expected number of placeholders
      ...
      Bytef* pcmp = (Bytef*)mxCalloc(cmp_buf_len, sizeof(Bytef));
        ...
        if (pec[i]&&plen[i]>MIN_LEN_ZLIB) {
		  ...
		  //following 3 lines were  used to replace
		  //compressBound function
		  //which seems have version conflicts
          ulong sourceLen1 = plen[i];
          sourceLen1 = sourceLen1 + (sourceLen1 >> 12) + (sourceLen1 >> 14) + 11;
          const uLongf max_len = sourceLen1;
		  //original: const uLongf max_len = compressBound(plen[i]);
