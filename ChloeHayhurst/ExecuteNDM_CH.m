...
for f=1:length(FolderName)
    ...
    disp(FolderName{f})
    load('ExpeInfo.mat')
%     NewFolderName=strrep(FolderName{f}(max(findstr(FolderName{f},'NicotineETM')):end),'/','_');
        NewFolderName=strrep(FolderName{f}(max(findstr(FolderName{f},'HeadRestraint')):end),'/','_');
