function [pval , stats_out] = MakeSpreadAndBoxPlot2_CH(A,Cols,X,Legends,varargin)
% Input variables
...
for i = 1:2:length(varargin)
    ...
    switch(lower(varargin{i}))
        ...
        case 'displaypval'
            displaypval = varargin{i+1};
            if displaypval~=0 && displaypval ~=1
        error('Incorrect value for property ''displaypval''.');
    end
        otherwise
...
if paired
    ...
    for k= 1: length(A)-1
        ...
        temp=A{k}(and(~isnan(A{k}) , ~isnan(A{k+1})));
        ind = and(~isnan(A{k}) , ~isnan(A{k+1}));
        try
            line([ones(1,sum(ind))*X(k)+.3 ; ones(1,sum(ind))*X(k+1)-.3],[A{k}(ind)' ; A{k+1}(ind)'], 'color',[.7 .7 .7])
        catch
            line([ones(1,sum(ind))*X(k)+.3 ; ones(1,sum(ind))*X(k+1)-.3],[A{k}(ind) ; A{k+1}(ind)], 'color',[.7 .7 .7])
        end
    end
        ...
        if strcmp(optiontest,'ranksum')
            ...
            for c=1:length(column_test)
                ...
                if sum(~isnan(A{i}))>2 && sum(~isnan(A{j}))>2
                    ...
                    if displaypval
                        y_max = max([MaxA(i), MaxA(j)]);
                        text(mean([x(i), x(j)]), y_max * 1.05, sprintf('p=%.2f', p), ...
