%PlotRipRaw
%
% [M,T] = PlotRipRaw_SB(LFP, events, durations, cleaning, PlotFigure,NewFig)
%
%
%%INPUT
% events : in seconds
...
function [M,T] = PlotRipRaw_CH(LFP, events, durations, cleaning, PlotFigure, NewFig)
...
%         plot(((1:nBins)'-ceil(nBins/2))/nBins*diff(durations),mean(T)-std(T),'r--');
%         title(['raw data n=' num2str(size(events,1))])
%         xlim(durations)
        
%         subplot(1,2,2), hold on
%         plot(((1:nBins)'-ceil(nBins/2))/nBins*diff(durations), zscore(T')', 'k'); hold on
        plot(((1:nBins)'-ceil(nBins/2))/nBins*diff(durations), mean(zscore(T')'), 'r', 'linewidth',2); hold on
