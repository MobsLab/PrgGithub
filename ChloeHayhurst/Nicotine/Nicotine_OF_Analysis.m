...
for group = 1:3
    ...
    for sess=1:2
        for mouse=1:length(Mouse_names)
            ...
            [~,Distance,~,~,~] = Thigmotaxis_OF_CH(AlignedXtsd, AlignedYtsd,'OF','figure',0,'percent_inner',0.7);
...
x = time(:,1:15); 
y1 = m(:,1:15);
y2 = ThigmoTemp.NicotineOF.OFPost(:,1:15); 
y3 = RipDensTemp2.NicotineOF.OFPost(:,1:15);
