function [numF,filename]=UMazeTracking_IRComp_ObjectOrientated_SBCheckTwoCamSolution(~,~)
...
arduinoDictionary.HPCStimOff = 6;
arduinoDictionary.HPCStimOn = 8;
    ...
    function PerformTracking(obj,event)
        ...
        while enableTrack
            ...
            if StartChrono
                ...
                %% Specific to ripples inhibition protocol
%                 if (strcmp('Cond',ExpeInfo.namePhase) | strcmp('CondWallShock',ExpeInfo.namePhase) | strcmp('CondWallSafe',ExpeInfo.namePhase)) &  sum(isnan(Pos))==0
%                     % Mouse is in shock
%                     where= Zone{1}(max(floor(Pos(2).*TrObjLocal.Ratio_IMAonREAL),1),max(floor(Pos(1).*TrObjLocal.Ratio_IMAonREAL),1));
%                     moving = im_diff(end,2)>th_immob & size(im_diff,1)>3; % if mouse is moving
% 
%                     if (where== 1 | moving==1) %& not(HPCOff)
%                         fwrite(a,arduinoDictionary.HPCStimOff);
%                         HPCOff = 1;
%                         disp(HPCOff)
%                     elseif (where~=1 & moving==0) %& not(HPCOff)
%                         fwrite(a,arduinoDictionary.HPCStimOn);
%                         HPCOff = 0;
%                         disp(HPCOff)
%                    end
%                 end
                ...
                if (strcmp('Cond',ExpeInfo.namePhase) | strcmp('CondWallShock',ExpeInfo.namePhase) | strcmp('CondWallSafe',ExpeInfo.namePhase)) &  sum(isnan(Pos))==0
                    ...
                    if etime(clock,KeepTime.LastStim)<5
                        HPCOff = 1;
                        disp(HPCOff)
                    end
                end
        ...
        save([ExpeInfo.name_folder,filesep,'TrObjLocalect.mat'],'TrObjLocal');
        ...
        %% generate figure that gives overview of the tracking session
        figbilan=figure;
        subplot(2,3,1:2)
