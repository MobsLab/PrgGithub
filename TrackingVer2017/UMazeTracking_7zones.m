function [numF,filename]=UMazeTracking_7zones(~,~)
...
arduinoDictionary.SleepOn=4;
arduinoDictionary.SleepOff=5;
...
global nametypes; nametypes={'Hab','TestPre','Cond','CondWallShock','CondWallSafe','TestPost','Ext','BlockedWall',...
    ...
    'Calibration','CalibrationEyeshock','SleepSession', 'ManualSleepScoring','BlockedWallShock', 'ExploratoryStimulation'};
mat1=ones(1,30);
...
maskbutton(10)= uicontrol(Fig_UMaze,'style','pushbutton',...
    'units','normalized','position',[0.25 0.15 0.15 0.05],...
    'string','Sleep On!','callback', @SwitchSleepOn);
set(maskbutton(10),'enable','off','FontWeight','bold')

maskbutton(11)= uicontrol(Fig_UMaze,'style','pushbutton',...
    'units','normalized','position',[0.25 0.08 0.15 0.05],...
    ...
    function giv_inputs(obj,event)
        ...
        function savProtoc(obj,event)
            ...
            elseif strcmp('CondWallShock',ExpeInfo.namePhase)
                ...
                set(inputDisplay(11),'string','Remove door at 180','ForegroundColor','k');
            ...
            elseif strcmp('ManualSleepScoring',ExpeInfo.namePhase)
                ExpeInfo.lengthPhase=12000;
                set(inputDisplay(10),'string','No Shock');
                set(chronostim,'ForegroundColor','k');
                set(inputDisplay(11),'string','You can relax','ForegroundColor','k');
            ...
            if strcmp('New PulsePal',Stimulator)
                if strcmp('Cond',ExpeInfo.namePhase) || strcmp('Calibration',ExpeInfo.namePhase) || strcmp('ManualSleepScoring',ExpeInfo.namePhase) || strcmp('CondWallShock',ExpeInfo.namePhase)
                ...
                if OPF
                    if num_fr > 15
                        ...
                        if nodetect == 0
                            ...
                            if runmean(sqrt(diff(PosMatV(end-15:end,2)).^2+diff(PosMatV(end-15:end,3)).^2)./(diff(PosMatV(end-15:end,1))),7) > 2
...
%% Make manual sleep scoring
    function SwitchSleepOn(obj,event)
        fwrite(a,arduinoDictionary.SleepOn);
        set(maskbutton(10),'enable','off','FontWeight','bold')
...
%% Define behavioural zones
    function DefineZones(obj,event)
        ...
        elseif strcmp(ExpeInfo.namePhase,'SoundCnd') | strcmp(ExpeInfo.namePhase,'SoundHab') | strcmp(ExpeInfo.namePhase,'SoundTest') | strcmp(ExpeInfo.namePhase,'Calibration')  | strcmp(ExpeInfo.namePhase,'SleepSession') | strcmp(ExpeInfo.namePhase,'CalibrationEyeshock') | strcmp(ExpeInfo.namePhase,'ExploratoryStimulation')
            Zone = []; ZoneEpoch = []; ZoneIndices = []; ZoneLabels = [];
            ...
            title('FarShock');[x1,y1,FarShock,x2,y2]=roipoly(); Zone{6}=uint8(FarShock);plot(x2,y2,'linewidth',2)
            title('FarNoShock');[x1,y1,FarNoShock,x2,y2]=roipoly(); Zone{7}=uint8(FarNoShock);plot(x2,y2,'linewidth',2)
            stats = regionprops(TrObjLocal.mask, 'Area');
            tempmask=TrObjLocal.mask;
            AimArea=stats.Area/2;
            ActArea=stats.Area;
            ...
            Zone{8}=uint8(tempmask); % I]
            Zone{9}=uint8(TrObjLocal.mask-tempmask);% Outside area
