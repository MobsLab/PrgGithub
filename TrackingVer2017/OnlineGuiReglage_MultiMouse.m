function guireg_fig=OnlineGuiReglage_MultiMouse(MouseNum,colorval)

% function guireg_fig=OnlineGuiReglage(obj,event);
% let online control of paramteres for image treatments

color_on = [0 0 0];
global BW_threshold smaller_object_size sm_fact strsz SrdZone se

typical_size=300;
MaxSmoothFact=20;
maxsrtsz=20;
maxsrndsz=80;

% create figure
guireg_fig=figure('units','normalized',...
    'position',[0.1 0.1 0.4 0.6],...
    'numbertitle','off',...
    'name','Online Mouse Tracking : Setting Parameters',...
    'menubar','none',...
    'tag','figure reglage');
set(guireg_fig,'Color',colorval);

% create names of sliders
text1=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.05 0.85 0.10 0.05],...
    'string','threshold');

text2=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.25 0.85 0.10 0.05],...
    'string','min size');

text3=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.45 0.85 0.10 0.05],...
    'string','erode obj size');

text4=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.65 0.85 0.10 0.05],...
    'string','Fz Srnd Sz');

text5=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.85 0.85 0.10 0.05],...
    'string','smoothing');

% create sliders
slider_threshold = uicontrol(guireg_fig,'style','slider',...
    'units','normalized',...
    'position',[0.06 0.1 0.08 0.7],...
    'callback', @threshold);
set(slider_threshold,'Value',BW_threshold{MouseNum});

slider_small=uicontrol(guireg_fig,'style','slider',...
    'units','normalized',...
    'position',[0.26 0.1 0.08 0.7],...
    'callback', @elimination);
set(slider_small,'Value',smaller_object_size{MouseNum}/typical_size);

slider_strel = uicontrol(guireg_fig,'style','slider',...
    'units','normalized',...
    'position',[0.46 0.1 0.08 0.7],...
    'callback', @setstrelsize);
set(slider_strel,'Value',strsz{MouseNum}/maxsrtsz);

slider_srndsz = uicontrol(guireg_fig,'style','slider',...
    'units','normalized',...
    'position',[0.66 0.1 0.08 0.7],...
    'callback', @setsrndsz);
set(slider_srndsz,'Value',SrdZone{MouseNum}/maxsrndsz);

slider_smoothing = uicontrol(guireg_fig,'style','slider',...
    'units','normalized',...
    'position',[0.86 0.1 0.08 0.7],...
    'callback', @setsmoothval);
set(slider_smoothing,'Value',sm_fact{MouseNum}/MaxSmoothFact);

% create labels with actual values
text6=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.06 0.05 0.08 0.03],...
    'string',num2str(BW_threshold{MouseNum}));

text7=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.26 0.05 0.08 0.03],...
    'string',num2str(smaller_object_size{MouseNum}));

text8=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.46 0.05 0.08 0.03],...
    'string',num2str(strsz{MouseNum}));

text9=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.66 0.05 0.08 0.03],...
    'string',num2str(SrdZone{MouseNum}));

text10=uicontrol(guireg_fig,'style','text', ...
    'units','normalized',...
    'position',[0.86 0.05 0.08 0.03],...
    'string',num2str(sm_fact{MouseNum}));

%get threshold value
    function threshold(obj,event)
        BW_threshold{MouseNum} = get(slider_threshold, 'value');
        set(text6,'string',num2str(BW_threshold{MouseNum}))
    end

%get min object size
    function elimination(obj,event)
        smaller_object_size{MouseNum} = round(get(slider_small,'value')*typical_size);
        set(text7,'string',num2str(smaller_object_size{MouseNum}))
    end

%get object size for erosion
    function setstrelsize(~,~)
        strsz{MouseNum} = round(get(slider_strel,'value')*maxsrtsz);
        se{MouseNum} = strel('disk',strsz{MouseNum});
        set(text8,'string',num2str(strsz{MouseNum}));
    end

%get surround zone size
    function setsrndsz(~,~)
        SrdZone{MouseNum} = round(get(slider_srndsz,'value')*maxsrndsz);
        set(text9,'string',num2str(SrdZone{MouseNum}));
    end

%get surround zone size
    function setsmoothval(~,~)
        sm_fact{MouseNum} = round(get(slider_smoothing,'value')*MaxSmoothFact);
        set(text10,'string',num2str(sm_fact{MouseNum}));
    end
end
