    ...
    function start_Expe(obj,event)
        ...
        if length(ExpeInfo.voltage)>1
            ExpeInfo.name_folder=['NosePoke-Mouse-' num2str(ExpeInfo.nmouse) '-'...
                ExpeInfo.strtostim '-' num2str(ExpeInfo.nPhase) '-' num2str(ExpeInfo.voltage(1)) '-' num2str(ExpeInfo.voltage(2))...
                'V-' ExpeInfo.TodayIs];
        else
            ExpeInfo.name_folder=['NosePoke-Mouse-' num2str(ExpeInfo.nmouse) '-'...
...
%% track mouse
    function PerformTracking(obj,event)
            ...
            if StartChrono
                ...
                if strcmp(TrObj.camera_type,'Webcam')
                    if strcmp(TrObj.vid.VideoFormat,'RGB24_320x240')
                        ...
                        frame.colormap = [];
                        try,writeVideo(writerObj,frame);catch,disp('missed frame video'),end
                    else
                        frame.cdata = cat(3,IM,IM,IM);
                        frame.colormap = [];
                        try,writeVideo(writerObj,frame);catch,disp('missed frame video'),end
        ...
        save([ExpeInfo.name_folder,filesep,'behavResources.mat'],'PosMat','PosMatInit','im_diff','im_diffInit','Vtsd','Xtsd','Ytsd','Imdifftsd',...
        ...
        save([ExpeInfo.name_folder,filesep,'behavResources.mat'],'FreezeEpoch','th_immob','thtps_immob','-append');
...
%% stop tracking
    function stop_Phase(obj,event)
        ...
        set(maskbutton.StopSession,'enable','off','FontWeight','normal')
        fwrite(a,arduinoDictionary.Off);disp('Intan OFF');
        close(writerObj);
        set(maskbutton.StartExpe,'enable','off')
    end
