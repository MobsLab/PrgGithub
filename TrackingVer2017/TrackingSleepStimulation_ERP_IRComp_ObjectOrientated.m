function [numF,filename]=TrackingSleepStimulation_ERP_IRComp_ObjectOrientated(~,~)
...
global stimcount; 
global stims; stims(1) = 0; stims(2)=0;
...
inputDisplay.stim1=uicontrol(Fig_SleepSession,'style','text','units','normalized','position',[0.05 0.3 0.1 0.02],'string','Shock','FontSize',8);
inputDisplay.stim2=uicontrol(Fig_SleepSession,'style','text','units','normalized','position',[0.05 0.2 0.1 0.02],'string','Shock','FontSize',8);
...
stimcount{1}=uicontrol('style','edit', 'units','normalized','position',[0.05 0.25 0.1 0.05],...
    'string',num2str(floor(0)),'ForegroundColor','c','BackgroundColor','k','FontSize',14);
stimcount{2}=uicontrol('style','edit', 'units','normalized','position',[0.05 0.15 0.1 0.05],...
    'string',num2str(floor(0)),'ForegroundColor','r','BackgroundColor','k','FontSize',14);
...
%% Ask for all inputs and display
    function giv_inputs(obj,event)
        ...
        if strcmp('New PulsePal',Stimulator)
                    ...
                    ProgramPulsePalParam(2,'Phase2Duration', 0.001);
                    ProgramPulsePalParam(2,'InterPulseInterval', 0);
                    ProgramPulsePalParam(2,'BurstDuration', 0.002);
                    ProgramPulsePalParam(2,'InterBurstInterval', 0);
                    ProgramPulsePalParam(2,'PulseTrainDelay', 0);
                    ProgramPulsePalParam(2,'PulseTrainDuration', 0.002);
        ...
        function savProtoc
            ...
            if strcmp('New PulsePal',Stimulator)
                ...
                for mm = 1:2
                    ...
                    default_answer{2}='9999999';
                    default_answer{3}='2';
            
                    answer = inputdlg({'NumberMouse','Session duration (s)', 'Voltage to stimulate'},'INFO',1,default_answer);
                    ...
                    ExpeInfo.voltage{mm} = str2num(answer{3});
                    ExpeInfo.nPhase=0;
                    
                    ProgramPulsePalParam(MouseNum,'Phase1Voltage', ExpeInfo.voltage{mm});
                    ProgramPulsePalParam(MouseNum,'Phase2Voltage', -ExpeInfo.voltage{mm});
...
%% track mouse
    function PerformTracking(obj,event)
        ...
        htrack{1} = imagesc(TrObj.ref);axis image; hold on
        ...
        htrack{2} = imagesc(TrObj.ref);axis image; hold on
        caxis([0.1 0.9])
        ...
        htrack2{1} = imagesc(zeros(size((TrObj.ref))));axis image; caxis([0 1]);hold on
        xlabel('tracking online','Color','w')
        g{1}=plot(0,0,'c+');
        subplot(5,2,[6,8]),
        htrack2{2} = imagesc(zeros(size((TrObj.ref))));axis image; caxis([0 1]);hold on
        xlabel('tracking online','Color','w')
        g{2}=plot(0,0,'r+');
        ...
        PosMatTemp{1}=[];im_diffTemp{1}=[]; 
        PosMatTemp{2}=[];im_diffTemp{2}=[];

        IM=TrObj.GetAFrame;
        OldIm{1}=TrObj.mask;OldIm{2}=TrObj.mask;
        OldZone{1}=TrObj.mask;OldZone{2}=TrObj.mask;
        ...
        while enableTrack
            ...
            if StartChrono
                ...
                if sum(isnan(Pos))==0
                    ...
                    PosMatTemp{MouseNum}(num_fr,:)=[KeepTime.chrono;NaN;NaN;NaN];
                ...
                if im_diffTemp{MouseNum}(end,2)<th_immob{MouseNum} & size(im_diffTemp{MouseNum},1)>3
                    SleepChrono{MouseNum}=SleepChrono{MouseNum}+im_diffTemp{MouseNum}(end,1)-im_diffTemp{MouseNum}(end-1,1);
                    set(stimcount{MouseNum},'string',num2str(stims(MouseNum)));
                    if SleepChrono{MouseNum}>MinSleepTime
                        PeriodChrono{MouseNum}=PeriodChrono{MouseNum}+im_diffTemp{MouseNum}(end,1)-im_diffTemp{MouseNum}(end-1,1);
                        fwrite(a,arduinoDictionary.StartSleep{MouseNum});
                        if etime(clock,KeepTime.LastStim{MouseNum})>1.5*delStim
                            KeepTime.LastStim{MouseNum}=clock;
                            KeepTime.LastStim{MouseNum}(end)=KeepTime.LastStim{MouseNum}(end)-(delStim-delStimreturn);
                        end
                        if etime(clock,KeepTime.LastStim{MouseNum})>delStim +2*rand
                            KeepTime.LastStim{MouseNum}=clock;
                            fwrite(a,arduinoDictionary.SendStim{MouseNum});
                            PosMatTemp{MouseNum}(end,4)=1;
                            stims(MouseNum) = stims(MouseNum)+1;
                            disp(['stim Mouse ' num2str(MouseNum)]);
                        end
                    else
                        fwrite(a,arduinoDictionary.StopSleep{MouseNum});
                    end
                elseif im_diffTemp{MouseNum}(end,2)>th_immob{MouseNum} & size(im_diffTemp{MouseNum},1)>3    
                    SleepChrono{MouseNum}=0;
                end
        ...
        for MouseNum=1:2
            ...
            save([MouseDir,filesep,'behavResources.mat'],'PosMat','PosMatInit','im_diff','im_diffInit','Vtsd','Xtsd','Ytsd','Imdifftsd',...
            ...
            save([MouseDir,filesep,'behavResources.mat'],'FreezeEpoch','th_immob','thtps_immob','-append');
