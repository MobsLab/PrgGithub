function [numF,filename]=TrackingManualSleepStimulation_IRComp_ObjectOrientated(~,~)
...
arduinoDictionary.StartSleep = 4; % Show when the animal is sleeping
arduinoDictionary.StopSleep = 5;
arduinoDictionary.SendStim=9; % This is sent to stimulate the mouse
...
InterfaceButton.Stimulation= uicontrol(Fig_SleepSession,'style','pushbutton',...
    'units','normalized','position',[0.01 0.2 0.15 0.05],...
    'string','STIMULATE','callback', @manual_stimulation);
set(InterfaceButton.Stimulation,'enable','off')
...
%% Ask for all inputs and display
    function giv_inputs(obj,event)
        ...
        function savProtoc(obj,event)
            ...
            default_answer{2}=num2str(28800);
...
%% track mouse
    function PerformTracking(obj,event)
            ...
            if StartChrono
                ...
                if im_diff(end,2)<th_immob & size(im_diff,1)>3
                    ...
                    if SleepChrono>MinSleepTime
                        ...
                        fwrite(a, arduinoDictionary.StartSleep);
                        set(InterfaceButton.Stimulation,'enable','on','FontWeight','bold')
                    else
                        fwrite(a, arduinoDictionary.StopSleep);
                        set(InterfaceButton.Stimulation,'enable','off','FontWeight','bold')
                    end
        ...
        fwrite(a,arduinoDictionary.StopSleep); % switch off intan
...
%% Manual stimulation
    function st = manual_stimulation(obj, event)
        % Clear the arduino
        if a.BytesAvailable>0
            fread(a,a.BytesAvailable);
        end
...
%% stop tracking
    function stop_Phase(obj,event)
        ...
        set(maskbutton.StopSession,'enable','off','FontWeight','normal')
        try fwrite(a,arduinoDictionary.Off); fwrite(a,arduinoDictionary.StopSleep); disp('Intan OFF');end
        close(writerObj);
        set(maskbutton.StartExpe,'enable','off')
    end
