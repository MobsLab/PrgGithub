function [numF,filename]=NosePokeTracking_IRComp_ObjectOrientated_SL(~,~)
%%--------------------------------------------------------------------------------------------
%
% DESCRIPTION: This is a nosepoke tracking code compatible with IR camera based on earlier nosepoke code
%   and compatible with new, standard for whole lab variables
% 
% UPDATES:
% Modified by Dima in February-March 2018
% Modified by Dima in October-November2018
% Modified by SL on April and May 2019 to include:
%   - Heatmaps
%   - Flashing light only when animal is in nose poke
%   - Better control of GUI color
%   - Diary of session
%   - Output on command line time and stimulations
%   - Stims are written into PosMat (column 4)
%   - Added a Part Tree below and marked every sections with headers
% Modified by SL on April and May 2019 to include:
%   - Button for Manual Stimulation
%
% PARTS TREE:
%   - Global variables
%   - Graphical interface with all the pushbuttons
%   - Called Functions
%       - Quit the programm
%       - Input window (Mouse #, Session #, Voltage
%           - setprotoc(obj,event)
%           - savProtoc(obj,event)
%       - Get light threshold
...
%    - StartSession
%    - Track mouse
%    - Stop tracking button(emergency)
%    - Sliders settings for tracking
%        
%----------------------------------------------------------------------------------------------
...
% shortcuts for multi-sessions - saves last choices
global MName; MName='';
...
arduinoDictionary.FlashingLight=5; % Switch on synchronizing light (for light threshold)
arduinoDictionary.USGH_sync=7; % Send sync pulse to intan then to USGH recorder
arduinoDictionary.SendStim1=8; %send stimulation (manual)
arduinoDictionary.SendStim2=9; %send stimulation (manual)


% For figure - Size of the occupation map
nx = 32;
ny = 32;
...
maskbutton.ManualStim1= uicontrol(Fig_NosePoke,'style','pushbutton',...
    'units','normalized','position',[0.01 0.2 0.1 0.05],...
    'string','Stim #1','callback', @ManualStim1);
set(maskbutton.ManualStim1,'enable','on')

maskbutton.ManualStim2= uicontrol(Fig_NosePoke,'style','pushbutton',...
    'units','normalized','position',[0.12 0.2 0.1 0.05],...
    'string','Stim #2','callback', @ManualStim2);
set(maskbutton.ManualStim2,'enable','on')
...
%=============================================
    ...
    function quit(obj,event)
        enableTrack=0;
%         diaryFolder = ['SessionLog-M' MName '-' SessionName '-'  ExpeInfo.TodayIs];
%         if ~exist(diaryFolder, 'dir')    
%             mkdir(diaryFolder);
%         end
         diary off;
%         movefile('diary',diaryFolder);
        ...
        function savProtoc(obj,event)
            ...
            if strcmp('NosePokeOneStim',ExpeInfo.namePhase)
                if strcmp('New PulsePal',Stimulator)
                    ...
                    default_answer{2}= SessionName;
                    default_answer{3}= SessionNum;
                    default_answer{4}= SessionVolt;
    ...
    function start_Expe(obj,event)
        ...
        enableTrack=1;
        StartChrono=0;
        if strcmp('NosePokeTwoStimSimultan',ExpeInfo.namePhase)
            ExpeInfo.name_folder=['NosePoke-Mouse-' num2str(ExpeInfo.nmouse) '-'...
                ExpeInfo.strtostim '-' num2str(ExpeInfo.nPhase) '-MFB' num2str(ExpeInfo.voltage(1)) ...
                'V-PAG' num2str(ExpeInfo.voltage(2)) 'V-' ExpeInfo.TodayIs];
        else
            ExpeInfo.name_folder=['NosePoke-Mouse-' num2str(ExpeInfo.nmouse) '-'...
    ...
    function StartSession(obj,event)
        ...
        disp(['Start of trial: ' datestr(now,'HH:MM:SS')]);
        ...
        %Keep all promtp to Command Line into a text file saved in
        %directory
        cd(ExpeInfo.name_folder)
        diary on	
        cd ../ 
    end
    ...
    function PerformTracking(obj,event)
            ...
            if StartChrono
                ...
                if sum(sum(sum(IM.*DiodMask)))>DiodThresh
                    if FirstStim
                        ...
                        tStart = KeepTime.chrono;
                        tLastStim = KeepTime.chrono;
                        PosMat(num_fr-1,4)=1;
                        disp([datestr(now) ' - Stim']);
                        FirstStim=0;                        
                    else
                        if KeepTime.chrono-tLastStim>1
                            tLastStim = KeepTime.chrono;
                            PosMat(num_fr-1,4)=1;
                            disp([datestr(now) ' - Stim']);
                        end
                ...
                if  mod(num_fr,1000)==0
                    ...
                    fwrite(a,arduinoDictionary.USGH_sync);
                end
                ...
                if ~(FirstStim) && (KeepTime.chrono-tStart>100)
                    enableTrack=0;
                    diary off    
                end
        ...
        % Display total number of Stimulation during the trial
        if strcmp('NosePokeTwoStimSimultan',ExpeInfo.namePhase)
            disp(['Total number of stimulation at PAG ' num2str(ExpeInfo.voltage(2)) 'V: ' num2str(nansum(PosMat(:,4)))]);    
        else
            disp(['Total number of stimulation at ' num2str(ExpeInfo.Voltage) 'V: ' num2str(nansum(PosMat(:,4)))]);
        end
        ...
        %*********************************************
        %         FIGURES AFTER SESSION
        %*********************************************
        
        %figure1 - might scrap it
        ...
        % Figure2 - figure with heatmap of movements and animal's
        % trajectory (on map, hole is north of the figure (90 degrees
        % rotation)
        
        
        fshock = 0;
        fshock = PosMat(find(PosMat(:,4)==1,1):end,4);
        fshock(isnan(fshock))=0;
        foccup = figure;
        if fshock
            [occH, x1, x2] = hist2(Data(Xtsd(fshock:end)), Data(Ytsd(fshock:end)), 240, 320);
        else
            [occH, x1, x2] = hist2(Data(Xtsd), Data(Ytsd), 240, 320);
        end
%         xpos = Data(Xtsd); 
%         ypos = Data(Ytsd);
%         [occH, x1, x2] = hist2(xpos(fshock:end), ypos(fshock:end), 240, 320);
    ...
    %  Manual stimulation 
    %----------------------------------
    function ManualStim1(obj,event)
        fwrite(a,arduinoDictionary.SendStim1);
        disp([datestr(now) ' - Manual Stim - Electrode 1']);
    end
    
    function ManualStim2(obj,event)
        fwrite(a,arduinoDictionary.SendStim2);
        disp([datestr(now) ' - Manual Stim - Electrode 2']);
    end
