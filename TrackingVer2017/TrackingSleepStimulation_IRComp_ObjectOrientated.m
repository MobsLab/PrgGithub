function [numF,filename]=TrackingSleepStimulation_IRComp_ObjectOrientated(~,~)

%% This is the simplest possible tracking code - you can use it as a framework to buld more personal codes
...
MinSleepTime=10;StimOnTime=1000;StimOffTime=10; ITI=13;
...
%% Ask for all inputs and display
    function giv_inputs(obj,event)
        ...
        function savProtoc
            ...
            ExpeInfo.nmouse{MouseNum}=str2double(answer{1});
...
%% Interface of analysis
    function start_Expe(obj,event)
        ...
        ExpeInfo.name_folder=['SLEEP-Mouse-' num2str(ExpeInfo.nmouse{1}) '&' num2str(ExpeInfo.nmouse{2}) '-' ExpeInfo.TodayIs];
...
%% StartSession
    function StartSession(obj,event)
        ...
        disp(['Sent arduino' num2str(arduinoDictionary.On)])
    ...
    function PerformTracking(obj,event)
        ...
        while enableTrack
            ...
            if StartChrono
                ...
                for MouseNum=1:2
                    % Find the mouse
                    [Pos,ImDiff,PixelsUsed,NewIm,FzZone]=Track_ImDiff(IM,TrObjLocal.mask.*MouseMask{MouseNum},TrObjLocal.ref,BW_threshold{MouseNum},smaller_object_size{MouseNum},sm_fact{MouseNum},...
                        se{MouseNum},SrdZone{MouseNum},TrObjLocal.Ratio_IMAonREAL,OldIm{MouseNum},OldZone{MouseNum},TrObjLocal.camera_type);
                    ...
                    if sum(isnan(Pos))==0
                        ...
                        switch TrObjLocal.camera_type
                            case 'IRCamera'
                                ...
                                MouseTemp{MouseNum}(num_fr,2)=max(max(double(IM).*double(MouseMask{MouseNum})));
                            case 'Webcam'
                    ...
                    if im_diffTemp{MouseNum}(end,2)<th_immob{MouseNum} & size(im_diffTemp{MouseNum},1)>3
                        ...
                        if SleepChrono{MouseNum}>MinSleepTime
                            ...
                            disp(['Sent arduino' num2str(arduinoDictionary.StartSleep{MouseNum})])
                            
                        else
                            fwrite(a,arduinoDictionary.StopSleep{MouseNum});
                            disp(['Sent arduino' num2str(arduinoDictionary.StopSleep{MouseNum})])
                            
                        end
                        if PeriodChrono{MouseNum}>StimOnTime & StimOnOff{MouseNum}==1
                            StimOnOff{MouseNum}=0;
                            PeriodChrono{MouseNum}=0;
                        elseif PeriodChrono{MouseNum}>StimOffTime & StimOnOff{MouseNum}==0
                            StimOnOff{MouseNum}=1;
                            PeriodChrono{MouseNum}  =0;
                        end
                        PosMatTemp{MouseNum}(num_fr,4)=StimOnOff{MouseNum};
                ...
                if  mod(num_fr,1000)==0
                    ...
                    disp(['Sent arduino' num2str(arduinoDictionary.ThousandFrames)])
        ...
        disp(['Sent arduino' num2str(arduinoDictionary.Off)])
        disp(['Sent arduino' num2str(arduinoDictionary.StopSleep{1})])
        disp(['Sent arduino' num2str(arduinoDictionary.StopSleep{2})])
        ...
        for MouseNum=1:2
            ...
            [PosMat,PosMatInit,im_diff,im_diffInit,Vtsd,Xtsd,Ytsd,Imdifftsd]=CommonInterpPosMatImDiff(im_diffTemp{MouseNum},KeepTime.chrono,PosMatTemp{MouseNum}, dist_thresh);
            ref=TrObjLocal.ref;
