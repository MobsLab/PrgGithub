...
function []=LocalGlobalOmissionEffect(start,choice)
%%

MMNfilegenesis(start);
...
if choice==2;
  for i=1:length(LFP);
    ...
    for a=0:150:600
        hold on, plot(a,min(axis):max(axis),'b','linewidth',10);
    end
    ...
    hold on, title(['effet LOCAL: FreqBBBBB (black) vs FreqAAAAB (red)   +  freqAAAAA (green) - channeln�',num2str(i)])
    ...
    hold on, title(['effet GLOBAL: FreqBBBBA (black) vs RareBBBBA (red) +  freqBBBBB (green) - channeln°',num2str(i)])
    ...
    hold on, title(['effet GLOBAL: FreqAAAAB (black) vs RareAAAAB  (red)   +  freqAAAAA (green) - channeln°',num2str(i)])
    hold on, axis([-100 1100 -1500 1000]) 
    ...
    [h,p]=ttest2(Data(matVal{i,9})',Data(Restrict(matVal{i,10},matVal{i,8}))');
    rg=Range(matVal{i,1},'ms');
    pr=rescale(p,500, 600);
    hold on, plot(rg(p<pval),pr(p<pval),'bx')  
    end
