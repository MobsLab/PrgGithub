...
function [Epoch, NameEpoch] = SubstagesScoring_VaryN3Definition(featuresNREM, NoiseEpoch, IBI,NumDelta,varargin)
...
% N3
% this is the piece of cide that allows you to change how you define delta
% waves
load('DeltaWaves.mat', 'deltas_PFCx')
deltas_PFCx = dropShortIntervals(deltas_PFCx, min_delta_duration);
tdeltas = ts(Start(deltas_PFCx));

[BurstDeltaEpoch, burst_data] = FindDeltaBurst2(tdeltas, IBI, 0);
BurstDeltaEpoch=subset(BurstDeltaEpoch, find(burst_data(:,1)>=NumDelta));

N3 = and(BurstDeltaEpoch, SWS);
