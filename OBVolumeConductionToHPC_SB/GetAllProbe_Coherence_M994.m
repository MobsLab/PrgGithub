...
for shanknum = 2:5
    ...
    load(RipplesFile.(InfoLFP.hemisphere{find(InfoLFP.channel==HPCOrderChans{shanknum}(1))}))
    %% Trigger on ripples
    clear HPCTrigRipples
    for c=1:length(HPCOrderChans{shanknum})
        load(['LFPData/LFP',num2str(HPCOrderChans{shanknum}(c)),'.mat'])
        [M,T]=PlotRipRaw(LFP,ripples(:,2),500,0,0);
        HPCTrigRipples(c,:)=M(:,2);
    end
    save([SaveFolder ,'HPCTriggered_Sk',num2str(shanknum),'.mat'],'HPCTrigRipples','-append')
end

    %% Spetrogram
    for c=1:length(HPCOrderChans{shanknum})
        chan2=HPCOrderChans{shanknum}(c);
        if exist([SaveFolder 'Spectro_HPC',num2str(chan2),'.mat'])==0
            disp('calc')
            load(['LFPData/LFP',num2str(HPCOrderChans{shanknum}(c)),'.mat'])
    ...
    for c=1:length(HPCOrderChans{shanknum})
        ...
        if exist([SaveFolder 'Cohgram_OB_HPC',num2str(chan2),'.mat'])==0
            disp('calc')
        ...
        if exist([SaveFolder 'Cohgram_PFCx_HPC',num2str(chan2),'.mat'])==0
            disp('calc')
    ...
    for el=1:size(subtraction,2)
        ...
        if exist([SaveFolder 'Cohgram_PFCx_HPCDiff_Sk',num2str(shanknum),'_El',num2str(el),'.mat'])==0
            disp('calc')
            [C,phi,S12,S1,S2,t,f,confC,phistd]=cohgramc(Data(LFPPFC),Data(subtsd),movingwin,params);
            chan1=PFCxChan;
            chan2=el;
            save([SaveFolder 'Cohgram_PFCx_HPCDiff_Sk',num2str(shanknum),'_El',num2str(el),'.mat'],'C','phi','S12','confC','t','f','chan1','chan2')
            clear C phi S12 S1 S2 t f confC phist chan1 chan2
        end
        if exist([SaveFolder 'Cohgram_OB_HPCDiff_Sk',num2str(shanknum),'_El',num2str(el),'.mat'])==0
            disp('calc')
    ...
    for el=1:size(csd,2)
        ...
        if exist([SaveFolder 'Cohgram_PFCx_HPCCSD_Sk',num2str(shanknum),'_El',num2str(el),'.mat'])==0
            disp('calc')
            [C,phi,S12,S1,S2,t,f,confC,phistd]=cohgramc(Data(LFPPFC),Data(ctsd),movingwin,params);
            chan1=PFCxChan;
            chan2=el;
            save([SaveFolder 'Cohgram_PFCx_HPCCSD_Sk',num2str(shanknum),'_El',num2str(el),'.mat'],'C','phi','S12','confC','t','f','chan1','chan2')
            clear C phi S12 S1 S2 t f confC phist chan1 chan2
        end
        
        if exist([SaveFolder 'Cohgram_OB_HPCCSD_Sk',num2str(shanknum),'_El',num2str(el),'.mat'])==0
            disp('calc')
    ...
    if exist([SaveFolder ,'HPCTriggered_Sk',num2str(shanknum),'.mat'])==0
        disp('calc')
    ...
    for c=1:length(HPCOrderChans{shanknum})
        load(['LFPData/LFP',num2str(HPCOrderChans{shanknum}(c)),'.mat'])
        [M,T]=PlotRipRaw(LFP,ripples(1,2),500,0,0);
        HPCTrigRipples(c,:)=M(:,2);
    end
    save([SaveFolder ,'HPCTriggered_Sk',num2str(shanknum),'.mat'],'HPCTrigRipples','-append')
