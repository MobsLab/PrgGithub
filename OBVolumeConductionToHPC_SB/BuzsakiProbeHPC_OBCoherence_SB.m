...
save([SaveFolder 'Epochs.mat'],'RunEpoch','FreezeEpoch')

%% Get order of channels on probe
...
for el=1:size(subtraction,2)
    ...
    chan1=OBchan;
    chan2=el;
    save([SaveFolder 'Cohgram_OB_HPCDiff',num2str(el),'.mat'],'C','phi','S12','confC','t','f','chan1','chan2')
    clear C phi S12 S1 S2 t f confC phist chan1 chan2
end
...
for el=1:size(csd,2)
    ...
    chan1=OBchan;
    chan2=el;
    save([SaveFolder 'Cohgram_OB_HPCCSD',num2str(el),'.mat'],'C','phi','S12','confC','t','f','chan1','chan2')
    clear C phi S12 S1 S2 t f confC phist chan1 chan2
end
...
AllPeaks=FindPeaksForFrequency(LFP,Options,0);
...
for c=1:length(HPCOrderChans)
    load(['LFPData/LFP',num2str(HPCOrderChans(c)),'.mat'])
    [M,T]=PlotRipRaw(LFP,Start(TopBreath,'s'),300,0,0);
    HPCTrigTheta_Freeze(c,:)=M(:,2);
end

save([SaveFolder 'HPCTriggered.mat'],'HPCTrigBreath_Run','HPCTrigBreath_Freeze','HPCTrigTheta_Freeze','HPCTrigTheta_Run')

        
        %% Figures %%%%
        %% HPC triggered on breathing
        
        load([SaveFolder 'HPCTrigBreath.mat'])
