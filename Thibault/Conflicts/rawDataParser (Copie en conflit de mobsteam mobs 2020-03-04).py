...
def get_position(folder):
    ...
    if not os.path.exists(folder + 'nnBehavior.mat'):
        if not os.path.exists(folder + 'behavResources.mat'):
            raise ValueError('this file does not exist :'+folder+'behavResources.mat')
        else:
            print('Converting behaviour data to python.')

            log_path  = folder + 'mlog.out'
            call = 'matlab -nodesktop -nosplash -nodisplay -r'
            matlabCommand = 'cd(\'' + os.path.expanduser('~/Dropbox/Kteam/PrgMatlab/OnlinePlaceDecoding') + '\');'
            matlabCommand += 'LoadPATHencoder(\'' + getuser() + '\');'
            matlabCommand += 'cd(\'' + folder + '\');'
            matlabCommand += 'export_spikes;'
            matlabCommand += 'exit'
            process = subprocess.run(call.split() + [matlabCommand])
...
class SpikeDetector:
    ...
    def __init__(self, path):
        ...
        self.filter = Filter(350., self.samplingRate, self.list_channels)
        ...
        while spl < filteredBuffer.shape[0]:
            for chnl in range(len(self.list_channels[group])):
                ...
                if triggered:
                    ...
                    # Do nothing unless after beahviour data has started
...
if __name__ == '__main__':
    print(rawDataParser.get_params(os.path.expanduser('~/Documents/dataset/RatCatanese/rat122-20090731.xml')))
    sys.exit(1)