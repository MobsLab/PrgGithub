...
temp2 = temp.argsort()
thresh = temp[temp2[int(len(temp2)*0.2)]]
selectionCNN = np.array(Standdev)<thresh

# LSTM
results = np.load(os.path.expanduser('~/Documents/dataset/RatCatanese/_resultsForRnn_2019-09-25_trueUnshuffled.npz'))
Y_test = results['arr_0']
speed = results['arr_1']
testOutput = results['arr_2']
trainLosses = results['arr_3']
...
_, edges, _ = plt.hist(ErrorCNN, label = 'CNN + bayes', bins=200, density=True, alpha=0.5, edgecolor='k')
# fig= plt.figure(figsize=(9,9))
plt.hist(ErrorLSTM*maxPos, bins=edges, label='CNN + LSTM', density=True, alpha=0.5, edgecolor='k')
# fig= plt.figure(figsize=(9,9))
plt.hist(ErrorCNN - ErrorLSTM[0:len(ErrorCNN)]*maxPos, label='difference', bins=edges, density=True, alpha=0.5, edgecolor='k')
...
# ax1.plot([n*0.036 for n in range(len(Y_test))], ErrorLSTM*maxPos, label=None, color='b', linewidth=5)
ax1.plot([n*0.036 for n in range(len(Y_test))], Y_test[:,0]*maxPos, label=None, color='b', linewidth=5)
# ax2 = plt.subplot2grid((2,1),(1,0), sharex=ax1)
