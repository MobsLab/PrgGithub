...
def scattWithError(metric, Error, label, ax):
    xy = np.vstack([Error, metric])
    z = gaussian_kde(xy)(xy)
    # fig, ax1 = plt.subplots(figsize=(15,15))
    # ax1 = plt.subplot2grid((3,1),(0,0), rowspan=2)
    ax.scatter(Error, metric[:], c=z, s=13, label='error and selection metric for each time bin')
    ...
    ax.errorbar(
        ...
        yerr = err, 
        label='median '+r'$\pm$' + ' 20 percentile',
        linewidth=2)
    # ax2.errorbar(
    #     [(edges[n+1]+edges[n])/2 for n in range(nBins)],
...
# file = '~/Documents/dataset/RatCatanese/_resultsForRnn_2019-11-19_aligned.npz'
...
try:
    ...
    Data = np.load('/home/mobshamilton/Documents/dataset/RatCatanese/mobsEncoding_2019-05-06_15:10/_data.npy', allow_pickle=True).item()
...
def synchronize(pos1, pos2):
    costs = []
    offset = [0, 1]
    indices = np.array([n for n in range(len(pos1[0])//4, 3*len(pos1[0])//4)])
    cost = lambda posA, posB, offset, indices: np.sum(np.abs(posA[indices] - posB[indices-offset]))
    costs.append(cost(pos1[0], pos2[0], offset[-2], indices))
    costs.append(cost(pos1[0], pos2[0], offset[-1], indices))
...
# True
Results = np.load(os.path.expanduser('~/Documents/dataset/RatCatanese/mobsEncoding_2019-10-04_shuffledTrueLabels/_simDecodingUnshuffled.npz'))
position_proba=Results['position_proba']
position = Results['position'].tolist()
for n in range(len(position_proba)):
    if np.sum(position_proba[n,:,:])==0.0:
...
X_err = [np.abs(X_true[n] - X_guessed[n]) for n in range(len(X_true))]
Y_err = [np.abs(Y_true[n] - Y_guessed[n]) for n in range(len(Y_true))]
ErrorTrue=np.array([np.sqrt(X_err[n]**2 + Y_err[n]**2) for n in range(len(X_err))])
X_standdev = np.sqrt([np.sum([X_proba[n][x]*(Bins[0][x]-X_guessed[n])**2 for x in range(Bins[0].size)]) for n in range(len(position_proba))])
Y_standdev = np.sqrt([np.sum([Y_proba[n][y]*(Bins[1][y]-Y_guessed[n])**2 for y in range(Bins[1].size)]) for n in range(len(position_proba))])
StanddevTrue   = np.sqrt(np.power(X_standdev,2) + np.power(Y_standdev,2))

temp = StanddevTrue
temp2 = temp.argsort()
thresh = temp[temp2[int(len(temp2)*0.2)]]
selectionTrue = np.array(StanddevTrue)<thresh

# CNN
Results = np.load(os.path.expanduser('/home/mobshamilton/Documents/dataset/RatCatanese/mobsEncoding_2019-09-30_shuffledSpikes/_simDecodingUnshuffled.npz'))
position_proba=Results['position_proba']
position = Results['position'].tolist()
for n in range(len(position_proba)):
    if np.sum(position_proba[n,:,:])==0.0:
...
X_err = [np.abs(X_true[n] - X_guessed[n]) for n in range(len(X_true))]
Y_err = [np.abs(Y_true[n] - Y_guessed[n]) for n in range(len(Y_true))]
ErrorCNN=np.array([np.sqrt(X_err[n]**2 + Y_err[n]**2) for n in range(len(X_err))])
X_standdev = np.sqrt([np.sum([X_proba[n][x]*(Bins[0][x]-X_guessed[n])**2 for x in range(Bins[0].size)]) for n in range(len(position_proba))])
Y_standdev = np.sqrt([np.sum([Y_proba[n][y]*(Bins[1][y]-Y_guessed[n])**2 for y in range(Bins[1].size)]) for n in range(len(position_proba))])
StanddevCNN   = np.sqrt(np.power(X_standdev,2) + np.power(Y_standdev,2))

temp = StanddevCNN
temp2 = temp.argsort()
thresh = temp[temp2[int(len(temp2)*0.2)]]
selectionCNN = np.array(StanddevCNN)<thresh

# LSTM
results = np.load(os.path.expanduser('~/Documents/dataset/RatCatanese/_resultsForRnn_2019-11-19_aligned.npz'))
Y_test = results['pos']
speed = results['spd']
...
temp2 = temp.argsort()
thresh = temp[temp2[int(len(temp2)*lossSelection)]]
selectionLSTM = testOutput[:,2]<thresh

xError = np.abs(testOutput[:,0]*maxPos - Y_test[:,0])
yError = np.abs(testOutput[:,1]*maxPos - Y_test[:,1])
ErrorLSTM = np.array([np.sqrt(xError[n]**2 + yError[n]**2) for n in range(len(xError))])

offset = synchronize([np.array(Y_test[:,0]*maxPos), np.array(Y_test[:,1]*maxPos)], [np.array(X_true), np.array(Y_true)])
offset = 5933-5887
...
# plt.bar(np.arange(3), 
#     [np.mean(ErrorCNN), 
#         np.mean(ErrorLSTM[offset:]*maxPos), 
#         np.mean(np.abs(ErrorCNN - ErrorLSTM[offset:]*maxPos))], 
#     yerr=[np.std(ErrorCNN), 
#         np.std(ErrorLSTM[offset:]*maxPos), 
#         np.std(np.abs(ErrorCNN - ErrorLSTM[offset:]*maxPos))], 
#     width=barwidth, color='grey', edgecolor='black', label='mean '+r'$\pm$'+' '+r'$\sigma$')
# plt.bar([x+barwidth for x in np.arange(3)], 
#     [np.median(ErrorCNN), 
#         np.median(ErrorLSTM[offset:]*maxPos), 
#         np.median(np.abs(ErrorCNN - ErrorLSTM[offset:]*maxPos))], 
#     yerr=[(np.percentile(ErrorCNN,70)-np.percentile(ErrorCNN,30))/2, 
#         (np.percentile(ErrorLSTM[offset:]*maxPos,70)-np.percentile(ErrorLSTM[offset:]*maxPos,30))/2,
#         (np.percentile(np.abs(ErrorCNN - ErrorLSTM[offset:]*maxPos),70)-np.percentile(np.abs(ErrorCNN - ErrorLSTM[offset:]*maxPos),30))/2],
#     width=barwidth, color='white', edgecolor='black', label='median '+r'$\pm$'+' 20 percentile')
# plt.xticks([r + barwidth/2 for r in range(3)], ['CNN + bayes', 'CNN + LSTM', 'difference'])
# plt.legend(loc="upper right")
# plt.suptitle('Inferred error')


# boxplot
fig, ax = plt.subplots()
ax = plt.subplot2grid((1,3),(0,1))
ax.title.set_text('CNN + bayesian decoder')
ax.boxplot([ErrorCNN, ErrorCNN[selectionCNN], ErrorCNN[selectionLSTM[:-1]]], showfliers=False)
plt.xticks([1.0, 2.0, 3.0], ['All bins', 'Selected Bins', 'Selected using\nLSTM inferred error'])

ax = plt.subplot2grid((1,3),(0,2))
ax.title.set_text('CNN + LSTM decoder')
ax.boxplot([ErrorLSTM, ErrorLSTM[selectionLSTM], ErrorLSTM[selectionLSTM]], showfliers=False)
plt.xticks([1.0, 2.0, 3.0], ['All bins', 'Selected Bins', 'Selected using\nLSTM inferred error'])

ax = plt.subplot2grid((1,3),(0,0))
ax.title.set_text('Bayesian decoder')
ax.boxplot([ErrorTrue, ErrorTrue[selectionTrue], ErrorTrue[selectionLSTM[:-1]]], showfliers=False)
plt.xticks([1.0, 2.0, 3.0], ['All bins', 'Selected Bins', 'Selected using\nLSTM inferred error'])

plt.suptitle("Mean Error of decoding")
...
## Overview aligned between CNN and LSTM
# fig = plt.subplots(figsize=(20,12))
# ax1 = plt.subplot2grid((2,1),(0,0))
# # ax1.plot(np.where(selectionCNN)[0]*0.036, np.array(X_guessed)[selectionCNN], label=None)
# # ax1.plot([(n+offset)*0.036 for n in range(len(X_true))], ErrorCNN, label=None, color='k', linewidth=5)
# ax1.plot([(n+offset)*0.036 for n in range(len(X_true))], X_true, label=None, color='k', linewidth=5)
# ax2 = plt.subplot2grid((2,1),(1,0), sharex=ax1)
# # ax2.plot(np.where(selectionCNN)[0]*0.036, np.array(Y_guessed)[selectionCNN], label='guessed position')
# ax2.plot([(n+offset)*0.036 for n in range(len(X_true))], Y_true, label='true position', color='k', linewidth=5)
# ax2.legend(loc="lower right", fontsize=25)
# plt.xlim(100,170)
# # plt.suptitle("CNN + bayes")


## LSTM inferred error against bayesian true error
fig, ax1 = plt.subplots(figsize=(20,12))
# ax1 = plt.subplot2grid((2,1),(0,0))
# ax1.plot(np.where(selectionLSTM)[0]*0.036, testOutput[selectionLSTM,0], label=None)
ax1.plot([n*0.036 for n in range(len(Y_test)-1)], ErrorTrue/10, label='Error of bayesian model')
ax1.plot([n*0.036 for n in range(len(Y_test))], testOutput[:,2]*maxPos, label='Inferred error of decoder')
# ax2 = plt.subplot2grid((2,1),(1,0), sharex=ax1)
# ax2.plot(np.where(selectionLSTM)[0]*0.036, testOutput[selectionLSTM,1], label='guessed position')
# ax2.plot([n*0.036 for n in range(len(Y_test))], Y_test[:,1]*maxPos, label='true position', color='b', linewidth=5)
ax1.legend(loc="upper right", fontsize=15)
plt.xlim(100,170)
plt.suptitle("LSTM")
fig, ax = plt.subplots(figsize=(15,15))
scattWithError(testOutput[:-1,2]*maxPos, ErrorTrue,'inferred error', ax)
plt.xlim(0,100)
plt.ylim(0,12)


# histogram of errors and comparison
fig, ax = plt.subplots(figsize=(15,9))
ax = plt.subplot2grid((2,3),(0,0))
_, edges, _ = ax.hist(ErrorTrue, bins=75, color='tab:orange', edgecolor='k', alpha=0.7)
ax.set_xlabel('true error')
ax.set_ylabel('number of windows')
ax.set_title('Classical bayesian decoder', size=15)

ax = plt.subplot2grid((2,3),(0,1), sharex = ax, sharey=ax)
ax.hist(ErrorCNN, bins = edges, color='tab:blue', edgecolor='k', alpha=0.7)
ax.set_xlabel('true error')
ax.set_ylabel('number of windows')
ax.set_title('CNN + modified\n bayesian decoder', size=15)

ax = plt.subplot2grid((2,3),(0,2), sharex = ax, sharey=ax)
ax.hist(ErrorLSTM, bins = edges, color='tab:green', edgecolor='k', alpha=0.7)
ax.set_xlabel('true error')
ax.set_ylabel('number of windows')
ax.set_title('CNN + LSTM decoder', size=15)

ax = plt.subplot2grid((2,3),(1,0), sharex = ax)
scattWithError(StanddevTrue, ErrorTrue,'width of probability distribution', ax)

ax = plt.subplot2grid((2,3),(1,1), sharex = ax, sharey=ax)
scattWithError(StanddevCNN, ErrorCNN,'width of probability distribution', ax)
plt.ylim(0,120)

ax = plt.subplot2grid((2,3),(1,2), sharex = ax)
scattWithError(testOutput[:,2]*maxPos, ErrorLSTM,'inferred error', ax)
# plt.savefig(os.path.expanduser('~/Dropbox/Mobs_member/Thibault/Poster Chicago Thibault/allErrorsAndComparisons.png'), bbox_inches='tight')


# # compraison CNN / LSTM
# disagreement = np.abs(np.array(Error) - np.array(Error[1]))
...
## some bins where guys don't agree
...
plt.hist(times[1:],bins=100, alpha=0.5,edgecolor='k', label='Model: CNN + modified bayesian')

Results = np.load('/home/mobshamilton/Documents/dataset/RatCatanese/_resultsForRnn_2019-10-10_normalWithTimeStamps.npz')
times = Results['times']
plt.hist(times[1:],bins=100, color='tab:green', alpha=0.5,edgecolor='k', label='Model: CNN + LSTM')
