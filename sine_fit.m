function [Xf,Yf,Yf2,param]=sine_fit(x,y,x2,fixed_params,initial_params,plot_flag)
...
if nargin==3 %simplest valid input
...
x_vector=min(x):mean(diff(x)):max(x);  
x_vector=x;
Xf=x_vector;
Yf=f(BETA,x_vector);
Yf2=f(BETA,x2);
