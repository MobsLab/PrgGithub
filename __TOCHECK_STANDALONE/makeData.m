...
% feb 2015 : old function
% see rather makeDataBulb or makeDataIntan

fff=1;
plo=0;
setCu=0;

Art=150;% old camera Art=100;
AreaLim=[0 600]; % old data AreaLim=[0 400];
...
try 
    ...
    try
        ...
        try
            ...
            listLFP=[0 1 2 3 4 5 7 8 9 10 11 12 13 14 15];
...
if 1
    ...
    Pos;
            ...
            global DATA
            filPos=[DATA.session.path,'/',DATA.session.basename,'.pos'];
...
%                 Art=100;
                ...
                plot(PosTemp(:,2),PosTemp(:,3)), xlim(AreaLim), ylim(AreaLim)
                plot(PosC(:,2),PosC(:,3),'r'), xlim(AreaLim), ylim(AreaLim),
                try
...
%                                                
                                           ...
                                           %     Art=100;
                                                ...
                                                subplot(2,1,1),hold on
                                                plot(PosTemp(:,2),PosTemp(:,3)), xlim(AreaLim), ylim(AreaLim)
                                                plot(PosC(:,2),PosC(:,3),'r'), xlim(AreaLim), ylim(AreaLim),title(namePos{i}(27:end-9));
                                                
                                                try
                                                
                                                plot(Data(xplo),Data(yplo),'k','linewidth',2), xlim(AreaLim), ylim(AreaLim),
                                                xploC=tsd(PosC(:,1)*1E4,PosC(:,2));
                                                yploC=tsd(PosC(:,1)*1E4,PosC(:,3));
                                                xploC=Restrict(xploC,FinalEpoch);
                                                yploC=Restrict(yploC,FinalEpoch);
                                                plot(Data(xploC),Data(yploC),'g','linewidth',2), xlim(AreaLim), ylim(AreaLim),    
...
%             end
           ...
           % tpsGood=1/FreqVideo:tps(end):tps(end)
