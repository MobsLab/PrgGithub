...
function [ptab,htab] = MultimodalTest(data,max_k,AnalyType)
...
if AnalyType %very slow , goes through all the h in interval 0 1
...
if 0
    ...
    hold on
        yl=ylim;
     line([x(modes)' x(modes)']',yl'*ones(length(modes),1)','color','r');    
    %Lines(x(modes),[],'r');
