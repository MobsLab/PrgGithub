...
function [C,CP,f,p,phi,S1l,S2l,fl]=CoherencyAstroFieldforLoopLR(chF,Fup)
...
% catch
%                 Y=locdetrend(Y,chF,[100,10]);                                   
%             F=locdetrend(F,chF,[100,10]);  
% end
...
%     Ajustement manuel des limites de l'enregistrement à analyser
            ...
            figure('Color',[1 1 1]),
%             plotyy(tps,F,tps,Y)
            hold on, plot(zscore(Y))            
            hold on, plot(zscore(F),'r')
            ...
            save DataCompLFP10 Y F tps chF
...
%   suite figure finale
            ...
            tempS1=(S1l(fl<Fup))/max(S1l(fl<Fup));
            tempS2=(S2l(fl<Fup))/max(S2l(fl<Fup));
            tempsmo=4;
            
            subplot(4,1,2)
            plot(fl(fl<Fup),smooth(tempS1,tempsmo),'r','linewidth',2)
            hold on, plot(fl(fl<Fup),smooth(tempS2,tempsmo),'b','linewidth',2)
...
% S2l=10*log10(S2);
  ...
  save Coherency C CP p phi S12 S1 S2 S1l S2l f confC phierr Cerr params  
