...
[Cl,lag,p1,CPL,CPl] = StatsXcorr(Data(Fl),Data(Yl),chF,100);
[Cm,lag,p2,CPM,CPm] = StatsXcorr(Data(Fm),Data(Ym),chF,100);
[Ch,lag,p3,CPH,CPh] = StatsXcorr(Data(Fh),Data(Yh),chF,100);
[Cuh,lag,p4,CPUH,CPuh] = StatsXcorr(Data(Fuh),Data(Yuh),chF,100);

if plott==1
        subplot(2,3,[2,5])
        hold on, plot(lag/chF,Cl,'k'), plot(lag/chF,CPL,'Color',[0.6 0.6 0.6]), plot(lag/chF,CPl,'Color',[0.6 0.6 0.6])
        hold on, plot(lag/chF,Cm,'b'), plot(lag/chF,CPM,'Color',[0.6 0.6 0.6]), plot(lag/chF,CPm,'Color',[0.6 0.6 0.6])
        hold on, plot(lag/chF,Ch,'r'), plot(lag/chF,CPH,'Color',[0.6 0.6 0.6]), plot(lag/chF,CPh,'Color',[0.6 0.6 0.6])
        hold on, plot(lag/chF,Cuh,'g'), plot(lag/chF,CPUH,'Color',[0.6 0.6 0.6]), plot(lag/chF,CPuh,'Color',[0.6 0.6 0.6]),xlim([-50 50])
        yl=ylim;
...
save DataCrossCorrPartFreq chF rLisa pLisa lags lag Cl Cm Ch Cuh CPM CPm CPL CPl CPH CPh CPUH CPuh p1 p2 p3 p4 DelaisOne FLow FMedium FHigh FUltraHigh YLow YMedium YHigh YUltraHigh zfl zfm zfh zfuh zyl zym zyh zyuh r p R freqFl freqFm freqFh freqFuh% f S1 S2 C CPMn CPmn
