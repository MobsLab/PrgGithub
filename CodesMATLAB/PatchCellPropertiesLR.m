function [Res,numfig,HistoUp,binUp,HistoDown,binDown,Spect,freq]=PatchCellPropertiesLR(yy,tps,spikes,DebutUp,FinUp,fig,data,nom)
...
for i=1:20
 if DebutUp(1)*1E4-8000<tps(1)*1E4
...
for i=1:20 
  if DebutUp(end)*1E4+8000>tps(end)*1E4
...
for i=1:20 
  if FinUp(end)*1E4+8000>tps(end)*1E4
     DebutUp=DebutUp(1:end-1);
     FinUp=FinUp(1:end-1);
 end
end
...
if fig
    ...
    try
        ...
        figure(nn), 
        [fh, rasterAx, histAx, matValS] = ImagePETH(tsd(tps*1E4,zscore(yy)), ts(DebutUp(2:end-1)*1E4), -8000, +8000,'BinSize',100);
    end
    try
        figure('Color',[1 1 1]), 
        mm=gcf; 
        [fh, rasterAx, histAx, matValE] = ImagePETH(tsd(tps*1E4,zscore(yy)), ts(FinUp*1E4), -8000, +8000,'BinSize',100);
    catch
        figure(mm), 
        [fh, rasterAx, histAx, matValE] = ImagePETH(tsd(tps*1E4,zscore(yy)), ts(FinUp(2:end-1)*1E4), -8000, +8000,'BinSize',100);
    end
...
try
     if sS==1
        percSm(i)=fitsigmoid(tts(20:end-20),dds(20:end-20,i));
     elseif sS==2
        percSm(i)=fitsigmoid(tts(60:end-60),dds(60:end-60,i));
     else
...
try
    if sE==1
    percEm(i)=fitsigmoid(tte(20:end-20),dde(20:end-20,i));
    elseif sE==2
    percEm(i)=fitsigmoid(tte(60:end-60),dde(60:end-60,i));
...
[Cross, Bcross] = CrossCorr(spikes*1E4, spikes*1E4, binsize,nombreBins);
Cross(Bcross==0)=0;
...
if thresholdManual
    eval(['save DataFinalPlotthresholdManual',nomL,' labels Res spikes Delai Bins nBinns th Csp Bsp Bs Be Cs Ce pow f NFFT Bcross Cross h1 h2 x1 x2 j1 j2 j3 j4 DebutUp FinUp nbspk temps maxAuto Skew Kurto Indice nbspkUp DureeUpTotal DureeDownTotal nbupStates DureeUp DureeDown amplUp percS percE percSm percEm PBimod PUnimod p MembPot PotUp PotDown AmpSpk AhpSpk FreqS0 freq Spect M idt pc1'])
else
...
if thresholdManual
try
    saveFigure(numfig,'figure1ThM',pwd)
saveFigure(nn,'figure2ThM',pwd)
saveFigure(mm,'figure3ThM',pwd)
saveFigure(numfig2,'figure4ThM',pwd)
end
else
