...
function [idX,Ppr0,Tpr0,binCrossTzs]=ObsBinCohrT(Mat,smoo)
...
if size(binCrossT)~=size(Mat)
load ResultsCompLFPSpectre
elseif size(binCrossT)~=size(Mat)
load ResultsCompLFPCross
elseif size(binCrossT)~=size(Mat)
load ResultsCompLFPS
...
title('Ranking according to the maximum')
...
title('Ranking according to the positive Peak the closest to zero')
