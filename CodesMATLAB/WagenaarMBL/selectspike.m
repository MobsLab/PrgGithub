...
% SELECTSPIKE - User interaction for posthoc spike classification
...
plot(0,0,'b.','markersi',15,'tag','lowerdots','buttondownfcn',@phsc_click);
plot([0 0],[0 0],'g','linew',2,'tag','upperline','buttondownfcn',@phsc_click);
plot(0,0,'g.','markersi',15,'tag','upperdots','buttondownfcn',@phsc_click);
...
switch phsc_data{figh}.src.type
  case 'spikes'
    phsc_data{figh}.C = 1;
    phsc_data{figh}.T = max([max(phsc_data{figh}.src.spk.tms) 60]);
  case 'histo'
