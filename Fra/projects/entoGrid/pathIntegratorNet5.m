...
switch Simulation
  ...
  %    synapticMatrix = 'Load';
        ...
        connectionSpan = 8; % the spread of the conncetion matrix
        connectionSpanAH = connectionSpan;
        connectionSpanHA = connectionSpan;        
        
        N = 576;  % the number of cells
        ...
        G = 10; % the gain of the threshold-linear function
        threshold = 0.; % the threshold of the threshold-linear function

        fixAct = 2; % the maximum average activity allowed by the inhibition mechanism
        % gamma = 50 first value 
        gamma = 200; % the relative importance of the asymmetric component in the synapric matrix
...
switch synapticMatrix
    ...
    case 'TriangularPeriodic'
        ...
        vq = [xh(:),yh(:)];
        vm = vq * M;
        xh = (vm(:,1)-0.5)*latticeSpacing;
        yh = (vm(:,2))*latticeSpacing;
        ...
        for i = -2:2
            for j = -2:2
                [XC, X] = meshgrid(xh, x+i*v1(1)+j*v2(1), xh);
                [YC, Y] = meshgrid(yh, y+i*v1(2)+j*v2(2));
                D = sqrt((X-XC).^2 + (Y-YC).^2);
                zeta = atan2(Y-YC, X-XC);
                Jha = Jha + max(cos(zeta-Phi) .* D.* exp(-D  /connectionSpan)/connectionSpan, 0);
            end
        end
...
switch trajectory
    ...
    case 'RandomForaging'
         [xRat, yRat] = randomForaging(nSteps, cageSize);
end
