...
Figures_dir = [parent_dir(A) filesep 'ReactTimeCourse' filesep 'ReactThetaMvt'];
...
XS1 = Restrict(XS,sleep1Epoch);
YS1 = Restrict(YS,sleep1Epoch);
...
sub1 = subplot(1,2,1);
	...
	plot(times - times(1),RsmF,'LineWidth',2,'Color','k')
...
sub2 = subplot(1,2,2);
	...
	Rsmooth = conv(Data(reactTimeCourseS2),gausswin(1200))/sum(gausswin(1200));
	Rsmooth = Rsmooth(600:end-600);
	RsmF = Rsmooth(rg);
	maxY = max(RsmF)+0.005;
	minY = min(RsmF)-0.005;
...
ylim1 = get(sub1,'YLim');
ylim2 = get(sub2,'YLim');
set(sub1,'YLim',[min(min(ylim1),min(ylim2)) max(max(ylim1),max(ylim2))]);
set(sub2,'YLim',[min(min(ylim1),min(ylim2)) max(max(ylim1),max(ylim2))]);
