...
/*       for(n = 0 ; n < 20; n++) */
/* 	{ */
/* 	  mexPrintf("%d: t0 = %g t1 = %g ix_first = %d, t = %g if_last = %d t = %g\n",  */
/* 		    n, t0[n], t1[n], ix_first[n],  */
/* 		    t[ix_first[n]], ix_last[n], t[ix_last[n]]); */
/* 	} */
...
/* 	  mexPrintf("### 2\n"); /\* debug *\/ */
	  ...
	  if(f == g) /* we're outside all intervals, so don't shuffle */
	  ...
	  else
	    {
	      ...
	      to[i] = t[i] - t0[ix] + t0[shuffle_idx[ix]];
	      /* if(i < 10 && i < n_points) /\* debug *\/ */
/* 		{ */
