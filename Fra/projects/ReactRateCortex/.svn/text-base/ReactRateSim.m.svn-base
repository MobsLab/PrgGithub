function ReactRateSim()
  ...
  x0 = randn(n_cells, 1) * sqrt(var_X0);
  logFrate_S1 = x0 + randn(n_cells, 1) * sqrt(var_S1);
  logFrate_S2 = x0 + randn(n_cells, 1) * sqrt(var_S2);
  logFrate_M = x0 + randn(n_cells, 1) * sqrt(var_M);  
...
% $$$   ylabel( 'X_S1S2', 'interpreter', 'none');;
  ...
  corrcoef(X_MS1, X_S2S1)
  corrcoef(X_MS2, X_S2S1)
  
  display('Expected variance')

  [EV, EVr] = ReactEV(logFrate_S1, logFrate_S2, logFrate_M)
  
  [a, b, r, p] = regression_line(logFrate_S1, logFrate_M);
  
  r_M = logFrate_M - a - b * logFrate_S1;
  
  [a, b, r, p] = regression_line(logFrate_S1, logFrate_S2);
  
  r_S2 = logFrate_S2 - a - b * logFrate_S1;
  ...
  % with the three variances made equal, we get a correlation of about
  % 0.5
  
  keyboard
  % now let's see how this correlation varies as a function of var_M,
  % leaving var_S1, var_S2 at 1
  ...
  plot(variances, EV, 'go');
  plot(variances, EVr, 'mo');
  
  legend({'slope', 'correlation', 'EV', 'EVr'});
  xlabel('variances')
