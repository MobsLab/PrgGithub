function AO = ReactRate(A)
  ...
  A = registerResource(A, 'CellRow', 'numeric', dim_by_cell, ...
		       'cell_row', ...
		       ['the row (antero/posterior) ', ...
		    ' in the warpdrive'], 1);

  A = registerResource(A, 'CellColumn', 'numeric', dim_by_cell, ...
		       'cell_column', ...
		       ['the column (left/right) ', ...
		    ' in the warpdrive'], 1);
  ...
  cell_row = zeros(size(cellnames));
  cell_column = zeros(size(cellnames));  
  
  for i = 1:length(cellnames)
    cn = cellnames{i};
    x1 = findstr(cn, '_');
    cn = cn((x1(1)+1):(x1(2)-1));
    cell_row(i) = double(cn(1)-64);
    cell_column(i) = str2num(cn(2:end));
  end
