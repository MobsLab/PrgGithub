function fig_st = CRAMGlobalFigs()
...
% $$$   pyr = find(~int);
  ...
  [r, clo, chi] = nancorrcoef(X_MS1, X_S2S1, 0.05, 'bootstrap');
  
  log_string = ...
      ['global rate react, direct, all cells',  newline, ...
       'slope = ' num2str(b), ' intercept = ', num2str(a), newline, ...
       'correlation = ', num2str(r(1,2)^2), ' (', num2str(clo(1,2)), ',', ...
      num2str(chi(1,2)), ')'];
  ...
  fig_direct.figureName = 'CRAMGlobalReactDirect';  
  ...
  fig_inverse.figureName = 'CRAMGlobalReactInverse';  
  
  [r, clo, chi] = nancorrcoef(X_MS2, - X_S2S1, 0.3, 'bootstrap');
