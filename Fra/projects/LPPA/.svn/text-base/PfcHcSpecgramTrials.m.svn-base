function A = PfcHcSpecgramTrials(A)
...
A = registerResource(A, 'PfcSpecgramTrials', 'tsdArray', {1,1},...
    'pfcSpecgramTrials', ...
    ['specgram  for prefrontal  EEG', ...
    'for teh trial period']);

A = registerResource(A, 'HcSpecgramTrials', 'tsdArray', {1,1},...
    'hcSpecgramTrials', ...
...
EEGpfc = Restrict(EEGpfc, Range(startTrial), Range(trialOutcome));
...
EEGhc = Restrict(EEGhc, Range(startTrial), Range(trialOutcome));
...
times = times(ix);
 ...
 pfcSpecgramTrials = tsd(times, S);

 [S,t,f,Serr]=mtspecgramc(deegHc,movingwin,params);
 hcSpecgramTrials = tsd(times, S);
