...
try
    ...
    load behavResources
    Epoch=intervalSet(tpsdeb{1}*1E4,tpsfin{1}*1E4);
end
...
if sleep
    try
        SleepSelEpoch;
        idd=Data(Restrict(burst,SleepSelEpoch));
    catch
...
if 1
    ...
    for a=2:size(data1{2},2)
    subplot(size(data1{2},2)-1,1,a-1), hold on
    for j=1:10
    plot(data1{idd(j)}(debtemp:fintemp,a),'k','linewidth',1), ylim([-3000 3000])
    end
    end
     end
subplot(size(data1{1},2)-1,1,1),title('raw data from tetrode 1 centered to ICRS')
...
subplot(size(data2{1},2)-1,1,1),title('raw data from tetrode 2 centered to ICRS')
...
if sleep
                ...
                for j=spknum
                    try
                      ...
                      if id(1)>81
                        ...
                        %if temp(78)<thresh
                          Tot1{a-1}=[Tot1{a-1};(data1{idd(j)}(id(1)-80:id(1),a))'];
            ...
            for a=2:size(data2{1},2)
                ...
                for j=spknum
                     ...
                     if id(1)>81
                    temp=data1{idd(j)}(id(1)-80:id(1),5);
                    Tot2{a-1}=[Tot2{a-1};(data2{idd(j)}(id(1)-80:id(1),a))'];
                       % if temp(78)<thresh
            ...
            for a=2:size(data1{1},2)
                Tot1Resc{a-1}=[];
                if tetrode==2;
                temp=Tot2{channel}(:,1:end-1)';    
                else
                temp=Tot1{channel}(:,1:end-1)';   
                end
                %temp=Tot1{a-1}(:,1:end-1)';    
                [BE, id]=min(temp(75:80,:)); 
                temp=Tot1{a-1}(:,1:end-1)'; 
                subplot(size(data1{1},2)-1,1,a-1), hold on
                for i=1:length(id)
                    if length(temp(75+id(i)-10:min([id(i)+85,80]),i))>12
                        temp2=temp(75+id(i)-10:min([id(i)+85,80]),i)';
                    Tot1Resc{a-1}=[Tot1Resc{a-1}; temp2(1:13)];
                    plot(temp(75+id(i)-10:min(id(i)+85,80),i), 'color','k')
                    end
                end
                xlim([0 13])
                ylim([-3000 3000])
            ...
            for a=2:size(data2{1},2)
                ...
                [BE, id]=min(temp(75:80,:));  
                temp=Tot2{a-1}(:,1:end-1)'; 
                subplot(size(data2{1},2)-1,1,a-1), hold on
                for i=1:length(id)
                    if length(temp(75+id(i)-10:min([id(i)+85,80]),i))>12
                        temp2=temp(75+id(i)-10:min([id(i)+85,80]),i)';
                        Tot2Resc{a-1}=[Tot2Resc{a-1}; temp2(1:13)];
                        plot(temp(75+id(i)-10:min([id(i)+85,80]),i), 'color','k')
                    end
                end
                xlim([0 13])
            ...
            figure('color',[1 1 1]), imagesc(Tot2Resc{2}), caxis([-1000 1000])
            
            figure('color',[1 1 1]), 
            for ki=1:size(data1{1},2)-1
            subplot(size(data1{1},2)-1,1,ki), hold on
                plot(mean(Tot1Resc{ki}),'k','linewidth',2)
                plot(mean(Tot1Resc{ki})+stdError(Tot1Resc{ki}),'k')
                plot(mean(Tot1Resc{ki})-stdError(Tot1Resc{ki}),'k')
                xlim([0 13])
             ylim([-3000 3000])
            end
            ...
            figure('color',[1 1 1]), 
            for ki=1:size(data2{1},2)-1
            subplot(size(data2{1},2)-1,1,ki), hold on
                plot(mean(Tot2Resc{ki}),'k','linewidth',2)
                plot(mean(Tot2Resc{ki})+stdError(Tot2Resc{ki}),'k')
                plot(mean(Tot2Resc{ki})-stdError(Tot2Resc{ki}),'k')
                xlim([0 13])
             ylim([-3000 3000])
            end
            ...
            for ki=1:size(data1{1},2)-1
            subplot(size(data1{1},2)-1,1,ki), hold on
                plot((Tot1Resc{ki}'),'k','linewidth',1)
            ...
            for ki=1:size(data2{1},2)-1
            subplot(size(data2{1},2)-1,1,ki), hold on
                plot((Tot2Resc{ki}'),'k','linewidth',1)
            ...
            figure('color',[1 1 1]), 
            if tetrode==2;
            [BE,idx]=sort(Tot2Resc{channel}(:,10));
            for ki=1:size(data2{1},2)-1
            subplot(size(data2{1},2)-1,1,ki), hold on
                plot((Tot2Resc{ki}(idx(1:80),:)'),'k','linewidth',1)
            ...
            [BE,idx]=sort(Tot1Resc{channel}(:,10)); 
            for ki=1:size(data1{1},2)-1
            subplot(size(data1{1},2)-1,1,ki), hold on
                plot((Tot1Resc{ki}(idx(1:80),:)'),'k','linewidth',1)
...
%%
            ...
            subplot(size(data1{1},2)-1,1,1),title('Raw data from tetrode 1 recentered to ICRS')
