...
% Corrected for MOBS Paris by Dima Bryzgalov 2019
% bryzgalovdm@gmail.com
...
%     PCAs_global = zeros(3,sum(kcoords==kcoords3),length(ia{i}));
    PCAs_global = zeros(4,sum(kcoords==kcoords3),length(ia{i})); % Changed by Dima Bryzgalov 10/04/2019
    ...
    %%%%%%%%%%% % Changed by Dima Bryzgalov 24/04/2019 %%%%%%%%%%%%%%%
    for j=1:size(waveforms,1)
        if j==1
            wpowers = int64(sum(waveforms(j,11:22,:).^2,2)/size(waveforms,2)/100); % Calculate power on 5 points around the peak
        else
            wpowers = [wpowers; (int64(sum(waveforms(j,11:22,:).^2,2)/size(waveforms,2)/100))]; % Calculate power on 5 points around the peak
        end
    end
    %%%%%%%%%%% % Changed by Dima Bryzgalov 24/04/2019 %%%%%%%%%%%%%%%
    rmpath(genpath([dropbox,'/Kteam/PrgMatlab/MuTE_onlineVersion/']));  %added by Sam 14/06/2019
...
%             PCAs_global(:,k,:) = pca(zscore(permute(waveforms(k,:,:),[2,3,1]),[],2),'NumComponents',3)';
            PCAs_global(:,k,:) = pca(zscore(permute(waveforms(k,11:28,:),[2,3,1]),[],2),'NumComponents',4)'; % Changed by Dima Bryzgalov 10/04/2019 - Calculate PCs on 5 points before and 12 points after the peak
        end
    else
...
%             PCAs_global(:,k,:) = pca(zscore(permute(waveforms(k,:,:),[2,3,1]),[],2),'NumComponents',3)';
    ...
    Fet = double([PCAs_global2; wranges; squeeze(wpowers); spikeTimes(ia{i})']);
...
fprintf('\nComplete!')
	...
	function waveforms_all = Kilosort_ExtractWaveforms(rez)
        ...
        ops = rez.ops;
        NT = ops.NT;
        if exist(ops.fbinary) == 0 % changed by Dima Bryzgalov 09/04/19 it was <if exist('ops.fbinary') == 0>
...
%                 [channel_order,channel_index] = sort(xml.AnatGrps(kcoords2(i)).Channels+1);
               %%%%%%%%%%%%%% % Changed by Dima Bryzgalov 20/04/2019 %%%%%%
               chantoan = chanMap(find(rez.connected));
                [channel_order,channel_index] = sort(chantoan(find(kcoords==kcoords3)));
                %%%%%%%%%%%%%% % Changed by Dima Bryzgalov 20/04/2019 %%%%%%
