% PSTH_behav_accelero2

% similar to PSTH_behav_accelero, but plot 
% - either all CS +, averaged_2_by_2
%CSoption = 'AllCS avg2by2';
% disp ('CSoption = AllCS avg2by2')
% - either the first 4 CS +, no averaged
...
%% GET DATA TO PLOT
% try
%     load(['Spectro_ChR2/PSTH_behav_' manipname ]);
...
%             end
    ...
    if ~isdir([ res '/Spectro_ChR2'])
        mkdir([ res '/Spectro_ChR2']);
    end
    save ([res '/Spectro_ChR2/PSTH_behav_' manipname  ],'C','P','E', 'C0','C1','C2', 'C3','C4','C5','C6','C7','P0','P1','P2','P3','P3','P4','P5','P6','P7','E0','E1','E2','E3','C3','E4','E5','E6','E7',...
...
if plo
    ...
    if sav 
        for i=1:length(nameMice), 
            saveas(allfig(i), ['Spectro_ChR2/' nameMice{i} '_PSTH_allCS+.fig'])
            set(allfig(i),'PaperPosition',[0 0 12 20])
            saveas(allfig(i), ['Spectro_ChR2/' nameMice{i} '_PSTH_allCS+.png'])
        end
    end
...
for i=1:2;
    ...
    if i==1
        saveas(gcf,'AcceleroChR2_AvgPSTHmatrix.fig')
        saveas(gcf,'AcceleroChR2_AvgPSTHmatrix.png')
        
    elseif i==2
        saveas(gcf,'AcceleroChR2_indivPSTHmatrix.fig')
        saveas(gcf,'AcceleroChR2_indivPSTHmatrix.png')
