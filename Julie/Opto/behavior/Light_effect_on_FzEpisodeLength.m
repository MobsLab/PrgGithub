...
for k=0:3
    ...
    title(['gfp  ' num2str(sum(n1_gfp)) ' chr2  ' num2str(sum(n1_ch))])
...
subplot(131), PlotErrorBarN(NbEp_gfp(:,[1,2]),0,0,'ranksum',1), title('gfp')
subplot(132), PlotErrorBarN(NbEp_ch(:,[1,2]),0,0,'ranksum',1), title('chr2')
subplot(133), 
[p_an,table_an,stats_an,Pkw_g,table_kw_g,p_mw,stats_mw]=BarPlotBulbSham_gen({NbEp_gfp(:,[1,2]),NbEp_ch(:,[1,2])},'nb Fz Episodes',colori,'ranksum',1,3,3);
if sav
...
subplot(131), PlotErrorBarN(MeanDurEp_gfp,0,0,'ranksum',1), title('gfp')
subplot(132), PlotErrorBarN(MeanDurEp_ch,0,0,'ranksum',1), title('chr2')
subplot(133), 
[p_an,table_an,stats_an,Pkw_g,table_kw_g,p_mw,stats_mw]=BarPlotBulbSham_gen({MeanDurEp_gfp(:,[1,2]),MeanDurEp_ch(:,[1,2])},'Episodes Duration',colori,'ranksum',1,3,3);
