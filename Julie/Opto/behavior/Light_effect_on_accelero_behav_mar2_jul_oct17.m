...
%     period='fullperiod'; optionfullper='close2sound'; % option used for
%     paper
...
%                     saveFigure (gcf,['Movacctsd_' num2str(m) ],[res '/MovAccFig'])            
    ...
    %             line([Start(NosoundWithLaserPer)*1E-4 End(NosoundWithLaserPer)*1E-4],[aa aa]);aa=aa+1;
        ...
        bilanFreezeAccEpoch{man}=FreezeAccEpoch;
        Ep=FreezeAccEpoch;
        if strcmp(period,'fullperiod')
            %percentage of freeing during the different periods (four CS- and each block of four CS+)
            try
                bilan{stepN}(man,:)=[length(Data(Restrict(MovAccSmotsd,and(Ep,NosoundNoLaserPer))))/length(Data(Restrict(MovAccSmotsd,NosoundNoLaserPer))),...
                    length(Data(Restrict(MovAccSmotsd,and(Ep,NosoundWithLaserPer))))/length(Data(Restrict(MovAccSmotsd,NosoundWithLaserPer))),...
                    length(Data(Restrict(MovAccSmotsd,and(Ep,CsminPer))))/length(Data(Restrict(MovAccSmotsd,CsminPer))),...
                    length(Data(Restrict(MovAccSmotsd,and(Ep,CspluPer0))))/length(Data(Restrict(MovAccSmotsd,CspluPer0))),...
                    length(Data(Restrict(MovAccSmotsd,and(Ep,CspluPer1))))/length(Data(Restrict(MovAccSmotsd,CspluPer1))),...
                    length(Data(Restrict(MovAccSmotsd,and(Ep,CspluPer2))))/length(Data(Restrict(MovAccSmotsd,CspluPer2))),...
                    length(Data(Restrict(MovAccSmotsd,and(Ep,CspluPer3))))/length(Data(Restrict(MovAccSmotsd,CspluPer3)))];
...
%                 sounds1=intervalSet(Start(subset(CSmiInt,[1:4])),Start(subset(CSmiInt,[1:4]))+30*1e4) ;
%                 sounds3=intervalSet(Start(subset(CSplInt,[1:2])),Start(subset(CSplInt,[1:2]))+30*1e4) ;%=subset(CSplInt,[1:2]);
%                 sounds4=intervalSet(Start(subset(CSplInt,[3:4])),Start(subset(CSplInt,[3:4]))+30*1e4) ;%=subset(CSplInt,[3:4]);       
%                 sounds5=intervalSet(Start(subset(CSplInt,[5:8])),Start(subset(CSplInt,[5:8]))+30*1e4) ;%=subset(CSplInt,[5:8]);
%                 sounds6=intervalSet(Start(subset(CSplInt,[9:12])),Start(subset(CSplInt,[9:12]))+30*1e4) ;%=subset(CSplInt,[9:12]);
            else 
                sounds1=intervalSet(Start(subset(CSmiInt,[1:2])),Start(subset(CSmiInt,[1:2]))+45*1e4) ;
...
if sav
    cd(res)
    save([StepName{stepN} '_' period '_' optionfullper '_acc2'], 'bilan','gfpmice','chr2mice','StepName', 'Dir','period','bilanMovAccSmotsd','optionfullper',...
        'bilanMovAccSmotsd','bilanFreezeAccEpoch','CSmiInt','CSplInt')
    save([StepName{stepN} '_' period '_' optionfullper '_acc2'],'csm','csp','CSplInt','CSmiInt','-Append')

end

%% Barplot bilan
figure ('Position',[ 287         517       1792         312])
n=4; k=1;colCSplu=4; colCSpluLas=5;
% Chr2 mice
if ~strcmp(manip_name, 'Fear-CTRL');
    ...
    PlotErrorBarN(bilan{stepN}(chr2mice,:),0,1,'ranksum',colCSplu-1) % newfig=0; paired=1,columntest=2
    for man=1:size(bilan{stepN}(chr2mice,:),1)
            plot(bilan{stepN}(chr2mice(man),:),'Color',colori(man,:),'Marker','o','MarkerSize',3,'MarkerEdgeColor',colori(man,:),'MarkerFaceColor',colori(man,:))
    end
    ylim([0 1]), title('ChR2')
...
PlotErrorBarN(bilan{stepN}(gfpmice,:),0,1,'ranksum',colCSplu-1)
for man=1:size(bilan{stepN}(gfpmice,:),1)
        plot(bilan{stepN}(gfpmice(man),:),'Color',colori(man,:),'Marker','o','MarkerSize',3,'MarkerEdgeColor',colori(man,:),'MarkerFaceColor',colori(man,:))
end
...
if sav
    cd(res)
    saveas (gcf,[StepName{stepN} '_' period '_' optionfullper 'mean_acc.fig'])
    saveas (gcf,[StepName{stepN} '_' period '_' optionfullper 'mean_acc.png'])
    save([manip_name '_' StepName{stepN} '_acc'],'Dir','bilan','gfpmice','chr2mice','StepName','period','optionfullper');% 'Table'
    saveFigure(gcf,[StepName{stepN} '_' period '_' optionfullper 'mean_acc'],res)
end
