...
for st = 1:length(StepSizes)
    ...
    load([dropbox '/Mobs_member/SophieBagur/Figures/BehaviourOptoDurationEvents/SimulationFzOpto' num2str(stepsize),'NoMemory.mat'])
...
for l = 2:size(DurFzEp_GFP,2)
    for k=1:size(DurFzEp_GFP,1)
        ...
        PercFz_Rem{l-1} = [PercFz_Rem{l-1};[PercFz(row),PercFz_GFP(k,l)]];
        PercFz_remErr{l-1} = [PercFz_remErr{l-1};[((PercFz(row)-PercFz_GFP(k,l)).^2)./(PercFz_GFP(k,l).^2)]];
        Err_PercFz_GFP(st,k,l) = (PercFz_GFP(k,l)-PercFz(row)).^2;
        subplot(2,2,l+1)
        plot(FreqInit(row),FreqInit_GFP(k,l),'.','color',[0.4 0.8 0.4],'MarkerSize',20),hold on
        Err_FreqInit_GFP(st,k,l) = (FreqInit_GFP(k,l)-FreqInit(row)).^2;
        FreqInit_rem{l-1} = [FreqInit_rem{l-1};[FreqInit(row),FreqInit_GFP(k,l)]];
        FreqInit_remErr{l-1} = [FreqInit_remErr{l-1};[((FreqInit(row)-FreqInit_GFP(k,l)).^2)./(FreqInit_GFP(k,l).^2)]];
...
for l = 2:size(DurFzEp_GFP,2)
    for k=1:size(DurFzEp_GFP,1)
        ...
        PercFz_Rem{l-1} = [PercFz_Rem{l-1};[PercFz(row),PercFz_CHR2(k,l)]];
        PercFz_remErr{l-1} = [PercFz_remErr{l-1};[((PercFz(row)-PercFz_CHR2(k,l)).^2)./(PercFz_CHR2(k,l).^2)]];

        Err_PercFz_CHR2(st,k,l) = (PercFz_CHR2(k,l)-PercFz(row)).^2;
        subplot(2,2,l+1)
        plot(FreqInit(row),FreqInit_CHR2(k,l),'.','color',[0.4 0.4 0.8],'MarkerSize',20),hold on
        Err_FreqInit_CHR2(st,k,l) = (FreqInit_CHR2(k,l)-FreqInit(row)).^2;
        FreqInit_rem{l-1} = [FreqInit_rem{l-1};[FreqInit(row),FreqInit_CHR2(k,l)]];
        FreqInit_remErr{l-1} = [FreqInit_remErr{l-1};[((FreqInit(row)-FreqInit_CHR2(k,l)).^2)./(FreqInit_CHR2(k,l).^2)]];
