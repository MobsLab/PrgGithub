...
for st = 3%1:length(StepSizes)
    ...
    load([dropbox '/Mobs_member/SophieBagur/Figures/BehaviourOptoDurationEvents/DataFzingStatesopto_' num2str(stepsize),'.mat'])
    cols = {'k','b','r'}
...
title('no laser')
subplot(223)
line([0 0.15]*100,[0 0.15]*100,'color',[0.6 0.6 0.6],'linewidth',1.5), xlim([0 0.15]*100),ylim([0 0.15]*100), hold on
...
for l = 2:size(DurFzEp_GFP,2)
    ...
    for k=1:size(DurFzEp_GFP,1)
        ...
        PercFz_Rem{l-1} = [PercFz_Rem{l-1};[PercFz(row,col),PercFz_GFP(k,l)]];
        PercFz_remErr{l-1} = [PercFz_remErr{l-1};[((PercFz(row,col)-PercFz_GFP(k,l)).^2)./(PercFz_GFP(k,l).^2)]];
        Err_PercFz_GFP(st,k,l) = (PercFz_GFP(k,l)-PercFz(row,col)).^2;
        X1{l}(num) = PercFz(row,col);
        Y1{l}(num) = PercFz_GFP(k,l);
        
        
        subplot(2,2,l+1)
        plot(FreqInit(row,col)*100,FreqInit_GFP(k,l)*100,'.','color',[0.4 0.8 0.4],'MarkerSize',20),hold on
        Err_FreqInit_GFP(st,k,l) = (FreqInit_GFP(k,l)-FreqInit(row,col)).^2;
        FreqInit_GFP_model(k,l) = FreqInit(row,col);
        FreqInit_rem{l-1} = [FreqInit_rem{l-1};[FreqInit(row,col),FreqInit_GFP(k,l)]];
        FreqInit_remErr{l-1} = [FreqInit_remErr{l-1};[((FreqInit(row,col)-FreqInit_GFP(k,l)).^2)./(FreqInit_GFP(k,l).^2)]];
        X2{l}(num) = FreqInit(row,col);
        Y2{l}(num) = FreqInit_GFP(k,l);
        num=num+1;
    end
end
...
for l = 2:size(DurFzEp_GFP,2)
    for k=1:size(DurFzEp_GFP,1)
        ...
        PercFz_Rem{l-1} = [PercFz_Rem{l-1};[PercFz(row,col),PercFz_CHR2(k,l)]];
        PercFz_remErr{l-1} = [PercFz_remErr{l-1};[((PercFz(row,col)-PercFz_CHR2(k,l)).^2)./(PercFz_CHR2(k,l).^2)]];
        Err_PercFz_CHR2(st,k,l) = (PercFz_CHR2(k,l)-PercFz(row,col)).^2;
        X1{l}(num) = PercFz(row,col);
        Y1{l}(num) = PercFz_CHR2(k,l);
        
        subplot(2,2,l+1)
        plot(FreqInit(row,col)*100,FreqInit_CHR2(k,l)*100,'.','color',[0.4 0.4 0.8],'MarkerSize',20),hold on
        Err_FreqInit_CHR2(st,k,l) = (FreqInit_CHR2(k,l)-FreqInit(row,col)).^2;
        FreqInit_CHR2_model(k,l) = FreqInit(row,col);
        FreqInit_rem{l-1} = [FreqInit_rem{l-1};[FreqInit(row,col),FreqInit_CHR2(k,l)]];
        FreqInit_remErr{l-1} = [FreqInit_remErr{l-1};[((FreqInit(row,col)-FreqInit_CHR2(k,l)).^2)./(FreqInit_CHR2(k,l).^2)]];
        X2{l}(num) = FreqInit(row,col);
        Y2{l}(num) = FreqInit_CHR2(k,l);
        num=num+1;
