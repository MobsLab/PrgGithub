...
for st = 1:length(StepSizes)
    ...
    load(['/home/vador/Dropbox/Mobs_member/SophieBagur/BehaviourOptoDurationEvents/SimulationFzOpto' num2str(stepsize),'DecisionModel.mat'])
...
load(['/home/vador/Dropbox/Mobs_member/SophieBagur/BehaviourOptoDurationEvents/Figures/SimulationFzOpto' num2str(1.5),'DecisionModel.mat'])
load(['/home/vador/Dropbox/Mobs_member/SophieBagur/BehaviourOptoDurationEvents/Figures/DataFzingStatesopto_' num2str(1.5),'.mat'])
...
title('laser')
for l = 2:size(DurFzEp_GFP,2)
    ToCorr_PercFz{l-1}=[];ToCorr_FreqInit{l-1}=[];
    for k=1:size(DurFzEp_GFP,1)
        ErrMat = (DurFzEp-DurFzEp_GFP(k,l)).^2+(DurActEp-DurActEp_GFP(k,l)).^2;
        ErrMat = naninterp(ErrMat);
        ErrMat = SmoothDec(ErrMat,2);
        ...
        Pdec_GFP(k,l) = row*0.005;
        PFz_GFP(k,l) = col*0.005;
        ...
        ToCorr_PercFz{l-1} = [ToCorr_PercFz{l-1};[PercFz(row,col),PercFz_GFP(k,l)]];
        ToCorr_FreqInit{l-1} = [ToCorr_PercFz{l-1};[FreqInit(row,col),FreqInit_GFP(k,l)]];
    end
end
...
for l = 2:size(DurFzEp_GFP,2)
    for k=1:size(DurFzEp_GFP,1)
        ...
        Pdec_CHR2(k,l) = row*0.005;
        PFz_CHR2(k,l) = col*0.005;
        ...
        ToCorr_PercFz{l-1} = [ToCorr_PercFz{l-1};[PercFz(row,col),PercFz_CHR2(k,l)]];
        ToCorr_FreqInit{l-1} = [ToCorr_PercFz{l-1};[FreqInit(row,col),FreqInit_CHR2(k,l)]];
...
subplot(212)
hold on
handles = plotSpread({PFz_CHR2(:,1);PFz_CHR2(:,2);PFz_CHR2(:,3)},'distributionColors',[0.4 0.4 0.8],'xValues',[1 2 3]+0.1);
set(handles{1},'MarkerSize',15)
handles = plotSpread({PFz_GFP(:,1);PFz_GFP(:,2);PFz_GFP(:,3)},'distributionColors',[ 0.4 0.8 0.4],'xValues',[1 2 3]-0.1);
set(handles{1},'MarkerSize',15)
for k = 1:3
    line([0.82 0.98]+k-1,[1 1]*nanmean(PFz_GFP(:,k)),'linewidth',2,'color','k')
    line([1.02 1.18]+k-1,[1 1]*nanmean(PFz_CHR2(:,k)),'linewidth',2,'color','k')
end
for k=1:3
    [p,h]=ranksum(PFz_CHR2(:,k),PFz_GFP(:,k))
    if p<0.05
        p
        plot(k,1,'k*')
    end
...
subplot(211)
handles = plotSpread({Pdec_CHR2(:,1);Pdec_CHR2(:,2);Pdec_CHR2(:,3)},'distributionColors',[0.4 0.4 0.8],'xValues',[1.1 2.1 3.1]);
set(handles{1},'MarkerSize',15)
handles = plotSpread({Pdec_GFP(:,1);Pdec_GFP(:,2);Pdec_GFP(:,3)},'distributionColors',[ 0.4 0.8 0.4],'xValues',[0.9 1.9 2.9]);
set(handles{1},'MarkerSize',15)
for k = 1:3
    line([0.82 0.98]+k-1,[1 1]*nanmean(Pdec_GFP(:,k)),'linewidth',2,'color','k')
    line([1.02 1.18]+k-1,[1 1]*nanmean(Pdec_CHR2(:,k)),'linewidth',2,'color','k')
end
for k=1:3
    [p,h]=ranksum(Pdec_CHR2(:,k),Pdec_GFP(:,k));
    p
    if p<0.05
        plot(k,1,'k*')
    end
...
for k=1:3
    [p,h,stats]=ranksum(DurFzEp_CHR2(:,k),DurFzEp_GFP(:,k))
    if p<0.05
        plot(k,35,'k*')
    end
end
...
for k=1:3
    [p,h,stats]=ranksum(DurActEp_CHR2(:,k),DurActEp_GFP(:,k))
    if p<0.05
        plot(k,1,'k*')
