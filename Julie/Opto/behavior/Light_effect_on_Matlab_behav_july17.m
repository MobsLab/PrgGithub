...
%% INITIALIZE
% group CS+=WN
% CSplu_WN_GpNb=[465 466 496 498 502 504 506 ];
...
cd /media/DataMOBs57/Fear_July2017/behavior/
res=pwd;
    ...
    if rawPlo
        ...
        subplot(length(Dir.path),5,(man-1)*5+[1:3])
         plot(Range(Movtsd,'s'),Data(Movtsd),'k')
            ...
            title([ num2str(m) ' ' Dir.path{man}(end-17:end)  'max ' num2str(max(Data(Movtsd)))])
            ylim([0 30])
            xlim([0 1420])
            aa=21;
