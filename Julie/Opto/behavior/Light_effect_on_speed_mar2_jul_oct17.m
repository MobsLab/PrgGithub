...
try 
    ...
    for man=1:length(Dir.path)
        ...
        cd(Dir.path{man})
        try 
            load behavResources Movtsd X Y MovAccSmotsd FreezeAccEpoch
            Movtsd;X;Y; 
            V=diff(Range(X)
        catch
            keyboard
...
%                     saveFigure (gcf,['Movacctsd_' num2str(m) ],[res '/MovAccFig'])            
    ...
    %             line([Start(NosoundWithLaserPer)*1E-4 End(NosoundWithLaserPer)*1E-4],[aa aa]);aa=aa+1;
        ...
        if strcmp(period,'fullperiod')
            ...
            try
                bilan{stepN}(man,:)=[nanmean(Data(Restrict(V,NosoundNoLaserPer))),...
                    nanmean(Data(Restrict(V,NosoundWithLaserPer))),...
                    nanmean(Data(Restrict(V,CsminPer))),...
                    nanmean(Data(Restrict(V,CspluPer0))),...
                    nanmean(Data(Restrict(V,CspluPer1))),...
                    nanmean(Data(Restrict(V,CspluPer2))),...
                   nanmean(Data(Restrict(V,CspluPer3)))];
            catch
                bilan{stepN}(mousenb,:)=NaN(1,4);
                disp(['no  value for M' num2str(m) ' ' StepName{stepN} ])
            end
        elseif strcmp(period,'soundonly')
            disp('this option is not adapted in that code')
            keyboard
...
%     ylim([0 1]), 
    ...
    if ~isempty(strfind(Dir.path{man},'EXT'))
        ylabel([StepName{stepN} ' ' optionfullper ' speed'])
    end
...
%     ylim([0 1])
    ...
    text(0.05,0.85,[ 'Col test ' num2str(colCSplu) ' and ' num2str(colCSpluLas) ],'FontSize',8,'units','normalized')
    if 0
        for sp=1:3
...
if sav
    cd(res)
    saveas (gcf,[StepName{stepN} '_' period '_' optionfullper 'mean_speed.fig'])
    saveas (gcf,[StepName{stepN} '_' period '_' optionfullper 'mean_speed.png'])
    save([manip_name '_' StepName{stepN} '_speed'],'Dir','bilan','gfpmice','chr2mice','StepName','period','optionfullper');% 'Table'
    saveFigure(gcf,[StepName{stepN} '_' period '_' optionfullper 'mean_speed'],res)
end
