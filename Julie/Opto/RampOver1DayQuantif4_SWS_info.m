% RampOver1DayQuantif4_SWS_Info.m
% 29.08.2017

% appelée par RampOver1DayQuantif4_SWS.m
% 28.08.2017
...
try
    ...
    disp(['Loading existing data from DataRampOver1DayQuantif_no_selection_SWS_info.mat']);
catch
...
%     load(['DataRampOver1DayQuantif_no_selec_HPC_PiCx.mat'], 'StimInfoAllMan');
    ...
    for man=1:length(Dir.path) 
        ...
        for i=1:length(structlist)
            ...
            for fq=1:nb_fq
                ...
                % Condition : choose stim that started during SWS (1st second)
                ...
                for k=1:length(Start(LaserIntOneSecBef{man,fq}))
                    if ~isempty(Start(and(subset(LaserIntOneSecBef{man,fq},k),SWSEpoch)))
                        ind_IN_SWS=[ind_IN_SWS;1];
                    else
                        ind_IN_SWS=[ind_IN_SWS;0];
                    end
                end
                LFPpower_Info(1,i,man,1:length(ind_IN_SWS),1,fq)=ind_IN_SWS;
            end
        end
    end
    ...
    if sav
         save DataRampOver1DayQuantif_no_selection_SWS_Info LFPpower_Info  f1  structlistname fq_list Dir LaserInt  nb_fq fq_list
    end
end
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% for the following steps, see RampOver1DayQuantif4_SWS_Info
