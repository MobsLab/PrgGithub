...
% res='/media/DataMOBsRAIDN/ProjetAversion/RampOver1dayQuantif/Ramp_4_no_selection';
% res='/media/DataMOBsRAIDN/ProjetAversion/RampOver1dayQuantif/Ramp_4_no_selection/7mice_458-467';
% res='/media/DataMOBsRAIDN/ProjetAversion/RampOver1dayQuantif/Ramp_4_no_selection/4_3_mice_paper_july17';
...
for k=1:length(Dir.path)
    ...
    mousenames{k}=Dir.path{k}(ind_mouse+5:ind_mouse+8);
    if strcmp(Dir.name{k}(end),'/'), Dir.name{k}(end)=[];end
end

% mousenames={'458';'458';'459';'465';'465';'467';'467'};%;'466';'466';'468';'468'
...
%     cd(res)
%     
%     load(['DataRampOver1DayQuantif_no_selection_REM_Dima.mat'],'LFPpower','LFPpower_Before','fq_list','nb_fq','f1');
%     Epoch='';
%     disp(['Loading existing data from DataRampOver1DayQuantif_no_selection_Dima.mat']);

    
        load(['DataRampOver1DayQuantif_no_selection.mat'],'LFPpower','LFPpower_Before','fq_list','nb_fq','f1');
    Epoch='';
...
%     
%         load(['DataRampOver1DayQuantif_no_selection_SWS.mat']);
%     disp(['Loading existing data from DataRampOver1DayQuantif_no_selection_SWS.mat']);
...
try 
%     load RampOver1Day_quantifPlot_REM.mat
    load RampOver1Day_quantifPlot_REM_Dima.mat
catch
    for man=1:length(Dir.path), 
...
if 1
    ampl_th=200;
%     ampl_th=1000;
    disp (['remove bulbs for which efficiency inferior to ' num2str(ampl_th)])
...
if sav
    ...
    saveas(h_mice_mean,'power_at_stim_fq_all_mice.png')
    try saveFigure(h_mice_mean,'power_at_stim_fq_all_mice',res),end
end
...
if ratio_plo
    ...
    if sav
        ...
        saveas(h_mice_dur_bef_ratio,'power_during_before_ratio_all_mice.png')
        try saveFigure(h_mice_dur_bef_ratio,'power_during_before_ratio_all_mice',res), end
    end
end
...
if sav
    ...
    saveas(h_mice_dur_bef,'power_during-before_all_mice.png')
    try saveFigure(h_mice_dur_bef,'power_during-before_all_mice',res), end
end

end % end of avg_plo

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  difference DURING - BEFORE
%% bilan moyenne left and right
...
text(-0.2,1.2,'average left & right','units', 'normalized')
...
XL=xlim; line([XL(1) XL(2)],[0 0 ],'Color',[0.7 0.7 0.7])
ylim([-0.5E4 1.5E4]),
end
try
subplot(322)
...
if sav
    ...
    saveas(gcf,'power_during-before_all_mice_2_hpc_local.png')
    try saveFigure(gcf,'power_during-before_all_mice_2_hpc_local',res), end
    try
        eval(['save(''power_dur-bef_left_right'',''Mbulb'',''Mpfc'',''Mhpc'',''Mpicx'' )'])
    catch
        keyboard
        save('power_dur-bef_left_right','Mbulb','Mpfc','Mhpc','Mpicx')
    end
end
...
if sav
    ...
    saveas(h_mice_perc,'power_perc_all_mice.png')
    try saveFigure(h_mice_perc,'power_perc_all_mice',res), end
end
end
...
title(Epoch), ylabel('modulation index (4-10)/(4+10)')
