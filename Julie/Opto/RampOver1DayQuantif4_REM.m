...
try
    ...
    LFPpower_Before_REM=LFPpower_Before;
    LFPpower_REM=LFPpower;
...
%                 events not in REM are put to nan
                ...
                LFPpower_Before_REM(1,i,man,ind_evt,1:length(f1),fq)=nan;
                LFPpower_REM(1,i,man,ind_evt,1:length(f1),fq)=nan;
            end  
        end
    end
LFPpower=LFPpower_REM;
LFPpower_Before=LFPpower_Before_REM;
    cd(res)
    
    if sav
        save DataRampOver1DayQuantif_no_selection_REM LFPpower LFPpower_Before f1 f structlistname fq_list Dir LaserInt StimInfoAllMan f Sp_On Sp_Off nb_fq fq_list
    end
end
end
...
%                     sub{i,fq}=subplot(length(structlist),nb_fq,nb_fq*(i-1)+fq); hold on
        ...
        if sav
            saveas(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_' hemi '_RampOver1Day_mean_REM.fig'])
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end)  '_' hemi '_RampOver1Day_mean_REM'],res)
        end
