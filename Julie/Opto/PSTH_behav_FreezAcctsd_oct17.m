%PSTH_behav_FreezAcctsd2_july.m
% 22.08.2017
% code pas rtès approprié. designé au départ pour traiter des données sans
% manip control avec souris GFP
...
% mice_included=[363 367 458 459];
% mice_included=[465 466 467 468];
...
% group CS+=bip
CSplu_bip_GpNb=[467 468, 497 498, 503 505];
CSplu_bip_Gp={};
...
% group CS+=WN
CSplu_WN_GpNb=[363 367, 458 459, 465 466, 496 497, 502 504 506, 537 540 542 543, 610 611 612 613 614];
CSplu_WN_Gp={};
...
try
    ...
    for man=1:length(Dir.path) 
       ...
       if sum(strcmp(num2str(m),CSplu_bip_Gp))==1
        ...
        if strfind(Dir.path{man},'EXT')&strfind(Dir.path{man},'24') % EXT-24
            disp(StepName{1})
             for k=0:7
                 eval(['C' num2str(k) '=[C' num2str(k) ';f' num2str(k) '];']);
             end
            
        elseif strfind(Dir.path{man},'EXT')&strfind(Dir.path{man},'48') % EXT-48
            disp(StepName{2})
            for k=0:7
                eval(['P' num2str(k) '=[P' num2str(k) ';f' num2str(k) '];']);
            end

        elseif strfind(Dir.path{man},'EXT')&strfind(Dir.path{man},'72')  % EXT-72
            disp(StepName{3})
            for k=0:7
...
try
    ...
    try
    load(['PSTH_behav_' manipname  '_' num2str(SndDur) 's' ])
    catch
    end
end
...
for m=1:2
    for k=0:2:6
        ...
        if k==0& m==1
            text(0,1.2,num2str(mice_included),'units','normalized')
        end
