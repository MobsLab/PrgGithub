...
try
    ...
    load(['DataRampOver1DayQuantif_no_selection_SWS_Info.mat']);
    end
    LFPpower_Before_SWS=LFPpower_Before;
    LFPpower_SWS=LFPpower;
...
%                 events not in SWS are put to nan
                ind_evt=logical(1-ind_evt);
                LFPpower_Before_SWS(1,i,man,ind_evt,1:length(f1),fq)=nan;
                LFPpower_SWS(1,i,man,ind_evt,1:length(f1),fq)=nan;
            end  
        end
    end
LFPpower=LFPpower_SWS;
LFPpower_Before=LFPpower_Before_SWS;
    cd(res)
    
    if sav
        save DataRampOver1DayQuantif_no_selection_SWS LFPpower LFPpower_Before f1 f structlistname fq_list Dir LaserInt StimInfoAllMan f Sp_On Sp_Off nb_fq fq_list
    end
end
end
...
%                     sub{i,fq}=subplot(length(structlist),nb_fq,nb_fq*(i-1)+fq); hold on
        ...
        if sav
            saveas(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_' hemi '_RampOver1Day_mean_SWS.fig'])
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end)  '_' hemi '_RampOver1Day_mean_SWS'],res)
        end
