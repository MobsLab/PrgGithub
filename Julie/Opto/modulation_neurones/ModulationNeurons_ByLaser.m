...
% 09.05.2017
% plots histogram of phase for all neurons of a mouse, 1 figure per
% stimulation frequen
...
figure('Position',[201          69        1616         899])
for man=1:length(Dir.path)
    clear K
    K=KappaAr{man};
    ...
    PlotErrorSpreadN_KJ(K,'plotcolors',[0.7 0.7 0.7],'newfig',0);hold on
    ...
    title([Dir.path{man}(end-11:end-9) ' - N tot neurons = ' num2str(size(KappaAr{man},1)) ] )
   ...
   bar(sum(~isnan(K))/ size(KappaAr{man},1))
    ylim([0 0.7]),set(gca,'XTickLabel',[1 2 4 7 10 13 15 20])
   if man==1,ylabel('% modulated neurons'),end;
    if man==length(Dir.path),xlabel('stim frequency'),end;
end
cd(resRslt)
saveas(gcf,['ModByMouse' suffix '.fig']);
saveFigure(gcf,['ModByMouse' suffix ],resRslt);

%% all concatenated
figure('Position',[ 1958          67         307         899])
K=Kappa;
K(pval>0.05)=nan;
...
title([Dir.path{man}(end-11:end-9) ' - N tot neurons = ' num2str(size(Kappa,1)) ] )

subplot(312)
try PlotErrorBarN(K,0);end
set(gca,'XTickLabel',fq_list)
ylabel('Kappa')

subplot(313)
bar(sum(~isnan(K)/ size(Kappa,1)))
ylim([0 0.7]),set(gca,'XTickLabel',fq_list)
ylabel('% modulated neurons')
xlabel('stim frequency')

cd(resRslt)
saveas(gcf,['ModAllMice' suffix '.fig']);
saveFigure(gcf,['ModAllMice' suffix ],resRslt);
