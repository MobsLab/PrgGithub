...
sav=1;
savindiv=0;
cd /media/DataMOBsRAIDN/ProjetAversion/SleepStim/doubleFq
resRslt=pwd;
...
%     suffix='aft';
    ...
    save(['ModNeurons_doubleFq_' suffix],'int_laser','fq_list')
        ...
        for fq=1:8
            for num=1:length(NumNeurons)
                ...
                if size(Data(Restrict(S{NumNeurons(num)},subset(int_laser,ind_OI{fq}))),1)>1
                    ...
                    [Ph] = ModulationSquaredSignal_doubleFq(Restrict(LFPlaser,subset(int_laser,ind_OI{fq})),Restrict(S{NumNeurons(num)},subset(int_laser,ind_OI{fq})), 1,0);
                    Pht{num,fq}=[Pht{num,fq};Ph];
                end
            end
        end
        save(['ModNeurons_doubleFq_' suffix],'Pht','-Append')
    end
    
    if ~isdir([res '/modulation_doubleFq/']);
        mkdir([res '/modulation_doubleFq/']);
    end
    ...
    try
        ...
        for fq=1:8
            ...
            if savindiv
                saveas(gcf,[res '/modulation_doubleFq/freq' num2str(fq_list(fq)) 'Hz'])
                saveFigure(gcf,['freq' num2str(fq_list(fq)) '_' suffix],[res '/modulation_doubleFq/'])
            end
        end
        save(['ModNeurons_doubleFq_' suffix],'mu','Kappa','pval','NumNeurons','numtt','TT','-Append')
    end
...
for man=1:length(Dir.path)
    cd(Dir.path{man})
    temp=load(['ModNeurons_doubleFq_' suffix]);
...
cd(resRslt)
if sav
save(['ModNeurons_doubleFq_' suffix],'mu','Kappa','pval','mu','fq_list')
end
...
%% compare Kappa et Kappa doubleFq
sav=1;suffix='dur'; 
resRslt='/media/DataMOBsRAIDN/ProjetAversion/SleepStim/doubleFq';
cd /media/DataMOBsRAIDN/ProjetAversion/SleepStim
temp=load(['ModNeurons_' suffix],'mu','Kappa','pval','mu','fq_list');
cd /media/DataMOBsRAIDN/ProjetAversion/SleepStim/doubleFq
tempdouble=load(['ModNeurons_doubleFq_' suffix],'mu','Kappa','pval','mu','fq_list');
cd /media/DataMOBsRAIDN/ProjetAversion/SleepStim
choose_Py_orIN='IN';
load Py_or_IN_ID
% Py
if strcmp(choose_Py_orIN,'Py')
    temp.Kappa=temp.Kappa(Py_or_IN==1,:);
    temp.pval=temp.pval(Py_or_IN==1,:);
    tempdouble.Kappa=tempdouble.Kappa(Py_or_IN==1,:);
    tempdouble.pval=tempdouble.pval(Py_or_IN==1,:);
elseif strcmp(choose_Py_orIN,'IN')
    temp.Kappa=temp.Kappa(Py_or_IN==-1,:);
    temp.pval=temp.pval(Py_or_IN==-1,:);
    tempdouble.Kappa=tempdouble.Kappa(Py_or_IN==-1,:);
    tempdouble.pval=tempdouble.pval(Py_or_IN==-1,:);
...
figure, subplot(121)
plot(temp.Kappa(:,2),tempdouble.Kappa(:,2),'.','Color',[0.8 0.8 0.8 ])
temp.K=temp.Kappa; 
temp.K(temp.pval>0.05)=nan;
tempdouble.K=tempdouble.Kappa;
tempdouble.K(tempdouble.pval>0.05)=nan;
hold on, plot(temp.Kappa(:,2),tempdouble.K(:,2),'.g')
hold on, plot(temp.K(:,2),tempdouble.Kappa(:,2),'.c')
hold on, plot(temp.K(:,2),tempdouble.K(:,2),'.m')
legend('no mod','mod4','mod2', 'mod 2 & 4')
ylabel('Kappa 4Hz'), xlabel('Kappa 2Hz')
title(['Stim 2Hz - n = ' num2str(size(tempdouble.Kappa,1))])
YL=ylim; XL=xlim; hold on, plot([0 max(YL(2), XL(2))], [0 max(YL(2), XL(2))] ,'Color',[0.5 0.5 0.5]) 

subplot(122)
%plotSpread([temp.K(:,2) tempdouble.Kappa(:,2)])
plotErrorBarN([temp.K(:,2) tempdouble.Kappa(:,2)],0)
set(gca,'XTick',[1 2 ],'XTickLabel', {'mod 4','mod 2'})
title([' n = ' num2str(size(tempdouble.Kappa,1))])
if sav
    saveas(gcf,['/media/DataMOBsRAIDN/ProjetAversion/SleepStim/doubleFq/Mod4_vs_2_at2Hz_stim_' choose_Py_orIN '.fig'])
    saveFigure(gcf,['Mod4_vs_2_at2Hz_stim_' choose_Py_orIN],resRslt)
end
...
if 0
    ...
    bef=load('ModNeurons_doubleFq_bef');
    dur=load('ModNeurons_doubleFq_dur');
    aft=load('ModNeurons_doubleFq_bef');
