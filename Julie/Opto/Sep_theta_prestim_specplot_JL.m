...
% Dima
%% Parameters
plo_inf_1=0; plo_inf_1_avg=0; plo_sup_2=1; plo_sup_2_avg=1; 
sav=0; 
chan = 'dHPC_deep';
chan2 = 'dHPC_rip';
...
for k=1:length(Dir.path)
    ...
    Dir.name{k}=Dir.path{k}([ind_mouse ind_mouse+5:ind_mouse+7]);
end
% res = '/media/DataMOBsRAIDN/ProjetAversion/RampOver1dayQuantif/Ramp_4_no_selection/3mice_march17'; % Folder to save the pictures
res='/media/DataMOBsRAIDN/ProjetAversion/RampOver1dayQuantif/Ramp_4_no_selection/7mice_458-467';

% Load res_matrix
load /media/DataMOBsRAIDN/ProjetAversion/DATA-Fear/f_0-20;

% % List of structures to plot and compute
% structlist={'dHPC_rip', 'PFCx_deep_right', 'PFCx_deep_left', 'dHPC_deep'};%, 'dHPC_rip_left', 'vHPC_rip'
% structlistname={'dHPC rip', 'PFCx deep right', 'PFCx deep left', 'dHPC deep'};

% 
structlist={'Bulb_deep_right','Bulb_deep_left','PFCx_deep_right','PFCx_deep_left','dHPC_rip','PiCx_right','PiCx_left'};
structlistname={'Bulb_deep_right','Bulb_deep_left','PFCx_deep_right','PFCx_deep_left','dHPC_rip','PiCx_right','PiCx_left'};
% Memory allocation
...
%% Load data
try 
    disp('Loading power in 3-6 and 6-10Hz bands')
    load('DataRampOver1DayQuantif_Power_36_610', 'LFPpower36','LFPpower610');
catch
    disp('Computing power in 3-6 and 6-10Hz bands')
    for m = 1:length(Dir.path)
        ...
        % Spectrum loading
        if (exist([Dir.path{m},'/ChannelsToAnalyse/', chan,'.mat']) | exist([Dir.path{m},'/ChannelsToAnalyse/', chan2,'.mat']) )
           try
            temp=load([Dir.path{m},'/ChannelsToAnalyse/',chan,'.mat']);
           catch
               temp=load([Dir.path{m},'/ChannelsToAnalyse/',chan2,'.mat']);
           end
            try
    ...
    if sav
        cd(res)
        save DataRampOver1DayQuantif_Power_36_610 LFPpower36 LFPpower610 f1 f fq_list Dir Prestim 
    end
end

%% Plot ratio

h=figure('Position',[ 141          99        1536         867]);
k=1;
for m=1:length(Dir.path)
    for fq = 1:length(fq_list)
        figplot{m,fq} = subplot(length(Dir.path),length(fq_list),k); hold on
        if length(Dir.path) == 1
            a = squeeze(nanmean(LFPpower610,3))./squeeze(nanmean(LFPpower36,3)); % Ratio
            b = 1:1:sum(~isnan(a(:,fq))); % number of events per frequency per day
            a = (a(b,fq))'; % Ratio for each event
            plot (b, a);
            ylim([0 5]);
            k=k+1;
            if m==1 & fq==1; xlabel('# event'); ylabel('6-10 / 3-6 ratio'); end
            title([ 'm ' num2str(m) ' fq ' num2str(fq)])
        else
            try
            a = squeeze(nanmean(LFPpower610,3))./squeeze(nanmean(LFPpower36,3)); % Ratio
            b = 1:1:sum(~isnan(a(m,:,fq))); % number of events per frequeny per day
            a = a(m,b,fq); % Ratio for each event
            plot (b, a); 
            ylim([0 5]);
            k=k+1;
%                         if m==1 & fq==1; xlabel('# event'); ylabel('6-10 / 3-6 ratio'); end
%             title([ 'm ' num2str(m) ' fq ' num2str(fq)])
            if m==1 & fq==1; text(0,1.5,'6-10 / 3-6 ratio','Units','normalized'); end
            if m==1; title([num2str(fq_list(fq)) 'Hz']); end
            if fq==1, ylabel(Dir.name{m}); end
            if m==length(Dir.path), xlabel('# event'); end
            catch
                keyboard
            end
        end
...
if sav
    ...
    saveFigure(h,'ratio_all',res)
end

StepName='Ramp';nb_fq=8;
%% Plot spectra
...
try
    ...
    temp=load(['DataRampOver1DayQuantif_no_selection.mat'],'LFPpower','LFPpower_Before','structlistname');
    LFPpower=temp.LFPpower;LFPpower_Before=temp.LFPpower_Before;
    
    LFPpower_temp=nan(length(StepName),length(structlist), length(Dir.path),50,67,15); % 50 : nb d'events (max estimé)/ 67 : nb fqce dans le spectre / 15 : nb de fqce de stim dans la rampe (max)
    LFPpower_Before_temp=nan(length(StepName),length(structlist), length(Dir.path),50,67,15);
    for i=1:length(structlist)
        i_temp=find(strcmp(temp.structlistname,structlistname{i}));
        if strcmp(structlistname{i},'dHPC deep'),i_temp=find(strcmp(temp.structlistname,'dHPC rip')),end
        LFPpower_Before_temp(1,i,:,:,:,:)=LFPpower_Before(1,i_temp,:,:,:,:);
        LFPpower_temp(1,i,:,:,:,:)=LFPpower(1,i_temp,:,:,:,:);
    end
    LFPpower_Before=LFPpower_Before_temp;
    LFPpower=LFPpower_temp;
catch
    % Memory allocation
...
%                     ylim([0 4E5])
        ...
        if sav
            ...
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_Spec_low_freq'],res)
        end
...
%                     ylim([0 4E5])
        ...
        if sav
            ...
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_Spec_low_freq_av'],res)
        end
end   
...
%                     ylim([0 4E5])
        ...
        if sav
            ...
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_spec_theta_state'],res)
            LFPpower=LFPpower_high;
            LFPpower_Before=LFPpower_Before_high;
            save DataRampOver1DayQuantif_no_selection_REM_Dima LFPpower LFPpower_Before Dir f1 structlist structlistname 
        end
    ...
    idx_sum_high = nan(1,length(fq_list));
        ...
        for i=1:length(structlist)
            ...
            for fq=1:nb_fq
                ...
                if i==length(structlist), xlabel(['stim ' num2str(fq_list(fq)) 'Hz']);end
                if i==1&&fq==1, text(-0.2,1.3,[' high frequency state Average'],'units','normalized');end
...
%                     ylim([0 4E5])
        ...
        if sav
            ...
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_Spec_high_freq_av'],res)
        end
end
...
%                     ylim([0 4E5])
        ...
        if sav
            ...
            saveFigure(manSpfig,[ Mousename '_' Dir.path{man}(end-7:end) '_all'],res)
        end
end
