function AnalysisManipBulbectomyCB(datalocation,manipname,freezeTh,varargin)

% AnalysisManipBulbectomyCB
...
% 2015.02.25
% Can be modified to exclude mice which may have been exchanged.

% INPUTS
% datalocation : 'server' ou 'manip' or 'server_windows'
% (24/02/16 datalocation can also be 'server_windows' to adapt the path to server using windows)
% manipname : 'ManipDec14Bulbectomie' or 'ManipFeb15Bulbectomie' or 'ManipNov15Bulbectomie'
...
hab='grille';
period='fullperiod';
verif=1; % check results without the mice potentially exchanged (273,283,284,275). verif=0 all mice included, verif=1 suspected mice excluded

 for i = 1:2:length(varargin),%    disp('only on mice which were not exchanged at the end of the experiment (perfusion day (feburay 2016)')
...
StepName{1}=['HAB ' hab];
StepName{2}='COND';
POI1=3;% plot of interest
POI2=4;
if strcmp(manipname,'ManipDec14Bulbectomie')
    StepName{3}='EXT pleth';%    disp('only on mice which were not exchanged at the end of the experiment (perfusion day (feburay 2016)')
    StepName{4}='EXT envB';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    disp('only on mice which were not exchanged at the end of the experiment (perfusion day (feburay 2016)')
...
elseif strcmp(manipname,'ManipNov15Bulbectomie')
    if verif==1
        shammice=[280:282,285:290]';
        bulbmice=[269:272,274,276:279]';
    else
        shammice=[280:290]';
        bulbmice=[269:279]';
    end
...
% to run the code only on mice which were not exchanged at the end of the
% experiment (perfusion day (feburay 2016))
    ...
    for mousenb=1:length(group)
        ...
        for step=1:length(StepName)
            ...
            %try
                ...
                if strcmp(manipname,'ManipNov15Bulbectomie')|(strcmp(manipname,'ManipDec15BulbectomiePlethysmo')& step~=1)
                ...
                elseif (strcmp(manipname,'ManipDec15BulbectomiePlethysmo')& step==1)  
                    CSplInt=[];
                    CSmiInt=[];
                    ...
                    if ~isempty(CSmiInt)
                        ...
                        elseif step==2 % COND
                            sounds1=subset(CSmiInt,[1:4]);
                            sounds2=subset(CSplInt,[1:4]);
                            sounds3=subset(CSmiInt,[5:8]);
                            sounds4=subset(CSplInt,[5:8]);
                        elseif step==3
                        ...
                        elseif step==4 % EXT env C/Plth  et EXTenvB
                            if m==285||m==275
    %                             sounds1=intervalSet(0,0);
    %                             sounds2=intervalSet(0,0);
    %                             sounds3=intervalSet(0,0);
    %                             sounds4=intervalSet(0,0);
                            else
                                sounds1=CSmiInt;
                                sounds2=subset(CSplInt,[1:4]);
                                sounds3=subset(CSplInt,[5:8]);
...
%             end % of try catch
    ...
    if gpfg
        ...
        PlotErrorBarNJL((bilan{POI1}),'newfig',0,'paired',1,'optiontest','ranksum','ColumnTest', ColTest)
        title(StepName{POI1})
        ylim([0 1])
        ...
        PlotErrorBarNJL((bilan{POI2}),'newfig',0,'paired',1,'optiontest','ranksum','ColumnTest', ColTest)
        title(StepName{POI2})
        ylim([0 1])
         ylabel(['col test = ' num2str(ColTest)])
        set(gca,'XTick',[1 2 3 4],'XTickLabel',{'noS','CS-','CS+1','CS+2','CS+3'})
    ...
    if gpfg
        ...
        % EXT pleth
        subplot(1,2,1)
        PlotErrorBarNJL((bilan{POI1}),'newfig',0,'paired',1,'optiontest','ranksum','ColumnTest', ColTest) % test : paired ranksum relative to column 1
        title(StepName{POI1})
        ylim([0 1])
        ylabel(['col test = ' num2str(ColTest)])
        set(gca,'XTick',[1 2 3 4],'XTickLabel',{'noS','CS-','CS+1','CS+2','CS+3'})
...
res=pwd;
if sav && gpfg
    saveFigure(bilanFig,'AllMiceAverage',res)
end
