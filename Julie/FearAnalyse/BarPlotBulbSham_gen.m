function [p_an,table_an,stats_an,Pkw_g,table_kw_g,p_mw,stats_mw]=BarPlotBulbSham_gen(Table,TableName,Gpcolor,test,n,p,j,varargin)

% generalized 19.10.2017
% INPUTS
% - Table:
...
% plot
if strcmp (errorbarflag,'median')
    h1=bar(X1,nanmedian(Table{1,1})','FaceColor',Gpcolor{1}, 'BarWidth', 0.35);hold on; % important to flip the matrices to have them correctlmy plotted
    h2=bar(X2,nanmedian(Table{1,2})','FaceColor',Gpcolor{2}, 'BarWidth', 0.35);
else
    h1=bar(X1,nanmean(Table{1,1})','FaceColor',Gpcolor{1}, 'BarWidth', 0.35);hold on; % important to flip the matrices to have them correctlmy plotted
    h2=bar(X2,nanmean(Table{1,2})','FaceColor',Gpcolor{2}, 'BarWidth', 0.35);
...
if indivdots==1
    h4=scatter(multiX1(:),Table{1,1}(:),'Marker','o','MarkerEdgeColor','k', 'MarkerFaceColor',[0.5 0.5 0.5],'LineWidth', 1,'SizeData',12); % 'SizeData',15
    h5=scatter(multiX2(:),Table{1,2}(:),'Marker','o','MarkerEdgeColor','k', 'MarkerFaceColor',[0.5 0.5 0.5], 'LineWidth', 1,'SizeData',12);
end
set(gca, 'XTick',[]);
Ylimit=get(gca,'YLim');
set(gca, 'YLim', [0 Ylimit(2)])
ylabel(TableName)
xlim([0 size(Table{1,1},2)+1])
...
title (['An P_s_e_s ' sprintf('%.3f',(Pan_ses)) '   P_g_ ' sprintf('%.3f',(Pan_gp)) '   P_i_n_t ' sprintf('%.3f',(Pan_int))  '  /  KW P_g_p ' sprintf('%.3f',(Pkw_g))], 'FontSize',8);
...
%     values=values_for_ANOVA;
    ...
    if size(values_for_ANOVA,2)==4
        xlabel([test '    P_' Leg{1} '=' sprintf('%.3f',(p_kw(1))) '   P_' Leg{2} '=' sprintf('%.3f',(p_kw(2)))  '   P_' Leg{3} '=' sprintf('%.3f',(p_kw(3)))  '   P_' Leg{4} '=' sprintf('%.3f',(p_kw(4)))  ]);
    elseif size(values_for_ANOVA,2)==5
        xlabel([test '    P_' Leg{1} '=' sprintf('%.3f',(p_kw(1)))  '   P_' Leg{2} '=' sprintf('%.3f',(p_kw(2))) '   P_' Leg{3} '=' sprintf('%.3f',(p_kw(3)))  '   P_' Leg{4} '=' sprintf('%.3f',(p_kw(4)))  '   P_' Leg{5} '=' sprintf('%.3f',(p_kw(5)))  ]);
    end
...
%     if size(values_for_ANOVA,2)==4
%         xlabel(['ranksum    P_p_r_e=' sprintf('%.3f',(p_mw(1))) '   P_p_o_s_t=' sprintf('%.3f',(p_mw(2)))  '   P_6_j=' sprintf('%.3f',(p_mw(3)))  '   P_3_w_k=' sprintf('%.3f',(p_mw(4)))  ]);
%     elseif size(values_for_ANOVA,2)==5
%         xlabel(['ranksum    P_n_o_S=' sprintf('%.3f',(p_mw(1)))  '   P_p_r_e=' sprintf('%.3f',(p_mw(2))) '   P_p_o_s_t=' sprintf('%.3f',(p_mw(3)))  '   P_6_j=' sprintf('%.3f',(p_mw(4)))  '   P_3_w_k=' sprintf('%.3f',(p_mw(5)))  ]);
%     end
%     if size(values_for_ANOVA,2)==4
%         xlabel([test '    P_' Leg{1} ' ' sprintf('%.3f',(p_mw(1))) '   P_' Leg{2} ' ' sprintf('%.3f',(p_mw(2)))  '   P_' Leg{3} ' ' sprintf('%.3f',(p_mw(3)))  '   P_' Leg{4} ' ' sprintf('%.3f',(p_mw(4)))  ]);
%     elseif size(values_for_ANOVA,2)==5
%         xlabel([test '    P_' Leg{1} ' ' sprintf('%.3f',(p_mw(1)))  '   P_' Leg{2} ' ' sprintf('%.3f',(p_mw(2))) '   P_' Leg{3} ' ' sprintf('%.3f',(p_mw(3)))  '   P_' Leg{4} ' ' sprintf('%.3f',(p_mw(4)))  '   P_' Leg{5} ' ' sprintf('%.3f',(p_mw(5)))  ]);
%      elseif size(values_for_ANOVA,2)==6
% %         xlabel(['P_' Leg{1} ' ' sprintf('%.3f',(p_mw(1)))  ' P_' Leg{2} ' ' sprintf('%.3f',(p_mw(2))) ' P_' Leg{3} ' ' sprintf('%.3f',(p_mw(3)))  ' P_' Leg{4} ' ' sprintf('%.3f',(p_mw(4))) ...
% %             ' P_' Leg{5} ' ' sprintf('%.3f',(p_mw(5)))  ' P_' Leg{6} ' ' sprintf('%.3f',(p_mw(6)))  ],'Fontsize',6);
%         xlabel([' ' sprintf('%.3f',(p_mw(1)))  '  ' sprintf('%.3f',(p_mw(2))) '  ' sprintf('%.3f',(p_mw(3)))  '  ' sprintf('%.3f',(p_mw(4))) ...
%             '  ' sprintf('%.3f',(p_mw(5)))  '  ' sprintf('%.3f',(p_mw(6)))  ],'Fontsize',6);
%     
%     end
    
    xlabelname=[];
    for k=1:size(values_for_ANOVA,2)
        xlabelname=strcat([ xlabelname '  ' sprintf('%.3f',(p_mw(k))) ]);
    end
    xlabel(xlabelname,'Fontsize',6);
    if savestats
        file2save=['Stats_' TableName '.mat'];
