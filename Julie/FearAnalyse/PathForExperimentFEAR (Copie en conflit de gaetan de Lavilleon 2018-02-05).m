...
nameTreatment={'control','CNO1','CNO2'};
    
OBX=[207 208 209 210 214 215 216 , 222 226 228 232 234 236 238 240 , 230 247,  249 250];
hemiOBX=[245 246];

CTRL=[211 212 213 217 218 219 220, 223 224 225 227 229 233 235 237 239 , 231 248, 241 242,  243 244, 253 254]; 

implanted=[230 231 241 242 247 248 245 246  243 244 249 250 253 254 ];
...
elseif strcmp(experiment, 'FearMLavr2015')
   FolderPath='/media/DataMOBsRAID/ProjetAstro/';
    ...
    for m=245:246;
        a=a+1; Dir.path{a}=[FolderPath  'Mouse' num2str(m) '/20150410/COND']; 
...
%     a=a+1; Dir.path{a}='/media/DataMOBs29/M253/20150713-15_manip-CNO2/20150715-EXT-48h-envB/FEAR-Mouse-253-15072015';      
    ...
    a=a+1; Dir.path{a}='/media/DataMOBsRAID/ProjetAversion/DATA-Fear/Mouse254/20150703-EXT-24h-envC/FEAR-Mouse-254-03072015'; 
...
%     nameEnvt={'envC','envB','pleth'};
    ...
    for mi=1:length(nameSession)
        if ~isempty(strfind(Dir.path{i},nameSession{mi}))
            tempS{i}=nameSession{mi};
        end
    end
    for mi=1:length(nameEnvt)
        if ~isempty(strfind(Dir.path{i},nameEnvt{mi}))
   ...
   if exist('tempS','var') , Dir.Session{i}=[tempS{i},Environment{i}];end
end
...
%     nameEnvt={'envC','envB','pleth'};
   ...
   if exist('tempT','var') , Dir.Treatment{i}=[tempT{i}];end
end
