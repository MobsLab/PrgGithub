function AnalysisManipBulbectomySoundBySoundCB(datalocation,manipname,freezeTh,varargin)

% Clara Dec 15

% 2015.12.08
% similar to AnalysisManipBulbectomyCB modif to compute freezing for each sound
% similar to AnalysisManipBulbectomy. Modif to deal with
% ManipNov15Bulbectomie

% INPUTS
% datalocation : 'server' or 'manip' or 'server_windows'  (24/02/16 datalocation can also be 'server_windows' to adapt the path to server using windows)
...
%             end % of try catch
    ...
    if gpfg
        ...
        PlotErrorBarNJL((bilan_indiv{3}),'newfig',0,'paired',1,'optiontest','ranksum','ColumnTest', ColTest)
        title(StepName{3})
        ylim([0 1])
        ...
        if sav
            ...
            saveas(bilanFig,[groupname{g} 'AllMiceAverage_' num2str(freezeTh) '.eps'])
            currFold=pwd;
            %saveFigure(bilanFig, [groupname{g} 'AllMiceAverage_' num2str(freezeTh)], currFold)
        end
    end
    ...
    if gpfg
        ...
        if sav
            ...
            saveas(groupFig,[groupname{g} 'Average_' num2str(freezeTh) '.eps'])
...
if sav && gpfg
    ...
    saveFigure(bilanFig,'AllMiceAverage.fig')
    saveFigure(bilanFig,'AllMiceAverage.png')
end
