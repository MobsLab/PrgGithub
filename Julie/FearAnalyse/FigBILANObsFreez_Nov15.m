function FigBILANObsFreez_Nov15(datalocation, manipname,varargin)
%09.12.2015
...
elseif strcmp(manipname,'ManipNov15Bulbectomie')
    StepName{3}='EXT24-envC';
    StepName{4}='EXT48-envB';
end
...
for i=1:length(list)
    ...
    if list(i).isdir==1&list(i).name(1)~='.'&list(i).name(1)=='M' % if the folder is indeed a mouse name folder
        ...
        if strcmp(manipname,'ManipDec14Bulbectomie')||strcmp(manipname,'ManipFeb15Bulbectomie')   
...
%         line([csp csp+29],[0 0],'Color',[1 0.8 0])
%         line([csm csm+29],[0 0],'Color',[0 0.8 0])
        % compute PSTH
        [m2,s2,t2]=mETAverage(csp(1:4)*1E4,Range(Movtsd),Data(Movtsd),bi,lim); 
        [m3,s3,t3]=mETAverage(csp(5:8)*1E4,Range(Movtsd),Data(Movtsd),bi,lim);
        ...
        C1=[C1,m1];
        if plo
            figure(hFigIndiv),subplot(3,1,1);
        ...
        clear csp csm CStimes CSevent  times event TTL
        %%%%%%%%%%%%%%%%%%%%%%%%%  PLETH  %%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        try
            ...
            if ((exist('Behavior.mat'))~=0)
                ...
                [m2,s2,t2]=mETAverage(csp(1:4)*1E4,Range(Movtsd),Data(Movtsd),bi,lim);
                [m3,s3,t3]=mETAverage(csp(5:12)*1E4,Range(Movtsd),Data(Movtsd),bi,lim);
                [m1,s1,t1]=mETAverage(csm*1E4,Range(Movtsd),Data(Movtsd),bi,lim);
            else
            ...
            P1=[P1,m1];
            if plo
                figure(hFigIndiv),subplot(3,1,2);
        ...
        try
            ...
            [m1,s1,t1]=mETAverage(csm*1E4,Range(Movtsd),Data(Movtsd),bi,lim);
            if plo
                figure(hFigIndiv),subplot(3,1,3);
