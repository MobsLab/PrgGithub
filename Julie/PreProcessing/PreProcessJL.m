%  PreProcessJL.m
% from PreProcessDeltaFeedback

% 09.110.2017 - modif JL to fit my processing
% - remove the foler 'acclero' and 'analog'
% - add conditions for creation for folders for 2 mice and for SpkRefSubtraction
% - 
do_klustakwik=0;
do_clean_folders=0;
...
if ~init_done_done
    ...
    defaultvalues1={'FEAR-Mouse-'};
    Prompt = {'Name of the Experiment'};
    
    defaultvalues1{end+1} = '1';
    ...
    defaultvalues2 = {[experiment_name '.dat'],'36','1'};
    idx_animals = [];
    for m=1:nb_mice
        ...
        defaultvalues2{end+1} = '[32 33 34 35]';
...
if ~move_file_done
    ...
    for m=1:nb_folders
        ...
        %analog
        try
            analog_name = [experiment_name '-' sessions_name{m} '-lasersti.dat'];
...
if ~create_links_done
    ...
    for m=1:length(accelero_files)
        command = ['ln -s ' accelero_files(m).name ' ' wideband_folder];
        system(command);
    end
    analog_files = dir('*-lasersti.dat');
    for m=1:length(analog_files)
        command = ['ln -s ' analog_files(m).name ' ' wideband_folder];
        system(command);
    end
...
if ~refsub_done
    ...
    % with one mouse, rename to the original filename
    if str2num(answer_refsub{3})==1
        movefile([answer_refsub{1}(1:end-4) '_' answer_refsub{4} '.dat'], [answer_refsub{1} ])
...
if (str2num(answer_refsub{3})>1) % for 2 mice
        keyboard
    if ~mice_folders_done
        ...
        for m=1:length(mice_name)
            ...
            cd(mouse_expe{m}); mouse_path{m} = pwd; 
            cd('..'); %path to the principal subfolder, with data
...
%         cd(mouse_path{m})
%         eval('cd ..')
        cd( [ mainFolder '/' experiment_name])
        % Julie
        mouse_expe{m} = experiment_name;% ['Breath-' mice_name{m} '-' experiment_name(end-7:end)]; 
