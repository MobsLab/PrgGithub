function [numNeurons,numMUA,pvalAll,KappaAll,phAll,meanPh,pthres,Kth,Duration,PercTimeval,criterion,SpkNbThr,Spectrum,f]=ModulationPFCneuronsByLFP(FreqRange,lim2,lim3,LFPregion,varargin);
...
% - LFPregion : 'Bulb_deep', PFC_deep, dHPC_rip
...
LogPMax=-10; % for plots (to avoid saturation of xscale or color scale)
...
% get LFP Bulb
res=pwd; 
if strcmp(LFPregion,'dHPC_rip') % because sometimes dHPC_rip does not exist
    try load([res,'/ChannelsToAnalyse/' LFPregion]); 
    catch load([res,'/ChannelsToAnalyse/dHPC_deep']);
    end
else
load([res,'/ChannelsToAnalyse/' LFPregion]); 
end
...
if strongperonly==1
    ...
    Stsd=tsd(t*1E4,Sp);
    Spectrum=mean(Restrict(Stsd,StrongOscAndFreezeEpoch));
else
    load([res,'/SpectrumDataL/Spectrum',num2str(channel)]);
    Stsd=tsd(t*1E4,Sp);
    Spectrum=mean(Restrict(Stsd,Epoch));
end

FolderName=['SpkModul_' LFPregion '_' EpochName '_' suffix ];
if ~isdir(FolderName);  
    mkdir(FolderName);
end
...
try 
    ...
    for i=1:length(numNeurons)
        ...
        if a==1
            text(-0.5,1.25, res(46:end),'units','normalized')
            text(-0.5,1.15, EpochName,'units','normalized')
            text(-0.5,1.05, [BandFq ' Hz'],'units','normalized')
            if strongperonly==1
                text(-0.5,0.95, ['Strong Osc Per Only Crit' num2str(criterion)],'units','normalized')
            elseif strongperonly==0
                text(-0.5,0.95, 'All Epochs included','units','normalized')
            end
            
            text(-0.5,0.85, [num2str(round(Duration*1E-4*PercTimeval*1E-2)) ' sec'],'units','normalized')
            text(-0.5,0.75, ['minSpkNb ' sprintf('%.0f',SpkNbThr) ],'units','normalized')
        end
    ...
    saveFigure(hfigMod,(['Modul_' BandFq '_' suffix ]),[res '/' FolderName])

pvalAllforplot=pvalAll;% avoid saturation of color scale and pb with value "zero" (log)
pvalAllforplot(pvalAllforplot<10^(LogPMax))=10^(LogPMax);
...
scatter(log10(pvalAllforplot), KappaAll,'MarkerFaceColor','b','MarkerEdgeColor','k')
hold on, scatter(log10(pvalAllforplot((pvalAll<pthres) & (KappaAll>Kth))), KappaAll((pvalAll<pthres) & (KappaAll>Kth)), 'MarkerFaceColor','r','MarkerEdgeColor','k')
scatter(log10(pvalAllforplot(numMUA)), KappaAll(numMUA),'Marker', '+','MarkerEdgeColor',[0.8 0.8 0.8])
xlim([LogPMax 0])
...
A=round(abs(log10(pvalAllforplot)));
% cols=jet(max(A)+1);
...
for nn=numNeurons
    if ~isnan(KappaAll(nn))
        ...
        if pvalAll(nn)>pthres
            ...
            scatter(meanPh(nn), KappaAll(nn),'MarkerFaceColor',cols(round(abs(log10(pvalAllforplot(nn))))+1, :),'MarkerEdgeColor','k'),hold on
        end
...
saveFigure(hperc,(['K_Distrib_',BandFq , '_OB_' suffix ]),[res '/' FolderName])
saveas(hperc,(['K_Distrib_',BandFq , '_OB_' suffix '.fig']))

save(['Modul_',BandFq '_' suffix '.mat'], 'KappaAll', 'pvalAll','phAll','meanPh', 'strongperonly', 'criterion', 'numNeurons', 'numMUA', 'Kth', 'modulatedNeurons','NoiseRemoved','Stsd','f','Spectrum','Epoch','EpochName')
end
cd(res)
%verification
...
if RayleighFreq2switch
    ...
    saveFigure(gcf, ['RayleighFreq2_' suffix ],[res '/' FolderName])
end % 
