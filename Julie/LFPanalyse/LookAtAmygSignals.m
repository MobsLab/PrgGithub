...
%                     end
                    ...
                    %try
                        ...
                        imagesc(t,f,10*log10(Sp')), axis xy, caxis([10 60])
                        ...
                        plot(f,10*log10(nanmean(Data(Restrict(Stsd, FreezeEpochL)))), 'Color', 'k','LineWidth',2), hold on
                        plot(f,10*log10(nanmean(Data(Restrict(Stsd, TotEpoch-FreezeEpoch)))), 'Color', 'c','LineWidth',2), 
                        plot(f,10*log10(nanmedian(Data(Restrict(Stsd, FreezeEpochL)))), '--','Color', 'k','LineWidth',1),
                        plot(f,10*log10(nanmedian(Data(Restrict(Stsd, TotEpoch-FreezeEpoch)))), '--','Color', 'c','LineWidth',1)
...
%                     end
            ...
            disp(['Saving data in local path, Amy' suffix '_' nameMice{m} 'Freezing.mat']);
            save (['AmyFreezing' suffix '_' nameMice{m}],'MatInfo','MatSpF','MatSpNoF','params','structlist','nameMice','nameGroups','Dir','f','th_immob','thtps_immob','Rogne','thtps_immob_for_rogne')
            saveas (allfig(m,man), [pwd '/Amy_' suffix 'Freez',nameMice{m}, '_',num2str(man)]),
            saveFigure(allfig(m,man),['Amy_' suffix 'Freez',nameMice{m}, '_',num2str(man)],pwd);
...
saveas (gcf, [pwd '/Amy' suffix '_' struct2comp '_AllStt.fig'])
saveFigure(gcf,['Amy' suffix '_' struct2comp '_AllStt'],pwd)
