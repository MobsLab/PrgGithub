% SpectroFearChR2BulbDiffLocations.m
% 23.08.016
...
MatSpFON=nan(length(Dir.path),length(structlist),67); % avant 164
MatSpNoFON= MatSpFON;   MatInfo=nan(length(Dir.path),length(structlist),4); 

MatSpFOFF=nan(length(Dir.path),length(structlist),67); % avant 164
MatSpNoFOFF= MatSpFOFF;   MatInfo=nan(length(Dir.path),length(structlist),4); 

try
    load(['DataSpectro_ChR2_BulbDiffLoc_' Treatment ]);MatInfo;
    disp(['Loading existing data from DataSpectro_ChR2_BulbDiffLoc_' Treatment '.mat']);
    [B,IX] = sort(Dir.group);
            ...
            for i=1:length(structlist)
               ...
               % try
                    ...
                    if length(f)>67%164
                        ...
                        MatSpFON(man,i,:)= interp1(f,mean(Data(Restrict(Stsd,FreezeEpochStimON))),f1);
                        MatSpNoFON(man,i,:)=interp1(f,mean(Data(Restrict(Stsd,And(TotEpoch-FreezeEpoch,StimLaserON)))),f1);
                        MatSpFOFF(man,i,:)= interp1(f,mean(Data(Restrict(Stsd,FreezeEpochStimOFF))),f1);
                        MatSpNoFOFF(man,i,:)=interp1(f,mean(Data(Restrict(Stsd,And(TotEpoch-FreezeEpoch,StimLaserOFF)))),f1);
                        f=f1;
                    else
                        MatSpFON(man,i,1:length(f))=mean(Data(Restrict(Stsd,FreezeEpochStimON)));
                        MatSpNoFON(man,i,1:length(f))=mean(Data(Restrict(Stsd,And(TotEpoch-FreezeEpoch,StimLaserON))));
                        MatSpFOFF(man,i,1:length(f))=mean(Data(Restrict(Stsd,FreezeEpochStimOFF)));
                        MatSpNoFOFF(man,i,1:length(f))=mean(Data(Restrict(Stsd,And(TotEpoch-FreezeEpoch,StimLaserOFF))));
                        disp(Dir.path{man})
                    end

                    MatInfoON(man,i,1:4)=[find(strcmp(Dir.group{man},nameGroups)),...
                        find(strcmp(Dir.name{man},nameMice)),tot_length(FreezeEpochStimON,'s'),tot_length(And(TotEpoch-FreezeEpoch,StimLaserON),'s')];
                    MatInfoOFF(man,i,1:4)=[find(strcmp(Dir.group{man},nameGroups)),...
                        find(strcmp(Dir.name{man},nameMice)),tot_length(FreezeEpochStimOFF,'s'),tot_length(And(TotEpoch-FreezeEpoch,StimLaserOFF),'s')];
...
%                 end
    ...
    disp(['Saving data in local path, DataSpectro_ChR2_BulbDiffLoc__' Treatment ]);
    save (['DataSpectro_ChR2_BulbDiffLoc_' Treatment ], 'MatInfoON','MatSpFON','MatSpNoFON', 'MatInfoOFF','MatSpFOFF','MatSpNoFOFF','params','structlist','nameMice','nameGroups','Dir','f','th_immob','thtps_immob','Rogne','thtps_immob_for_rogne')
...
for i=1:length(structlist)
    ...
    for man=1:length(Dir.path)
        if SpInfoON(man,3)<th_for_plot
            MatSpFON(man,i,1:length(f))=NaN;
            MatSpFOFF(man,i,1:length(f))=NaN;
        end
        if SpInfoON(man,4)<th_for_plot
            MatSpNoFON(man,i,1:length(f))=NaN;
            MatSpNoFOFF(man,i,1:length(f))=NaN;
        end        
    end
end
    ...
    for man=1:length(Dir.path)
        try
            if SpInfoON(man,3)>th_for_plot          
                ...
                %plot(f,SpFON(man,:),'Color',colori(SpInfoON(man,2),:),'LineStyle','--');
                plot(f,SpFON(man,:),'Color',colori(man,:),'LineStyle','--');
                ylabel(structlist{i}); ylim(yl);xlim([0 20]);
                legF=[legF,{[Dir.name{man}([1,6:8]),' ',num2str(floor(SpInfoON(man,3))),'s']}];
            end
            if SpInfoOFF(man,3)>th_for_plot          
                subplot(length(structlist),6,6*(i-1)+[1:2]),hold on,
                %plot(f,SpFOFF(man,:),'Color',colori(SpInfoOFF(man,2),:),'LineStyle','-');
                plot(f,SpFOFF(man,:),'Color',colori(man,:),'LineStyle','-');
                ylabel(structlist{i}); ylim(yl);xlim([0 20]);
                legF=[legF,{[Dir.name{man}([1,6:8]),' ',num2str(floor(SpInfoOFF(man,3))),'s']}];
            end
        end
        try
            if SpInfoON(man,4)>th_for_plot  
                subplot(length(structlist),6,6*(i-1)+[3:4]),hold on,
                %plot(f,SpNoFON(man,:),'Color',colori(SpInfoON(man,2),:),'LineStyle','--');
                plot(f,SpNoFON(man,:),'Color',colori(man,:),'LineStyle','--');
                ylim(yl);xlim([0 20]);
                legNoF=[legNoF,{[Dir.name{man}([1,6:8]),' ',num2str(floor(SpInfoON(man,4))),'s']}];
            end
            if SpInfoOFF(man,4)>th_for_plot  
                subplot(length(structlist),6,6*(i-1)+[3:4]),hold on,
                %plot(f,SpNoFOFF(man,:),'Color',colori(SpInfoOFF(man,2),:),'LineStyle','-');
                plot(f,SpNoFOFF(man,:),'Color',colori(man,:),'LineStyle','-');
                ylim(yl);xlim([0 20]);
                legNoF=[legNoF,{[Dir.name{man}([1,6:8]),' ',num2str(floor(SpInfoOFF(man,4))),'s']}];
            end
        end
    end
