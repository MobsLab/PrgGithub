function SpectroSleepShamBulb2(SpPplot, mean_or_med, SessionName, Expe)
...
file2load= 'StateEpoch';
nameEpoch{1}='SWSEpoch';
nameEpoch{2}='REMEpoch';
nameEpoch{3}='MovEpoch';% wake period
PColor={'r', 'g', 'b'}; % plotting colors sleep scoring Marie
...
for m=1:size(mouselist,1)
    ...
    for i=1:size(structlist,1)
        ...
        cd(DataPath{m})
        eval(['load ' file2load ' '  nameEpoch{1} ' ' nameEpoch{2} ' ' nameEpoch{3} ' MovEpoch NoiseEpoch GndNoiseEpoch WeirdNoiseEpoch'])
        ...
        Epoch1=Epoch1-NoiseEpoch-GndNoiseEpoch-WeirdNoiseEpoch;
        
        Epoch2=REMEpoch;
        Epoch2_length=sum(End(Epoch2,'s')-Start(Epoch2,'s'));
        LegSpectroG={};
        for j=1:length(nameEpoch)
            eval([   'line([Start(subset(' nameEpoch{j} ',1),''s'') End(subset(' nameEpoch{j} ',1),''s'')],[14+j 14+j],''color'',PColor{j},''linewidth'',4)'   ])
            LegSpectroG=[LegSpectroG, nameEpoch{j}(1:end-5)];
        end
        legend(LegSpectroG)
        for j=1:length(nameEpoch)
            eval([   'line([Start(' nameEpoch{j} ',''s'') End(' nameEpoch{j} ',''s'')],[14+j 14+j],''color'',PColor{j},''linewidth'',4)'   ])
        end

        % restriction of period just before and just after each SWS event to clean up the spectrum
