...
struct2comp='PFCx_deep';
structlist={'Bulb_deep','dHPC_rip'};%'PiCx',%,'Amyg','PiCx'
    ...
    for man=1:length(Dir.path)
        ...
        cd /media/DataMOBsRAID/ProjetAversion/DATA-Fear
                try
                    ...
                    % save in matrice
                    if first, MatSpF=nan(length(Dir.path),length(structlist),67); MatSpNoF= MatSpF;first=0;end % 04.02.2016                  
                    if length(f)>67 %164
                        load f_0-20;
                        ...
                        MatSpF(man,i,:)= interp1(f,mean(Data(Restrict(Ctsd,FreezeEpoch))),f1);
                        MatSpNoF(man,i,:)=interp1(f,mean(Data(Restrict(Ctsd,TotEpoch-FreezeEpoch))),f1);
                        f=f1;
                        disp('use interpolation')
                    else
                        MatSpF(man,i,:)=mean(Data(Restrict(Ctsd,FreezeEpoch)));
                        MatSpNoF(man,i,:)=mean(Data(Restrict(Ctsd,TotEpoch-FreezeEpoch)));
                    end
                    %disp(Dir.path{man})
