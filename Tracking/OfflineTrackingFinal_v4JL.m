function OfflineTrackingFinal_v4JL
% fait le 19.09.2016
% pas très abouti... j'ai pas obtenu ce que je voulais. à jeter sans doute
...
%% Track from Matlab generated frames
    ...
    function track(~,~)
        ...
        j = imshow(uint8(ones(size(ref,1),size(ref,2))));
        ...
        while start==1
            ...
            if go==3 % answer 'TrackOffLine' to 'Do you need to improve tacking? '
                ...
                if length(tosave)==8 | length(tosave)==4 %  'Tracking' or 'Both'
                    review=figure;
                    subplot(131), imshow(ref), colormap gray, axis image
                    hold on
                ...
                if savetrack{1}=='y'
                    if length(tosave)==8 %'Tracking'
                        PosMrat(:,2:3)=PosMat(:,2:3)*pixratio;
                        saveas(review,[nameFile 'review.fig']);
                        saveas(review,[ nameFile 'review.png']);
                        save('PosMat.mat','PosMat')
                        try
                            save('InfoTrackingOffline.mat','pixratio','ref','mask','smaller_object_size','BW_threshold','shape_ratio','strsz','ref','-append')
                        catch
                            save('InfoTrackingOffline.mat','pixratio','ref','mask','smaller_object_size','BW_threshold','shape_ratio','strsz','ref')
                        end
                        close(review);
                        go=0;
                    elseif length(tosave)==7 % 'Im Diff'
                        im_diff(:,2)=im_diff(:,2)*pixratio*pixratoio;
                        save('Imdiff.mat','im_diff')
                        try
                            save('InfoTrackingOffline.mat','pixratio','mask','ref','smaller_object_size_imdiff','BW_threshold_imdiff','ref','-append')
                        catch
                            save('InfoTrackingOffline.mat','pixratio','ref','mask','smaller_object_size_imdiff','BW_threshold_imdiff','ref')
                        end
                        go=0;
                    elseif length(tosave)==4 %'Both'
                        ...
                        save('Imdiff.mat','im_diff')
                        try
                            save('InfoTrackingOffline.mat','pixratio','mask','smaller_object_size','BW_threshold','shape_ratio','smaller_object_size_imdiff','BW_threshold_imdiff','strsz','ref','-append')
                        catch
                            save('InfoTrackingOffline.mat','pixratio','mask','smaller_object_size','BW_threshold','shape_ratio','smaller_object_size_imdiff','BW_threshold_imdiff','strsz','ref')
                        end
                        close(review);
                        go=0;
                    end
        ...
        function create_ref(~,~)
            ...
            function GetFileInfo
                tempref=load('InfoTrackingOffline.mat','ref');
                IM=uint8(tempref.ref);
            end
        ...
        function GenMask
            ...
            subplot(1,2,1), imshow(IM),axis image
            ...
            function Reset_mask(obj,event)
                ...
                ref2=IM;
                subplot(122), imshow(ref2), axis image
            end
        ...
        function GenTracking
            ...
            if showfig==1
                subplot(bnw_watchin)
                imshow(diffshow), colormap hot, freezeColors
            ...
            nopath=(occupation==0); % endroit oï¿½ la souris n'est jamais passï¿½e
            
            RGBoccupation(:,:,3)=nopath;
            RGBoccupation(:,:,2)=nopath;
            RGBoccupation(:,:,1)= log_occupation_norm+nopath; %en blanc lï¿½ oï¿½ elle n'est jamais passï¿½e ; en rouge ailleurs ;
