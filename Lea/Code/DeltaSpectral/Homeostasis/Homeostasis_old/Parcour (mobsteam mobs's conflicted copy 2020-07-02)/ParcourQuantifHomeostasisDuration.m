%% ParcourQuantifHomeostasisDuration
%
% 23/04/2020 LP
%
...
% on event duration, for delta waves, sup slow waves, deep slow waves, down states.
% Duration = proportion of time spent in this event
% 
% -> Global fit on whole session or multiple fits on separate sleep episodes 
% (when wake duration > wake_thresh) 
%
% SEE : QuantifHomeostasisDuration ParcourQuantifHomeostasisDurationPlot
...
%% GET DURATION HOMEOSTASIS FOR ALL SESSIONS
...
% FOR EACH TYPE OF EVENTS : 
    ...
    % ------ Get Event Duration during Epoch ------ :
    ...
    % ------ Extract Homeostasis Data ------ :
        [GlobalFit, MultiFit] = HomeostasisMultiFit_LP(evt_duration,extendedWAKE,wake_thresh,'plot',0,'newfig',0,'ZTtime',NewtsdZT,'fit_start',fit_start) ; % Get homeostasis data (and plot) for multiple fits
...
if afterREM1
    eval ([' save ParcourQuantifHomeostasisDuration_multifit' num2str(wake_thresh) '_afterREM1.mat Homeo_res windowsize cooccur_delay slowwave_clean_window merge_closewake wake_thresh afterREM1']) ;
else
    eval ([' save ParcourQuantifHomeostasisDuration_multifit' num2str(wake_thresh) '.mat Homeo_res windowsize cooccur_delay slowwave_clean_window merge_closewake wake_thresh afterREM1']) ;
end