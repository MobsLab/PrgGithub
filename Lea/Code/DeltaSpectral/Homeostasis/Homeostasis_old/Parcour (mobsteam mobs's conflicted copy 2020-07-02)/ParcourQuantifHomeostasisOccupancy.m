...
% 23/04/2020 LP
%
% Store homeostasis info for all sessions in a structure. 
% "Homeostasis" computed here through linear regression of local maxima 
% on event occupancy, for delta waves, sup slow waves, deep slow waves, down states.
% Occupancy = proportion of time spent in this event
% 
% -> Global fit on whole session or multiple fits on separate sleep episodes 
% (when wake duration > wake_thresh) 
%
% SEE : QuantifHomeostasisOccupancy ParcourQuantifHomeostasisOccupancyPlot
...
%% GET OCCUPANCY HOMEOSTASIS FOR ALL SESSIONS
...
% FOR EACH TYPE OF EVENTS : 
    ...
    % ------ Get Event Occupancy during Epoch ------ :
        evt_occupancy = EventsOccupancy_LP(events{q},epoch_timewindows, 'union', 0) ;
        % normalize event_density by mean density on entire epoch
        mean_evt_occupancy = EventsOccupancy_LP(events{q},epoch_timewindows, 'union', 1) ;
        evt_occupancy = tsd(Range(evt_occupancy),Data(evt_occupancy)/mean_evt_occupancy*100) ; % expressed as a percentage of mean event density during whole epoch (SWS)
    ...
    % ------ Assign 'NaN' value out of Epoch ------ : 
        ...
        if mean(length(outofepoch)) ~= 0 
            ...
            evt_occupancy = concat_tsd(evt_occupancy,outofepoch_tsd) ; 
        end
    
    % ------ Extract Homeostasis Data ------ :
        [GlobalFit, MultiFit] = HomeostasisMultiFit_LP(evt_occupancy,extendedWAKE,wake_thresh,'plot',0,'newfig',0,'ZTtime',NewtsdZT,'fit_start',fit_start) ; % Get homeostasis data (and plot) for multiple fits
...
if afterREM1
    eval ([' save ParcourQuantifHomeostasisOccupancy_multifit' num2str(wake_thresh) '_afterREM1.mat Homeo_res windowsize cooccur_delay slowwave_clean_window merge_closewake wake_thresh afterREM1']) ;
else
    eval (['save ParcourQuantifHomeostasisOccupancy_multifit' num2str(wake_thresh) '.mat Homeo_res windowsize cooccur_delay slowwave_clean_window merge_closewake wake_thresh afterREM1']) ;
end
