%% ParcourQuantifHomeostasisOccupancyPlot
%
% 23/04/2020 LP
%
...
% on event occupancy (delta, slow waves, down states). 
%
% -> Global fit on whole session or multiple fits on separate sleep episodes 
% (when wake duration > wake_thresh) 
%
% SEE : QuantifHomeostasisOccupancy ParcourQuantifHomeostasisOccupancy
...
try 
    if afterREM1
        ...
        eval(['load ' FolderDataLP_DeltaSpectral 'Data/ParcourQuantifHomeostasisOccupancy_multifit' num2str(wake_thresh) '.mat']) % path if on lab computer
    end    
    cd([FolderDataLP_DeltaSpectral 'Results/Homeostasis'])  
catch
    if afterREM1
        eval(['load /Users/leaprunier/Dropbox/Mobs_member/LeaPrunier/DeltaSpectral/Data/ParcourQuantifHomeostasisOccupancy_multifit' num2str(wake_thresh) '_afterREM1.mat']) % if on personal computer
    else
        eval(['load /Users/leaprunier/Dropbox/Mobs_member/LeaPrunier/DeltaSpectral/Data/ParcourQuantifHomeostasisOccupancy_multifit' num2str(wake_thresh) '.mat']) % if on personal computer
    end    
...
for p = ToPlot
    ...
    for q = 1:length(events_names) 
       ...
       % for each fit :
        ...
        xlabel('ZT Time (hours)') ; ylabel('Normalized Event Occupancy (% of mean NREM Occupancy)') ;
    ...
    % Save Plot :   
    saveas(gcf,['Occupancy/ParcourQuantifHomeostasisOccupancyPlot' num2str(p) '_' Homeo_res.name{p} '.png']) 
    saveas(gcf,['epsFigures/ParcourQuantifHomeostasisOccupancyPlot' num2str(p) '_' Homeo_res.name{p} '.eps'],'epsc')
...
for q = 1:length(events_names) 
    ...
    xlabel('Time (hours)'), ylabel('Normalized Event Occupancy (% of mean NREM Occupancy)');
    ...
    legend([d,l],{'Occupancy Local Maxima (all)', 
    ...
    % Save Plot :   
saveas(gcf,['Occupancy/ParcourQuantifHomeostasisOccupancyPlot_GlobalMean.png']) 
saveas(gcf,['epsFigures/ParcourQuantifHomeostasisOccupancyPlot_GlobalMean.eps'],'epsc')
...
for q = 1:length(events_names) 
    ...
    title([events_names{q} ' (Windowsize: ' num2str(windowsize) 's)'],'FontSize',12) ;
    mean_reg_coeff = mean(vertcat(Homeo.reg_coeff{ToPlot})) ; % mean slope coeff (linear regression)
    legend([d,l],{'Occupancy Local Maxima (all)',[sprintf('Linear Regression, mean R2diff = %.2f', nanmean(all_R2diff)) newline sprintf('Mean Slope = %.3e', nanmean(all_slopes))] },'FontSize',11)  % mean R2diff value
    ...
    % Save Plot :   
saveas(gcf,['Occupancy/ParcourQuantifHomeostasisOccupancyPlot_MultipleMean.png']) 
saveas(gcf,['epsFigures/ParcourQuantifHomeostasisOccupancyPlot_MultipleMean.eps'],'epsc')
