...
%% ParcourQuantifHomeostasisDensityPlot
%
% 21/04/2020 LP
%
...
try 
    if afterREM1
        ...
        eval(['load ' FolderDataLP_DeltaSpectral 'Data/ParcourQuantifHomeostasisDensity_multifit' num2str(wake_thresh) '.mat']) % path if on lab computer
    end    
    cd([FolderDataLP_DeltaSpectral 'Results/Homeostasis'])  
catch
    if afterREM1
        eval(['load /Users/leaprunier/Dropbox/Mobs_member/LeaPrunier/DeltaSpectral/Data/ParcourQuantifHomeostasisDensity_multifit' num2str(wake_thresh) '_afterREM1.mat']) % if on personal computer
    else
        eval(['load /Users/leaprunier/Dropbox/Mobs_member/LeaPrunier/DeltaSpectral/Data/ParcourQuantifHomeostasisDensity_multifit' num2str(wake_thresh) '.mat']) % if on personal computer
    end    
...
for p = ToPlot
    ...
    % Save Plot :   
    saveas(gcf,['Density/ParcourQuantifHomeostasisDensityPlot' num2str(p) '_' Homeo_res.name{p} '.png']) 
    saveas(gcf,['epsFigures/ParcourQuantifHomeostasisDensityPlot' num2str(p) '_' Homeo_res.name{p} '.eps'],'epsc')
...
for q = 1:length(events_names) 
    ...
    % Save Plot :   
saveas(gcf,['Density/ParcourQuantifHomeostasisDensityPlot_GlobalMean.png']) 
saveas(gcf,['epsFigures/ParcourQuantifHomeostasisDensityPlot_GlobalMean.eps'],'epsc')
...
for q = 1:length(events_names) 
    ...
    title([events_names{q} ' (Windowsize: ' num2str(windowsize) 's)'],'FontSize',12) ;
    mean_reg_coeff = mean(vertcat(Homeo.reg_coeff{ToPlot})) ; % mean slope coeff (linear regression)
    legend([d,l],{'Density Local Maxima (all)',[sprintf('Linear Regression, mean R2diff = %.2f', nanmean(all_R2diff)) newline sprintf('Mean Slope = %.3e', nanmean(all_slopes))] },'FontSize',11)  % mean R2diff value
    ...
    % Save Plot :   
saveas(gcf,['Density/ParcourQuantifHomeostasisDensityPlot_MultipleMean.png']) 
saveas(gcf,['epsFigures/ParcourQuantifHomeostasisDensityPlot_MultipleMean.eps'],'epsc')
