%% QUANTIFY OCCUPANCY HOMEOSTASIS (Local Maxima) during sleep
%
% 23/04/2020 LP
%
%
...
% for event occupancy (delta waves, slow waves, down states)
% -> global fit and multiple fits in separate sleep episodes   
%
%
% SEE : ParcourQuantifHomeostasisOccupancy
...
% ------ Get rid of aberrant values (> artefact_thresh %) ------ :     
    ...
    if  artefact_thresh ~= 'none' 
        evt_occupancy_t = Range(evt_occupancy) ; evt_occupancy_y = Data(evt_occupancy) ; 
        artefacts = (evt_occupancy_y>artefact_thresh) ; 
        evt_occupancy = tsd(evt_occupancy_t(~artefacts), evt_occupancy_y(~artefacts)) ; 
    end
...
% Global Fit & Multiple Fits
    ...
    [GlobalFit, MultiFit] = HomeostasisMultiFit_LP(evt_occupancy,extendedWAKE,wake_thresh,'plot',1,'newfig',0,'ZTtime',NewtsdZT,'fit_start',fit_start) ; % Get homeostasis data (and plot) for multiple fits
...
% ------ Plot LFP around Events ------ : 
    ...
    epoch_events_t = all_events_t(belong(epoch,all_events_t)) ;
