...
if quantif_choice == 1 % All Slow Waves 
    ...
    quantif_colors = {[0 0 0],[0.5 0.5 0.5],[0.9882,0.7137,0.302],[0.9686 0.5765 0.3137],[0.9373 0.3098 0.5765],[0.7137 0.4392 0.6824],[0.4392 0.5843 0.7176],[0.2627 0.7216 0.5843],[0.4392 0.5843 0.2980],[0.2880 0.4331 0.2920]} ;      
...
if mean_across_mice   % ----- Mean Data across mice (if mean_across_mice == true) ----- :
    ...
    for q = 1:length(quantif_list)
        ...
        c = plot(allmiceR2,-allmiceSlope,'x','Color',quantif_colors{q},'MarkerSize',15,'HandleVisibility','off');
        uistack(c,'bottom'); % put crosses in background

        % Plot mean of all sessions as dots : 
        plot(nanmean(allmiceR2),nanmean(-allmiceSlope),'.','Color',quantif_colors{q},'MarkerSize',30);
        ...
        xlim(xl_r2) ; ylim(yl_slope) ; 
        xlabel('mean R2') ; ylabel('mean (- Slope)') ;   
        legend(quantif_names,'Location','eastoutside') ; 
        set(gca,'FontSize',16)  
        title(['1st Fit : ' '0-' num2str(firstfit_duration) 'h'],'FontSize',20) ;
    end
    ...
    for q = 1:length(quantif_list) 
        ...
        c = plot(cell2mat(twofitR2_1),-twofitreg_coeff_1 (:,1)','x','Color',quantif_colors{q},'MarkerSize',15,'HandleVisibility','off');
        uistack(c,'bottom'); % put crosses in bakground

        % Plot mean of all sessions as dots : 
        plot(nanmean(cell2mat(twofitR2_1)),nanmean(-twofitreg_coeff_1 (:,1)'),'.','Color',quantif_colors{q},'MarkerSize',30);
...
if mean_across_mice   % ----- Mean Data across mice (if mean_across_mice == true) ----- :
    ...
    for q = 1:length(quantif_list)
        ...
        legend(quantif_names,'Location','eastoutside') ; 
        set(gca,'FontSize',16)  
        title(['' newline '2nd Fit : ' num2str(firstfit_duration) 'h-end'],'FontSize',20) ;
    end
    ...
    for q = 1:length(quantif_list) 
        ...
        c = plot(cell2mat(twofitR2_2),-twofitreg_coeff_2 (:,1)','x','Color',quantif_colors{q},'MarkerSize',15,'HandleVisibility','off');
        uistack(c,'bottom'); % put crosses in background

        % Plot mean of all sessions as dots : 
        plot(nanmean(cell2mat(twofitR2_2)),nanmean(-twofitreg_coeff_2 (:,1)'),'.','Color',quantif_colors{q},'MarkerSize',30);
        ...
        title(['2nd Fit : ' num2str(firstfit_duration) 'h-end'],'FontSize',20) ;
...
set(gcf, 'Position',  [1, 1, 705, 522])
print([PathToSave 'MiceHomeoDensitySlopeR2'], '-dpng', '-r300') ; 
 
print([PathToSave 'MiceHomeoDensityComparisonBarplot'], '-dpng', '-r300') ; 
